...
This service is the blueprint for a future `/project-chat` voice intake experience. The branch has not landed yet, so LiveKit dependencies are not installed locally; use this document as the integration guide when we are ready to hook everything up.

When implemented, it will pair the LiveKit WebRTC session used by the client widget with a streaming agent that listens for turns, generates replies, invokes the same Mastra agents/tools we already ship for text chat, and keeps the discovery/post-sales forms in sync.
...
3. **Realtime pipeline** – once connected, the agent wires AssemblyAI Slam-1 for streaming transcription, OpenAI (configurable via `VOICE_AGENT_MODEL`) for dialogue + structured extraction, ElevenLabs for TTS, and a timeout-based turn detector so the AI waits for user pauses. Mastra workflows would supply the reasoning/tools invoked inside each turn so we do not duplicate logic.
...
> **Tip:** until LiveKit is installed, you can still iterate on the Mastra workflows/tools that the voice agent will call by running the existing text-chat paths. The same helpers will be invoked by the voice runtime once connected.
