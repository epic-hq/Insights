# Trigger.dev Guide (v4)

**Audience:** Developers implementing background tasks and async workflows.

This guide covers Trigger.dev v4 SDK patterns for the Insights project. For deployment instructions, see `docs/30-howtos/trigger-dev-deployment.md`.
...
| Pattern | Use Case |
|---------|----------|
| `task()` | Basic async operations |
| `schemaTask()` | Type-safe tasks with Zod validation |
| `schedules.task()` | Cron-based scheduled tasks |
| `queue()` | Concurrency control |
| `wait.for()` | Delays and timeouts |
