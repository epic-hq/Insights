...
**theme_evidence**(`theme_id`, `evidence_id`, `rationale?`)
...
2) Evidence review UI (approve AI‑proposed snippets; add tags, personas, journey stage).
3) Auto cluster → **Themes** (rule‑based + semantic assist) with manual merge/split; create `theme_evidence` join.
4) Create **Insights** automatically from ≤3 themes; show counts & coverage (no probabilistic score yet).
...
**B. Evidence Inbox** (default after setup)
- Filters left; cards center; transcript drawer right; Accept/Edit/Reject

**C. Theme Studio**
- Suggested clusters; drag merge/split; inspector for naming & scope

**D. Insight Workshop**
- Select ≤3 themes → AI Draft → counts/coverage; persona wedge panel

**E. Opportunities Backlog**
- Kanban with HMW cards; Generate via AI button

**F. Persona × Theme Heatmap**
- Grid by persona; click to drill to evidence
...
# propose_themes.baml
input: { evidence_items: Evidence[], existing_themes: Theme[] }
output: [ { theme_id?: string, name?: string, statement?: string, rationale: string, confidence: float } ]

# draft_insight.baml
input: { themes: Theme[], personas: string[] }
output: { statement: string, theme_ids: string[], personas: string[], assumptions: string[], risks: string[] }

# draft_opportunities.baml
input: { insight: Insight }
output: [ { how_might_we: string, audience?: string, expected_benefit?: string, measure?: string } ]
```
