...
function getLangfuse(): Langfuse | null {
  ...
  if (!process.env.LANGFUSE_PUBLIC_KEY || !process.env.LANGFUSE_SECRET_KEY) {
    consola.debug(
      "[BAML Tracing] Langfuse credentials not found, tracing disabled",
    );
    return null;
  }
...
function summarizeUsage(
  collector: Collector,
  costOptions?: RunBamlOptions<unknown>["costOptions"],
  ...
  if (typeof billedInputTokens === "number")
    summary.billedInputTokens = billedInputTokens;
  ...
  if (
    costOptions?.promptCostPer1KTokens &&
    typeof promptTokenBase === "number"
  ) {
    summary.promptCostUsd = Number(
      ((promptTokenBase / 1000) * costOptions.promptCostPer1KTokens).toFixed(6),
    );
  }
  if (
    costOptions?.completionCostPer1KTokens &&
    typeof summary.outputTokens === "number"
  ) {
    summary.completionCostUsd = Number(
      (
        (summary.outputTokens / 1000) *
        costOptions.completionCostPer1KTokens
      ).toFixed(6),
    );
  }
  if (
    typeof summary.promptCostUsd === "number" ||
    typeof summary.completionCostUsd === "number"
  ) {
    summary.totalCostUsd = Number(
      ((summary.promptCostUsd ?? 0) + (summary.completionCostUsd ?? 0)).toFixed(
        6,
      ),
    );
  }
...
function mapUsageToLangfuse(usage: BamlUsageSummary | null) {
  ...
  if (typeof usage.cachedInputTokens === "number")
    payload.cached_tokens = usage.cachedInputTokens;
...
export async function runBaml<TResult>({
  ...
  model,
  costOptions,
}: RunBamlOptions<TResult>): Promise<{
  result: TResult;
