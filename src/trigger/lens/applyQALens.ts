...
export const applyQALensTask = task({
	...
	run: async (payload: ApplyQALensPayload) => {
		const { interviewId, accountId, projectId, computedBy, focusAreas } = payload
		const client = createSupabaseAdminClient()

		consola.info(`[applyQALens] Extracting Q&A pairs from interview ${interviewId}`)
		setProgress("loading")
		...
		consola.info(`[applyQALens] Found ${evidence.length} evidence items (${questionCount} questions)`)

		// 3. Build context
		const interviewContext = [
			interview.title ? `Title: ${interview.title}` : null,
			interview.interview_date ? `Date: ${interview.interview_date}` : null,
			interview.duration_sec ? `Duration: ${Math.round(interview.duration_sec / 60)} minutes` : null,
		]
			.filter(Boolean)
			.join("\n") || "Interview"
		...
		try {
			const { result } = await runBaml({
				...
				bamlCall: (b) =>
					b.ExtractQAPairs(evidenceJson, interviewContext, focusAreas || null),
			})
			extraction = result
		} catch (error) {
