...
export type SynthesizeLensSummaryPayload = z.infer<
  typeof synthesizeLensSummarySchema
>;
...
export const synthesizeLensSummaryTask = schemaTask({
  ...
  run: async (payload, { ctx }) => {
    const { projectId, templateKey, accountId, customInstructions, force } =
      payload;
    const client = createSupabaseAdminClient();

    consola.info(
      `[synthesize-summary] Starting synthesis for project=${projectId}, template=${templateKey}`,
    );
    ...
    const { data: analyses, error: analysesError } = (await (client as any)
      ...
      .eq("template_key", templateKey)
      .eq("status", "completed")
      .order("processed_at", { ascending: false })) as {
    ...
    if (!analyses || analyses.length === 0) {
      consola.info(
        `[synthesize-summary] No completed analyses found for project=${projectId}, template=${templateKey}`,
      );
      return { status: "no_data", interviewCount: 0 };
    }

    consola.info(
      `[synthesize-summary] Found ${analyses.length} analyses to synthesize`,
    );
    ...
    if (!force) {
      ...
      if (
        ...
        existingSummary.interview_count === analyses.length
      ) {
        consola.info(
          `[synthesize-summary] Summary already up-to-date, skipping`,
        );
    ...
    if (upsertError || !summaryRecord) {
      throw new Error(
        `Failed to create summary record: ${upsertError?.message || "Unknown error"}`,
      );
    }
    ...
    try {
      // 6. Call BAML synthesis function with billing
      consola.info(
        `[synthesize-summary] Calling SynthesizeLensInsights BAML function`,
      );
      ...
      const { result: synthesisResult } = await runBamlWithBilling(
        billingCtx,
        {
          functionName: "SynthesizeLensInsights",
          traceName: `lens.synthesize.${templateKey}`,
          input: {
          ...
          resourceType: "lens_summary",
          resourceId: summaryRecord.id,
          bamlCall: (client) =>
            client.SynthesizeLensInsights(
        ...
        `lens-summary:${projectId}:${templateKey}`,
      );
      ...
      consola.info(
        `[synthesize-summary] Successfully synthesized ${analyses.length} analyses for ${templateKey}`,
      );
