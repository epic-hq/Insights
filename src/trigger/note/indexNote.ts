...
export const indexNoteTask = schemaTask({
  ...
  run: async (payload) => {
    ...
    consola.info(
      `[indexNote] Extracted ${extraction.evidence.length} evidence units`,
    );
    ...
    if (deleteError) {
      consola.warn(
        `[indexNote] Failed to delete existing evidence:`,
        deleteError,
      );
    }
    ...
    consola.success(
      `[indexNote] Indexed note ${interviewId}: ${evidenceCount} evidence records`,
    );
