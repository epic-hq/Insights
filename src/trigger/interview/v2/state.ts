...
export async function loadWorkflowState(
  ...
  if (error) {
    consola.error(
      `Failed to load workflow state for interview ${interviewId}:`,
      error,
    );
    return null;
  }
  ...
  if (!state) {
    consola.warn(
      `Workflow state is null for interview ${interviewId}, but conversation_analysis exists`,
    );
    return null;
  }
  ...
  const mergedState = {
    ...state,
    completedSteps:
      conversationAnalysis.completed_steps || state.completedSteps || [],
...
export async function saveWorkflowState(
  ...
  if (state.fullTranscript !== undefined)
    merged.fullTranscript = state.fullTranscript;
  if (state.language !== undefined) merged.language = state.language;
  if (state.transcriptData !== undefined)
    merged.transcriptData = state.transcriptData;
  if (state.evidenceIds !== undefined) merged.evidenceIds = state.evidenceIds;
  if (state.evidenceUnits !== undefined)
    merged.evidenceUnits = state.evidenceUnits;
  ...
  if (state.completedSteps !== undefined)
    merged.completedSteps = state.completedSteps;
  ...
  if (error) {
    consola.error(
      `Failed to save workflow state for interview ${interviewId}:`,
      error,
    );
  ...
  consola.success(
    `[saveWorkflowState] State saved successfully for interview ${interviewId}`,
  );
}
...
export async function updateAnalysisJobProgress(
  ...
  if (error) {
    consola.warn(
      `Failed to update interview progress for ${interviewId}:`,
      error,
    );
  }
}
...
export function shouldExecuteStep(
  ...
  if (resumeFrom) {
    // IMPORTANT: Must match all steps in WorkflowStep type
