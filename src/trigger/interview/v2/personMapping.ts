...
import { isPlaceholderPerson, normalizeSpeakerLabel, upsertPersonWithCompanyAwareConflict } from "~/features/interviews/peopleNormalization.server"
...
export async function mapRawPeopleToInterviewLinks({
	...
	for (const raw of rawPeople) {
		...
		if (isPlaceholderPerson(baseName)) {
			consola.info(`[personMapping] Skipping placeholder person "${baseName}" (${raw.person_key})`)
			continue
		}
		...
		const payload: PersonUpsertPayload = {
			...
			company: raw.organization ?? null,
			role: raw.role ?? null,
		}

		const upserted = await upsertPersonWithCompanyAwareConflict(db, payload, null)
