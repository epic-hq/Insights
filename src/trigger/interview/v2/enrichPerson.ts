...
export const enrichPersonTaskV2 = task({
  ...
  run: async (
    payload: EnrichPersonPayload,
    { ctx },
  ): Promise<EnrichPersonResult> => {
    const { interviewId, projectId, accountId, personId, analysisJobId } =
      payload;
    ...
    try {
      ...
      if (linkError) {
        throw new Error(
          `Failed to load interview people: ${linkError.message}`,
        );
      }

      const peopleIds =
        interviewPeople?.map((link) => link.person_id).filter(Boolean) ?? [];

      if (peopleIds.length === 0) {
        consola.warn("[enrichPerson] No people found for interview", {
          interviewId,
        });
        ...
        try {
          ...
          if (!person.description) {
            try {
              ...
              if (updateError) {
                consola.error(
                  `[enrichPerson] Failed to update person ${pid}:`,
                  updateError,
                );
              } else {
                descriptionGenerated = true;
                consola.success(
                  `[enrichPerson] Generated description for person ${pid}`,
                );
              }
            } catch (descError) {
              consola.warn(
                `[enrichPerson] Could not generate description for person ${pid}:`,
                descError,
              );
            }
          } else {
            consola.info(
              `[enrichPerson] Person ${pid} already has description, skipping`,
            );
          }
          ...
          consola.error(
            `[enrichPerson] Failed to enrich person ${pid}:`,
            personError,
          );
          enrichedPeople.push({
            personId: pid,
      ...
      consola.success(
        `[enrichPerson] Enriched ${enrichedPeople.length} people`,
      );
