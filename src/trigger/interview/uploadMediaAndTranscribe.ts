    ...
    try {
      ...
      if (isRetry && payload.mediaUrl) {
        ...
        if (!hasMeaningfulTranscript) {
          console.log(
            "📝 No meaningful transcript data found - will transcribe",
          );
          needsTranscription = true;
        } else {
          console.log(
            "📋 Already have transcript data - skipping transcription",
          );
        }
      }
      ...
      if (needsTranscription && payload.mediaUrl) {
        ...
        try {
          ...
          const {
            ...
            validateR2PresignedUrl,
            extractR2KeyFromUrl,
          } = await import("~/utils/r2.server");
          let transcriptionUrl = payload.mediaUrl;

          // Check if mediaUrl is already a presigned URL
          if (
            payload.mediaUrl.startsWith("http://") ||
            payload.mediaUrl.startsWith("https://")
          ) {
            console.log(
              "- Provided URL appears to be presigned, validating...",
            );
            ...
            if (!isValid) {
              console.log(
                "- ❌ Presigned URL is invalid/expired, attempting to regenerate...",
              );
              ...
              if (r2Key) {
                console.log(
                  "- Extracted R2 key, generating fresh presigned URL...",
                );
                ...
                if (freshPresigned) {
                  ...
                  console.log(
                    "- ✅ Generated fresh presigned URL for transcription:",
                    transcriptionUrl.slice(0, 100) + "...",
                  );
                } else {
                  console.error("- ❌ Failed to generate fresh presigned URL!");
                  throw new Error(
                    "Failed to generate fresh presigned URL for transcription",
                  );
                }
              } else {
                console.error(
                  "- ❌ Could not extract R2 key from invalid URL!",
                );
                throw new Error(
                  "Could not extract R2 key from invalid presigned URL",
                );
              }
            } else {
            ...
            if (!presigned) {
              ...
              throw new Error(
                "Failed to generate presigned URL for transcription",
              );
            }

            transcriptionUrl = presigned.url;
            console.log(
              "- ✅ Generated presigned URL for transcription:",
              transcriptionUrl.slice(0, 100) + "...",
            );
          }

          console.log(
            "🔄 Calling transcribeAudioFromUrl with URL:",
            transcriptionUrl.slice(0, 100) + "...",
          );
          ...
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(
              () =>
                reject(
                  new Error(
                    "Transcription timeout: no response after 10 minutes",
                  ),
                ),
              10 * 60 * 1000,
            );
          ...
          const progressPromise = new Promise((resolve, reject) => {
            const progressInterval = setInterval(() => {
              ...
              metadata.set(
                ...
                Math.min(
                  25,
                  ((metadata.get("progressPercent") as number) || 20) + 1,
                ),
              );
          ...
          throw new Error(
            `Transcription failed: ${transcriptionError instanceof Error ? transcriptionError.message : String(transcriptionError)}`,
          );
        }
      } else {
        console.log(
          "⏭️ Skipping transcription - no mediaUrl or already has transcript",
        );
      }
      ...
      const { extractEvidenceAndPeopleTask } =
        await import("./extractEvidenceAndPeople");
      metadata.set("stageLabel", "Extracting evidence and participants");
      metadata.set("progressPercent", 55);
      const evidenceResultRun =
        await extractEvidenceAndPeopleTask.triggerAndWait({
          ...uploadResult,
      ...
      if (!evidenceResultRun.output) {
        throw new Error(
          "Evidence extraction completed without a result payload.",
        );
      }
      ...
      if (refreshedInterview) {
        interviewForAnalysis =
          refreshedInterview as typeof interviewForAnalysis;
      }

      const { analyzeThemesAndPersonaTask } =
        await import("./analyzeThemesAndPersona");
      ...
      if (!analysisResult.ok) {
        const message =
          ...
          "Failed to analyze themes and personas for interview.";
        throw new Error(message);
      }

      if (!analysisResult.output) {
        throw new Error(
          "Interview analysis completed without a result payload.",
        );
      }
      ...
      try {
        ...
        } = await import("~/utils/conversationAnalysis.server");
        ...
        const conversationAnalysis = await generateConversationAnalysis({
          ...
          context: {
            meetingTitle:
              payloadMetadata.interviewTitle ||
              interviewForAnalysis.title ||
              undefined,
        ...
        const enrichedConversationAnalysis =
          enrichConversationAnalysisWithEvidenceIds(
        ...
        console.log(
          "✅ Conversation analysis generated and stored as conversation-overview lens",
        );
      } catch (convAnalysisErr) {
        // Non-fatal — the rest of the pipeline already succeeded
        console.error(
          "⚠️ Conversation analysis generation failed (non-fatal):",
          convAnalysisErr,
        );
      }
