...
export const generateThumbnail = schemaTask({
  ...
  run: async (payload) => {
    ...
    try {
      ...
      if (!hasVideoStream) {
        console.log(
          `Skipping thumbnail for ${normalizedMediaKey} - no video stream (audio-only file)`,
        );
        return {
          success: false,
          skipped: true,
      ...
      console.warn(
        "Failed to probe file, attempting thumbnail anyway:",
        probeError,
      );
    ...
    try {
      ...
      console.log(
        `Extracting thumbnail from ${normalizedMediaKey} at ${timestampSec}s`,
      );
      ...
      if (!updateResponse.ok) {
        ...
        throw new Error(
          `Failed to update interview: ${updateResponse.status} ${errorText}`,
        );
      }
