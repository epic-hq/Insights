...
- URLs for editing, public access, and viewing responses`,
	inputSchema: z.object({
		...
		search: z.string().nullish().describe("Search surveys by name or description"),
		isLive: z.boolean().nullish().describe("Filter by live status (true = only live, false = only draft)"),
		includeQuestions: z.boolean().nullish().describe("Include full question definitions in response (default: false)"),
		limit: z.number().int().min(1).max(100).nullish().describe("Maximum number of surveys to return (default: 20)"),
	}),
