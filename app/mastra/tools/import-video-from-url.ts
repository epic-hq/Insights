...
The tool will scan webpages to find video/audio URLs, prioritizing HLS/DASH streams for best quality. For streaming formats, processing is handled via Trigger.dev with ffmpeg for reliable conversion.`,
  inputSchema: z.object({
    videoUrl: z
      ...
      .url()
      .describe(
        "URL to import - can be a direct media link or a webpage containing video/audio.",
      ),
    title: z
      .string()
      .optional()
      .describe("Optional title to use for the new interview."),
    participantName: z
      .string()
      .optional()
      .describe(
        "Name of the participant in the conversation. Will create a new person record if provided.",
      ),
    participantOrganization: z
      .string()
      .optional()
      .describe(
        "Organization/company name for the participant. Will be created if it doesn't exist.",
      ),
    participantSegment: z
      .string()
      .optional()
      .describe(
        "Segment for the participant (e.g., 'customer', 'prospect', 'churned_user').",
      ),
    customInstructions: z
      .string()
      .optional()
      .describe("Optional custom instructions to guide analysis."),
  }),
  execute: async (input, context?) => {
    ...
    if (!isDirectMediaUrl(inputUrl) && !isStreamingUrl(inputUrl)) {
      consola.info(
        `[importVideoFromUrl] URL doesn't appear to be direct media, scanning page: ${inputUrl}`,
      );
      ...
      if (extractionResult.assets.length > 1 && uniqueTypes.size > 1) {
        ...
        const options = extractionResult.assets.map((asset, i) => ({
          ...
          format: asset.format,
          url:
            asset.url.length > 80 ? asset.url.slice(0, 77) + "..." : asset.url,
        }));

        const videoCount = extractionResult.assets.filter(
          (a) => a.type === "video",
        ).length;
        const audioCount = extractionResult.assets.filter(
          (a) => a.type === "audio",
        ).length;
        const streamCount = extractionResult.assets.filter(
          (a) => a.type === "stream",
        ).length;
    ...
    consola.info(
      `[importVideoFromUrl] Delegating to Trigger.dev task for URL: ${inputUrl}`,
    );
    ...
    try {
          ...
          urls: [
            {
              ...
              participantSegment: participantSegment || undefined,
            },
          ],
          projectId,
          accountId,
      ...
      consola.error(
        "[importVideoFromUrl] Failed to trigger import task:",
        triggerError,
      );
      return {
        success: false,
