...
const toolInputSchema = z.object({
  ...
  nameSearch: z
    .string()
    .nullish()
    .describe("Optional case-insensitive name search for list"),
  limit: z
    .number()
    .int()
    .min(1)
  ...
  confirmName: z
    ...
    .nullish()
    .describe(
      "For delete (non-dryRun): must match the person's current name exactly (case-insensitive)",
    ),
});
...
async function countLinks(db: SupabaseClient<Database>, person_id: string) {
  const [
    interview_people_res,
    project_people_res,
    people_organizations_res,
    people_personas_res,
  ] = await Promise.all([
    db
...
export const managePeopleTool = createTool({
  ...
  execute: async (input, context?) => {
    ...
    const validated_person_id = personId
      ? validateUUID(personId, "personId", "manage-people")
      : null;
    ...
    try {
      ...
      if (action === "delete") {
        if (!validated_person_id) {
          return {
            success: false,
            message:
              "personId is required for delete and must be a valid UUID.",
            person: null,
          };
        }
        ...
        if (
          !expected_name ||
          !provided_name ||
          expected_name.toLowerCase() !== provided_name.toLowerCase()
        ) {
          return {
            success: false,
      ...
      return {
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to manage people.",
        person: null,
      };
    }
  },
