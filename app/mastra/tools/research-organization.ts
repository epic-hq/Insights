...
const SIZE_RANGES = ["1-10", "11-50", "51-200", "201-500", "501-1000", "1001-5000", "5001-10000", "10000+"] as const;
...
function extractCompanyData(text: string): {
	...
	const foundedPatterns = [/(?:founded|established|started)\s+(?:in\s+)?(\d{4})/i, /since\s+(\d{4})/i];
	...
	if (firstSentenceMatch && firstSentenceMatch[1].length > 20 && firstSentenceMatch[1].length < 300) {
...
- User asks "what do we know about [company]" and internal search has no results`,
	...
	execute: async (input, context?) => {
		...
		if (!apiKey) {
			return {
				success: false,
				message: "Organization research is not configured (EXA_API_KEY missing).",
			};
		}
		...
		try {
			...
			if (!response || !response.ok) {
				return {
					success: false,
					message: "Failed to search for company information (API error or rate limit).",
				};
			}
			...
			const markdownContent = formatResearchAsAnnotation(organizationName, data.results, extracted);
			...
			if (!orgId && createIfNotFound) {
				...
				if (insertError) {
					consola.error("[research-organization] Failed to create org:", insertError);
				} else if (newOrg) {
					orgId = newOrg.id;
					wasCreated = true;
			...
			if (orgId && !wasCreated) {
				...
				if (extracted.headquarters_location) updateData.headquarters_location = extracted.headquarters_location;
				...
				if (Object.keys(updateData).length > 0) {
					...
					if (updateError) {
						consola.error("[research-organization] Failed to update org:", updateError);
					} else {
						consola.info("[research-organization] Updated organization:", updateData);
					}
				}
			}

			// Sync org data to person facets for all linked people
			let peopleEnriched = 0;
			if (orgId && (extracted.industry || extracted.size_range)) {
				try {
					const { syncOrgDataToPersonFacets } = await import("../../features/people/syncOrgDataToPersonFacets.server");
					...
					for (const person of linkedPeople ?? []) {
						try {
							...
							consola.error("[research-organization] Error syncing facets for person:", person.id, err);
						}
					}

					if (peopleEnriched > 0) {
						consola.info(`[research-organization] Synced org facets to ${peopleEnriched} people`);
					}
				} catch (err) {
					consola.error("[research-organization] Error in org facet sync:", err);
				}
			}
			...
			if (orgId) {
				...
				if (annotationError) {
					consola.error("[research-organization] Failed to create annotation:", annotationError);
				} else {
					annotationId = annotation?.id;
					consola.info("[research-organization] Created annotation:", annotationId);
				}
			}
			...
			for (const result of data.results) {
				try {
					...
					consola.error("[research-organization] Error creating evidence:", err);
				}
			}
			...
			const fieldCount = Object.keys(extracted).filter((k) => extracted[k as keyof typeof extracted]).length;

			let message: string;
			const enrichedSuffix = peopleEnriched > 0 ? ` Enriched ${peopleEnriched} linked people with org facets.` : "";
			if (wasCreated) {
				message = `Created organization "${organizationName}" with ${fieldCount} extracted fields (${extracted.industry || "unknown industry"}, ${extracted.size_range || "unknown size"}).${enrichedSuffix}`;
			} else if (orgId) {
				message = `Updated organization "${organizationName}" with ${fieldCount} fields.${enrichedSuffix}`;
			} else {
			...
			return {
				success: false,
				message: error instanceof Error ? error.message : "Failed to research organization.",
			};
		}
	},
});
...
function formatResearchAsAnnotation(
	...
	if (extracted.industry) extractedFields.push(`**Industry:** ${extracted.industry}`);
	if (extracted.size_range) extractedFields.push(`**Company Size:** ${extracted.size_range}`);
	if (extracted.employee_count) extractedFields.push(`**Employees:** ~${extracted.employee_count.toLocaleString()}`);
	if (extracted.headquarters_location) extractedFields.push(`**Headquarters:** ${extracted.headquarters_location}`);
	if (extracted.founded_year) extractedFields.push(`**Founded:** ${extracted.founded_year}`);
