  ...
  execute: async (input, context?) => {
    ...
    try {
      ...
      if (orgId && (extracted.industry || extracted.size_range)) {
        try {
          const { syncOrgDataToPersonFacets } =
            await import("../../features/people/syncOrgDataToPersonFacets.server");
          ...
          for (const person of linkedPeople ?? []) {
            try {
              ...
              consola.error(
                "[research-organization] Error syncing facets for person:",
                person.id,
                err,
              );
            }
          ...
          if (peopleEnriched > 0) {
            consola.info(
              `[research-organization] Synced org facets to ${peopleEnriched} people`,
            );
          }
        } catch (err) {
          consola.error(
            "[research-organization] Error in org facet sync:",
            err,
          );
        }
      }
      ...
      const enrichedSuffix =
        peopleEnriched > 0
          ? ` Enriched ${peopleEnriched} linked people with org facets.`
          : "";
      if (wasCreated) {
