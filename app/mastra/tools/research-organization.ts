...
- User asks "what do we know about [company]" and internal search has no results`,
	...
	execute: async (input, context?) => {
		...
		const { organizationId, organizationName, additionalQueries } = input
		...
		try {
			...
			return {
				success: true,
				message: orgId
					? `Researched "${organizationName}" and updated organization record with ${Object.keys(extracted).filter((k) => extracted[k as keyof typeof extracted]).length} fields.`
					: `Researched "${organizationName}" but no matching organization found to update. Created ${evidenceCount} evidence records.`,
				organizationId: orgId,
				extractedData: {
