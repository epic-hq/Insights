...
export const fetchThemeStakeholdersTool = createTool({
	...
	inputSchema: z.object({
		...
		themeIds: z.array(z.string()).min(1).describe("One or more theme IDs to look up stakeholders for."),
		requireAll: z
			.boolean()
			.nullish()
			.describe("If true, only return people linked to ALL specified themes. Default false (any theme)."),
		limit: z.number().int().min(1).max(100).nullish().describe("Max people to return. Default 50."),
	}),
	...
	execute: async (input, context?) => {
		...
		try {
			...
			if (facetError) {
				consola.error("[fetch-theme-stakeholders] Error fetching facets:", facetError);
				// Continue without facets rather than failing
			}

			// Build person -> facets map
			const personFacets = new Map<string, Array<{ kind: string; label: string }>>();
