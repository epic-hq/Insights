...
import { z } from "zod";
import { SEGMENT_KIND_LABELS, SEGMENT_KIND_SLUGS } from "../../features/segments/services/segmentData.server";
...
const kindDistributionSchema = z.object({
	kind: z.string(),
	kindLabel: z.string(),
	totalTagged: z.number().describe("Number of people with at least one value for this kind"),
...
export const fetchStakeholderDemographicsTool = createTool({
	...
	inputSchema: z.object({
		projectId: z.string().nullish().describe("Project ID. Defaults to runtime context project_id."),
		kinds: z
			.array(z.string())
			.nullish()
			.describe(
		...
		themeId: z
			.string()
			.nullish()
			.describe("Optional: restrict analysis to people linked to a specific theme. If provided, personIds is ignored."),
	}),
	...
	execute: async (input, context?) => {
		...
		try {
			...
			if (input.themeId) {
				...
				const { getUsersWithThemes } = await import("../../features/themes/services/segmentThemeQueries.server");
			...
			const facetAccountMap = new Map<number, { kindSlug: string; label: string }>();
			...
			const wantsOrgIndustry = requestedKinds.includes("industry") || requestedKinds.includes("company_industry");
			...
			if (wantsOrgIndustry || wantsOrgSize) {
				const { data: peopleWithOrgs } = await supabase
					.from("people")
					.select("id, default_organization_id, organizations!default_organization_id(industry, size_range)")
					.in("id", personIds)
				...
				if (peopleWithOrgs && peopleWithOrgs.length > 0) {
					for (const person of peopleWithOrgs) {
						...
						if (wantsOrgIndustry && org.industry) {
							...
							if (!kindAgg.has(org.industry)) kindAgg.set(org.industry, new Set());
						...
						if (wantsOrgSize && org.size_range) {
							...
							const kindAgg = agg.get(slug)!;
							if (!kindAgg.has(org.size_range)) kindAgg.set(org.size_range, new Set());
