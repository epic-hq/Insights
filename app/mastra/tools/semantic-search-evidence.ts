...
export const semanticSearchEvidenceTool = createTool({
  ...
  inputSchema: z.object({
    ...
    projectId: z
      ...
      .nullish()
      .describe(
        "Project ID to search within. Defaults to the current project in context.",
      ),
    interviewId: z
      .string()
      .nullish()
      .describe(
        "Optional: Limit search to a specific interview. Omit to search all interviews.",
      ),
    matchThreshold: z
      .number()
      .min(0)
  ...
  execute: async (input, context?) => {
    ...
    if (!projectId) {
      ...
      return {
        success: false,
        message:
          "Missing projectId. Pass one explicitly or ensure the runtime context sets project_id.",
        query: query || "",
        evidence: [],
        totalCount: 0,
    ...
    if (!query) {
      ...
      return {
        success: false,
        message:
          "Missing search query. Provide a natural language query to search for.",
        query: "",
        evidence: [],
        totalCount: 0,
        threshold: matchThreshold,
    ...
    try {
      ...
      if (!embeddingResponse.ok) {
        ...
        throw new Error(
          `OpenAI API error: ${embeddingResponse.status} ${errorText}`,
        );
      }
      ...
      if (interviewId) {
        ...
        if (evidenceError) {
          consola.error(
            "semantic-search-evidence: database error",
            evidenceError,
          );
          throw evidenceError;
        }
        ...
        const interviewIds = [
          ...new Set(
            evidenceData?.map((_e) => interviewId).filter(Boolean) || [],
          ),
        ];
        ...
        const interviewTitles = new Map(
          interviewData?.map((i) => [i.id, i.title]) || [],
        );
      ...
      consola.info(
        "semantic-search-evidence: calling find_similar_evidence + find_similar_evidence_facets RPCs",
        {
          projectId,
          matchThreshold,
          matchCount,
      ...
      const [
        ...
        { data: facetsData, error: facetsError },
      ...
      const evidenceMatchRows =
        (evidenceData as EvidenceMatchRow[] | null) ?? [];
      ...
      if (evidenceError) {
        consola.error(
          "semantic-search-evidence: evidence query error",
          evidenceError,
        );
        throw evidenceError;
      }
      ...
      if (facetsError) {
        consola.error(
          "semantic-search-evidence: facets query error",
          facetsError,
        );
        throw facetsError;
      }
      ...
      if (evidenceIds.size === 0) {
        ...
        consola.info(
          "semantic-search-evidence: running keyword fallback (no embedding hits)",
          {
            query,
            projectId,
            matchCount,
        ...
        const { data: textMatches, error: textError } = await supabase
          .from("evidence")
          .select(
            "id, verbatim, gist, chunk, interview_id, pains, gains, thinks, feels, anchors, context_summary",
          )
        ...
        if (textError) {
          consola.error(
            "semantic-search-evidence: keyword fallback text query error",
            textError,
          );
        }
        ...
        if (peopleError) {
          consola.error(
            "semantic-search-evidence: keyword fallback people query error",
            peopleError,
          );
        }
        ...
        const peopleMatchRows =
          (peopleMatches as PeopleMatchRow[] | null) ?? [];
        ...
        if (fallbackEvidenceIds.size > 0) {
          ...
          const { data: fallbackEvidence } = await supabase
            .from("evidence")
            .select(
              "id, verbatim, gist, chunk, interview_id, pains, gains, thinks, feels, anchors",
            )
          ...
          const { data: fallbackInterviewData } = fallbackInterviewIds.length
            ? await supabase
                .from("interviews")
                .select("id, title")
                .in("id", fallbackInterviewIds)
            : { data: [] };

          const fallbackInterviewTitles = new Map(
            fallbackInterviewData?.map((i) => [i.id, i.title]) || [],
          );
          ...
          const fallbackRanked =
            fallbackEvidence
              ?.map((row) => {
                const textBlob = [row.verbatim, row.gist, row.chunk]
                  .filter(Boolean)
                  .join(" ")
                  .toLowerCase();
                const tokenMatches =
                  tokens.length > 0 ? scoreText(textBlob) / tokens.length : 0;
          ...
          if (fallbackRanked.length > 0) {
            consola.info(
              "semantic-search-evidence: returning keyword fallback results",
              {
        ...
        const { data: topMatches } = await supabase.rpc(
          ...
          diagnostics: {
            ...
            percentWithEmbeddings:
              totalEvidenceCount && totalEvidenceCount > 0
                ? Math.round(
                    ((evidenceWithEmbeddings || 0) / totalEvidenceCount) * 100,
                  )
                : 0,
            highestSimilarity: topMatches?.[0]?.similarity || 0,
            thresholdTooHigh:
              (topMatches?.[0]?.similarity || 0) < matchThreshold,
          },
        });
      ...
      const { data: fullEvidence } = await supabase
        .from("evidence")
        .select(
          "id, verbatim, gist, interview_id, pains, gains, thinks, feels, anchors",
        )
      ...
      const evidence = evidenceIdsArray
        .map((id) => {
          ...
          return {
            ...
            interviewTitle: fullData.interview_id
              ? interviewTitles.get(fullData.interview_id) || null
              : null,
