...
function inferStatus(tool_name: string): ToolStatus {
  ...
  if (
    ...
    return "deleting";
  if (name.includes("update") || name.includes("edit") || name.includes("set"))
    return "updating";
  if (
...
function summarizeToolIO(value: unknown): Record<string, unknown> | string {
  ...
  if (typeof value !== "object")
    return String(value).slice(0, IO_SUMMARY_MAX_LEN);
...
async function emitToolStatus(params: {
  context?: unknown;
  tool: string;
  status: ToolStatus;
  message?: string;
...
export function wrapToolWithStatusEvents<T extends ToolLike>(
  tool: T,
  tool_name: string,
): T {
...
export function wrapToolsWithStatusEvents<T extends Record<string, unknown>>(
  tools: T,
): T {
