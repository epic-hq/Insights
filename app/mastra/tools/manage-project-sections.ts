...
export const fetchProjectSectionTool = createTool({
  ...
  description:
    "Fetch a specific project section by kind (e.g., target_orgs, target_roles, research_goal)",
  inputSchema: z.object({
    kind: z
      .string()
      .describe("Section kind to fetch (e.g., target_orgs, target_roles)"),
  }),
...
export const updateProjectSectionMetaTool = createTool({
  ...
  inputSchema: z.object({
    ...
    mergeMeta: z
      ...
      .default(false)
      .describe(
        "If true, merge with existing meta (append to arrays, merge objects)",
      ),
    contentMd: z
      .string()
      .optional()
      .describe("Optional markdown content to update"),
  }),
  outputSchema: baseOutput.extend({
    section: sectionOutputSchema.nullable().optional(),
  }),
...
export const deleteProjectSectionMetaKeyTool = createTool({
  ...
  description:
    "Delete specific keys from a section's meta JSON object, or remove items from meta arrays",
  inputSchema: z.object({
    kind: z.string().describe("Section kind (e.g., target_orgs, target_roles)"),
    keys: z
      .array(z.string())
      .optional()
      .describe("Meta keys to delete entirely"),
    removeFromArrays: z
      .record(z.string(), z.array(z.string()))
      .optional()
      .describe(
        "Remove specific items from array fields. Format: { fieldName: [items to remove] }",
      ),
  }),
  outputSchema: baseOutput.extend({
    section: sectionOutputSchema.nullable().optional(),
  ...
  execute: async (input, context?) => {
    try {
      ...
      if (input.removeFromArrays) {
        for (const [field, itemsToRemove] of Object.entries(
          input.removeFromArrays,
        )) {
          if (Array.isArray(meta[field])) {
            meta[field] = meta[field].filter(
      ...
      const message = error instanceof Error ? error.message : "Unknown error";
      consola.error(
        "deleteProjectSectionMetaKeyTool unexpected error",
        message,
      );
      return { success: false, message };
    }
