...
export const fetchOpportunitiesTool = createTool({
  ...
  inputSchema: z.object({
    ...
    kanbanStatus: z
      .string()
      .nullish()
      .describe("Filter by kanban status (Explore, Validate, Build)"),
    ...
    opportunityIds: z
      .array(z.string())
      .nullish()
      .describe("Specific opportunity IDs to retrieve"),
    responseFormat: z
      .enum(["concise", "detailed"])
      .nullish()
      .describe(
        "Use concise for short lists (id/title/stage/status/amount/closeDate).",
      ),
  }),
  ...
  execute: async (input, context?) => {
    try {
      ...
      const mapped = (data ?? []).map((row) =>
        mapOpportunity(row as RawOpportunity, projectPath),
      );
      const opportunities =
      ...
      return {
        ...
        message: mapped.length
          ? `Found ${mapped.length} opportunities`
...
const sharedOpportunityFields = {
  ...
  kanbanStatus: z
    .string()
    .nullish()
    .describe("Pipeline column (Explore, Validate, Build)"),
  stage: z
    .string()
    .nullish()
    .describe("Sales stage (e.g., Discovery, Proposal)"),
  ...
  relatedInsightIds: z
    .array(z.string())
    .nullish()
    .describe("Insight IDs linked to this opportunity"),
  linkedInterviewId: z
    .string()
    .nullish()
    .describe("Interview ID that inspired this opportunity"),
  metadata: z
    .record(z.unknown())
    .nullish()
    .describe("Additional structured metadata to store"),
};
...
export const createOpportunityTool = createTool({
  ...
  execute: async (input, context?) => {
    try {
      ...
      if (input?.relatedInsightIds?.length)
        insertData.related_insight_ids = input.relatedInsightIds;
...
export const updateOpportunityTool = createTool({
  ...
  execute: async (input, context?) => {
    try {
      ...
      if (typeof input?.kanbanStatus === "string")
        updateData.kanban_status = input.kanbanStatus;
      ...
      if (typeof input?.closeDate === "string")
        updateData.close_date = input.closeDate;
      if (typeof input?.ownerId === "string")
        updateData.owner_id = input.ownerId;
      if (Array.isArray(input?.relatedInsightIds))
        updateData.related_insight_ids = input.relatedInsightIds;
