...
type ThemeEvidenceRow =
  Database["public"]["Tables"]["theme_evidence"]["Row"] & {
    evidence?: Pick<
      EvidenceRow,
      | "id"
      | "gist"
...
type OrganizationNameSummary = Pick<
  Database["public"]["Tables"]["organizations"]["Row"],
  "name"
>;
type ProjectPeopleRow =
  Database["public"]["Tables"]["project_people"]["Row"] & {
    person?:
...
type PeoplePersonaRow =
  Database["public"]["Tables"]["people_personas"]["Row"] & {
    people?: Pick<
      Database["public"]["Tables"]["people"]["Row"],
      "id" | "name" | "segment" | "job_function"
    > | null;
  };
type PersonaInsightsRow =
  Database["public"]["Tables"]["persona_insights"]["Row"];
type InterviewRow = Database["public"]["Tables"]["interviews"]["Row"] & {
  insights?: Array<{ id: string | null }> | null;
  evidence?: Array<{ id: string | null }> | null;
};
type InterviewPeopleRow =
  Database["public"]["Tables"]["interview_people"]["Row"] & {
    interview?: Pick<
...
>;
type ProjectPersonSummary = Pick<
  Database["public"]["Tables"]["people"]["Row"],
  "id" | "title"
> & {
  default_organization?:
...
type ProjectPeopleSummaryRow = Pick<
  Database["public"]["Tables"]["project_people"]["Row"],
  "person_id"
> & {
...
function resolveSummaryPerson(
  value: ProjectPeopleSummaryRow["person"],
): ProjectPersonSummary | null {
  if (!value) return null;
...
export const fetchProjectStatusContextTool = createTool({
  ...
  inputSchema: z.object({
    projectId: z
      ...
      .nullish()
      .describe(
        "Project ID to load. Defaults to the current project in context.",
      ),
    scopes: z
    ...
    peopleSearch: z
      ...
      .nullish()
      .describe(
        "Optional case-insensitive search string to match people by name or display name.",
      ),
    includePersonEvidence: z
      .boolean()
      .nullish()
      .describe(
        "When true, include recent evidence snippets linked to the matched people.",
      ),
    personEvidenceLimit: z
      ...
      .nullish()
      .describe(
        "Maximum number of evidence snippets to include per person when includePersonEvidence is true.",
      ),
    includeEvidence: z
      .boolean()
      .nullish()
  ...
  execute: async (input, context?) => {
    ...
    const runtimeAccountIdString = runtimeAccountId
    ...
    const includeEvidence =
      input.includeEvidence ?? scopes.includes("evidence");
    ...
    const includePersonEvidence =
      input.includePersonEvidence ?? sanitizedPersonSearch.length > 0;
    const personEvidenceLimit =
      input.personEvidenceLimit ?? DEFAULT_PERSON_EVIDENCE_LIMIT;
    ...
    try {
      ...
      if (projectError) {
        consola.error(
          "fetch-project-status-context: failed to load project metadata",
          projectError,
        );
        return {
          success: false,
      ...
      const resolvedAccountId =
        runtimeAccountIdString || project.account_id || undefined;

      if (
        runtimeAccountIdString &&
        project.account_id &&
        project.account_id !== runtimeAccountIdString
      ) {
        consola.debug(
          "fetch-project-status-context: runtime account differs from project account",
          {
      ...
      if (!runtimeAccountIdString && project.account_id) {
        consola.debug(
          "fetch-project-status-context: using project.account_id as fallback for routes",
          {
      ...
      const projectPath =
        resolvedAccountId && projectId
          ? `/a/${resolvedAccountId}/${projectId}`
          : "";
      ...
      if (scopeSet.has("status")) {
        try {
          ...
          consola.error(
            "fetch-project-status-context: failed to load status scope",
            error,
          );
      ...
      if (scopeSet.has("sections")) {
        try {
          const { data: sections, error } = await supabase
            ...
            .select(
              "id, kind, content_md, meta, position, created_at, updated_at",
            )
          ...
          consola.error(
            "fetch-project-status-context: failed to load sections scope",
            error,
          );
      ...
      if (scopeSet.has("insights")) {
        try {
          ...
          (tagsRows || []).forEach((row) => {
            ...
            tagsMap.set(row.insight_id, [
              ...(tagsMap.get(row.insight_id) || []),
              row.tags.tag,
            ]);
          });
            ...
            insights?.map((insight: InsightRow) => {
              ...
              return {
                ...
                opportunity_ideas: Array.isArray(insight.opportunity_ideas)
                  ? insight.opportunity_ideas.filter(
                      (item): item is string => typeof item === "string",
                    )
                  : undefined,
                related_tags: Array.isArray(insight.related_tags)
                  ? insight.related_tags.filter(
                      (item): item is string => typeof item === "string",
                    )
                  : undefined,
                tags,
                ...
                url: projectPath
                  ? `${HOST}${routes.insights.detail(insight.id)}`
                  : null,
              };
            }) ?? [];
          ...
          consola.error(
            "fetch-project-status-context: failed to load insights scope",
            error,
          );
      ...
      if (scopeSet.has("evidence")) {
        if (!includeEvidence) {
          consola.debug(
            "fetch-project-status-context: evidence scope skipped due to includeEvidence=false",
          );
          data.evidence = [];
        } else {
          try {
            ...
            const serialized: EvidencePayload[] =
              evidence?.map((item: EvidenceRow) => ({
                ...
                url: projectPath
                  ? `${HOST}${routes.evidence.detail(item.id)}`
                  : null,
              })) ?? [];
            ...
            consola.error(
              "fetch-project-status-context: failed to load evidence scope",
              error,
            );
      ...
      if (scopeSet.has("themes")) {
        try {
          ...
          if (includeEvidence && themeIds.length > 0) {
            const { data: themeEvidence, error: themeEvidenceError } =
              await supabase
                .from("theme_evidence")
                .select(
            ...
            if (themeEvidenceError) {
              consola.warn(
                "fetch-project-status-context: failed to load theme evidence details",
                themeEvidenceError,
              );
            } else {
              ...
              for (const row of evidenceRows) {
                ...
                themeEvidenceCount.set(
                  row.theme_id,
                  (themeEvidenceCount.get(row.theme_id) ?? 0) + 1,
                );
              }
            }
          } else if (!includeEvidence) {
            consola.debug(
              "fetch-project-status-context: skipping theme evidence details due to includeEvidence=false",
            );
          }
          ...
          const serialized: ThemePayload[] = themeRows.map((theme) => ({
            ...
            url: projectPath
              ? `${HOST}${routes.themes.detail(theme.id)}`
              : null,
          }));
          ...
          consola.error(
            "fetch-project-status-context: failed to load themes scope",
            error,
          );
        ...
        try {
          ...
          if (sanitizedPersonSearch) {
            ...
            peopleQuery = peopleQuery.or(
              `person.name.ilike.${pattern},person.title.ilike.${pattern}`,
            );
          }
          ...
          const { data: allIcpScores, error: allIcpScoresError } =
            await supabase
              .from("person_scale")
          ...
          if (allIcpScoresError) {
            consola.warn(
              "fetch-project-status-context: failed to load person_scale rows",
              allIcpScoresError,
            );
          }
          ...
          const icpByPerson = new Map(
            ((allIcpScores as IcpScoreRow[] | null) ?? [])
              .filter(
                (score) => score.person_id && personIdSet.has(score.person_id),
              )
          ...
          if (personIds.length > 0) {
            ...
            if (interviewPeopleError) {
              ...
              for (const row of (interviewPeople as
                | InterviewPeopleRow[]
                | null) ?? []) {
          ...
          if (includePersonEvidence && interviewIds.size > 0) {
            const totalEvidenceLimit =
              personEvidenceLimit * Math.max(peopleRows.length, 1);
            const { data: evidenceRows, error: evidenceError } = await supabase
              .from("evidence")
              .select(
                "id, gist, verbatim, context_summary, modality, created_at, interview_id",
              )
              .eq("project_id", projectId)
              .is("deleted_at", null)
              .eq("is_archived", false)
            ...
            if (evidenceError) {
              consola.warn(
                "fetch-project-status-context: failed to load evidence for people scope",
                evidenceError,
              );
            } else {
              for (const evidence of (evidenceRows as EvidenceRow[] | null) ??
                ...
                const existing =
                  evidenceByInterview.get(evidence.interview_id) ?? [];
          ...
          const serialized: PersonPayload[] = peopleRows.map((row) => {
            ...
            const personas =
              person?.people_personas
                ...
                .filter(
                  (
                    persona,
                  ): persona is Database["public"]["Tables"]["personas"]["Row"] =>
                    Boolean(persona),
                )
                .map((persona) => ({
                  id: persona.id,
                  ...
                  color_hex:
                    (persona as { color_hex?: string | null })?.color_hex ??
                    null,
                })) ?? [];
            ...
            const interviewsForPerson = (interviewPeopleMap.get(personId) ?? [])
              .map((entry) => {
                ...
                const interviewDate = normalizeDate(
                  entry.interview?.interview_date,
                );
                const evidenceGroup =
                  evidenceByInterview.get(interviewId) ?? [];
                return {
                  id: interviewId,
            ...
            if (includePersonEvidence && interviewsForPerson.length > 0) {
              ...
              for (const interview of interviewsForPerson) {
                const evidenceGroup =
                  evidenceByInterview.get(interview.id) ?? [];
            ...
            return {
              ...
              company:
                resolveOrganizationName(person?.default_organization) ?? null,
              ...
              icpMatch: icpByPerson.get(personId)
                ...
                : null,
              url: projectPath
                ? `${HOST}${routes.people.detail(personId)}`
                : null,
            };
          });
          ...
          const { data: summaryPeopleRows, error: summaryPeopleError } =
            await supabase
              .from("project_people")
              .select(
          ...
          if (summaryPeopleError) {
            consola.warn(
              "fetch-project-status-context: failed to load full people summary scope",
              summaryPeopleError,
            );
          }
          const summaryPeople =
            (summaryPeopleRows as ProjectPeopleSummaryRow[] | null) ?? [];
          const summaryProfileByPersonId = new Map<
            string,
            { hasTitle: boolean; hasCompany: boolean }
          >();
          for (const row of summaryPeople) {
            ...
            const summaryCompanyName =
              resolveOrganizationName(summaryPerson?.default_organization) ??
              null;
          ...
          for (const score of (allIcpScores as IcpScoreRow[] | null) ?? []) {
            if (!score.person_id || !summaryPersonIdSet.has(score.person_id))
              continue;
          ...
          consola.error(
            "fetch-project-status-context: failed to load people scope",
            error,
          );
      ...
      if (scopeSet.has("personas")) {
        try {
          ...
          if (personaIds.length > 0) {
            const [personaInsightsResult, personaPeopleResult] =
              await Promise.all([
                supabase
                ...
                supabase
                  ...
                  .select(
                    "persona_id, people:person_id(id, name, segment, job_function)",
                  )
                  .in("persona_id", personaIds),
              ]);

            if (!personaInsightsResult.error) {
              for (const row of (personaInsightsResult.data ??
                []) as PersonaInsightsRow[]) {
                if (!row.persona_id || !row.insight_id) continue;
                const insights =
                  personaInsightMap.get(row.persona_id) ?? new Set<string>();
            ...
            if (!personaPeopleResult.error) {
              for (const row of (personaPeopleResult.data ??
                []) as PeoplePersonaRow[]) {
              ...
              consola.warn(
                "fetch-project-status-context: failed to load people_personas",
                personaPeopleResult.error,
              );
            }
          }
          ...
          const serialized: PersonaPayload[] = personaRows.map((persona) => ({
            ...
            linkedPeople: personaPeopleMap.has(persona.id)
              ...
              : undefined,
            url: projectPath
              ? `${HOST}${routes.personas.detail(persona.id)}`
              : null,
          }));
          ...
          consola.error(
            "fetch-project-status-context: failed to load personas scope",
            error,
          );
      ...
      if (scopeSet.has("interviews")) {
        try {
          const { data: interviews, error } = await supabase
            .from("interviews")
            .select(
              "id, title, participant_pseudonym, segment, status, interview_date, created_at, updated_at",
            )
            .eq("project_id", projectId)
          ...
          evidenceRows?.data?.forEach((row) => {
            ...
            evidenceMap.set(
              row.interview_id,
              (evidenceMap.get(row.interview_id) || 0) + 1,
            );
          });
          ...
          insightRows?.data?.forEach((row) => {
            ...
            insightMap.set(
              row.interview_id,
              (insightMap.get(row.interview_id) || 0) + 1,
            );
          });
          ...
          const serialized: InterviewPayload[] =
            (interviews as InterviewRow[] | null)?.map((interview) => ({
              ...
              url: projectPath
                ? `${HOST}${routes.interviews.detail(interview.id)}`
                : null,
            })) ?? [];
          ...
          consola.error(
            "fetch-project-status-context: failed to load interviews scope",
            error,
          );
      ...
      const missingScopes = scopes.filter(
        (scope) => !loadedScopes.includes(scope),
      );
