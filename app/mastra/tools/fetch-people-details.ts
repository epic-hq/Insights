...
export const fetchPeopleDetailsTool = createTool({
  ...
  inputSchema: z.object({
    projectId: z
      .string()
      .describe(
        "Project ID to fetch people from. Required - no default project context available.",
      ),
    peopleSearch: z
      .string()
      .nullish()
      .describe(
        "Case-insensitive search string to match people by name or display name.",
      ),
    peopleLimit: z
      .number()
      .int()
    ...
    includeEvidence: z
      .boolean()
      .nullish()
      .describe("Whether to include actual evidence snippets from interviews."),
    includePersonas: z
      .boolean()
      .nullish()
      .describe("Whether to include persona assignments for each person."),
    includeFacets: z
      .boolean()
      .nullish()
      .describe(
        "Whether to include facet attributes and scale measurements for each person.",
      ),
    specificPersonIds: z
      .array(z.string())
      .nullish()
      .describe("Specific person IDs to fetch details for."),
  }),
  ...
  execute: async (input, context?) => {
    ...
    try {
      ...
      if (peopleError) {
        consola.error(
          "fetch-people-details: failed to fetch people",
          peopleError,
        );
        throw peopleError;
      }
      ...
      if (sanitizedPersonSearch) {
        ...
        if (people.length === 0 && accountId) {
          consola.debug(
            "fetch-people-details: no project matches, expanding to account scope",
            {
              projectId,
              accountId,
              searchTerm: sanitizedPersonSearch,
          ...
          if (accountQuery.data && accountQuery.data.length > 0) {
            const accountPeople = (accountQuery.data as Person[]).filter(
              (person) => {
                const nameMatch = person.name
                  ?.toLowerCase()
                  .includes(searchLower);
                const titleMatch = person.title
                  ?.toLowerCase()
                  .includes(searchLower);
                const orgName = (person as any).default_organization?.name;
                const companyMatch = orgName
                  ?.toLowerCase()
                  .includes(searchLower);
                const roleMatch = person.role
                  ?.toLowerCase()
                  .includes(searchLower);
                return nameMatch || titleMatch || companyMatch || roleMatch;
              },
            ...
            if (accountPeople.length > 0) {
              ...
              searchScope = "account";
              consola.debug(
                "fetch-people-details: found matches at account scope",
                {
      ...
      if (specificPersonIds.length > 0) {
        people = people.filter((person) =>
          specificPersonIds.includes(person.id),
        );
      }
      ...
      const [personaData, interviewData, evidenceData, facetData, scaleData] =
        await Promise.all([
      ...
      if (evidenceData.data) {
        for (const evidence of evidenceData.data as EvidenceRow[]) {
          ...
          const interviewPeople = (
            interviewData.data as InterviewPeopleRow[]
          )?.find((ip) => ip.interview?.id === evidence.interview_id);
          if (interviewPeople) {
            const existing =
              evidenceByPerson.get(interviewPeople.person_id) ?? [];
      ...
      const result = people
        .map((person) => {
          ...
          return {
            ...
            contactInfo: person.contact_info as z.infer<
            ...
            personas: includePersonas
              ? personPersonas.map((pp) => ({
                  ...
                  url:
                    projectPath && pp.personas?.id
                      ? `${HOST}${routes.personas.detail(pp.personas.id)}`
                      : null,
                }))
              : undefined,
            ...
            evidence: includeEvidence
              ? personEvidence.slice(0, 20).map((ev) => ({
                  ...
                  url: projectPath
                    ? `${HOST}${routes.evidence.detail(ev.id)}`
                    : null,
                }))
              : undefined,
