...
- project: Projects themselves`,
  inputSchema: z.object({
    ...
    entityType: entityTypeEnum
      .nullish()
      .describe(
        "Type of entity to annotate (person, organization, opportunity, interview, persona, insight, project)",
      ),
    ...
    annotationType: annotationTypeEnum
      .nullish()
      .describe(
        "Type of annotation: note, comment, todo, ai_suggestion, flag, reaction",
      ),
    content: z
      .string()
      .nullish()
  ...
  execute: async (input, context?) => {
    ...
    const projectId = (input.projectId ?? runtimeProjectId ?? null) as
      | string
      | null;
    const accountId = (input.accountId ?? runtimeAccountId ?? null) as
      | string
      | null;
    ...
    if (!accountId) {
      return {
        success: false,
        message:
          "Missing accountId. Pass one explicitly or ensure the runtime context sets account_id.",
        annotation: null,
      };
    }
      ...
      if (operation === "create") {
        if (!entityType || !entityId || !annotationType) {
          return {
            success: false,
            message:
              "Create operation requires entityType, entityId, and annotationType",
            annotation: null,
          };
        }
