...
const columnMappingSchema = z.object({
  ...
  name: z
    .string()
    .nullish()
    .describe("Column name containing deal/opportunity name"),
  ...
  amount: z
    .string()
    .nullish()
    .describe("Column name containing deal amount/value"),
  ...
  pipeline: z
    .string()
    .nullish()
    .describe("Column name containing pipeline name"),
  forecast_category: z
    .string()
    .nullish()
    .describe("Column name containing forecast category"),

  // Dates
  close_date: z
    .string()
    .nullish()
    .describe("Column name containing expected close date"),
  created_date: z
    .string()
    .nullish()
    .describe("Column name containing created date"),

  // Probability/Confidence
  probability: z
    .string()
    .nullish()
    .describe("Column name containing win probability"),
  confidence: z
    .string()
    .nullish()
    .describe("Column name containing confidence score"),

  // Related entities
  account: z
    .string()
    .nullish()
    .describe("Column name containing account/company name"),
  contact: z
    .string()
    .nullish()
    .describe("Column name containing primary contact name"),
  contact_email: z
    .string()
    .nullish()
    .describe("Column name containing contact email"),
  ...
  description: z
    .string()
    .nullish()
    .describe("Column name containing description"),
  ...
  crm_id: z
    .string()
    .nullish()
    .describe("Column name containing external CRM ID"),
});
...
Requires the assetId from a previous parseSpreadsheet call.`,
  inputSchema: z.object({
    assetId: z
      .string()
      .uuid()
      .describe("The project_asset ID from parseSpreadsheet result"),
    columnMapping: columnMappingSchema
      .nullish()
      .describe(
        "Optional explicit column mappings. If not provided, auto-detects.",
      ),
    ...
    defaultStage: z
      .string()
      .nullish()
      .describe("Default stage for opportunities without a stage column"),
    defaultCurrency: z
      .string()
      .nullish()
      .transform((v) => v ?? "USD")
  ...
  outputSchema: z.object({
    ...
    organizationsCreated: z
      .number()
      .describe("Number of new organizations created"),
    results: z
      .array(
        z.object({
    ...
    try {
      ...
      if (!accountId || !projectId) {
        return {
          success: false,
          message:
            "Missing account or project context. Cannot import opportunities.",
          imported: 0,
          skipped: 0,
      ...
      consola.info(
        `[import-opportunities] Processing ${rows.length} rows from asset ${assetId}`,
      );
      ...
      if (!titleColumn) {
        return {
          success: false,
          message:
            "Could not detect opportunity name/title column. Please provide column mapping.",
          imported: 0,
          skipped: 0,
          organizationsCreated: 0,
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          if (skipDuplicates && crmId && existingCrmIds.has(crmId)) {
            consola.debug(
              `[import-opportunities] Skipping duplicate CRM ID: ${crmId}`,
            );
            skipped++;
            continue;
          }
          ...
          const confidence = parseProbability(
            getValue(row, mapping.probability) ||
              getValue(row, mapping.confidence),
          );
            ...
            if (orgCache.has(accountName)) {
              ...
              if (existingOrg) {
                ...
                if (newOrg && !orgError) {
                  ...
                  consola.debug(
                    `[import-opportunities] Created organization: ${accountName}`,
                  );
                }
              }
            }
