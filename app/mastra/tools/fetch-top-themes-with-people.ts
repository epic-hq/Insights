...
export const fetchTopThemesWithPeopleTool = createTool({
  ...
  inputSchema: z.object({
    projectId: z
      ...
      .nullish()
      .describe(
        "Project ID to analyze. Defaults to runtime context project_id.",
      ),
    limit: z
      .number()
      .int()
      ...
      .max(10)
      .nullish()
      .describe("How many top themes to return. Default 2."),
    peoplePerTheme: z
      .number()
      .int()
      .min(1)
  ...
  execute: async (input, context?) => {
    ...
    try {
      ...
      for (const row of (themeLinks ?? []) as ThemeLinkRow[]) {
        ...
        const existing =
          evidenceIdsByTheme.get(row.theme_id) ?? new Set<string>();
      ...
      const [facetPeopleResult, evidencePeopleResult, interviewPeopleResult] =
        await Promise.all([
          selectedEvidenceIdList.length
            ? supabase
                .from("evidence_facet")
      ...
      for (const row of (facetPeopleResult.data ?? []) as PersonLinkRow[]) {
        ...
        const existing =
          evidenceToPersons.get(row.evidence_id) ?? new Set<string>();
      ...
      for (const row of (interviewPeopleResult.data ??
        []) as InterviewPersonRow[]) {
        if (!row.interview_id || !row.person_id) continue;
        const existing =
          interviewToPersons.get(row.interview_id) ?? new Set<string>();
        existing.add(row.person_id);
      ...
      const topThemes = selectedThemes.map((theme) => {
        const evidenceIds =
          evidenceIdsByTheme.get(theme.id) ?? new Set<string>();
        ...
        for (const evidenceId of evidenceIds) {
          ...
          for (const personId of personIds) {
            personMentions.set(
              personId,
              (personMentions.get(personId) ?? 0) + 1,
            );
          }
        }
