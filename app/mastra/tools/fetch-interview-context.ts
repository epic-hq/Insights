...
function buildEmpathyMap(
  evidence: EvidenceRow[] | null | undefined,
): EmpathyMap {
...
function summarizePersonalFacets(
  participants: InterviewParticipantRow[],
): Array<{
...
export const fetchInterviewContextTool = createTool({
  ...
  inputSchema: z.object({
    interviewId: z
      ...
      .nullish()
      .describe(
        "Interview ID to load. Defaults to the interview ID in the runtime context.",
      ),
    includeEvidence: z
      .boolean()
      .nullish()
      .describe(
        "Set to false to omit evidence details when only high-level insights are required.",
      ),
    evidenceLimit: z
      .number()
      .int()
  ...
  execute: async (input, context?) => {
    ...
    const interviewId = validateUUID(
      input.interviewId || runtimeInterviewId,
      "interviewId",
    ...
    if (!projectId) {
      ...
      return {
        success: false,
        message:
          "Missing project_id in runtime context. This tool requires project scope to load interview data.",
      };
    }

    try {
      const [interviewResult, participantResult, themesResult, evidenceResult] =
        await Promise.all([
          supabase
            .from("interviews")
            .select(
      ...
      if (!interview) {
        return {
          success: false,
          message:
            "Interview not found or inaccessible for the current project.",
        };
      }
      ...
      const evidence = includeEvidence
        ...
        : null;
      const empathyMap = includeEvidence
        ? buildEmpathyMap(evidence)
        : emptyEmpathyMap();
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Unknown error fetching interview context",
      };
    }
  },
});
