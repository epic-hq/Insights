	...
	execute: async (input, context?) => {
		...
		try {
			...
			if (mode === "analyses" || mode === "both") {
				...
				const uniqueInterviewIds = [...new Set((analysesData || []).map((a) => a.interview_id))];
				let interviewMap = new Map<string, { title: string | null; personName: string | null }>();
				...
				if (uniqueInterviewIds.length > 0) {
					...
					const personIds = (interviewsData || []).map((i) => i.person_id).filter((id): id is string => !!id);
					const uniquePersonIds = [...new Set(personIds)];

					let personNameMap = new Map<string, string>();
					if (uniquePersonIds.length > 0) {
						const { data: peopleData } = await supabase.from("people").select("id, name").in("id", uniquePersonIds);
						personNameMap = new Map((peopleData || []).map((p) => [p.id, p.name ?? ""]));
					}
					...
					interviewMap = new Map(
						(interviewsData || []).map((i) => [
							...
							{
								title: i.title,
								personName: i.person_id ? (personNameMap.get(i.person_id) ?? null) : null,
							},
						])
					);
				}
				...
				result.analyses = (analysesData || []).map((a) => {
					...
					const lensDetailUrl = routes ? `${HOST}${routes.lenses.byTemplateKey(a.template_key)}` : null;
