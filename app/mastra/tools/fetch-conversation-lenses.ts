...
const lensAnalysisSchema = z.object({
  ...
  lensDetailUrl: z.string().nullable(),
});
...
export const fetchConversationLensesTool = createTool({
  ...
  inputSchema: z.object({
    ...
    projectId: z
      ...
      .nullish()
      .describe(
        "Project ID to scope the search. Defaults to current project in context.",
      ),
    interviewId: z
      .string()
      .nullish()
      .describe(
        "Filter analyses by specific interview ID. Only applies when mode includes 'analyses'.",
      ),
    templateKey: z
      .string()
      .nullish()
    ...
    status: z
      ...
      .nullish()
      .describe(
        "Filter analyses by processing status. Only applies when mode includes 'analyses'.",
      ),
    limit: z
      .number()
      .int()
      ...
      .max(100)
      .nullish()
      .describe("Maximum number of analyses to return. Default: 50"),
  }),
  ...
  execute: async (input, context?) => {
    ...
    try {
      ...
      if (mode === "templates" || mode === "both") {
        ...
        if (templatesError) {
          consola.error(
            "fetch-conversation-lenses: error fetching templates",
            templatesError,
          );
        ...
        result.templates = (templatesData || []).map((t) => {
          ...
          return {
            ...
            requiresProjectContext:
              definition?.requires_project_context ?? false,
            recommendationsEnabled:
              definition?.recommendations_enabled ?? false,
          };
        });
      ...
      if (mode === "analyses" || mode === "both") {
        if (!projectId && !interviewId) {
          consola.warn(
            "fetch-conversation-lenses: missing projectId or interviewId for analyses",
          );
          return {
            success: false,
            message:
        ...
        if (analysesError) {
          consola.error(
            "fetch-conversation-lenses: error fetching analyses",
            analysesError,
          );
          throw analysesError;
        }
        ...
        const uniqueTemplateKeys = [
          ...new Set((analysesData || []).map((a) => a.template_key)),
        ];
        ...
        const templateNameMap = new Map(
          (templatesForAnalyses || []).map((t) => [
            t.template_key,
            t.template_name,
          ]),
        );

        // Fetch interview titles and person names for analyses
        const uniqueInterviewIds = [
          ...new Set((analysesData || []).map((a) => a.interview_id)),
        ];
        let interviewMap = new Map<
          string,
          { title: string | null; personName: string | null }
        >();
        ...
        if (uniqueInterviewIds.length > 0) {
          const { data: interviewsData } = await supabase
            ...
            .in("id", uniqueInterviewIds);

          // Collect person IDs to fetch names
          const personIds = (interviewsData || [])
            .map((i) => i.person_id)
          ...
          interviewMap = new Map(
            (interviewsData || []).map((i) => [
              ...
              {
                title: i.title,
                personName: i.person_id
                  ? (personNameMap.get(i.person_id) ?? null)
                  : null,
              },
            ]),
          );
        ...
        const projectPath =
          runtimeAccountId && projectId
            ? `/a/${runtimeAccountId}/${projectId}`
            : "";
        const routes = projectPath ? createRouteDefinitions(projectPath) : null;

        result.analyses = (analysesData || []).map((a) => {
          const interviewInfo = interviewMap.get(a.interview_id);
          const personName = interviewInfo?.personName ?? null;
          const interviewTitle = interviewInfo?.title ?? null;
          const lensDetailUrl = routes
            ? `${HOST}${routes.lenses.byTemplateKey(a.template_key)}`
            : null;
          ...
          return {
            ...
            interviewUrl: routes
              ? `${HOST}${routes.interviews.detail(a.interview_id)}`
              : null,
            interviewTitle,
            personName,
            lensDetailUrl,
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Unexpected error fetching conversation lenses.";
      return {
        success: false,
