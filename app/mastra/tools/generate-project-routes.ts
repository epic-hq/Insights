...
export const generateProjectRoutesTool = createTool({
	...
	inputSchema: {
		...
		properties: {
			...
			entityId: {
				type: "string",
				description: "The ID of the entity (for survey_response, this is the responseId)",
			},
			parentEntityId: {
				type: "string",
				description: "Parent entity ID for nested resources (e.g., surveyId for survey_response)",
			},
			action: {
				type: "string",
				enum: ["detail", "edit"],
