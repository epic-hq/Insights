...
export const fetchWebContentTool = createTool({
  ...
  inputSchema: z.object({
    ...
    includeMedia: z
      ...
      .transform((val) => val ?? false)
      .describe(
        "Whether to extract and return media URLs (video/audio) from the page. Default: false",
      ),
    maxContentLength: z
      .number()
      .nullish()
      ...
      .describe(
        `Maximum length of text content to return. Default: ${MAX_CONTENT_LENGTH}`,
      ),
  }),
  ...
  execute: async (input) => {
    ...
    try {
      ...
      consola.info(
        `[fetchWebContent] Successfully fetched ${url}, content length: ${textContent.length}`,
      );
      return result;
    } catch (error) {
