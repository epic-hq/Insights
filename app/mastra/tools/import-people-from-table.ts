...
5. Return summary with created/updated/skipped counts`,
  ...
  outputSchema: z.object({
    ...
    imported: z.object({
      ...
      surveyResponses: z
        ...
        .nullish()
        .describe(
          "Number of survey Q&A pairs created as evidence_facet records",
        ),
      skipped: z
        .number()
  ...
  execute: async (input, context?) => {
    try {
      ...
      const surveyColumns = allFacetColumns.filter(
        (fc) => fc.facetKind === "survey_response",
      );
      const regularFacetColumns = allFacetColumns.filter(
        (fc) => fc.facetKind !== "survey_response",
      );
      ...
      if (columnMapping) {
        ...
        if (Object.keys(cleanedColumnMapping).length === 0) {
          ...
          consola.warn(
            "[import-people] columnMapping had only null values - using auto-detection instead",
          );
        }
      }
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!existingPersonId && (firstname || fullName)) {
          ...
          if (existingByName) {
            ...
            consola.info(
              `[import-people] Row ${i}: Found existing person by name+organization: ${displayName}`,
            );
          }
        }
        ...
        if (existingPersonId) {
          ...
          if (organizationId) {
            await (supabase as any).from("people_organizations").insert({
              ...
              job_title:
                getValue(row, mapping.title) || getValue(row, mapping.role),
              is_primary: true,
            });
          }
        }
      ...
      if (facetObservationsByPerson.length > 0 && projectId) {
        try {
          ...
          const facetsByPersonId = new Map<
            string,
            Map<string, FacetObservation>
          >();
          ...
          const deduplicatedObservations = Array.from(
            facetsByPersonId.entries(),
          ).map(([personId, facetMap]) => ({
            personId,
          ...
          facetsCreated = deduplicatedObservations.reduce(
      ...
      if (surveyColumns.length > 0 && importedDetails.length > 0 && projectId) {
        try {
          ...
          const assetTitle =
            (asset as { title?: string }).title || "Imported Survey";
          const { data: interview, error: interviewError } = await (
            supabase as any
          )
            .from("interviews")
            .insert({
          ...
          if (interviewError || !interview) {
            consola.error(
              "[import-people] Failed to create survey interview:",
              interviewError,
            );
          } else {
            const interviewId = interview.id;
            consola.info(
              `[import-people] Created survey interview: ${interviewId}`,
            );
            ...
            if (!surveyKindId) {
              consola.error(
                "[import-people] survey_response facet kind not found",
              );
            } else {
              ...
              for (const surveyCol of surveyColumns) {
                ...
                if (existingFacet) {
                  ...
                  const { data: newFacet, error: facetError } = await (
                    supabase as any
                  )
                    .from("facet_account")
                    .insert({
              ...
              if (evidenceRows.length > 0) {
                ...
                const { data: insertedEvidence, error: evidenceError } = await (
                  supabase as any
                )
                  .from("evidence")
                  .insert(evidenceRows)
                ...
                if (evidenceError) {
                  consola.error(
                    "[import-people] Failed to insert evidence:",
                    evidenceError,
                  );
                } else if (insertedEvidence && insertedEvidence.length > 0) {
                  const evidenceIds = insertedEvidence.map(
                    (e: { id: string }) => e.id,
                  );
                  ...
                  for (const detail of importedDetails) {
                    ...
                    for (const surveyCol of surveyColumns) {
                      const answer = getValue(row, surveyCol.column);
                      if (answer) {
                        const facetAccountId = questionToFacetAccountId.get(
                          surveyCol.column,
                        );
                        if (facetAccountId) {
                  ...
                  if (interviewPeopleRows.length > 0) {
                    ...
                    if (ipError) {
                      consola.warn(
                        "[import-people] Failed to link interview to people:",
                        ipError,
                      );
                    }
                  }
                  ...
                  if (evidenceFacetRows.length > 0) {
                    const { error: efError } = await (supabase as any)
                      .from("evidence_facet")
                      .insert(evidenceFacetRows);

                    if (efError) {
                      consola.error(
                        "[import-people] Failed to insert evidence_facet:",
                        efError,
                      );
                    } else {
                      consola.info(
          ...
          consola.error(
            "[import-people] Failed to process survey responses:",
            surveyError,
          );
        }
      }
      ...
      return {
        ...
        imported: {
          ...
          surveyResponses:
            surveyResponsesCreated > 0 ? surveyResponsesCreated : undefined,
          skipped,
        },
