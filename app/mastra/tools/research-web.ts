...
export const webResearchTool = createTool({
  ...
  inputSchema: z.object({
    ...
    numResults: z
      ...
      .default(5)
      .describe(
        "Number of results to return (1-10). Use higher values instead of multiple separate searches.",
      ),
    type: z
      .enum(["neural", "keyword", "auto"])
      .default("auto")
      .describe(
        "Search type: neural (semantic), keyword (traditional), or auto",
      ),
    useAutoprompt: z
      .boolean()
      .default(true)
  ...
  outputSchema: z.object({
    ...
    evidenceCount: z
      .number()
      .optional()
      .describe("Number of evidence records created"),
  ...
  execute: async (input, context?) => {
    ...
    try {
      const { query, numResults, type, useAutoprompt, includeText, category } =
        input;
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        if (attempt > 0) {
          ...
          consola.info(
            `[web-research] Rate limited, waiting ${waitTime}ms before retry ${attempt + 1}`,
          );
          ...
          if (noteError) {
            ...
            for (const result of results) {
              try {
                ...
                if (evidenceError) {
                  consola.error(
                    "[web-research] Failed to save evidence:",
                    evidenceError,
                  );
                } else {
                  evidenceCount++;
                ...
                consola.error(
                  "[web-research] Error creating evidence:",
                  evidenceErr,
                );
              }
            }
            ...
            consola.info(
              `[web-research] Created ${evidenceCount} evidence records`,
            );
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        if (attempt > 0) {
          ...
          consola.info(
            `[find-similar] Rate limited, waiting ${waitTime}ms before retry ${attempt + 1}`,
          );
          await delay(waitTime);
        }
          ...
          if (noteError) {
            ...
            for (const result of results) {
              try {
                ...
                if (evidenceError) {
                  consola.error(
                    "[find-similar] Failed to save evidence:",
                    evidenceError,
                  );
                } else {
                  evidenceCount++;
                ...
                consola.error(
                  "[find-similar] Error creating evidence:",
                  evidenceErr,
                );
              }
            }
            ...
            consola.info(
              `[find-similar] Created ${evidenceCount} evidence records`,
            );
