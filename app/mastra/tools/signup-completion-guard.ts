...
const OutputSchema = z.object({
	...
	next_question_key: z.enum(["problem", "need_to_learn", "challenges", "content_types"]).nullable(),
...
export const signupCompletionGuardTool = createTool({
	...
	execute: async (input, _context) => {
		...
		const questions: Record<string, string> = {
			...
			need_to_learn: "What do you need to learn to help you achieve that goal?",
		...
		return {
			completed: false,
			missing,
			next_question_key: nextKey as "problem" | "need_to_learn" | "challenges" | "content_types",
