...
const optionSchema = z.object({
	...
	description: z.string().optional().describe("Optional helper text shown below the label"),
});
...
export const requestUserInputTool = createTool({
	...
	inputSchema: z.object({
		prompt: z.string().describe("The question or instruction shown to the user above the options"),
		options: z.array(optionSchema).min(2).max(10).describe("The options the user can choose from (2-10 items)"),
		selectionMode: z
			.enum(["single", "multiple"])
			.default("single")
			.describe("Whether the user picks one option (radio) or multiple (checkboxes)"),
		allowFreeText: z
			.boolean()
			.default(true)
			.describe("Whether to show a free-text input as an alternative to the predefined options"),
	}),
	outputSchema: z.object({
		...
		userInput: z
			...
			.describe("Payload detected by the chat renderer to show inline input UI"),
	}),
	execute: async (input) => {
