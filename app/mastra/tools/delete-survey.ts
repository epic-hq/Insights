...
Use hardDelete=true only when explicitly requested to permanently remove the survey and all its responses.`,
  inputSchema: z.object({
    ...
    hardDelete: z
      ...
      .default(false)
      .describe(
        "If true, permanently deletes the survey and all responses. Default is false (soft delete/archive).",
      ),
    reason: z
      .string()
      .nullish()
      .describe(
        "Optional reason for deletion (stored in description for soft delete)",
      ),
  }),
    ...
    try {
      ...
      if (input.hardDelete) {
        ...
        if (responsesDeleteError) {
          consola.error(
            "delete-survey: responses delete error",
            responsesDeleteError,
          );
          return {
            success: false,
        ...
        if (surveyDeleteError) {
          consola.error(
            "delete-survey: survey delete error",
            surveyDeleteError,
          );
          return {
            success: false,
