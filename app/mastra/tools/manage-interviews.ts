...
const toolInputSchema = z.object({
  ...
  titleSearch: z
    .string()
    .nullish()
    .describe("Optional case-insensitive title search for list"),
  limit: z
    .number()
    .int()
    .min(1)
    .max(200)
    .nullish()
    .describe("Max rows for list (default 50)"),
  dryRun: z
    .boolean()
    .nullish()
    .describe("For delete: return what would be deleted without mutating"),
  force: z
    .boolean()
    .nullish()
    .describe("For delete: proceed even if linked records exist"),
  confirmTitle: z
    .string()
    .nullish()
    .describe(
      "For delete (non-dryRun): must match the interview's current title exactly (case-insensitive)",
    ),
});
...
export const manageInterviewsTool = createTool({
  ...
  execute: async (input, context?) => {
    ...
    const runtime_account_id = context?.requestContext?.get?.("account_id") as
      | string
      | undefined;
    const runtime_project_id = context?.requestContext?.get?.("project_id") as
      | string
      | undefined;

    const resolved_account_id = validateUUID(
      account_override || runtime_account_id,
      "accountId",
      "manage-interviews",
    );
    const resolved_project_id = validateUUID(
      project_override || runtime_project_id,
      "projectId",
      "manage-interviews",
    );
    const validated_interview_id = interviewId
      ? validateUUID(interviewId, "interviewId", "manage-interviews")
      : null;
      ...
      if (action === "get") {
        if (!validated_interview_id) {
          return {
            success: false,
            message:
              "interviewId is required for get and must be a valid UUID.",
            interview: null,
          };
        }
      ...
      if (action === "delete") {
        if (!validated_interview_id) {
          return {
            success: false,
            message:
              "interviewId is required for delete and must be a valid UUID.",
            interview: null,
          };
        }
        ...
        const linked_counts = await countLinks(
          supabase,
          interview_row.id,
          resolved_project_id,
        );
        const total_links =
          linked_counts.evidence + linked_counts.interview_people;
        ...
        if (
          !expected_title ||
          !provided_title ||
          expected_title.toLowerCase() !== provided_title.toLowerCase()
        ) {
          return {
            success: false,
