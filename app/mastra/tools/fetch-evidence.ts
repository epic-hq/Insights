...
export const fetchEvidenceTool = createTool({
  ...
  inputSchema: z.object({
    projectId: z
      ...
      .nullish()
      .describe(
        "Project ID to fetch evidence from. Defaults to the current project in context.",
      ),
    interviewId: z
      .string()
      .nullish()
      .describe("Filter evidence by specific interview ID."),
    personId: z
      .string()
      .nullish()
      .describe("Filter evidence by specific person ID."),
    evidenceSearch: z
      .string()
      .nullish()
      .describe(
        "Case-insensitive search string to match evidence content (gist, verbatim, chunk).",
      ),
    evidenceLimit: z
      .number()
      .int()
  ...
  execute: async (input, context?) => {
    ...
    const projectId = validateUUID(
      input.projectId || runtimeProjectId,
      "projectId",
      "fetch-evidence",
    );
    ...
    const interviewId = validateUUID(
      input.interviewId,
      "interviewId",
      "fetch-evidence",
    );
    ...
    const sanitizedEvidenceSearch = evidenceSearch
      .replace(/[%*"'()]/g, "")
      .trim();
    ...
    try {
      ...
      if (evidenceError) {
        consola.error(
          "fetch-evidence: failed to fetch evidence",
          evidenceError,
        );
        throw evidenceError;
      }
      ...
      if (personId && evidenceIds.length > 0) {
        ...
        if (personEvidenceIds) {
          const interviewIds = personEvidenceIds
            .map((ip) => ip.interview_id)
            .filter(Boolean);
          evidenceRows = evidenceRows.filter(
            (row) =>
              row.interview_id && interviewIds.includes(row.interview_id),
          );
        }
      }
      ...
      if (insightsData?.data) {
        const insightsWithInterviewId = (insightsData.data as Insight[]).filter(
          (insight): insight is Insight & { interview_id: string } =>
            Boolean(insight.interview_id),
        );
        for (const insight of insightsWithInterviewId) {
          const existing =
            insightsByInterviewId.get(insight.interview_id) ?? [];
          existing.push({
            id: insight.id,
            name: insight.name,
      ...
      const projectPath =
        accountId && projectIdStr ? `/a/${accountId}/${projectIdStr}` : "";
      const routes = projectPath ? createRouteDefinitions(projectPath) : null;

      // Build the final result
      const evidence = evidenceRows.map((row) => {
        const person = row.interview_id
          ? (peopleByInterviewId.get(row.interview_id)?.[0] ?? null)
          : null;
        return {
          id: row.id,
          ...
          interviewTitle: row.interview_id
            ? (interviewsById.get(row.interview_id)?.title ?? null)
            : null,
          interviewDate: row.interview_id
            ? (interviewsById.get(row.interview_id)?.interviewDate ?? null)
            : null,
          interviewStatus: row.interview_id
            ? (interviewsById.get(row.interview_id)?.status ?? null)
            : null,
          personName: person?.name ?? null,
          personId: person?.id ?? null,
          personRole: person?.role ?? null,
          insightCount: row.interview_id
            ? (insightsByInterviewId.get(row.interview_id)?.length ?? 0)
            : 0,
          url: routes ? `${HOST}${routes.evidence.detail(row.id)}` : null,
          interviewUrl:
            routes && row.interview_id
              ? `${HOST}${routes.interviews.detail(row.interview_id)}`
              : null,
          personUrl:
            routes && person?.id
              ? `${HOST}${routes.people.detail(person.id)}`
              : null,
        };
      });
