    ...
    try {
      ...
      const evidence = evidenceRows.map((row) => ({
        ...
        interviewStatus: row.interview_id
          ...
          : null,
        personName: row.interview_id
          ? (peopleByInterviewId.get(row.interview_id)?.[0]?.name ?? null)
          : null,
        personRole: row.interview_id
          ? (peopleByInterviewId.get(row.interview_id)?.[0]?.role ?? null)
          : null,
