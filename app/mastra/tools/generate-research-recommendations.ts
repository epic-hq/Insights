...
const RecommendationSchema = z.object({
  ...
  reasoning: z
    .string()
    .describe("Why this recommendation is important, with evidence context"),
  confidence_current: z
    .number()
    .optional()
    .describe("Current confidence score 0-1"),
  confidence_target: z
    .number()
    .optional()
    .describe("Target confidence score 0-1"),
  evidence_refs: z
    .array(
      z.object({
  ...
  action_data: z
    .record(z.unknown())
    .optional()
    .describe("Data needed to execute action"),
...
Returns recommendations sorted by priority (1 = highest) with full reasoning and evidence links.`,
  inputSchema: z.object({
    projectId: z
      .string()
      .describe("Project ID to generate recommendations for"),
    accountId: z
      .string()
      .optional()
      .describe("Account ID (can be extracted from context)"),
  ...
  execute: async (input, context?) => {
    ...
    const accountId =
      input.accountId ?? context?.requestContext?.get?.("account_id");
    ...
    try {
      ...
      const { data: unansweredQuestions, error: questionsError } =
        await supabase
      ...
      const { data: recentInterviewPeople, error: interviewPeopleError } =
        await supabase
      ...
      if (recentInterviewPeople) {
        ...
        for (const ip of recentInterviewPeople) {
          ...
          if (
            !personLastInterviewMap.has(personId) ||
            interviewDate > personLastInterviewMap.get(personId)!
          ) {
        ...
        for (const [personId, lastDate] of personLastInterviewMap.entries()) {
          ...
          if (daysSince >= 14) {
            const personData = recentInterviewPeople.find(
              (ip) => ip.person_id === personId,
            )?.people;
            if (personData) {
              staleContacts.push({
                person_id: personId,
      ...
      const lowConfidenceThemes = themesWithConfidence.filter(
        (t) => t.confidence < 0.8 && t.evidence_count < 10,
      );
      ...
      const staleHighICPContacts = staleContacts.filter((contact) => {
        const icpScore = icpScores?.find(
          (s) => s.person_id === contact.person_id,
        );
      ...
      if (staleHighICPContacts.length > 0) {
        ...
        const icpScore = icpScores?.find(
          (s) => s.person_id === topStaleContact.person_id,
        );
      ...
      if (icpScores && icpScores.length > 0) {
        ...
        const neverInterviewedHighICP = icpScores.filter((icp) => {
          const hasInterview = recentInterviewPeople?.some(
            (ip) => ip.person_id === icp.person_id,
          );
          return !hasInterview;
        });
