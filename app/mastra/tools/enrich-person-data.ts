...
export const enrichPersonDataTool = createTool({
  ...
  inputSchema: z.object({
    ...
    accountId: z
      .string()
      .nullish()
      .describe("Account ID (from context if not provided)"),
    projectId: z
      .string()
      .nullish()
      .describe("Project ID (from context if not provided)"),
    writeToDb: z
      .boolean()
      .nullish()
      .transform((v) => v ?? true)
      .describe(
        "Whether to persist enriched data to the database (default: true)",
      ),
  }),
  ...
  execute: async (input, context?) => {
    ...
    const projectId =
      input.projectId || context?.requestContext?.get?.("project_id");
    ...
    if (input.writeToDb && result.enriched) {
      ...
      if (Object.keys(updates).length > 0) {
        ...
        if (updateError) {
          consola.error(
            `[enrich-person-data] Failed to update person ${person.id}:`,
            updateError,
          );
          return {
            success: false,
        ...
        consola.success(
          `[enrich-person-data] Updated person ${person.id}:`,
          Object.keys(updates),
        );
      }
    }
