...
const toolInputSchema = z
  .object({
    ...
    transcript: z
      ...
      .optional()
      .describe(
        "Natural language text describing one or more organization relationships.",
      ),
    ...
    defaultIsPrimary: z
      ...
      .optional()
      .describe(
        "Fallback flag applied when a relationship omits isPrimary (defaults to false).",
      ),
...
async function ensureOrganization(
  ...
  if (error)
    throw new Error(
      `Failed to create organization "${trimmed}": ${error.message}`,
    );
...
export const managePersonOrganizationsTool = createTool({
  ...
  execute: async (input, context?) => {
    ...
    const defaultIsPrimary =
      typeof (input as ToolInput).defaultIsPrimary === "boolean"
        ? (input as ToolInput).defaultIsPrimary
        : false;
    ...
    try {
      ...
      const resolvedAccountId =
        accountOverride || runtimeAccountId || person.account_id || "";
      const resolvedProjectId =
        projectOverride || runtimeProjectId || person.project_id || null;
      ...
      if (Array.isArray(relationships) && relationships.length > 0) {
        ...
        const extraction = await extractRelationships(
          person.name ?? "",
          transcript.trim(),
        );
      ...
      if (!normalizedRelationships.length) {
        return {
          success: false,
          message:
            "No organization relationships were detected in the provided input.",
          personId,
      ...
      for (const rel of normalizedRelationships) {
        try {
          ...
          if (
            ...
            !createdOrganizations.find((org) => org.id === organization.id)
          ) {
          ...
          consola.warn(
            "manage-person-organizations: failed to process relationship",
            {
              personId,
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to update person organizations.",
        personId,
        accountId: accountOverride ?? runtimeAccountId ?? null,
        projectId: projectOverride ?? runtimeProjectId ?? null,
      };
