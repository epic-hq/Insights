...
- industry: e.g., "B2B SaaS"`,
	...
	execute: async (input, context?) => {
		...
		if (input.account_id && input.account_id !== "current") {
			...
			consola.info("[saveAccountCompanyContext] Using input account_id:", accountId);
		}
		...
		if (!accountId && input.project_id) {
			try {
				...
				if (project?.account_id) {
					...
					consola.info("[saveAccountCompanyContext] Resolved from project:", accountId);
				}
			} catch (err) {
				consola.warn("[saveAccountCompanyContext] Failed to resolve from project:", err);
			}
		}
		...
		if (!accountId) {
			try {
				accountId = await resolveAccountId(context, "saveAccountCompanyContext");
			} catch {
		...
		if (input.company_description) updateData.company_description = input.company_description;
		if (input.customer_problem) updateData.customer_problem = input.customer_problem;
		...
		consola.info("[saveAccountCompanyContext] Saving to account:", accountId, Object.keys(updateData));

		try {
			const { error } = await supabaseAdmin.schema("accounts").from("accounts").update(updateData).eq("id", accountId);
