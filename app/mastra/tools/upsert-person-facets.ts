...
const extractionSchema = z.object({
  summary: z
    .string()
    .describe("One or two sentence recap of the participant facts."),
  facets: z
    .array(
      z.object({
        label: z.string(),
...
const toolInputSchema = z
  .object({
    ...
    projectId: z
      ...
      .nullish()
      .describe(
        "Project context for the facet rows. Defaults to the runtime or person record project.",
      ),
    accountId: z
      .string()
      .nullish()
      .describe(
        "Account context. Defaults to runtime headers or the person record.",
      ),
    transcript: z
      .string()
      .nullish()
      .describe(
        "Free-form text or transcript describing the person. Used when explicit facets are not provided.",
      ),
    facts: z
      .array(manualFacetSchema)
      .nullish()
      .describe("Optional structured facet facts to apply directly."),
    source: z
      .string()
      .nullish()
      .describe(
        "Source label to store on person_facet rows. Defaults to 'voice_note'.",
      ),
    mode: z
      .enum(["merge", "replace"])
      .nullish()
      .describe(
        "Merge keeps existing facets; replace removes unspecified ones after applying the new set.",
      ),
    confidence: z
      .number()
      .min(0)
      .max(1)
      .nullish()
      .describe("Override default confidence for added facets."),
    dryRun: z
      .boolean()
      .nullish()
      .describe("When true, returns the plan without writing to the database."),
  })
  .refine(
    (value) => {
      const hasTranscript = Boolean(
        value.transcript && value.transcript.trim().length > 0,
      );
    ...
    {
      message:
        "Provide either a transcript or at least one structured facet fact.",
      path: ["transcript"],
    },
  );
...
function resolveSourceLabel(rawSource?: string | null): {
  ...
  const candidate = normalizeLabel(rawSource)
    ?.toLowerCase()
    .replace(/\s+/g, "_");
...
async function fetchExistingFacets(
  ...
  if (error)
    throw new Error(`Failed to load existing person facets: ${error.message}`);
  return data ?? [];
}
...
async function runExtraction({
  ...
  const filtered = (object.facets ?? []).filter((facet) =>
    allowedKindSlugs.has(facet.kindSlug),
  );
  return {
...
function normalizeManualFacets(facts: ManualFacet[] = []): ManualFacet[] {
  return facts
    ...
    .filter(
      (fact) =>
        fact.action === "remove" || Boolean(fact.label && fact.kindSlug),
    );
}
  ...
  execute: async (input, context?) => {
    ...
    try {
      ...
      const resolvedAccountId =
        accountIdInput || runtimeAccountId || person.account_id;
      const resolvedProjectId =
        projectIdInput || runtimeProjectId || person.project_id;
      ...
      const allowedKindSlugs = new Set(
        kindRows.map((row) => row.slug).filter(Boolean) as string[],
      );
      const kindSummaries = kindRows
        .map(
          (row) =>
            `- ${row.slug}: ${row.label}${row.description ? ` — ${row.description}` : ""}`,
        )
        .join("\n");
      ...
      if (!normalizedFacts.length && transcript?.trim()) {
        try {
          ...
          return {
            ...
            warnings: [
              "LLM extraction failed. Provide structured facts or retry.",
            ],
          };
        }
      }
      ...
      for (const spec of filteredSpecs) {
        ...
        if (action === "remove") {
          ...
          const key =
            spec.kindSlug && spec.label
              ? labelKey(spec.kindSlug, spec.label)
              : null;
          if (key) {
        ...
        additions.push({
          ...
          status: existingFacetIdSet.has(facetAccountId)
            ? "updated"
            : "inserted",
        });
      }
      ...
      if (mode === "replace") {
        ...
        replaceRemovals = Array.from(existingFacetIdSet).filter(
          (id) => !keepIds.has(id),
        );
      ...
      for (const record of removalRecords) {
        removalById.set(record.facetAccountId, {
          ...
          kindSlug:
            record.kindSlug ?? removalById.get(record.facetAccountId)?.kindSlug,
          reason:
            record.reason ?? removalById.get(record.facetAccountId)?.reason,
        });
      }
      ...
      if (!dryRun) {
        if (rowsToUpsert.length) {
          ...
          if (upsertError)
            throw new Error(
              `Failed to upsert person facets: ${upsertError.message}`,
            );
        }
        ...
        if (removalIds.length) {
          ...
          if (deleteError)
            throw new Error(
              `Failed to remove person facets: ${deleteError.message}`,
            );
        }
      }
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to upsert person facets.",
