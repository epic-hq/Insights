...
const themeWithFrequencySchema = z.object({
	...
	frequencyPercent: z.number().describe("Percentage of segment members who mentioned this theme"),
...
export const fetchSegmentThemesTool = createTool({
	...
	inputSchema: z.object({
		...
		limit: z.number().int().min(1).max(50).nullish().describe("Max themes to return. Default 20."),
	}),
	...
	execute: async (input, context?) => {
		...
		const { getTopConcernsForSegment } = await import("../../features/themes/services/segmentThemeQueries.server");
		...
		try {
			...
			const effectiveLabel = input.segmentLabel ?? facets.map((f) => f.label).join(", ");
			...
			const personIds = [...new Set((personFacets ?? []).map((pf) => pf.person_id))];
