...
Use this when the user asks "what changed this week?" or "weekly review" or "research pulse".`,
  inputSchema: z.object({
    projectId: z
      .string()
      .nullish()
      .describe("Project ID. Falls back to runtime context."),
  }),
  ...
  execute: async (input, context?) => {
    ...
    try {
      ...
      const previous =
        (snapshots?.[0]?.data as SnapshotData | undefined) ?? null;
      ...
      if (accountId) {
        supabase
          ...
          .then(({ error }: { error: unknown }) => {
            if (error)
              consola.warn("[research-pulse] snapshot upsert failed", error);
          });
      }
      ...
      const prevTierIndex = previousTier
        ? TIER_ORDER.indexOf(previousTier)
        : tierIndex;
...
function buildDeltas(
  ...
  return metrics
    .filter(
      (m) =>
        (current[m.key] as number) > 0 ||
        (previous && (previous[m.key] as number) > 0),
    )
    .map((m) => {
...
function determineConfidenceTier(
  data: SnapshotData,
): "early_signal" | "growing_confidence" | "decision_ready" {
