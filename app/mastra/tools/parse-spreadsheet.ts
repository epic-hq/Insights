...
function parseTabularContent(
  ...
  const headers = parseCSVLine(headerLine, detectedDelimiter).map((h) =>
    h.replace(/^["']|["']$/g, "").trim(),
  );
...
Input can be raw CSV/TSV text. The first row is treated as headers.`,
  inputSchema: z.object({
    content: z
      .string()
      .describe("The raw tabular data content (CSV, TSV, or pipe-delimited)"),
    delimiter: z
      .enum([",", "\t", ";", "|"])
      .optional()
    ...
    title: z
      ...
      .optional()
      .describe(
        "Optional title for the saved asset. Auto-generated if not provided.",
      ),
    description: z
      .string()
      .optional()
      .describe(
        "Description of the table contents. Auto-generated if not provided.",
      ),
  }),
  outputSchema: z.object({
    ...
    sampleRows: z
      ...
      .optional()
      .describe("First few rows as structured data"),
    assetId: z
      .string()
      .uuid()
      .optional()
      .describe("ID of the saved project_asset, if saved"),
    assetUrl: z
      .string()
      .optional()
      .describe(
        "Full URL to view/edit the asset (use this for links, not assetId)",
      ),
    assetSaved: z
      .boolean()
      .optional()
      .describe("Whether the asset was saved to project_assets"),
    looksLikeContacts: z
      .boolean()
      .optional()
      .describe("Whether the data appears to be contact/people data"),
    contactColumns: z
      .array(z.string())
      .optional()
      .describe("Detected contact-related column names"),
    looksLikeOpportunities: z
      .boolean()
      .optional()
      .describe("Whether the data appears to be opportunity/deal data"),
    opportunityColumns: z
      .array(z.string())
      .optional()
      .describe("Detected opportunity-related column names"),
    ...
    columnMapping: z
      .object({
        ...
        name: z
          ...
          .nullable()
          .optional()
          .describe("Column with full name"),
        firstname: z
          .string()
          .nullable()
          .optional()
          .describe("Column with first name only"),
        lastname: z
          .string()
          .nullable()
          .optional()
          .describe("Column with last name only"),
        // Primary contact
        ...
        website: z
          ...
          .nullable()
          .optional()
          .describe("Column with website URL"),
        address: z
          .string()
          .nullable()
          .optional()
          .describe("Column with full address"),
        // Social profiles
        linkedin: z
          .string()
          .nullable()
          .optional()
          .describe("Column with LinkedIn"),
        twitter: z
          .string()
          .nullable()
          .optional()
          .describe("Column with Twitter/X"),
        instagram: z
          .string()
          .nullable()
          .optional()
          .describe("Column with Instagram"),
        ...
        title: z
          ...
          .nullable()
          .optional()
          .describe("Column with job title"),
        company: z
          .string()
          .nullable()
          .optional()
          .describe("Column with company name"),
        role: z
          .string()
          .nullable()
          .optional()
          .describe("Column with role/function"),
        industry: z
          .string()
          .nullable()
          .optional()
          .describe("Column with industry"),
        location: z
          .string()
          .nullable()
          .optional()
          .describe("Column with city/region"),
        // Company context
        company_stage: z
          .string()
          .nullable()
          .optional()
          .describe("Column with company stage"),
        company_size: z
          .string()
          .nullable()
          .optional()
        ...
        segment: z
          ...
          .nullable()
          .optional()
          .describe("Column with segment"),
        lifecycle_stage: z
          .string()
          .nullable()
          .optional()
          .describe("Column with lifecycle stage"),
      })
      .optional()
      .describe(
      ...
      .array(
        z.object({
          ...
          sampleValues: z
            .array(z.string())
            .optional()
            .describe("Example values from column"),
          reason: z.string().describe("Why this should be a facet"),
        }),
      )
      .optional()
      .describe(
        "Suggested facet mappings for unmapped columns (event signups, survey answers, etc.)",
      ),
    mappingConfidence: z
      .number()
      .optional()
      .describe("Confidence score 0-1 for the column mapping"),
    mappingWarnings: z
      .array(z.string())
      .optional()
      .describe("Warnings about the data or mapping"),
    // Duplicate detection for contacts
    duplicateCheck: z
      .object({
        totalEmailsInSpreadsheet: z
          .number()
          .describe("Number of emails found in spreadsheet"),
        existingEmailsInCRM: z
          .number()
          .describe("Number of emails that already exist in CRM"),
        newEmails: z
          .number()
          .describe("Number of new emails that would be created"),
        duplicatePercentage: z
          .number()
          .describe("Percentage of emails that are duplicates (0-100)"),
      })
      .optional()
      .describe(
        "Duplicate detection results - helps decide between create vs upsert mode",
      ),
    ...
    try {
      ...
      if (!content || content.trim().length === 0) {
        ...
        return {
          success: false,
          message:
            "No content provided. Please paste your CSV or tabular data.",
          markdownTable: "",
          headers: [],
          rowCount: 0,
          columnCount: 0,
      ...
      const stats = includeStats
        ? generateSummaryStats(headers, rows)
        : undefined;
      ...
      let suggestedFacets:
        | Array<{ column: string; facetKind: string; reason: string }>
        | undefined;
      ...
      if (looksLikeContacts) {
        try {
          ...
          consola.info(
            "[parse-spreadsheet] LLM column mapping:",
            columnMapping,
          );
          consola.info(
        ...
        try {
          ...
          if (uniqueSpreadsheetEmails.length > 0) {
            ...
            const existingEmailSet = new Set(
              (existingPeople || [])
                .map((p) => p.primary_email?.toLowerCase()?.trim())
                .filter(Boolean),
            );

            const existingCount = uniqueSpreadsheetEmails.filter((email) =>
            ...
            duplicateCheck = {
              ...
              duplicatePercentage: Math.round(
                (existingCount / uniqueSpreadsheetEmails.length) * 100,
              ),
            };

            consola.info(
              "[parse-spreadsheet] Duplicate check:",
              duplicateCheck,
            );
          }
        } catch (dupError) {
          consola.warn(
            "[parse-spreadsheet] Failed to check duplicates:",
            dupError,
          );
        }
      }
      ...
      const hasOpportunityName = headers.some((h) =>
        ["deal", "opportunity", "account", "prospect", "name", "title"].some(
          (kw) => h.toLowerCase().includes(kw),
        ),
      );
      ...
      const looksLikeOpportunities =
        hasOpportunityName &&
        hasOpportunityValue &&
        opportunityColumns.length >= 2;
      ...
      if (saveToAssets && accountId && projectId) {
        try {
          ...
          if (insertError || !asset) {
            consola.error(
              "[parse-spreadsheet] Failed to save asset:",
              insertError,
            );
          } else {
            assetId = asset.id;
            ...
            assetSaved = true;
            consola.info(
              "[parse-spreadsheet] Saved to project_assets:",
              assetId,
            );
            ...
            try {
              ...
              consola.info(
                "[parse-spreadsheet] Triggered embedding generation for",
                asset.id,
              );
            } catch (triggerError) {
              consola.warn(
                "[parse-spreadsheet] Failed to trigger indexing:",
                triggerError,
              );
            }
          }
        ...
        consola.warn(
          "[parse-spreadsheet] Cannot save: missing accountId or projectId",
        );
      }
      ...
      if (duplicateCheck && duplicateCheck.existingEmailsInCRM > 0) {
        ...
        if (duplicateCheck.duplicatePercentage > 30) {
          duplicateMessage +=
            " Consider using UPSERT mode to update existing contacts with new data.";
        }
      }
      ...
      return {
        ...
        opportunityColumns: looksLikeOpportunities
          ? opportunityColumns
          : undefined,
