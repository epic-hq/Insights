...
If the confidence is low, it will suggest clarifications.`,
  inputSchema: z.object({
    ...
    guidelines: z
      .string()
      .describe("Natural language guidelines to parse and add"),
    questionIds: z
      .array(z.string())
      .optional()
      .describe("Specific question IDs to apply guidelines to (optional)"),
  }),
  ...
  execute: async (input, context?) => {
    try {
      ...
      if (
        parseResult.overallConfidence === "LOW" ||
        parseResult.unparseableSegments.length > 0
      ) {
        return {
          success: false,
          ...
          clarificationsNeeded: [
            ...
            ...parseResult.unparseableSegments.map(
              (s) => `Could not understand: "${s}"`,
            ),
          ],
        };
      }
      ...
      for (const guideline of parseResult.guidelines) {
        ...
        const triggerIndex = updatedQuestions.findIndex(
          (q) => q.id === guideline.condition.questionId,
        );
        if (triggerIndex < 0) {
        ...
        const branchRule: z.infer<typeof BranchRuleSchema> = {
          ...
          source: "ai_generated",
          confidence: guideline.confidence.toLowerCase() as
            | "high"
            | "medium"
            | "low",
        };
      ...
      return {
        ...
        addedRules,
        clarificationsNeeded:
          parseResult.suggestedClarifications.length > 0
            ? parseResult.suggestedClarifications
            : undefined,
      };
    } catch (error) {
