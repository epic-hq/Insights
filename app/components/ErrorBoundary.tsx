...
const ErrorIllustration = () => (
	<svg viewBox="0 0 400 300" className="mx-auto h-48 w-64">
		...
		<g opacity="0.7">
			...
			{/* Broken code symbols */}
			<text x="100" y="200" fontSize="16" fill="#EF4444" opacity="0.8">
				{"</>"}
			</text>
			<text x="310" y="180" fontSize="14" fill="#EF4444" opacity="0.6">
				{"{ }"}
			</text>
		</g>
		...
		<g fill="#F59E0B" opacity="0.6">
			...
			<text x="70" y="195" fontSize="12" textAnchor="middle" fill="white">
				!
			</text>
		</g>
	</svg>
		...
		<div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 p-4 dark:from-slate-900 dark:via-red-900 dark:to-orange-900">
			<div className="w-full max-w-2xl text-center">
				...
				<div className="mb-10 space-y-4">
					<h1 className="bg-gradient-to-r from-red-600 via-orange-600 to-yellow-600 bg-clip-text font-bold text-7xl text-transparent">
						Oops!
					</h1>
					<h2 className="font-semibold text-3xl text-gray-800 dark:text-gray-100">Something went a bit sideways</h2>
					...
					{process.env.NODE_ENV === "development" && error && (
						<details className="mx-auto mt-6 max-w-lg rounded-lg bg-gray-100 p-4 text-left dark:bg-gray-800">
							...
							<pre className="overflow-auto whitespace-pre-wrap text-red-600 text-xs dark:text-red-400">
								{error.message}
								{error.stack && (
									<>
										{"\n\n"}
				...
				<div className="flex flex-col justify-center gap-4 sm:flex-row">
					...
					<Link
						to="/home"
						className="inline-flex cursor-pointer items-center justify-center rounded-xl border border-transparent bg-gradient-to-r from-red-600 to-orange-600 px-8 py-4 font-medium text-base text-white shadow-md transition-all duration-300 hover:scale-105 hover:from-red-700 hover:to-orange-700 hover:shadow-lg"
					>
						🏠 Take Me Home
					</Link>
				</div>
				...
				<div className="mt-12 border-gray-200 border-t pt-8 dark:border-gray-700">
					<p className="mb-4 text-gray-500 text-sm dark:text-gray-400">Need help? Try these options:</p>
					<div className="flex flex-wrap justify-center gap-3">
						<button
							onClick={() => window.location.reload()}
							className="rounded-md px-3 py-1 text-red-600 text-sm transition-colors hover:bg-red-50 hover:text-red-800 dark:text-red-400 dark:hover:bg-red-900/30 dark:hover:text-red-300"
						>
							Refresh Page
						</button>
						<Link
