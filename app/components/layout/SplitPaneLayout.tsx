...
import { CheckSquare, History, Map, MessageSquare, Minus, PanelLeft, Plus, Sparkles, X } from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { Link, Outlet, useFetcher, useLocation, useRouteLoaderData, useSearchParams } from "react-router";
...
function DesktopLayout({
	...
	useEffect(() => {
		...
		if (stored) {
			...
			if (!Number.isNaN(parsed) && parsed >= AGENT_WIDTH_MIN && parsed <= AGENT_WIDTH_MAX) {
	...
	const handleResizeStart = useCallback(
		(e: React.MouseEvent) => {
			...
			const onMouseMove = (ev: MouseEvent) => {
				...
				const newWidth = Math.min(AGENT_WIDTH_MAX, Math.max(AGENT_WIDTH_MIN, startWidth + delta));
			...
			<AnimatePresence mode="wait" initial={false}>
				{isTwoColumn ? (
					...
					>
						...
						>
							<AgentColumn accountId={accountId} projectId={projectId} systemContext={systemContext} />
					...
					<motion.main
						...
						transition={{ duration: 0.2 }}
					>
						<AgentCentered accountId={accountId} projectId={projectId} systemContext={systemContext} />
					</motion.main>
				)}
			</AnimatePresence>
		</div>
...
function MobileLayout({
	...
	return (
		<div className={cn("flex min-h-0 flex-1 flex-col", showMobileNav ? "pb-[72px]" : "")}>
	...
	return (
		<div className="rounded-lg bg-muted/50 p-2.5">
			<div className="mb-1.5 flex items-center justify-between">
				<span className="font-medium text-[10px] text-muted-foreground uppercase tracking-wider">Next Steps</span>
				<div className="flex items-center gap-1">
					{journeyBadge}
					<button
						type="button"
