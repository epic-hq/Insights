...
export default function MainNav() {
	...
	const isCustomerDiscoveryConsultantsPage = pathname === "/customer-discovery-for-consultants";
	const isDarkHeroPage = isHomePage || isPricingPage || isCustomerDiscoveryPage || isCustomerDiscoveryConsultantsPage;
	...
	return (
		<nav
			className={cn(
				...
				isMarketingPage && !isDarkHeroPage && "border-zinc-200/70 border-b bg-white/95 text-zinc-900 backdrop-blur",
				isMarketingPage && isDarkHeroPage && "border-white/10 border-b bg-zinc-950/95 text-white backdrop-blur"
			)}
		>
								...
								{marketingLinks.map(({ key, label, link }) => (
									<NavLink
										...
										className={cn(
											...
											isDarkHeroPage ? "text-zinc-300 hover:text-white" : "text-zinc-700 hover:text-zinc-950"
										)}
									>
										{label}
									</NavLink>
