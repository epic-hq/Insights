...
const navItems = [
	{ to: "/", label: "Dashboard", authOnly: true },
	{ to: "/themes", label: "Themes", authOnly: true },
	{ to: "/personas", label: "Personas", authOnly: true },
	{ to: "/opportunities", label: "Opportunities", authOnly: true },
	{ to: "/interviews", label: "Interviews", authOnly: true },
...
export default function MainNav() {
	const { user } = useAuth()
	const { pathname } = useLocation()
	const isMainRoute = pathname !== "/" && /^\/(themes|personas|opportunities|interviews|insights)/.test(pathname)
					...
					<div className="flex h-16 items-center justify-between">
						...
						{user && (
							<div className="hidden sm:ml-6 sm:flex sm:space-x-8">
								{navItems.filter(item => !item.authOnly || user).map(({ to, label }) => (
									<NavLink
										key={to}
										to={to}
										className={({ isActive }) =>
						...
						{/* Actions: always show UserProfile for authenticated users */}
						{user && (
