...
import { TOP_NAV_CATEGORIES, TOP_NAV_STANDALONE_ITEMS, type TopNavCategory, type TopNavItem } from "./top-nav.config";
				...
				{category.items.map((item) => {
					...
					return (
						<DropdownMenuItem key={item.key} asChild>
							<NavLink to={href} className="flex items-start gap-3 p-2" onClick={() => setOpen(false)}>
...
export function TopNavigation({ accounts = [], className }: TopNavigationProps) {
	...
	const getActiveKey = () => {
		...
		if (path.includes("/priorities")) return "tasks";
		...
		if (path.includes("/people") || path.includes("/organizations") || path.includes("/opportunities")) return "crm";
		return null;
	};
	...
	return (
		<>
			<header
				className={cn("sticky top-0 z-40 flex h-14 items-center border-b bg-background/95 backdrop-blur", className)}
			>
				<div className="flex w-full items-center gap-4 px-4">
					...
					<nav className="hidden flex-1 items-center justify-center gap-1 md:flex">
						...
						{/* Standalone nav links (no dropdown) */}
						{TOP_NAV_STANDALONE_ITEMS.map((item) => {
							const href = item.to(routes);
							const countValue = item.countKey ? counts[item.countKey] : undefined;
							return (
								<NavLink
									...
									className={cn(
										...
										activeKey === item.key ? "text-primary" : "text-foreground/70"
									)}
								>
					...
					<div className="flex min-h-0 flex-1 flex-col">
						<div className="min-h-0 flex-1 overflow-y-auto p-4">
							<div className="flex min-h-full flex-col gap-6">
								...
								{/* Standalone Items */}
								{TOP_NAV_STANDALONE_ITEMS.map((item) => (
									<MobileNavItem key={item.key} item={item} routes={routes} onClose={() => setMobileMenuOpen(false)} />
								))}
							</div>
						</div>
