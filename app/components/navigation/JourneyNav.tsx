    ...
    subItems: [
      {
        label: "Status",
        route: "/dashboard",
        description: "Project overview & recommendations",
      },
    ],
  },
  {
    ...
    subItems: [
      ...
      },
      {
        ...
        description: "Upload interviews & surveys",
      },
    ],
  },
  {
    ...
    subItems: [
      {
        label: "Personas",
        route: "/personas",
        description: "User archetypes & segments",
      },
      {
        label: "People",
        route: "/people",
      ...
      {
        label: "Evidence",
        route: "/personas/evidence",
        description: "Supporting quotes & data",
      },
    ],
  },
  {
    key: "insights",
    title: "Insights",
    description: "Themes, patterns & opportunities",
    icon: Lightbulb,
    ...
    subItems: [
      {
        label: "Overview",
        route: "/insights",
        description: "Dashboard with top themes",
      },
      {
        label: "Table",
        route: "/insights/table",
        description: "Grouped themes with priorities",
      },
      {
        label: "Opportunities",
        route: "/opportunities",
        description: "HMW backlog & experiments",
      },
    ],
  },
];
...
export function JourneyNav({
  ...
  const { accountId, projectId, projectPath, setLastProjectPath } =
    useCurrentProject();
  ...
  const accounts = (protectedData?.accounts || []).filter(
    (acct) => !acct.personal_account,
  );
  const currentAccount =
    accounts.find((acct) => acct.account_id === accountId) || accounts[0];
  const currentProject =
    currentAccount?.projects?.find((proj) => proj.id === projectId) ||
    currentAccount?.projects?.[0];
    ...
    for (const step of journeySteps) {
      ...
      if (step.subItems) {
        for (const item of step.subItems) {
          if (
            routeSegment.startsWith(item.route) &&
            item.route.length > bestMatchLength
          ) {
            bestMatch = step;
  ...
  const getRouteUrl = (route: string): string => {
    const routeMap: Record<string, () => string> = {
      ...
      "/insights/table": () => routes.insights.table(),
