import { Check, ChevronsUpDown, Crown, Plus } from "lucide-react";
...
export function TeamSwitcher({
  ...
  const allAccounts = useMemo<AccountRecord[]>(() => {
    ...
    if (typeof protectedData.accounts === "string") {
      try {
        ...
        console.error(
          "Failed to parse accounts from protected loader data",
          error,
        );
        return [];
      }
  ...
  const { currentProject, projectAccount } = useMemo(() => {
    ...
    if (projectId && currentAccount?.projects) {
      const urlProject = currentAccount.projects.find(
        (proj) => proj.id === projectId,
      );
      if (urlProject)
        return { currentProject: urlProject, projectAccount: currentAccount };
    }

    // Second priority: last_used_project_id - search across ALL accounts
    const userSettings = protectedData?.user_settings as
    ...
    if (lastUsedProjectId) {
      for (const account of allAccounts) {
        const project = account.projects?.find(
          (proj) => proj.id === lastUsedProjectId,
        );
        if (project) {
    ...
    const fallbackAccount = allAccounts.find(
      (acct) => !acct.personal_account && acct.projects?.length,
    );
    ...
    const anyAccountWithProjects = allAccounts.find(
      (acct) => acct.projects?.length,
    );
    return {
  ...
  const currentAccountLabel =
    projectAccount?.name || currentAccount?.name || "Select an account";
                ...
                {accounts.map((account) => (
                  ...
                  >
                    {(account.projects ?? []).map((project) => {
                      ...
                      const isActive =
                        account.account_id === projectAccount?.account_id &&
                      ...
                      return (
                        <CommandItem
                          ...
                          onSelect={() =>
                            handleSelectProject(account.account_id, project.id)
                          }
                        >
                          <Check
                            className={cn(
        ...
        <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Create a team workspace to collaborate with others on research
                projects.
              </DialogDescription>
            </DialogHeader>
            <CreateTeamForm
