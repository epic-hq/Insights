...
} from "~/components/chat/InlineUserInput";
...
function ThinkingWave({
  ...
  const normalizedMessage = progressMessage
    ? normalizeProgressMessage(progressMessage)
    : "";
  const shouldRotate =
    !progressMessage ||
    GENERIC_PROGRESS_LABELS.has(normalizedMessage) ||
    normalizedMessage.startsWith("routing");
  ...
  for (const part of message.parts) {
    ...
    if (anyPart.type === "data" && anyPart.data) {
      ...
      if (data?.type === "data-network" && typeof data?.data === "object") {
        ...
        const activeStep = networkData.steps?.findLast(
  ...
  for (const part of message.parts) {
    const anyPart = part as {
      ...
      toolInvocation?: {
        state: string;
        result?: {
          a2ui?: { __a2ui?: boolean; surfaceId?: string; messages?: unknown[] };
        };
      };
      result?: {
        a2ui?: { __a2ui?: boolean; surfaceId?: string; messages?: unknown[] };
    ...
    if (anyPart.type === "tool-invocation" || anyPart.toolInvocation) {
      ...
      if (
        ...
        toolData.result?.a2ui?.__a2ui === true
      ) {
    ...
    if (
      ...
      anyPart.result?.a2ui?.__a2ui === true
    ) {
...
/**
 * Detect requestUserInput payloads in tool-invocation message parts.
 * Returns the first __userInput payload found (with its toolCallId), or null.
 */
...
function extractUserInputPayloads(message: UpsightMessage): UserInputPayload[] {
  ...
  for (const part of message.parts) {
    ...
    if (anyPart.type === "tool-invocation" || anyPart.toolInvocation) {
      const toolData =
        anyPart.toolInvocation || (anyPart as typeof anyPart.toolInvocation);
      if (
        toolData &&
        toolData.toolName === "requestUserInput" &&
        toolData.state === "output-available" &&
        toolData.result?.userInput?.__userInput === true
      ) {
        const ui = toolData.result.userInput;
        payloads.push({
          toolCallId: toolData.toolCallId,
          prompt: ui.prompt || "",
          options: ui.options || [],
          selectionMode: ui.selectionMode || "single",
          allowFreeText: ui.allowFreeText ?? true,
        });
      }
    }
  }
...
function ProjectStatusAgentChatInner({
  ...
  // Track which requestUserInput tool calls have been answered to prevent re-submission
  const answeredUserInputsRef = useRef<Set<string>>(new Set());
    ...
    useChat<UpsightMessage>({
      ...
      onToolCall: async ({ toolCall }) => {
        ...
        if (toolCall.toolName === "navigateToPage") {
          ...
          const { resolved: normalizedPath, reason } = ensureProjectScopedPath(
            rawPath,
            accountId,
            projectId,
          );
  ...
  const toolSuggestions = useMemo(() => {
    ...
    if (displayableMessages.length === 0) {
      ...
      const interviews = interviewMatch
        ? Number.parseInt(interviewMatch[1], 10)
        : 0;
      const evidence = evidenceMatch
        ? Number.parseInt(evidenceMatch[1], 10)
        : 0;
      const insights = insightsMatch
        ? Number.parseInt(insightsMatch[1], 10)
        : 0;
    ...
    for (const part of lastAssistantMsg.parts) {
      ...
      if (anyPart.type === "tool-invocation" || anyPart.toolInvocation) {
        const toolData = anyPart.toolInvocation || anyPart;
        if (
          toolData.toolName === "suggestNextSteps" &&
          toolData.state === "output-available"
        ) {
  ...
  const handleAssistantLinkClick = (
  ...
  const chatContent = (
    <>
      ...
      {isGenUIEnabled &&
        a2uiSurface?.surface != null &&
        a2uiSurface.surface.ready && (
            ...
            <A2UIRenderer
              ...
              onAction={(action) => {
                ...
                persistCanvasAction(action, projectId).then((result) => {
                  if (result.error) {
                    consola.warn(
                      "[gen-ui] persistence skipped/failed",
                      result.error,
                    );
                  }
                });
      ...
      <div className="min-h-0 flex-1 overflow-hidden">
        {visibleMessages.length === 0 ? (
          ...
          <div
            ref={scrollRef}
            className="h-full overflow-y-auto text-xs sm:text-sm"
          >
            <div ref={contentRef} className="space-y-3">
              {visibleMessages.map((message, index) => {
                ...
                const filteredTextParts = textParts.filter(
                  (part) =>
                    typeof part === "string" && !isNetworkDebugText(part),
                );
                const messageText =
                ...
                const userInputPayloads =
                  !isUser && !isTool ? extractUserInputPayloads(message) : [];
                return (
                  <div
                    key={key}
                  ...
                  >
                    <div className="max-w-[85%]">
                      ...
                      {/* Inline user input rendered inside assistant bubble area */}
                      {userInputPayloads.map((payload) => {
                        const isAnswered = answeredUserInputsRef.current.has(
                          payload.toolCallId,
                        );
                        return (
                          <InlineUserInput
                            ...
                            onSubmit={(selectedIds, freeText) => {
                              answeredUserInputsRef.current.add(
                                payload.toolCallId,
                              );
                              ...
                              const responseText = freeText
                                ? `[UserInput] prompt: '${payload.prompt}', freeText: '${freeText}'`
                                : `[UserInput] prompt: '${payload.prompt}', selected: [${selectedIds.map((id) => `'${id}'`).join(", ")}] (${optionLabels})`;
      ...
      <Card className="flex h-full min-h-0 flex-col border-0 bg-background/80 shadow-none ring-1 ring-border/60 backdrop-blur sm:rounded-xl sm:shadow-sm">
        <CardHeader
          className={cn(
            ...
            isCollapsed ? "p-2" : "p-3 pb-2 sm:p-4",
          )}
        >
...
function describeCurrentProjectView({
  pathname,
  search,
  accountId,
  projectId,
}: ViewContextArgs): string {
  if (!pathname) return "";
...
function describeResourceContext(
  resource: string,
  remainder: string[],
): string {
