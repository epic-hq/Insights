...
export function ProjectStatusAgentChat({
      ...
      for (const part of msg.parts) {
        ...
        const isToolWithOutput =
          ...
          (partType === "tool-result" ||
            ...
            (partType?.startsWith("tool-") ?? false));

        if (isToolWithOutput) {
          const result = (anyPart.output ??
            anyPart.result ??
            anyPart.toolResult) as Record<string, unknown> | undefined;
  ...
  const handleFileUpload = useCallback(
    async (content: string, fileName: string, fileType: string) => {
      ...
      const endpoints = [
        `/a/${accountId}/${projectId}/api/people/import-csv`,
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          const result = (await response
            .json()
            .catch(() => ({}))) as PeopleImportApiResponse;
  ...
  useEffect(() => {
    if (!tts.isEnabled || status === "streaming" || status === "submitted")
      return;
    if (displayableMessages.length === 0) return;
    ...
    const textParts =
      lastMsg.parts?.filter((p) => p.type === "text").map((p) => p.text) ?? [];
            ...
            <div ref={contentRef} className="space-y-3">
              {visibleMessages.map((message, index) => {
                ...
                const isThisMessagePlaying =
                  tts.playingMessageId === message.id;
                return (
                  <div
                    key={key}
                      ...
                      <div
                        className={cn(
                          ...
                          isThisMessagePlaying &&
                            (embedded ? "ring-blue-500/30" : "ring-blue-300"),
                        )}
