...
import { Check, CircleDot, MessageSquare, Square, SquareCheck } from "lucide-react";
...
export function InlineUserInput({
	...
	const effectiveIds = effectiveAnswered && answeredIds.length > 0 ? new Set(answeredIds) : selectedIds;
	...
	const canSubmit = showFreeText ? freeText.trim().length > 0 : selectedIds.size > 0;
		...
		<div className="mt-2 space-y-2">
			...
			<div className="space-y-1.5">
				{options.map((option) => {
					...
					const Icon =
						selectionMode === "single" ? (isSelected ? CircleDot : CircleDot) : isSelected ? SquareCheck : Square;
					...
					return (
						...
						>
							<Icon
								className={cn("mt-0.5 h-4 w-4 flex-shrink-0", isSelected ? "text-primary" : "text-muted-foreground")}
							/>
							<div className="min-w-0 flex-1">
								<span className="font-medium">{option.label}</span>
								{option.description && (
									<span className="mt-0.5 block text-muted-foreground text-xs">{option.description}</span>
								)}
							</div>
							{effectiveAnswered && isSelected && <Check className="mt-0.5 h-4 w-4 flex-shrink-0 text-primary" />}
						</button>
					);
				})}
			</div>
			...
			{/* Submit button */}
			{!effectiveAnswered && (
				<Button size="sm" onClick={handleSubmit} disabled={!canSubmit} className="h-7 px-3 text-xs">
					{showFreeText ? "Send" : selectionMode === "single" ? "Confirm" : `Confirm (${selectedIds.size})`}
				</Button>
			)}
		</div>
	);
