	...
	return (
		...
		>
			<div className="text-center">
				<h3 className="font-semibold text-lg tracking-tight">What's your role?</h3>
			</div>
					...
					{JOB_FUNCTIONS.map((job) => {
						...
						return (
							<motion.div key={job.value} variants={itemVariants}>
								...
								>
									<RadioGroupItem value={job.value} id={`job-${job.value}`} className="sr-only" />
									<div
										className={cn(
									...
									<span
										className={cn("font-medium text-xs leading-tight transition-colors", isSelected && "text-primary")}
									>
										{job.label}
									</span>
									{isSelected && (
...
function UseCaseStep({ data, onChange }: StepProps) {
	...
	return (
		...
		>
			<div className="text-center">
				<h3 className="font-semibold text-lg tracking-tight">What brings you to UpSight?</h3>
				...
				{USE_CASES.map((useCase) => {
					...
					return (
						...
						>
							...
							<span className={cn("font-medium text-sm transition-colors", isSelected && "text-primary")}>
								{useCase.label}
							</span>
							{isSelected && (
								<motion.div
...
function CompanySizeStep({ data, onChange }: StepProps) {
	return (
		...
		>
			<div className="text-center">
				<h3 className="font-semibold text-lg tracking-tight">How big is your company?</h3>
			</div>
						...
						return (
							<motion.div key={size.value} variants={itemVariants}>
								...
								>
									...
									<span className={cn("font-semibold text-sm transition-colors", isSelected && "text-primary")}>
										{size.label}
									</span>
...
export function OnboardingWalkthrough({ open, onOpenChange, onComplete, initialData }: OnboardingWalkthroughProps) {
	...
	return (
		<Dialog open={open} onOpenChange={showCompletion ? undefined : onOpenChange}>
			<DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[480px]" showCloseButton={!showCompletion}>
					...
					{showCompletion ? (
						...
						<motion.div key="steps">
							<DialogHeader className="pb-3">
								<DialogTitle className="text-center text-lg">Welcome to UpSight</DialogTitle>
							...
							<div className="flex items-center justify-between gap-3 border-border/50 border-t pt-4">
								<Button
									...
									className={cn("transition-opacity duration-200", isFirstStep && "pointer-events-none opacity-0")}
								>
								...
								<Button
									...
									className={cn(
										...
										currentStep.canProceed && "shadow-md shadow-primary/20 hover:shadow-lg hover:shadow-primary/25"
									)}
								>
									{isLastStep ? (
										<>
