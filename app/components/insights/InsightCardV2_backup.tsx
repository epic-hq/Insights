		...
		<Card className="flex flex-col border-0 bg-white shadow-lg transition-all duration-300 hover:shadow-xl">
			...
			<CardContent className="flex-1 space-y-4">
				<div className="space-y-3">
					...
					{localInsight.emotionalResponse && (
						<div>
							...
							<InlineEdit
								...
								showEditButton={false}
								multiline
								placeholder="Emotional response level"
							/>
						</div>
					)}

					{/* Underlying Motivation */}
					{localInsight.underlyingMotivation && (
						<div>
							<h4 className="mb-1 text-gray-500 text-xs uppercase">Underlying Motivation</h4>
							<InlineEdit
								value={localInsight.underlyingMotivation || ""}
								onSubmit={async (newValue: string) => {
									setLocalInsight((prev) => ({ ...prev, underlyingMotivation: newValue }))
									await handleSaveField("underlyingMotivation", newValue)
								}}
								textClassName="text-gray-900 text-md"
								inputClassName="text-sm"
								submitOnBlur
								...
								<div className="flex gap-1">
									{[1, 2, 3, 4, 5].map((i) => (
										<div
											key={i}
											className={`h-2 w-2 rounded-full ${
												i <= (localInsight.confidence || 0) ? "bg-green-500" : "bg-gray-200"
											}`}
										/>
									))}
								</div>
					...
					{/* Related Tags */}
					{localInsight.relatedTags && localInsight.relatedTags.length > 0 && (
						<div>
							<h4 className="mb-2 text-gray-500 text-xs uppercase">Related Tags</h4>
							<div className="flex flex-wrap gap-1">
								{localInsight.relatedTags.map((tag, index) => (
									<Badge
										key={`${tag}-${index}`}
										variant="secondary"
