...
import { estimateTotalSeconds, formatEstimate, SURVEY_LENGTH_WARN_SECONDS } from "./coaching/time-estimates";
					...
					<div className="flex items-center justify-between gap-3">
						...
						<div className="flex items-center gap-1.5">
							<Button
								type="button"
								size="sm"
								className="h-7 bg-violet-600 px-3 text-white text-xs hover:bg-violet-700 dark:bg-violet-500 dark:hover:bg-violet-600"
								onClick={onCoach}
								disabled={isCoaching}
							>
								{isCoaching ? (
				...
				<div className="mb-2 flex items-center justify-between">
					...
					{onCoach && (
						<Button
							...
							variant="outline"
							size="sm"
							className="border-violet-500/20 bg-gradient-to-r from-violet-500/10 to-blue-500/10 text-violet-700 hover:from-violet-500/20 hover:to-blue-500/20 dark:border-violet-500/30 dark:from-violet-500/15 dark:to-blue-500/15 dark:text-violet-300"
							onClick={onCoach}
							disabled={isCoaching}
						>
							{isCoaching ? (
							...
							{isCoaching ? "Coaching..." : hasBeenCoached ? "Re-coach" : "Coach my questions"}
						</Button>
					)}
				</div>
			)}
