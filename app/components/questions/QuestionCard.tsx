					...
					<div className="flex items-start gap-3">
						<div className="mt-0.5 flex items-center gap-3">
							...
							{typeof props.index === "number" && (
								<div className="min-w-[1.5rem] font-medium text-foreground/60 text-sm">{(props.index ?? 0) + 1}</div>
							)}
						...
						<div className="min-w-0 flex-1">
							{isEditing ? (
								<div className="mb-2 flex items-center gap-2">
									...
									/>
									<Button variant="ghost" size="icon" onClick={onSaveEdit} className="text-green-600">
										<Check className="h-4 w-4" />
									</Button>
									<Button variant="ghost" size="icon" onClick={onCancelEdit} className="text-gray-500">
										<X className="h-4 w-4" />
									</Button>
								</div>
							) : (
						...
						<div className="flex items-center gap-1">
							...
							<DropdownMenu>
								...
								<DropdownMenuContent align="end">
									<DropdownMenuItem onClick={onToggleMustHave}>
										<TriangleAlert className="mr-2 h-4 w-4" />{" "}
										{question.isMustHave ? "Remove Must-Have" : "Mark Must-Have"}
									</DropdownMenuItem>
									<DropdownMenuItem onClick={onGenerateFollowups} disabled={!!generatingFollowUp}>
										{generatingFollowUp ? (
