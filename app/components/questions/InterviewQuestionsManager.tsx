...
import {
  fromManagerResearchMode,
  type ResearchMode,
  toManagerResearchMode,
} from "~/types/research";
...
const questionCategories = [
  {
    ...
    weight: 1.0,
    color:
      "border-blue-100 text-blue-800 dark:border-blue-900 dark:text-blue-200",
  },
  {
    id: "pain",
  ...
  },
  {
    ...
    weight: 1.1,
    color:
      "border-purple-100 text-purple-800 dark:border-purple-900 dark:text-purple-200",
  },
  {
    id: "goals",
    ...
    weight: 1.0,
    color:
      "border-green-100 text-green-800 dark:border-green-900 dark:text-green-200",
  },
  {
    id: "constraints",
    ...
    weight: 0.9,
    color:
      "border-orange-100 text-orange-800 dark:border-orange-900 dark:text-orange-200",
  },
  {
    id: "willingness",
    ...
    weight: 0.8,
    color:
      "border-indigo-100 text-indigo-800 dark:border-indigo-900 dark:text-indigo-200",
  },
  {
    id: "demographics",
    name: "Demographics",
...
const parseScores = (input: unknown): Question["scores"] => {
  const candidate =
    typeof input === "object" && input !== null
      ...
      : {};
  const importance =
    typeof candidate.importance === "number" ? candidate.importance : 0.5;
  const goalMatch =
    typeof candidate.goalMatch === "number" ? candidate.goalMatch : 0.5;
  const novelty =
    typeof candidate.novelty === "number" ? candidate.novelty : 0.5;
...
function QualityFlag({
  ...
  qualityFlag: Question["qualityFlag"];
}) {
        ...
        <TooltipContent>
          <div className="max-w-xs">
            ...
            <div className="text-xs">
              Score: {qualityFlag.score}/100 (
              {qualityFlag.assessment.toUpperCase()})
            </div>
            <div className="mt-1 text-xs">
              {qualityFlag.description ||
                "We can be more clear and specific. Try adding a concrete example."}
            </div>
          </div>
        </TooltipContent>
      </Tooltip>
...
function InterviewQuestionsManager(props: InterviewQuestionsManagerProps) {
  ...
  const [researchMode, setResearchMode] =
    useState<ManagerResearchMode>(defaultResearchMode);
  const [familiarity, setFamiliarity] =
    useState<Familiarity>(defaultFamiliarity);
  ...
  const setOrderedQuestionIds = useQuestionQueueStore(
    store,
    (s) => s.setOrderedIds,
  );
  ...
  const setMustHavesOnly = useQuestionQueueStore(
    store,
    (s) => s.setMustHavesOnly,
  );
  ...
  const [showingFollowupFor, setShowingFollowupFor] = useState<string | null>(
    null,
  );
  ...
  const [categoryTimeAllocations, setCategoryTimeAllocations] = useState<
    Record<string, number>
  >({});
  ...
  const [pendingInsertionChoices, setPendingInsertionChoices] = useState<
    Record<string, string>
  >({});
  const [_processingPendingId, setProcessingPendingId] = useState<
    string | null
  >(null);
  ...
  const [inlineQuestionCategory, setInlineQuestionCategory] =
    useState("context");
  ...
  const contextualSuggestionsApiPath =
    projectPath && projectPath.length > 0
      ? `${projectPath}/api/contextual-suggestions`
    ...
    const loadResearchGoal = async () => {
      try {
        const response = await fetch(
          `/api/load-project-goals?projectId=${projectId}`,
        );
  ...
  const commitSelection = useCallback(
    (nextBaseIds: string[]) => {
      console.log("🧮 commitSelection()", {
        mustHavesOnly,
        nextBaseIds,
        mustHaveCandidates: questions
          .filter((q) => q.isMustHave)
          .map((q) => q.id),
      });
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const [promptRes, settingsRes, questionsRes, answerRes] =
        await Promise.all([
          supabase
      ...
      if (promptRes.error)
        consola.warn(
          "Failed to load interview_prompts",
          promptRes.error.message,
        );
      if (settingsRes.error && settingsRes.error.code !== "PGRST116")
        throw settingsRes.error;
      if (questionsRes.error && questionsRes.error.code !== "PGRST116")
        throw questionsRes.error;
      if (answerRes.error)
        consola.warn("Failed to load project_answers", answerRes.error.message);

      existingPromptIdsRef.current = (promptRes.data ?? []).map(
        (row) => row.id,
      );
      ...
      const settingsMeta =
        (settingsRes.data?.meta as Record<string, unknown>) || {};
      ...
      if (storedModeRaw) {
        const storedMode: ResearchMode =
          storedModeRaw === "followup" ? "user_testing" : storedModeRaw;
        setResearchMode(storedMode);
      }

      // Load other settings if present
      if (typeof settingsMeta.familiarity === "string")
        setFamiliarity(settingsMeta.familiarity as Familiarity);
      if (typeof settingsMeta.goDeepMode === "boolean")
        setGoDeepMode(settingsMeta.goDeepMode);
      if (typeof settingsMeta.customInstructions === "string")
        setCustomInstructions(settingsMeta.customInstructions);
      if (typeof settingsMeta.custom_instructions === "string")
        setCustomInstructions(settingsMeta.custom_instructions);

      const catTimesRaw = (
        settingsMeta as
          | { categoryTimeAllocations?: Record<string, unknown> }
          | undefined
      ...
      const questionsMeta =
        (questionsRes.data?.meta as Record<string, unknown>) || {};
      ...
      if (promptRows.length > 0) {
        formattedQuestions = promptRows
          .filter(
            (row) => row.status !== "deleted" && row.status !== "rejected",
          )
          .map((row) => ({
            id: row.id,
            text: row.text,
            ...
            selectedOrder:
              typeof row.selected_order === "number"
                ? row.selected_order
                : null,
        ...
        selectedIds = promptRows
          ...
          .sort(
            (a, b) =>
              (a.selected_order ?? Number.POSITIVE_INFINITY) -
              (b.selected_order ?? Number.POSITIVE_INFINITY),
          )
          .map((row) => row.id);
      } else {
        existingPromptIdsRef.current = [];
        const legacyQuestions = (
          (questionsMeta.questions as QuestionInput[] | undefined) || []
        ).filter(
          (q) =>
            (q.status as Question["status"]) !== "deleted" &&
            (q.status as Question["status"]) !== "rejected",
        );
        const resolvedIds = legacyQuestions.map(
        ...
        formattedQuestions = legacyQuestions.map((q, idx) => ({
          ...
          source:
            (q as QuestionInput & { source?: "ai" | "user" }).source || "ai",
          isMustHave:
            (q as QuestionInput & { isMustHave?: boolean }).isMustHave || false,
          estimatedMinutes: (q as QuestionInput & { estimatedMinutes?: number })
            .estimatedMinutes,
          selectedOrder:
            typeof q.selectedOrder === "number" ? q.selectedOrder : null,
          isSelected:
            (q as QuestionInput & { isSelected?: boolean }).isSelected ?? false,
        }));
        selectedIds = legacyQuestions
          ...
          .filter(
            ({ q }) =>
              q &&
              ((q as QuestionInput & { isSelected?: boolean }).isSelected ||
                typeof q.selectedOrder === "number"),
          )
          .sort(
            (a, b) =>
              ((a.q.selectedOrder as number | undefined) ??
                Number.POSITIVE_INFINITY) -
              ((b.q.selectedOrder as number | undefined) ??
                Number.POSITIVE_INFINITY),
          )
        ...
        JSON.stringify(
          {
            ...
            selectedIds,
            followupCount: deduped.filter((q) =>
              q.rationale?.startsWith("Follow-up to:"),
            ).length,
  ...
  const markQuestionAsRecentlyAdded = useCallback((id: string) => {
    setRecentlyAddedQuestionIds((prev) =>
  ...
  useEffect(() => {
    ...
    if (
      ...
      now - lastLoadedRef.current.ts < 1500
    )
      return;
  ...
  const estimateMinutesPerQuestion = useCallback(
    (q: Question, mode: ManagerResearchMode, f: Familiarity): number => {
      const baseTimes = {
        exploratory: 3.5,
        validation: 2.5,
        user_testing: 2.0,
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.questionSet?.questions) {
          ...
          const formattedNewQuestions: Question[] = newQuestions.map(
            (q: QuestionInput) => {
              const baseQuestion: Question = {
                ...
                source: "ai" as const,
                isMustHave:
                  (q as QuestionInput & { isMustHave?: boolean }).isMustHave ||
                  false,
              };
              return {
                ...baseQuestion,
                estimatedMinutes:
                  (q as QuestionInput & { estimatedMinutes?: number })
                    .estimatedMinutes ??
          ...
          const deduplicatedQuestions = ensureUniqueQuestionIds(
            formattedNewQuestions,
            [...questions, ...pendingGeneratedQuestions],
          );

          setPendingGeneratedQuestions((prev) => [
            ...prev,
            ...deduplicatedQuestions,
          ]);
          setPendingInsertionChoices((prev) => ({
            ...prev,
            ...deduplicatedQuestions.reduce<Record<string, string>>(
              (acc, question) => {
                acc[question.id] = "end";
                return acc;
              },
          ...
          if (autoGenerateInitial) {
            ...
            toast.success(
              `Generated ${deduplicatedQuestions.length} initial questions`,
              {
                description:
                  "Questions have been saved and loaded from database.",
                duration: 5000,
              },
            );
          } else {
            toast.success(
              `Generated ${deduplicatedQuestions.length} new questions`,
              {
                description:
                  "Questions have been saved and loaded from database.",
                duration: 4000,
          ...
          toast.error("Failed to generate questions", {
            description:
              "The response was successful but contained no questions",
          });
        }
      } else {
  ...
  const calculateCompositeScore = useCallback((q: Question): number => {
    const categoryWeight =
      questionCategories.find((c) => c.id === q.categoryId)?.weight || 1;
    const s = q.scores;
    return (
      0.5 * (s.importance || 0) +
      0.35 * (s.goalMatch || 0) +
      0.15 * (s.novelty || 0) * categoryWeight
    );
  }, []);
  ...
  const questionPack = useMemo(() => {
    const targetCounts: Record<
      number,
      { base: number; validation: number; cold: number }
    > = {
    ...
    const validationBoost =
      researchMode === "validation" ? config.validation : 0;
    const targetCount = Math.max(
      4,
      config.base +
        validationBoost +
        (familiarity === "cold" ? config.cold : 0),
    );
    ...
    const visibleIdsRaw = mustHavesOnly
      ? canonicalSanitized.filter((id) => mustHaveSet.has(id))
      : canonicalSanitized;
    ...
    const totalEstimatedTime = orderedSelectedQuestions.reduce(
    ...
    const belowCount =
      overflowIndex >= 0 ? orderedSelectedQuestions.length - overflowIndex : 0;
    const selectedSet = new Set(canonicalSanitized);
    const remainingQuestions = allQuestionsWithScores.filter(
      (q) => !selectedSet.has(q.id),
    );
  ...
  const baseSelectedIdsForModal = useMemo(
    () => getBaseSelectedIds(),
    [getBaseSelectedIds],
  );
  const _baseSelectedQuestionsForModal = useMemo(
    () =>
      baseSelectedIdsForModal
  ...
  const saveQuestionsToDatabase = useCallback(
    async (
      ...
      if (!projectId) return;
      try {
        const followupCount = questionsToSave.filter((q) =>
          q.rationale?.startsWith("Follow-up to:"),
        ).length;
        console.log(
          "🛟 SAVE INVOKED",
        ...
        const missingSelected = selectedIds.filter(
          (id) => !questionLookup.has(id),
        );
        if (missingSelected.length > 0) {
          console.warn(
            "⚠️ SAVE WARNING: Missing selected questions in payload",
            {
              selectedIds,
              missingSelected,
        ...
        const withOrder = questionsToSave.map((q, _index) => {
          ...
          const estimated =
            q.estimatedMinutes ??
            estimateMinutesPerQuestion(q, researchMode, familiarity);
          return {
            ...q,
        ...
        const promptPayloads = withOrder.map((q, index) => {
          ...
          const payload = {
            ...
            selected_order:
              q.selectedOrder ?? (selectedIndex >= 0 ? selectedIndex : null),
          };
        ...
        console.log("📤 SENDING TO DATABASE:", {
          ...
          rejected_count: promptPayloads.filter((p) => p.status === "rejected")
            .length,
        ...
        if (newlyCreatedIds.length > 0) {
          ...
          if (missing.length > 0) {
            ...
            throw new Error(
              `Failed to confirm persistence for prompts: ${missing.join(", ")}`,
            );
          }
        }
        ...
        console.log(
          ...
          promptPayloads.filter((p) =>
            p.rationale?.startsWith("Follow-up to:"),
          ),
        );
        console.groupEnd();
        ...
        if (
          !suppressDeletionRef.current &&
          existingPromptIdsRef.current.length
        ) {
          const toDelete = existingPromptIdsRef.current.filter(
            (id) => !keepIds.has(id),
          );
          if (toDelete.length) {
        ...
        if (sectionError)
          consola.error("Error saving questions meta:", sectionError);
      } catch (e) {
  ...
  const _generateOneInCategory = useCallback(
    async (categoryId: string) => {
      ...
      try {
        ...
        const catName =
          questionCategories.find((c) => c.id === categoryId)?.name ||
          categoryId;
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      const baseIds = getBaseSelectedIds();
      console.log(
        "🔍 DEBUG: Debounced save triggered with",
        questions.length,
        "questions",
      );
    ...
    try {
      ...
      const baseQuestion = {
        ...
        qualityFlag: {
          ...
          description:
            evaluation.quick_feedback || "Quality assessment completed",
        },
      } as Question;
      const customQuestion: Question = {
        ...baseQuestion,
  ...
  const _getAnsweredCountColor = (count: number) => {
    ...
    if (count <= 3)
      return "bg-transparent text-yellow-600 dark:text-yellow-400";
  ...
  const _handlePendingInsertionChange = useCallback(
  ...
  const _handlePendingCategoryChange = useCallback(
  ...
  const handleAddContextualQuestion = useCallback(async () => {
    ...
    try {
      ...
      const newQuestion: Question = {
        ...baseQuestion,
        estimatedMinutes:
          savedQuestion.estimated_time_minutes ??
  ...
  const _syncExistingInterviews = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        `Synced questions for ${interviews?.length || 0} interviews`,
      );
    } catch (error) {
  ...
  const _handleAcceptPendingQuestion = useCallback(
    async (questionId: string, mode: "end" | "after") => {
      ...
      const pending = pendingGeneratedQuestions.find(
        (q) => q.id === questionId,
      );
      if (!pending) {
      ...
      if (mode === "after") {
        ...
        const anchorIndex =
          anchorId && baseIds.includes(anchorId)
            ? baseIds.indexOf(anchorId)
            : baseIds.length - 1;
        if (anchorIndex >= 0) {
          newBaseIds = [
            ...baseIds.slice(0, anchorIndex + 1),
            pending.id,
            ...baseIds.slice(anchorIndex + 1),
          ];
        } else {
  ...
  return (
    <div className="w-full space-y-2">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <h1 className="whitespace-nowrap font-bold text-2xl text-foreground">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            {researchModeLabel.charAt(0).toUpperCase() +
              researchModeLabel.slice(1)}{" "}
            • {timeMinutes}m
          </Button>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className={
                      showCategoryTime ? "border-blue-200 bg-blue-50" : ""
                    }
                  >
                    {showCategoryTime ? (
                ...
                <TooltipContent>
                  {showCategoryTime ? "Hide" : "Show"} category and time
                  information
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className={
                      mustHavesOnly ? "border-orange-200 bg-orange-50" : ""
                    }
                  >
                    <Filter className="h-4 w-4" />
                    <span className="ml-1 hidden sm:inline">
                      {mustHavesOnly ? "Must-Haves" : "All"}
                    </span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <label className="mb-3 block font-medium text-sm">
                  Interview Time: {timeMinutes} minutes
                </label>
                <Slider
                  value={[timeMinutes]}
              ...
              <div>
                ...
                <Select
                  value={researchMode}
                  onValueChange={(v: ManagerResearchMode) => setResearchMode(v)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    <SelectItem value="user_testing">
                      User Testing (product feedback)
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
      ...
      {showSettings && (
        <div className="rounded-lg bg-blue-50 p-4 text-sm">
          <p className="text-blue-800">
            ...
            interview with{" "}
            <strong>{familiarity === "cold" ? "new" : "familiar"}</strong>{" "}
            participants to gather insights for my research.
          </p>
        </div>
      )}
      ...
      <div className="space-y-4">
            ...
            {!showContextualInput ? (
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Select
                    value={contextualCategory}
                    onValueChange={setContextualCategory}
                  >
          ...
          <Card className="border-blue-100">
            <CardContent className="space-y-3 p-4">
              <div className="flex items-center gap-2">
                ...
                <Select
                  value={String(moreCount)}
                  onValueChange={(v) => setMoreCount(Number(v))}
                >
        ...
        <Dialog
          open={!!improvingId}
          onOpenChange={(o) => !o && setImprovingId(null)}
        >
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="space-y-4">
              <p className="text-muted-foreground text-sm">
                Replaces existing questions with new set. Custom instructions
                optional.
              </p>
              <Textarea
          ...
          {questions.length === 0 && !generating && (
            <div className="rounded-lg border-2 border-muted-foreground/25 border-dashed bg-muted/30 p-8 text-center">
              ...
              <p className="mb-4 text-muted-foreground text-sm">
                Generate AI-assisted questions based on your project context, or
                write your own using the input above.
              </p>
            </div>
          )}
                ...
                >
                  {questionPack.questions.map((question, index) => {
                    const isFirstOverflow =
                      index === questionPack.overflowIndex;
                    const cat = questionCategories.find(
                          ...
                          <div className="mt-6 border-orange-300 border-t-2 border-dashed pt-4">
                            <div className="mb-4 flex items-center gap-2">
                              ...
                              <span className="font-medium text-orange-600 text-sm">
                                Questions below may not fit in your{" "}
                                {timeMinutes}-minute time limit
                              </span>
                            </div>
                          </div>
                        )}
                                      ...
                                      <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                          ...
                                          <DropdownMenuItem
                                            onClick={async () => {
                                              console.group(
                                                `🗑️ DELETE QUESTION [${question.id.slice(0, 8)}]`,
                                              );
                                ...
                                <div className="mt-2 ml-8 rounded-lg border border-blue-200 bg-blue-50/50 p-4">
                                  ...
                                  <div className="mb-3 space-y-2">
                                    ...
                                    <Select
                                      ...
                                      onValueChange={setFollowupCategory}
                                    >
                                      ...
                                      <Button
                                        onClick={async () => {
                                          ...
                                          try {
                                            ...
                                            toast.error(
                                              "Failed to add follow-up question",
                                            );
                                          } finally {
                                            setAddingCustomQuestion(false);
                                          }
            ...
            <div className="mt-6">
              <div className="border-t pt-4">
                ...
                >
                  ...
                  {showAllQuestions ? "Hide" : "Show"} Backup Questions (
                ...
                {showAllQuestions && (
                  <div className="mt-4 space-y-3">
                    <p className="text-gray-600 text-sm">
                      Generated backup questions - Click{" "}
                      <Plus className="inline h-3 w-3" /> to add to your
                      interview plan:
                    </p>
                                ...
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button
                                      ...
                                      variant="outline"
                                      onClick={() =>
                                        addQuestionFromReserve(question)
                                      }
