	...
	"flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 bg-background font-medium text-xs",
	{
		variants: {
			status: {
				...
				completed: "border-primary bg-blue-500 text-primary-foreground",
				...
				<div key={item.id} className={cn(timelineItemVariants({ orientation }))}>
					...
					<div className="flex min-w-0 flex-1 flex-col gap-2">
						...
						{showTimestamps && timestampPosition === "top" && item.timestamp && (
							<time className="text-muted-foreground text-xs">{formatTimestamp(item.timestamp)}</time>
						)}
						...
						<div className="flex items-start justify-between gap-2">
							...
							{showTimestamps && timestampPosition === "inline" && item.timestamp && (
								<time className="shrink-0 text-muted-foreground text-xs">{formatTimestamp(item.timestamp)}</time>
							)}
						</div>
...
export function TimelineVariantsExample() {
	...
	return (
		<div className="space-y-8">
			...
			<div>
				<h3 className="mb-4 font-medium text-sm">Spacious</h3>
