...
/**
 * Prevents Radix DismissableLayer from closing Sheet/Dialog when user
 * interacts with a portaled sub-element (Select, Popover, Combobox, etc.).
 * These portals render outside the DismissableLayer bounds, so Radix
 * treats clicks on them as "outside" and dismisses the parent.
 */
function isInsideRadixPortal(target: EventTarget | null): boolean {
  if (!(target instanceof HTMLElement)) return false;
  return (
    target.closest(
      "[data-radix-popper-content-wrapper], [data-radix-select-viewport], [data-radix-menu-content], [role='listbox'], [cmdk-list]",
    ) !== null
  );
}

function handleDismissOutside(e: Event) {
