...
export function ConfidenceBarChart({ level, className, size = "md", variant = "bars" }: ConfidenceBarChartProps) {
	...
	if (variant === "icons") {
		...
		switch (normalizedLevel) {
			case "high":
				return <div title={getTooltipText()} className="inline-block"><CheckCircle {...iconProps} className={cn(iconProps.className, "text-green-600")} /></div>
			case "medium":
				return <div title={getTooltipText()} className="inline-block"><AlertCircle {...iconProps} className={cn(iconProps.className, "text-yellow-600")} /></div>
			case "low":
			default:
				return <div title={getTooltipText()} className="inline-block"><XCircle {...iconProps} className={cn(iconProps.className, "text-red-600")} /></div>
		}
	}
