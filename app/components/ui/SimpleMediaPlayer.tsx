...
export function SimpleMediaPlayer({
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.signedUrl) {
            ...
            consola.warn(
              "Thumbnail signed URL response missing signedUrl:",
              data,
            );
          }
        } else {
  ...
  const handleDownload = useCallback(async () => {
    try {
      ...
      if (data.signedUrl) {
        ...
        link.download =
          extractFilenameFromUrl(data.signedUrl) ??
  ...
  if (isLoading) {
    return (
      <div
        className={cn(
          "flex items-center justify-center rounded-md border bg-muted p-8",
          className,
        )}
      >
    ...
    return (
      <div className={cn("relative w-full", className)}>
        ...
        >
          ...
          {isAudio && (
            <div className="absolute left-4 text-muted-foreground text-sm">
              Load audio
            </div>
          )}
        </button>
        ...
        {showDebug && (
          <details className="mt-2 text-xs" open>
            ...
            <div className="mt-1 space-y-1 rounded bg-gray-50 p-2 text-xs dark:bg-gray-900">
              ...
              <div>
                Media type: {isAudio ? "audio" : isVideo ? "video" : "unknown"}
              </div>
            </div>
          </details>
        )}
    ...
    <div className={cn("relative w-full space-y-3", className)}>
      ...
      {startSeconds > 0 && (
        <div className="text-right text-muted-foreground text-xs">
          Start time: {Math.floor(startSeconds / 60)}:
          {String(Math.floor(startSeconds % 60)).padStart(2, "0")}
        </div>
      )}
