...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const customInstructions =
      formData.get("custom_instructions")?.toString() || null;

    if (!interviewId) {
      return Response.json(
        { ok: false, error: "Missing interview_id" },
        { status: 400 },
      );
    }
    ...
    const handle = await tasks.trigger<typeof regenerateAISummaryTask>(
      "sales.regenerate-ai-summary",
      {
