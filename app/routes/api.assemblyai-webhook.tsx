...
export async function loader() {
  ...
  return Response.json(
    { status: "ok", service: "assemblyai-webhook" },
    { status: 200 },
  );
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const { data: interview, error: interviewError } = await supabase
      .from("interviews")
      .select(
        "id, status, transcript, conversation_analysis, account_id, project_id, title",
      )
    ...
    if (interviewError || !interview) {
      if (mediaId) {
        ...
        return Response.json(
          { success: true, media_id: mediaId, status: payload.status },
          { status: 202 },
        );
      }

      consola.error(
        "Interview query failed for transcript:",
    ...
    if (
      ...
      (interview.status === "processing" &&
        (existingTriggerRunId || orchestratorPending))
    ) {
    ...
    if (payload.status === "completed") {
      ...
      consola.log(
        "AssemblyAI Webhook: Fetching transcript data for transcript:",
        payload.transcript_id,
      );
      ...
      consola.log(
        "AssemblyAI Webhook: Retrieved transcript data, length:",
      ...
      consola.log(
        "Audio file already stored during upload - skipping AssemblyAI download",
      );
      ...
      const updateData: Database["public"]["Tables"]["interviews"]["Update"] = {
        ...
        duration_sec: transcriptData.audio_duration
      ...
      if (interviewUpdateError) {
        throw new Error(
          `Failed to update interview: ${interviewUpdateError.message}`,
        );
      }
      ...
      const updatedConversationAnalysis = {
        ...
        completed_steps: [
          ...(conversationAnalysis.completed_steps || []),
          "transcription",
        ],
      };
      ...
      try {
        consola.info(
          "Triggering analysis orchestrator for interview:",
          interviewId,
        );
        ...
        const fetchedInterview = await supabase
          .from("interviews")
          .select(
            "account_id, project_id, title, participant_pseudonym, person_id",
          )
          .eq("id", interviewId)
          .single();
        ...
        if (!participantName && fetchedInterview.data?.participant_pseudonym) {
          ...
          if (
            pseudonym &&
            !pseudonym.match(/^(Participant|Anonymous)\s*\d*$/i)
          ) {
