...
describe("api/upload-file", () => {
	...
	describe("text file upload (.txt)", () => {
		...
		it("processes a .md file the same as .txt", async () => {
			const file = new File(["# Customer feedback\nThey loved the feature."], "notes.md", {
				type: "text/markdown",
			});
			const fd = makeFormData(file);
			...
			expect(response.status).toBe(200);
			expect(mockSanitize).toHaveBeenCalledWith(expect.objectContaining({ file_type: "text" }));
		});
	...
	describe("file type detection", () => {
		...
		it("does NOT use pdf-parse for audio files", async () => {
			...
			const { transcribeAudioFromUrl } = await import("~/utils/assemblyai.server");
