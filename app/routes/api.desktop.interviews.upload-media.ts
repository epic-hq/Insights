...
import { uploadToR2, createR2PresignedUploadUrl } from "~/utils/r2.server";
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      const { interview_id, file_name, file_type, file_size, action, r2_key } =
        body;
      ...
      if (action === "confirm") {
        if (!interview_id || !r2_key) {
          return Response.json(
            { error: "interview_id and r2_key are required for confirm" },
            { status: 400 },
          );
        }
        ...
        if (!r2_key.startsWith(expectedPrefix)) {
          return Response.json(
            { error: "Invalid R2 key for this interview" },
            { status: 400 },
          );
        }
        ...
        if (updateError) {
          consola.error(
            "[desktop-upload-media] Failed to confirm upload:",
            updateError.message,
          );
          return Response.json(
        ...
        consola.info(
          `[desktop-upload-media] Confirmed upload for interview ${interview_id}: ${r2_key}`,
        );
      ...
      if (!interview_id || !file_name) {
        return Response.json(
          { error: "interview_id and file_name are required" },
          { status: 400 },
        );
      }
      ...
      consola.info(
        `[desktop-upload-media] Created presigned URL for interview ${interview_id}`,
        { r2Key, file_size },
      );
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (!file || !interviewId) {
        return Response.json(
          { error: "file and interview_id are required" },
          { status: 400 },
        );
      }
      ...
      if (updateError) {
        consola.error(
          "[desktop-upload-media] Failed to update interview:",
          updateError.message,
        );
      }
