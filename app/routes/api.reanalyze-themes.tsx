...
function fallbackPersonName(options: {
  ...
  if (interviewTitle?.trim() && !interviewTitle.includes("Interview -"))
    return interviewTitle.trim();
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const customInstructions =
      conversationAnalysis.custom_instructions ?? undefined;
    ...
    const participantPersonId =
      interviewPeopleRows?.find((row) => row.role === "participant")
        ?.person_id ??
    ...
    let primaryPerson = primaryPersonId
      ? (peopleRows?.find((person) => person.id === primaryPersonId) ?? null)
      : null;

    if (!primaryPersonId) {
      const fallbackName = fallbackPersonName({
        fileName:
          (sanitizedTranscript?.original_filename as string | undefined) ??
          null,
      ...
      if (ensureErr || !ensuredPerson) {
        return Response.json(
          { error: "Failed to ensure participant record" },
          { status: 500 },
        );
      }
      ...
      if (linkErr && !linkErr.message?.includes("duplicate")) {
        return Response.json(
          { error: "Failed to link participant to interview" },
          { status: 500 },
        );
      }
    }
    ...
    if (!primaryPersonId) {
      return Response.json(
        { error: "Unable to resolve interview participant" },
        { status: 500 },
      );
    }
    ...
    const participantRole =
      primaryPerson?.role ??
      interviewPeopleRows?.find((row) => row.person_id === primaryPersonId)
        ?.role ??
      null;
    ...
    const evidenceUnits = evidenceRows.map((row) => {
      ...
      const gist =
        row.gist && row.gist.trim().length > 0 ? row.gist : row.verbatim;
      const chunk =
        row.chunk && row.chunk.trim().length > 0 ? row.chunk : row.verbatim;
        ...
        end_ms:
          typeof firstAnchor?.end_ms === "number"
            ...
            : typeof firstAnchor?.end === "string"
              ...
              : undefined,
        chapter_title:
          typeof firstAnchor?.chapter_title === "string"
            ? firstAnchor.chapter_title
            : undefined,
        char_span: Array.isArray(firstAnchor?.char_span)
          ? firstAnchor.char_span
          : undefined,
      };
    ...
    const evidenceFacetKinds = evidenceIds.map(
      (id) => facetByEvidence.get(id) ?? [],
    );

    const analysisMetadata = {
      accountId: interview.account_id,
      userId:
        claims.sub ?? interview.updated_by ?? interview.created_by ?? undefined,
      ...
      durationMin: interview.duration_sec
        ? interview.duration_sec / 60
        : undefined,
      fileName:
        (sanitizedTranscript?.original_filename as string | undefined) ??
        undefined,
    ...
    const evidenceResult = {
      ...
      primaryPersonName:
        primaryPerson?.name ?? interview.participant_pseudonym ?? null,
      ...
      primaryPersonSegments: primaryPerson?.segment
        ? [primaryPerson.segment]
        : [],
    ...
    if (useV2Workflow) {
      ...
      if (
        mediaUrlForTask &&
        !mediaUrlForTask.startsWith("http://") &&
        !mediaUrlForTask.startsWith("https://")
      ) {
        ...
        if (presigned) {
          ...
          consola.log(
            `Generated presigned URL for theme reprocessing: ${interviewId}`,
          );
        }
      }
