...
describe("onboarding data flow validation", () => {
	...
	it("job_function from onboarding maps to user_settings.job_function", () => {
		// The action sets job_function = onboardingData.jobFunction || existing.job_function
		const jobFunction = "product";
		const existingJobFunction = "founder";
		const resolvedJobFunction = jobFunction || existingJobFunction || null;

		expect(resolvedJobFunction).toBe("product");
	});

	it("falls back to existing job_function when jobFunction is empty", () => {
		const jobFunction = "";
		const existingJobFunction = "founder";
		const resolvedJobFunction = jobFunction || existingJobFunction || null;

		expect(resolvedJobFunction).toBe("founder");
	});

	it("uses null when both jobFunction and existing job_function are empty", () => {
		const jobFunction = "";
		const existingJobFunction = null;
		const resolvedJobFunction = jobFunction || existingJobFunction || null;

		expect(resolvedJobFunction).toBeNull();
	});
});
