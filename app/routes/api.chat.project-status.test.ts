...
describe("api.chat.project-status", () => {
  ...
  it("routes people and task operational prompts through deterministic projectStatus network", async () => {
    ...
    expect(mockedHandleChatStream).toHaveBeenCalledTimes(2);
    expect((mockedHandleChatStream.mock.calls[1][0] as any).agentId).toBe(
      "projectStatusAgent",
    );
  });
    ...
    expect(
      chunks.some((chunk) => {
        ...
        const data = (chunk as { data?: unknown }).data as
          ...
          | undefined;
        return (
          data?.path === "/a/acct-1/project-1/ask/survey-1/edit" &&
          chunk.id === "legacy-nav"
        );
      }),
    ).toBe(true);
  ...
  it("does not enforce CSV contract for researchAgent requests without csv format intent", async () => {
    ...
    const chatCall = mockedHandleChatStream.mock.calls[0][0] as any;
    expect(
      chatCall.params.requestContext.get("csv_requested_rows"),
    ).toBeUndefined();
    const csvContractContext = chatCall.params.context.find((entry: any) =>
      String(entry?.content ?? "").includes("Output Contract (MANDATORY)"),
    );
