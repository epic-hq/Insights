...
const mockedCreateUIMessageStreamResponse = vi.mocked(
  createUIMessageStreamResponse,
);
...
const mockedResolveAccountIdFromProject = vi.mocked(
  resolveAccountIdFromProject,
);
...
function buildArgs(options?: {
  ...
  const request = new Request(
    "http://localhost/api/chat/project-status/project-1",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function makeTextStream(options: {
  text?: string;
  toolName?: string;
  emitFinish?: boolean;
}) {
...
describe("api.chat.project-status", () => {
  ...
  it("routes fast standardized prompts to Chief of Staff with reduced step/tool streaming budget", async () => {
    ...
    const systemPart = contextText.replace(
      "## Context from the client's UI:\n",
      "",
    );
  ...
  it("falls back to projectStatusAgent when routing confidence is below threshold", async () => {
    ...
    const response = await action(
      buildArgs({ message: "status update please" }),
    );
    expect(response.status).toBe(200);
  ...
  it("routes top-theme questions through deterministic projectStatus network flow", async () => {
    ...
    const response = await action(
      buildArgs({ message: "what are top 2 themes and who has them?" }),
    );
    expect(response.status).toBe(200);
  ...
  it("routes people-comparison prompts through normal flow when classifier selects normal mode", async () => {
    ...
    const routingPrompt = (mockedGenerateObject.mock.calls.at(-1)?.[0] as any)
      ?.prompt as string;
  ...
  it("routes ICP lookup prompts to projectStatusAgent via deterministic routing", async () => {
    ...
    const response = await action(
      buildArgs({ message: "what icp matches do i have?" }),
    );
    expect(response.status).toBe(200);
  ...
  it("routes how-to prompts to howtoAgent with ux_research_mode", async () => {
    const response = await action(
      buildArgs({ message: "How do I run better user interviews?" }),
    );
    expect(response.status).toBe(200);
  ...
  it("routes how-to GTM prompts to howtoAgent with gtm_mode", async () => {
    const response = await action(
      buildArgs({ message: "Best way to position and launch this product?" }),
    );
    expect(response.status).toBe(200);
  ...
  it("enforces howto response contract sections and markdown links when missing", async () => {
    mockedHandleChatStream.mockResolvedValue(
      makeTextStream({ text: "Try a focused test this week." }) as any,
    );

    const response = await action(
      buildArgs({ message: "teach me how to improve our onboarding UX" }),
    );
    expect(response.status).toBe(200);
    expect(mockedHandleChatStream).toHaveBeenCalledTimes(1);
  ...
  it("injects a fallback message when stream finishes without assistant text", async () => {
    ...
    const call = mockedCreateUIMessageStreamResponse.mock.calls.at(
      -1,
    )?.[0] as any;
  ...
  it("returns non-empty howto fallback contract with links when upstream emits no text", async () => {
    mockedHandleChatStream.mockResolvedValue(
      makeTextStream({ emitFinish: true }) as any,
    );

    const response = await action(
      buildArgs({ message: "how do i validate product demand quickly?" }),
    );
    expect(response.status).toBe(200);
    expect(mockedHandleChatStream).toHaveBeenCalledTimes(1);
    ...
    expect(text.toLowerCase()).toContain("quick links");
    expect(text).toMatch(
      /\[[^\]]+\]\(\/a\/acct-1\/project-1\/(people|insights|ask)\)/,
    );
  });
  ...
  it("supports /debug prefix, strips it from execution prompt, and appends a debug trace", async () => {
    ...
    const response = await action(
      buildArgs({ message: "/debug help me prioritize this sprint" }),
    );
    expect(response.status).toBe(200);
    expect(mockedHandleChatStream).toHaveBeenCalledTimes(1);

    const chatCall = mockedHandleChatStream.mock.calls[0][0] as any;
    expect(chatCall.params.messages[0].content).toBe(
      "help me prioritize this sprint",
    );
  ...
  it("routes survey creation prompts to researchAgent through deterministic flow", async () => {
    ...
    const response = await action(
      buildArgs({ message: "create a waitlist survey for our beta launch" }),
    );
    expect(response.status).toBe(200);
    expect(mockedGenerateObject).not.toHaveBeenCalled();
    expect(mockedHandleChatStream).toHaveBeenCalledTimes(1);
  ...
  it("returns fallback + debug trace when survey execution stream init fails", async () => {
    ...
    mockedHandleChatStream.mockRejectedValueOnce(
      new Error("simulated stream init failure"),
    );

    const response = await action(
      buildArgs({
        message: "/debug create a survey to learn more from those people",
      }),
    );
    expect(response.status).toBe(200);
    expect(mockedHandleChatStream).toHaveBeenCalledTimes(1);
  ...
  it("routes people and task operational prompts through deterministic projectStatus network", async () => {
    ...
    expect(mockedGenerateObject).not.toHaveBeenCalled();
    expect(mockedHandleNetworkStream).toHaveBeenCalledTimes(1);
    expect((mockedHandleNetworkStream.mock.calls[0][0] as any).agentId).toBe(
      "projectStatusAgent",
    );
    ...
    expect((mockedHandleNetworkStream.mock.calls[1][0] as any).agentId).toBe(
      "projectStatusAgent",
    );
  });
  ...
  it("injects fallback even when upstream stream closes without finish chunk", async () => {
    mockedHandleChatStream.mockResolvedValue(
      makeTextStream({ emitFinish: false }) as any,
    );
    mockedGenerateObject.mockResolvedValue({
      object: {
        targetAgentId: "chiefOfStaffAgent",
  ...
  it("builds contextual quick links for people/ICP queries", async () => {
    ...
    expect(quickLinks).toContain(
      "[People](/a/acct-1/project-1/lenses?tab=people)",
    );
    expect(quickLinks).toContain(
      "[Insights](/a/acct-1/project-1/insights/table)",
    );
  });
});
