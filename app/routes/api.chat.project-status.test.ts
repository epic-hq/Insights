...
describe("api.chat.project-status", () => {
	...
	it("routes typed ui events to the sticky agent without classifier", async () => {
		...
		expect(mockedHandleChatStream).toHaveBeenCalledTimes(1);
		expect((mockedHandleChatStream.mock.calls[0][0] as any).agentId).toBe("projectSetupAgent");
	...
	it("enforces top-N CSV contract by appending corrected CSV from web research results", async () => {
		...
		const response = await action(
			buildArgs({
				message: "give me list of top 8 agentic ai systems like openClaw, Claude CoWork etc, in csv",
			})
		);
		expect(response.status).toBe(200);
		expect(mockedHandleChatStream).toHaveBeenCalledTimes(1);
