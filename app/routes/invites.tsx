...
export async function loader({ request }: { request: Request }) {
  ...
  if (error) {
    ...
    return data(
      { ok: false, error: error.message, invites: [] as InviteItem[] },
      { headers, status: 500 },
    );
  }
      ...
      <div className="container mx-auto max-w-3xl py-10">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-400">
            Accept invitations that were sent to your email.
          </p>
        </div>
        ...
        {invites.length === 0 ? (
          <div className="rounded-xl border bg-white/80 p-8 text-center dark:bg-slate-900/80">
            <p className="text-slate-600 dark:text-slate-400">
              No invitations found for your email.
            </p>
            <p className="mt-2 text-slate-500 text-sm dark:text-slate-500">
              Ask your teammate to resend an invite if you believe this is a
              mistake.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <a
                      href={`/accept-invite?invite_token=${encodeURIComponent(inv.token)}`}
                    >
