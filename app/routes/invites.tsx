    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800">
      <div className="container mx-auto max-w-3xl py-10">
        <div className="mb-8 text-center">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Your Invitations</h1>
          <p className="text-slate-600 dark:text-slate-400 mt-2">Accept invitations that were sent to your email.</p>
        </div>
        ...
        {invites.length === 0 ? (
          <div className="rounded-xl border bg-white/80 p-8 text-center dark:bg-slate-900/80">
            ...
            <p className="text-slate-500 dark:text-slate-500 text-sm mt-2">Ask your teammate to resend an invite if you believe this is a mistake.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {invites.map((inv) => (
              <div key={`${inv.account_id}-${inv.token}`} className="rounded-xl border bg-white/80 p-4 dark:bg-slate-900/80">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <a href={`/accept-invite?token=${encodeURIComponent(inv.token)}`}>
                      <Button>Accept</Button>
                    </a>
                    <button
                      className="text-xs text-slate-500 hover:text-slate-700 underline"
                      onClick={() => navigator.clipboard.writeText(`${window.location.origin}/accept-invite?token=${encodeURIComponent(inv.token)}`)}
                    >
                      Copy link
                    </button>
                  </div>
