  ...
  try {
    ...
    if (!projectId || !accountId) {
      return Response.json(
        { error: "project_id and account_id required" },
        { status: 400 },
      );
    }
    ...
    if (action === "reset") {
      ...
      if (linkDeleteError) {
        consola.error(
          "[diagnose-themes] Failed to delete theme_evidence:",
          linkDeleteError,
        );
        return Response.json(
          { error: `Failed to delete links: ${linkDeleteError.message}` },
          { status: 500 },
        );
      }

      consola.info(
        `[diagnose-themes] Deleted ${linksDeleted} theme_evidence links`,
      );
      ...
      if (themeDeleteError) {
        consola.error(
          "[diagnose-themes] Failed to delete themes:",
          themeDeleteError,
        );
        return Response.json(
          { error: `Failed to delete themes: ${themeDeleteError.message}` },
          { status: 500 },
        );
      }
      ...
      consola.success(
        `[diagnose-themes] Reset complete: ${result.themes.length} themes, ${result.link_count} links`,
      );
    ...
    consola.log(
      "[diagnose-themes] theme_evidence sample:",
      themeEvidenceLinks?.slice(0, 3),
    );
    ...
    const diagnosis = {
      ...
      themeEvidenceSample: (themeEvidenceLinks ?? [])
        .slice(0, 5)
        .map((te: any) => ({
      ...
      ratios: {
        evidencePerInterview: interviewCount
          ? ((evidenceCount ?? 0) / interviewCount).toFixed(1)
          : "N/A",
        linksPerTheme: themeCount
          ? ((linkCount ?? 0) / themeCount).toFixed(1)
          : "N/A",
        linksPerEvidence: evidenceCount
          ? ((linkCount ?? 0) / evidenceCount).toFixed(1)
          : "N/A",
      },
    ...
    if ((evidenceCount ?? 0) === 0) {
      diagnosis.issues.push(
        "No evidence found - interviews may not have been processed",
      );
    }
    if ((themeCount ?? 0) > 0 && (linkCount ?? 0) === 0) {
      diagnosis.issues.push(
        "Themes exist but no evidence links - run Consolidate Themes",
      );
    }
    if ((linkCount ?? 0) > (evidenceCount ?? 0) * 3) {
      diagnosis.issues.push(
        "Over-linking detected - too many links per evidence (run Consolidate)",
      );
    }
    if ((themeCount ?? 0) > 50) {
      diagnosis.issues.push(
        "Many themes - consider consolidating to reduce duplicates",
      );
    }
