...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const { data: interview, error: interviewError } = await supabase
      .from("interviews")
      .select(
        "id, title, transcript, transcript_formatted, media_url, account_id, project_id, created_by",
      )
      .eq("id", interviewId)
      .single();
    ...
    // forceReprocess bypasses idempotency key since this is an explicit reprocess request
