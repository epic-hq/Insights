...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const intent: MediaIntent =
    body.intent === "download" ? "download" : "playback";
  let key =
  ...
  if (!key) {
    ...
    return Response.json(
      {
        error:
          "Unable to resolve media key. Provide either 'key' (R2 path) or 'mediaUrl' (full URL or R2 path)",
      },
      { status: 400 },
    );
  }
  ...
  const responseContentDisposition =
    intent === "download"
      ? buildContentDisposition(sanitizeFilename(body.filename ?? key))
      : "inline";
  ...
  if (!presigned) {
    ...
    return Response.json(
      { error: "Cloudflare R2 signing is not available" },
      { status: 500 },
    );
  }
