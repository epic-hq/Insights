...
import {
  getMigrationStatus,
  migrateArrayDataToJunctions,
} from "~/utils/migrateArrayData.server";
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    if (action === "migrate") {
      ...
      consola.info(
        `Starting array-to-junction migration for account ${accountId}`,
      );
      ...
      consola.success(
        `Migration completed successfully: ${stats.totalProcessed} items migrated`,
      );
      return Response.json({
        success: true,
        stats,
