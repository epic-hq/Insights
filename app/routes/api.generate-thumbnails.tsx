...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const limit = Number.parseInt(
      formData.get("limit")?.toString() || "50",
      10,
    );
    ...
    if (action === "stats") {
      ...
      const videoInterviews =
        interviews?.filter((i) => isVideoFile(i.media_url)) || [];
    ...
    if (interviewId) {
      ...
      if (!interview.media_url) {
        return Response.json(
          { error: "Interview has no media file" },
          { status: 400 },
        );
      }
      ...
      if (!isVideoFile(interview.media_url)) {
        return Response.json(
          { error: "Interview media is not a video file" },
          { status: 400 },
        );
      }
      ...
      consola.info(
        `Triggering thumbnail generation for interview ${interviewId}`,
      );

      const handle = await tasks.trigger<typeof generateThumbnail>(
        "generate-thumbnail",
        {
          mediaKey,
    ...
    if (action === "backfill") {
      ...
      consola.info(
        `Batch triggering thumbnail generation for ${payloads.length} interviews`,
      );

      const batchHandle = await tasks.batchTrigger<typeof generateThumbnail>(
        "generate-thumbnail",
        payloads,
      );
    ...
    return Response.json(
      {
        error:
          "Invalid request. Provide interview_id, or action=stats/backfill",
      },
      { status: 400 },
    );
  } catch (error) {
