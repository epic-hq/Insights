...
const COMPONENT_GALLERY: Array<{
	...
	},
	{
		...
		data: {
			...
			actions: [
				...
				{
					id: "a2",
					action: "Create a comparison doc: your tool vs. current spreadsheet workflow",
					reasoning:
	...
	},
	{
		...
		data: {
			...
			overallSummary: "Strong budget signals and clear authority, but timeline is unclear.",
			insights: [
				{
					label: "Budget",
					summary: "Has approved $50K budget for this quarter. Mentioned willingness to expand if ROI proven.",
					confidence: 85,
				},
				{
					label: "Authority",
					summary: "Direct decision maker for tools under $100K. VP approval needed above.",
					confidence: 92,
				},
				{
					label: "Need",
					summary: "Current workflow takes 3x longer than competitors. Team morale affected.",
					confidence: 78,
				},
				{
					label: "Timeline",
					summary: "Mentioned 'sometime this quarter' but no hard deadline. Waiting on budget cycle.",
					confidence: 45,
				},
			],
		},
	},
	{
		...
		data: {
			...
			totalResponses: 47,
			completionRate: 82,
			topThemes: ["Ease of use", "Pricing concerns", "Integration needs", "Mobile support"],
			questionSummaries: [
				{
					question: "How disappointed would you be if you could no longer use this product?",
					topAnswer: "Very disappointed (62%)",
					responseCount: 47,
				},
				{
					question: "What is the primary benefit you receive from this product?",
					topAnswer: "Saves time on customer research (34%)",
					responseCount: 45,
				},
				{
					question: "What would you use as an alternative if this product didn't exist?",
...
function GenUITestInner() {
	...
	return (
		<div className="mx-auto max-w-5xl space-y-6 p-6">
			<div>
				<h1 className="font-bold text-2xl">Gen-UI Component Gallery</h1>
			...
			<Card>
				...
				<CardContent>
					{log.length === 0 ? (
						<p className="text-muted-foreground text-sm">Click a component above to start.</p>
					) : (
						<pre className="max-h-36 overflow-y-auto rounded bg-muted p-2 font-mono text-xs">{log.join("\n")}</pre>
					)}
				</CardContent>
			</Card>
...
function InlineUserInputTestSection({ addLog }: { addLog: (msg: string) => void }) {
	const handleSingleSubmit = useCallback(
		(selectedIds: string[], freeText?: string) => {
			addLog(`[Single] selected=${JSON.stringify(selectedIds)}, freeText=${freeText ?? "none"}`);
		},
		[addLog]
	);
	...
	const handleMultiSubmit = useCallback(
		(selectedIds: string[], freeText?: string) => {
			addLog(`[Multi] selected=${JSON.stringify(selectedIds)}, freeText=${freeText ?? "none"}`);
		},
		[addLog]
	);
	...
	return (
		<Card>
			<CardHeader className="pb-2">
				<CardTitle className="text-base">InlineUserInput (requestUserInput)</CardTitle>
			</CardHeader>
			<CardContent className="space-y-6">
				{/* Single selection (radio) */}
				<div>
					<p className="mb-2 font-medium text-muted-foreground text-xs uppercase">Single select (radio)</p>
				...
				<div>
					<p className="mb-2 font-medium text-muted-foreground text-xs uppercase">Multiple select (checkbox)</p>
					<div className="rounded-lg bg-background p-3 ring-1 ring-border/60">
						<InlineUserInput
				...
				<div>
					<p className="mb-2 font-medium text-muted-foreground text-xs uppercase">Already answered</p>
					<div className="rounded-lg bg-background p-3 ring-1 ring-border/60">
						<InlineUserInput
