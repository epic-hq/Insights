...
import { createSurveySends, getGmailConnection, sendGmailEmail } from "~/lib/integrations/gmail.server";
...
function buildSurveyLink(domain: string, slug: string, recipientEmail: string): string {
...
function buildEmailHtml(params: {
	...
	const message = params.customMessage || `I'd love to get your feedback. It only takes a few minutes.`;
...
export async function action({ context, request }: ActionFunctionArgs) {
	...
	const { accountId, projectId, surveyId, surveySlug, surveyName, subject, customMessage, recipients } = body as {
		accountId: string;
		projectId: string;
		surveyId: string;
		surveySlug: string;
	...
	if (!connection) {
		return Response.json({ error: "Gmail not connected. Please connect Gmail first." }, { status: 400 });
	}
