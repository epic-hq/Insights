...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (!teamAccountId) {
      ...
      const teamAccount =
        accounts?.find(
          (acc: Record<string, unknown>) => !acc.personal_account,
        ) || accounts?.[0];
    ...
    if (!project) {
      consola.error(
        "Failed to create project after multiple attempts:",
        projectError,
      );
      return Response.json(
    ...
    consola.log(
      `Created project: ${project.name} (${project.id}) for account ${teamAccountId}`,
    );
    ...
    if (posthog) {
      ...
      consola.debug(
        "[CREATE_PROJECT] PostHog client unavailable; skipping analytics",
      );
    }
