...
import {
  backfillMissingPeople,
  getInterviewPeopleStats,
} from "~/utils/backfillPeople.server";
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    if (!accountId) {
      return Response.json(
        { error: "Account ID not found in user claims" },
        { status: 400 },
      );
    }
    ...
    return Response.json(
      { error: "Invalid action. Use 'stats' or 'backfill'" },
      { status: 400 },
    );
  } catch (error) {
    return Response.json(
