...
function buildTranscriptPayload(
	...
	const fullTranscript = turns.map((turn) => `${turn.speaker}: ${turn.text}`).join("\n");
	...
	const speaker_transcripts = turns.map((turn, idx) => {
		...
		for (let i = idx + 1; i < turns.length; i += 1) {
			...
			if (typeof candidate === "number" && Number.isFinite(candidate) && candidate > start) {
				nextStart = candidate;
				break;
			}
		}
	...
	return safeSanitizeTranscriptPayload({
		...
		audio_duration: typeof durationSeconds === "number" ? durationSeconds : null,
		file_type: "realtime",
		speaker_transcripts,
	});
}
	...
	try {
		...
		if (people?.length) {
			for (const person of people) {
				...
				try {
					const resolution = await resolveOrCreatePerson(supabase, account_id, project_id, {
						name: person.person_name,
						primary_email: person.email || undefined,
						role: person.role || (person.is_host ? "interviewer" : "participant"),
		...
		if (resolvedPeopleMap.size > 0) {
			...
			const interviewPeopleRows = Array.from(resolvedPeopleMap.entries()).map(([personKey, personId]) => {
				const source = peopleByKey.get(personKey);
				return {
					interview_id,
					person_id: personId,
					project_id,
					role: source?.role || (source?.is_host ? "interviewer" : "participant") || null,
		...
		if (tasks?.length) {
			...
			for (const task of tasks) {
				try {
					...
					consola.error("[desktop-finalize] Failed to create task:", getErrorMessage(taskError));
				}
			}
		}
		...
		if (resolvedPeopleMap.size > 0) {
			try {
				...
				if (interviewPeopleError) {
					...
					if (evidenceError) {
						consola.warn("[desktop-finalize] Could not fetch evidence for linking:", evidenceError.message);
					} else {
						...
						for (const row of evidenceRows || []) {
							...
							const speakerLabel = anchors
								.map((anchor) => {
									...
									return (
										...
										(typeof record.speaker_label === "string" && record.speaker_label) ||
										""
									);
								})
								.find(Boolean);
							...
							const personId = resolvePersonIdForSpeakerLabel(speakerLabel, typedInterviewPeople);
				...
				consola.warn("[desktop-finalize] Evidence speaker linking failed:", getErrorMessage(linkError));
			}
		}
		...
		const statusUpdate: Record<string, unknown> = {
			...
			created_by: user?.id || interview.created_by || null,
			processing_metadata: {
				...((freshInterview?.processing_metadata as object) || (interview.processing_metadata as object) || {}),
				finalized_at: new Date().toISOString(),
		...
		return Response.json({ error: getErrorMessage(error) || "Finalization failed" }, { status: 500 });
	}
}
