...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const handle = await tasks.trigger<typeof enrichPeopleBatchTask>(
      "people.enrich-batch",
      {
    ...
    return Response.json({
      ...
      taskId: handle.id,
      publicAccessToken,
      message: personIds?.length
        ? `Enriching ${personIds.length} people`
        : "Enriching people with missing data",
    });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Unknown error";
    console.error("[enrich-people] Error:", error);
    return Response.json(
      { success: false, error: msg || "Failed to trigger enrichment" },
      { status: 500 },
    );
  }
}
