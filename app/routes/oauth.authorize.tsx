...
function buildErrorRedirect(redirectUri: string, error: string, state: string, errorDescription?: string): string {
...
export async function loader({ request }: Route.LoaderArgs) {
	...
	if (codeChallengeMethod !== "S256") {
		consola.warn("[oauth/authorize] Unsupported code_challenge_method:", codeChallengeMethod);
		throw new Response("Unsupported code_challenge_method. Only 'S256' is supported.", { status: 400 });
	}
	...
	const registeredUris: string[] = Array.isArray(client.redirect_uris) ? client.redirect_uris : [];
	...
	if (!registeredUris.includes(redirectUri)) {
		...
		throw new Response("redirect_uri does not match any registered redirect URIs for this client", { status: 400 });
	}
	...
	const accountsList = (Array.isArray(rawAccounts) ? rawAccounts : []) as Array<{
		account_id: string;
		name?: string | null;
		personal_account?: boolean | null;
	}>;

	const accountIds = accountsList.filter((a) => !a.personal_account).map((a) => a.account_id);
...
export async function action({ request }: Route.ActionArgs) {
	...
	if (!projectId || !clientId || !redirectUri || !codeChallenge || !codeChallengeMethod) {
...
export default function OAuthAuthorize({ loaderData }: Route.ComponentProps) {
	...
	const [selectedProjectId, setSelectedProjectId] = useState<string>(allProjects.length === 1 ? allProjects[0].id : "");
	const handleDeny = () => {
		...
		<div className="flex min-h-svh w-full items-center justify-center bg-gray-50 p-6 dark:bg-gray-950">
			<div className="w-full max-w-md">
				<div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
					{/* Header */}
					<div className="flex flex-col items-center gap-4 border-gray-200 border-b px-8 pt-8 pb-6 dark:border-gray-800">
						{/* Logo */}
						<div className="flex items-center gap-2">
							...
							<span className="font-semibold text-gray-900 text-xl dark:text-gray-100">UpSight</span>
						</div>

						<h1 className="text-center font-semibold text-gray-900 text-lg dark:text-gray-100">Connect to UpSight</h1>
						<p className="text-center text-gray-500 text-sm dark:text-gray-400">
							<span className="font-medium text-gray-700 dark:text-gray-300">{clientName}</span> wants to access your
							UpSight data
						</p>
					</div>
					...
					<Form method="post">
						...
						<input type="hidden" name="redirectUri" value={oauthParams.redirectUri} />
						<input type="hidden" name="codeChallenge" value={oauthParams.codeChallenge} />
						<input type="hidden" name="codeChallengeMethod" value={oauthParams.codeChallengeMethod} />
						...
						<div className="px-8 py-6">
							...
							<div className="mb-5 rounded-lg bg-gray-50 p-4 dark:bg-gray-800/50">
								<p className="mb-2 font-medium text-gray-700 text-sm dark:text-gray-300">This will allow access to:</p>
								<ul className="space-y-1.5 text-gray-600 text-sm dark:text-gray-400">
							...
							<div className="mb-6">
								...
								{allProjects.length === 0 ? (
									<p className="text-gray-500 text-sm dark:text-gray-400">
										You don't have any projects yet. Create a project in UpSight first.
									</p>
								) : accounts.length === 1 ? (
									...
									<select
										...
										className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-gray-900 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
									>
									...
									>
										...
										{accounts.map((account) => (
											<optgroup key={account.account_id} label={account.name || "Untitled team"}>
						...
						{/* Footer */}
						<div className="flex gap-3 border-gray-200 border-t px-8 py-6 dark:border-gray-800">
							<button
								type="button"
								onClick={handleDeny}
								className="flex-1 rounded-lg border border-gray-300 bg-white px-4 py-2.5 font-medium text-gray-700 text-sm shadow-sm transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
							>
								Deny
							</button>
							<button
