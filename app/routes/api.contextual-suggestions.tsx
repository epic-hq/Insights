...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const existing_items = JSON.parse(
      (formData.get("existingItems") as string) || "[]",
    );
    const shown_suggestions = JSON.parse(
      (formData.get("shownSuggestions") as string) || "[]",
    );
    const rejected_items = JSON.parse(
      (formData.get("rejectedItems") as string) || "[]",
    );
    const project_context = (formData.get("projectContext") as string) || "";
    const custom_instructions =
      (formData.get("customInstructions") as string) || "";
    const response_count = Number.parseInt(
      (formData.get("responseCount") as string) || "3",
      10,
    );
    const question_category =
      (formData.get("questionCategory") as string) || null;
    ...
    const rejected_items_array: string[] = Array.isArray(rejected_items)
      ? rejected_items
      : [];
