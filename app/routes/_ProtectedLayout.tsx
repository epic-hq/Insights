...
} from "~/lib/last-project-route.client";
...
export const middleware: Route.MiddlewareFunction[] = [
  async ({ request, context, params }) => {
    try {
      const { user, headers: authHeaders } =
      ...
      const supabase = jwt
        ? getRlsClient(jwt)
        : (await import("~/lib/supabase/client.server")).getServerClient(
            request,
          ).client;
      ...
      if (urlAccountId && Array.isArray(accounts)) {
        currentAccount = accounts.find(
          (acc: any) => acc.account_id === urlAccountId,
        );
      }
      ...
      if (
        !currentAccount &&
        user_settings?.last_used_account_id &&
        Array.isArray(accounts)
      ) {
        currentAccount = accounts.find(
          (acc: any) => acc.account_id === user_settings.last_used_account_id,
        );
      }
...
export async function loader({ context }: Route.LoaderArgs) {
  try {
    ...
    if (user.supabase) {
      try {
        const { data: rawInvites } = await user.supabase.rpc(
        ...
        consola.debug(
          "[PROTECTED_LAYOUT] Failed to fetch pending invites:",
          inviteError,
        );
      }
    }
    ...
    try {
      ...
      if (subError) {
        consola.error(
          "[PROTECTED_LAYOUT] Error fetching subscription",
          subError,
        );
      }
...
export default function ProtectedLayout() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [{ PostHogProvider }, ph] = await Promise.all([
  ...
  const isHomePage =
    ...
    location.pathname.match(/^\/a\/[^/]+\/home$/);
