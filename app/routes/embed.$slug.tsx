...
export default function EmbedPage() {
	...
	const themeClasses = useMemo(() => {
		...
		if (config.theme === "transparent") {
			// Transparent background - uses neutral gray colors that work on most backgrounds
			return {
				bg: "bg-transparent",
				...
				input:
					"bg-white/80 dark:bg-gray-800/80 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder:text-gray-500",
				button: "bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:opacity-90",
				card: "bg-gray-100/50 dark:bg-gray-800/50",
			}
		}
		// Dark theme (default)
		return {
				...
				<AnimatePresence mode="wait">
						...
						>
							...
							{walkthroughSignedUrl && config.videoThumbnail && !showVideo && (
								<button
									...
									className={cn("relative w-full overflow-hidden rounded-lg", themeClasses.border, "border")}
							...
							{walkthroughSignedUrl && showVideo && (
								<div className="relative overflow-hidden rounded-lg" style={{ borderRadius: "var(--border-radius)" }}>
									<video src={walkthroughSignedUrl} className="aspect-video w-full bg-black" controls autoPlay />
									<button
										type="button"
							...
							{list.hero_title && <h2 className={cn("font-semibold text-lg", themeClasses.text)}>{list.hero_title}</h2>}
					...
					{stage === "complete" && (
						...
						>
							...
							<p className={cn("font-medium", themeClasses.text)}>{config.successMessage}</p>
							{list.redirect_url && (
								<a href={list.redirect_url} className={cn("text-sm underline", themeClasses.textMuted)}>
									Continue
								</a>
							)}
						</motion.div>
				...
				<AnimatePresence mode="wait">
					...
					{stage === "complete" && (
						...
						>
							...
							<span className={cn("text-sm", themeClasses.text)}>{config.successMessage}</span>
						</motion.div>
					)}
				</AnimatePresence>
			</div>
				...
				<AnimatePresence mode="wait">
					...
					{stage === "form" && needsName && (
						...
						>
							{/* Video reveal */}
							{walkthroughSignedUrl && (
								<div className="overflow-hidden rounded-lg" style={{ borderRadius: "var(--border-radius)" }}>
									<video src={walkthroughSignedUrl} className="aspect-video w-full bg-black" controls autoPlay />
								</div>
							)}

							<p className={cn("text-sm", themeClasses.textMuted)}>One more step! Enter your name to continue.</p>
				...
				<AnimatePresence mode="wait">
					{stage === "initial" && (
						...
						>
							...
							{walkthroughSignedUrl && (
								<div className="overflow-hidden rounded-lg" style={{ borderRadius: "var(--border-radius)" }}>
									<video src={walkthroughSignedUrl} className="aspect-video w-full bg-black" controls />
								</div>
							)}
					...
					{stage === "form" && needsName && (
						<motion.form
							...
							className="space-y-4"
						>
							<p className={cn("text-sm", themeClasses.textMuted)}>Enter your name to continue to the survey.</p>
