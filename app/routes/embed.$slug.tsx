...
export async function loader({ params, request }: LoaderFunctionArgs) {
	...
	const config: EmbedConfig = {
		...
		theme: (url.searchParams.get("theme") as EmbedTheme) || "dark",
...
export default function EmbedPage() {
	const { slug, list, walkthroughSignedUrl, config } = useLoaderData() as LoaderData
	const emailId = useId()
	...
	async function handleEmailSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			// For email-only layouts, go straight to complete
	...
	async function handleNameSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const fullSurveyUrl = `/research/${slug}?email=${encodeURIComponent(email)}&responseId=${result.responseId}`
			if (typeof window !== "undefined") {
				window.location.href = fullSurveyUrl
			}
		} catch (caught) {
			setError(caught instanceof Error ? caught.message : "Something went wrong")
		} finally {
					...
					{stage === "initial" && (
						...
						>
							<Input
								...
								onChange={(e) => setEmail(e.target.value)}
								placeholder={config.placeholder}
								className={cn("flex-1 h-9", themeClasses.input)}
								style={{ borderRadius: `var(--border-radius)` }}
								required
							/>
							<Button
