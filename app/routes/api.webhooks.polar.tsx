 ...
 */
function extractAccountId(metadata?: Record<string, unknown> | null): string | undefined {
...
async function handleSubscriptionActive(payload: WebhookSubscriptionActivePayload) {
	...
	if (!accountId) {
		consola.error("[polar webhook] Missing account_id in subscription metadata", { subscriptionId: subscription.id });
		return;
	}
	...
	try {
		...
		if (posthogServer) {
			...
			const userId = (subscription.metadata?.user_id as string) || `account:${accountId}`;
		...
		consola.warn("[CHECKOUT_COMPLETED] PostHog tracking failed:", trackingError);
	}
}
 ...
 */
async function handleSubscriptionCreated(payload: WebhookSubscriptionCreatedPayload) {
	const subscription = payload.data;
	const customer = subscription.customer;
...
async function handleSubscriptionCanceled(payload: WebhookSubscriptionCanceledPayload) {
	...
	const accountId = extractAccountId(subscription.metadata);
	if (!accountId) {
		consola.warn("[polar webhook] Missing account_id in subscription.canceled", { subscriptionId: subscription.id });
		return;
	}
	...
	await upsertBillingSubscription({
		...
		currentPeriodEnd: subscription.currentPeriodEnd?.toISOString(),
		cancelAtPeriodEnd: subscription.cancelAtPeriodEnd,
		canceledAt: subscription.canceledAt?.toISOString() ?? new Date().toISOString(),
		metadata: subscription.metadata as Record<string, unknown>,
	});
	...
	try {
		...
		consola.warn("[SUBSCRIPTION_CANCELED] PostHog tracking failed:", trackingError);
	}
}
 ...
 */
async function handleSubscriptionRevoked(payload: WebhookSubscriptionRevokedPayload) {
	const subscription = payload.data;
	const customer = subscription.customer;
