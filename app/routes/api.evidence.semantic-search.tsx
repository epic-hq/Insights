...
export async function loader({ request, context }: LoaderFunctionArgs) {
	...
	const matchThreshold = Number.parseFloat(url.searchParams.get("matchThreshold") || "0.3");
	const matchCount = Number.parseInt(url.searchParams.get("matchCount") || "20", 10);

	if (!query || !projectId) {
		return Response.json({ error: "Missing query or projectId" }, { status: 400 });
	}
	...
	try {
		...
		console.log("[API] Evidence stats:", {
			...
			percentWithEmbeddings:
				totalEvidence && totalEvidence > 0 ? Math.round(((withEmbeddings || 0) / totalEvidence) * 100) : 0,
		});
		...
		const { data: facetsData, error: facetsError } = await supabase.rpc("find_similar_evidence_facets", {
			query_embedding: embedding as any,
			project_id_param: projectId,
			match_threshold: matchThreshold,
			match_count: matchCount,
		...
		// 5b. Fetch evidence facets for all results
		const { data: facetRows, error: facetErr } = await supabase
			.from("evidence_facet")
			.select("evidence_id, kind_slug, label, facet_account_id")
			.eq("project_id", projectId)
			.in("evidence_id", evidenceIdsArray);

		if (facetErr) {
			console.warn("[semantic-search] Failed to load facets:", facetErr);
		}

		// Build facets map
		const facetsByEvidence = new Map<string, Array<{ kind_slug: string; label: string; facet_account_id: number }>>();
		...
		// 6. Format response with similarity scores and facets
		const evidence = (fullEvidence || [])
			.map((ev) => ({
				...ev,
				similarity: similarityMap.get(ev.id) || 0,
				facets: facetsByEvidence.get(ev.id) || [],
			}))
