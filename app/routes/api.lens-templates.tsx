  ...
  try {
    ...
    switch (intent) {
      ...
      case "generate": {
        ...
        consola.info(
          "[lens-templates] Generating template from description:",
          description.substring(0, 100),
        );

        try {
          const generated = await b.GenerateLensTemplate(
            description,
            context || null,
          );
          ...
          return Response.json(
            {
              ok: false,
              error:
                "Failed to generate template. Please try a different description.",
            },
            { status: 500 },
          );
        }
      ...
      case "create": {
        ...
        const templateDefinition = formData
          .get("template_definition")
          ?.toString();
          ...
          return Response.json(
            {
              ok: false,
              error:
                "Missing required fields (template_name, template_definition, account_id)",
            },
            { status: 400 },
          );
        }
        ...
        consola.info(
          "[lens-templates] Created custom template:",
      ...
      case "update": {
        ...
        if (!templateKey || !accountId) {
          return Response.json(
            { ok: false, error: "Missing template_key or account_id" },
            { status: 400 },
          );
        }
        ...
        if (primaryObjective !== undefined)
          updates.primary_objective = primaryObjective || null;
        ...
        if (!templateDefinition && description && description.length >= 10) {
          consola.info(
            "[lens-templates] Regenerating template from description:",
            description.substring(0, 100),
          );
          try {
            ...
            return Response.json(
              {
                ok: false,
                error:
                  "Failed to regenerate template. Please try a different description.",
              },
              { status: 500 },
            );
          }
        ...
        consola.info(
          "[lens-templates] Updated template:",
          template.template_key,
        );
      ...
      case "generate_and_create": {
        ...
        consola.info(
          "[lens-templates] Generate and create from description:",
          description.substring(0, 100),
        );
