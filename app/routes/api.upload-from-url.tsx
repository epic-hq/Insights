...
export async function action({ request, context }: ActionFunctionArgs) {
	...
	try {
		...
		for (const item of urlItems) {
			...
			const defaultTitle = item.title || `Imported from URL - ${new Date().toISOString().slice(0, 10)}`;
			...
			if (insertError || !interview) {
				consola.error("[upload-from-url] Failed to create interview record", insertError);
