...
export async function action({ request }: ActionFunctionArgs) {
	...
	try {
		...
		const similarityThreshold = Number.parseFloat(formData.get("similarity_threshold")?.toString() || "0.85");
		...
		consola.info(`[consolidate-themes] Triggering consolidation task for project ${project.name} (${projectId})`);

		// Trigger the background task
		const handle = await tasks.trigger<typeof consolidateThemesTask>("themes.consolidate", {
			projectId,
			accountId,
			similarityThreshold,
		});

		consola.info(`[consolidate-themes] Task triggered with runId: ${handle.id}`);
		...
		const message = error instanceof Error ? error.message : "Failed to start theme consolidation";
