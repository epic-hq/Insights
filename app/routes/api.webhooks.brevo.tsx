...
type BrevoWebhookPayload =
  | BrevoMarketingWebhookPayload
  | BrevoTransactionalWebhookPayload;
...
function isMarketingWebhook(
  payload: BrevoWebhookPayload,
): payload is BrevoMarketingWebhookPayload {
...
export async function loader() {
  ...
  return Response.json(
    { status: "ok", service: "brevo-webhook" },
    { status: 200 },
  );
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const timestamp =
      payload.ts_event || payload.ts_epoch || payload.ts || Date.now() / 1000;
    ...
    if (isMarketingWebhook(payload)) {
      ...
      // Add URL for click events
      if (
        (eventType === "click" || eventType === "uniqueOpened") &&
        payload.link
      ) {
