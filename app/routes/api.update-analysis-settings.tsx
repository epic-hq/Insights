...
export async function action({ request }: ActionFunctionArgs) {
	...
	if (themeDedup < 0.5 || themeDedup > 0.95) {
		return Response.json({ error: "theme_dedup_threshold must be between 0.5 and 0.95" }, { status: 400 })
	}
	if (evidenceLink < 0.2 || evidenceLink > 0.7) {
		return Response.json({ error: "evidence_link_threshold must be between 0.2 and 0.7" }, { status: 400 })
	}
	...
	if (fetchError) {
		consola.error("[update-analysis-settings] Failed to fetch project:", fetchError)
	...
	if (updateError) {
		consola.error("[update-analysis-settings] Failed to update project:", updateError)
		return Response.json({ error: `Failed to update settings: ${updateError.message}` }, { status: 500 })
	}

	consola.info(`[update-analysis-settings] Updated settings for project ${projectId}:`, {
		themeDedup,
		evidenceLink,
	})
