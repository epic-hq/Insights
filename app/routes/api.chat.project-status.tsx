...
function buildStreamInitFailureMessage(
  errorMessage: string,
  ...
  if (isRateLimit) {
    const agentLabel =
      targetAgentId === "surveyAgent" ? "survey assistant" : "assistant";
...
function mapUsageToLangfuse(usage?: {
  ...
  if (typeof outputTokens === "number")
    usageDetailsPayload.output = outputTokens;
  ...
  return {
    ...
    usageDetails:
      Object.keys(usageDetailsPayload).length > 0
        ? usageDetailsPayload
        : undefined,
  };
}
...
function ensureSurveyEditPath(path: string): string {
  ...
  const search =
    typeof searchRaw === "string" && searchRaw.length > 0
      ...
      : "";
  const hash =
    typeof hashFragment === "string" && hashFragment.length > 0
      ? `#${hashFragment}`
      : "";
...
function convertLegacyDataPayloadToTypedChunks(
  payload: unknown,
  inheritedId?: string,
): StreamChunk[] {
  if (!payload) return [];
  ...
  const fromObject = (value: Record<string, unknown>): StreamChunk[] => {
    ...
    if (nestedType?.startsWith("data-")) {
      const normalized = toChunk(
        nestedType,
        "data" in value ? value.data : value,
      );
...
function normalizeStreamChunk(chunk: StreamChunk): StreamChunk[] {
  ...
  return convertLegacyDataPayloadToTypedChunks(
    chunk.data,
...
function extractInterviewIdFromSystemContext(
  ...
  const routeMatch = systemContext.match(
    new RegExp(`/interviews/(${UUID_PATTERN})\\b`, "i"),
  );
...
function normalizeMessagesForExecution(
  ...
  return messages.map((message) => {
    if (!message || message.role !== "user" || cleanedFirstUserMessage)
      return message;
...
export function buildQuickLinksMarkdown(options: {
  ...
  const mentionsPeople = /\b(people|person|contact|contacts|icp)\b/.test(
    normalizedPrompt,
  );
  const mentionsSurvey =
    /\b(survey|ask link|ask|questionnaire|responses?)\b/.test(normalizedPrompt);
  ...
  if (mentionsSurvey || targetAgentId === "researchAgent")
    links.push(`[Ask](${routes.ask.index()})`);
  if (mentionsLens) {
...
function augmentStreamForReliability(
  ...
  if (
    !stream ||
    typeof (stream as { pipeThrough?: unknown }).pipeThrough !== "function"
  ) {
    return stream;
  }
  ...
  const maybeEnqueueSafetyMessage = (
    controller: TransformStreamDefaultController<Record<string, unknown>>,
  ) => {
  ...
  const maybeEnqueueHowtoContractPatch = (
    controller: TransformStreamDefaultController<Record<string, unknown>>,
  ) => {
    if (options.targetAgentId !== "howtoAgent" || appendedHowtoContractPatch)
      return;
    const patchText = buildHowtoContractPatchText(
      accumulatedAssistantText,
      options.accountId,
      options.projectId,
    );
  ...
  const maybeEnqueueCsvCorrection = (
    ...
    if (responseSatisfiesCsvContract(accumulatedAssistantText, contract))
      return;
    ...
    new TransformStream<Record<string, unknown>, Record<string, unknown>>({
      transform(chunk, controller) {
        ...
        for (const normalizedChunk of normalizedChunks) {
          ...
          if (
            normalizedChunk?.type === "tool-input-available" &&
            typeof normalizedChunk.toolName === "string"
          ) {
          ...
          if (
            normalizedChunk?.type === "tool-output-available" ||
            normalizedChunk?.type === "tool-result"
          ) {
            const payload = (normalizedChunk.output ??
              normalizedChunk.result) as Record<string, unknown> | undefined;
            if (typeof payload === "object" && payload && "a2ui" in payload) {
              sawTextDelta = true;
            }

            const extractedResults =
              extractWebResearchResultsFromPayload(payload);
        ...
        if (options.userId && sawTextDelta) {
          ...
          if (posthog) {
            ...
            try {
              ...
              consola.error(
                "[PostHog] Failed to track agent_message_sent",
                error,
              );
            }
          }
...
function routeByDeterministicPrompt(
  ...
  const hasAny = (...tokens: string[]) =>
    tokens.some((token) => prompt.includes(token));
  const systemContext = options?.systemContext ?? "";
  const interviewIdFromContext =
    extractInterviewIdFromSystemContext(systemContext);
  ...
  const asksForIcpData =
    ...
    (hasAny("match", "matches", "score", "scored", "scoring") &&
      hasAny("person", "people", "who"));
  if (asksForIcpData) {
    return {
      targetAgentId: "projectStatusAgent",
      confidence: 1,
  ...
  const asksForInterviewOps =
    hasAny("interview", "interviews") &&
    hasAny("create", "draft", "generate", "write", "questions", "prompt");
  ...
  if (asksForInterviewOpenQuestionHelp) {
    return {
      ...
      responseMode: "normal",
      rationale:
        "deterministic routing for interview detail open-question guidance",
    };
  }
  ...
  const asksForFeedbackTriage =
    ...
    (hasAny("bug report", "feature request") &&
      hasAny("submit", "log", "track"));
  if (asksForFeedbackTriage) {
    return {
      ...
      responseMode: "normal",
      rationale:
        "deterministic routing for feedback triage and PostHog submission",
    };
  }
  ...
  const asksForPeopleOrTaskOps =
    ...
    (hasAny("task", "tasks", "todo", "to-do") &&
      hasAny("create", "add", "update", "complete", "done", "close"));
...
async function routeAgentByIntent(
  ...
  if (resourceId) {
    ...
    if (
      sticky &&
      (sticky.agentId === "surveyAgent" ||
        sticky.agentId === "projectSetupAgent") &&
      Date.now() - sticky.timestamp < STICKY_ROUTING_TTL_MS
    ) {
      return {
...
Message: """${prompt.slice(0, ROUTING_CLASSIFIER_MAX_PROMPT_CHARS)}"""`,
    ...
    consola.warn(
      "project-status: intent routing failed, falling back to projectStatusAgent",
      {
...
export async function action({ request, context, params }: ActionFunctionArgs) {
  ...
  const userTimezone =
    typeof body.userTimezone === "string" ? body.userTimezone : undefined;
  ...
  const threadTitleSeed =
    lastUserText === UI_EVENT_DISPATCH_TEXT
  ...
  const resolvedResponseMode: RoutingResponseMode =
    (routeDecision?.responseMode as RoutingResponseMode | undefined) ??
    "normal";
  ...
  const isFastStandardized =
    ...
    resolvedResponseMode === "fast_standardized";
  ...
  const systemContext =
    typeof system === "string" ? system.slice(0, systemContextLimit) : "";
  const defaultThreadResourceId = getPrimaryProjectStatusResourceId(
    userId,
    projectId,
  );
  ...
  const interviewIdFromSystemContext = extractInterviewIdFromSystemContext(
  ...
  try {
    ...
    const onboarding = (
      userSettings?.metadata as Record<string, unknown> | null
    )?.onboarding as Record<string, string> | undefined;
  ...
  if (
    targetAgentId === "researchAgent" &&
    resolvedResponseMode === "survey_quick_create"
  ) {
    try {
      ...
      const quickCreateCost =
        quickCreate.usage &&
        (quickCreate.usage.inputTokens || quickCreate.usage.outputTokens)
          ? estimateOpenAICost(
      ...
      if (
        ...
        await recordUsageOnly(
          ...
          consola.error(
            "[billing] Failed to record survey quick-create usage:",
            err,
          );
        });
      }
      ...
      await persistQuickCreateTurn({
        ...
        consola.warn(
          "project-status: failed to persist survey quick-create turn",
          {
            error: error instanceof Error ? error.message : String(error),
            threadId,
          },
      ...
      await persistQuickCreateTurn({
        ...
        consola.warn(
          "project-status: failed to persist survey quick-create failure",
          {
            error:
...
- If there are more candidates, keep only the top ${csvListContract.requestedRows}.`,
    ...
    onFinish: async (data: {
      ...
      if (
        ...
        const costUsd =
          traceCostUsd ?? estimateOpenAICost(model, inputTokens, outputTokens);
      ...
      requestGeneration?.end?.({
        output: {
          text: data.text ?? "",
          finishReason: data.finishReason ?? null,
          toolCallCount: Array.isArray(data.toolCalls)
  ...
  try {
    ...
    if (
      ...
      const debugSuffix = debugRequested
        ...
        : "";
      const failureText = buildStreamInitFailureMessage(
        errorMessage,
        targetAgentId,
      );
      return createUIMessageStreamResponse({
