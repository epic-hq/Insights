...
function buildDeterministicSurveyDraft(prompt: string) {
  ...
  const isMissingProfileData =
    ...
    (lower.includes("icp") &&
      (lower.includes("missing") || lower.includes("unscored")));

  if (isMissingProfileData) {
    return {
      name: "Profile Completion Survey",
      description:
        "Collect missing profile data and a short context update to improve ICP scoring.",
      questions: [
        {
        ...
        {
          prompt:
            "What is your primary goal right now related to this problem?",
          type: "auto",
          required: false,
        },
        {
  ...
  if (isWaitlist) {
    return {
      ...
      description:
        "Qualify waitlist signups and understand urgency and expected outcomes.",
      questions: [
        {
    ...
    stream =
      targetAgentId === "projectStatusAgent"
        ...
        : await handleChatStream({
            ...
            params: buildAgentParams(threadId),
            sendReasoning:
              targetAgentId === "researchAgent" && !isFastStandardized,
