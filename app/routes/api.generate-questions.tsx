  ...
  try {
    ...
    if (!project_id) {
      return Response.json(
        {
          error:
            "Project ID is required to generate and save interview questions",
        },
        { status: 400 },
      );
    }
    ...
    const requestedResearchMode = formData.get("research_mode") as
      | string
      | null;
    ...
    let research_mode: ResearchMode =
      coerceResearchMode(requestedResearchMode) ?? "exploratory";
    const initialCustomInstructions = (
      (formData.get("custom_instructions") as string) || ""
    ).trim();
    ...
    const interview_time_limit = Number(
      formData.get("interview_time_limit") ?? 30,
    );
    ...
    try {
      const projectContext = await getProjectContextGeneric(
        supabase,
        project_id,
      );
      ...
      target_orgs =
        meta.target_orgs?.join?.(", ") || meta.target_orgs || target_orgs || "";
      target_roles =
        meta.target_roles?.join?.(", ") ||
        meta.target_roles ||
        target_roles ||
        "";
      // Use research_goal, fall back to customer_problem if not set
      research_goal =
        meta.research_goal || meta.customer_problem || research_goal || "";
      research_goal_details =
        meta.research_goal_details || research_goal_details || "";
      assumptions =
        meta.assumptions?.join?.(", ") || meta.assumptions || assumptions || "";
      ...
      if (!initialCustomInstructions) {
        const projectLevelInstructions =
          (typeof meta.custom_instructions === "string" &&
            meta.custom_instructions.trim()) ||
          (typeof meta.settings?.customInstructions === "string" &&
            meta.settings?.customInstructions.trim()) ||
          "";
    ...
    try {
      ...
      consola.warn(
        "Failed to load research structure for question generation:",
        error,
      );
    }
    ...
    if (researchQuestionTexts.length > 0) {
      structureNotes.push(
        "Ensure questions help answer these research questions:",
      );
    ...
    try {
      ...
      consola.warn(
        "[api.generate-questions] Failed to load enhanced context:",
        error,
      );
    }

    const combinedCustomInstructions = customInstructionParts
      .filter(Boolean)
      .join("\n\n");
    ...
    if (
      typeof questionCountRaw === "string" &&
      questionCountRaw.trim().length > 0
    ) {
    ...
    if (!research_goal?.trim()) {
      ...
      consola.info(
        `[api.generate-questions] No research_goal for project ${project_id}, using default`,
      );
    }
    if (!target_roles?.trim()) {
      target_roles = "Target users";
      consola.info(
        `[api.generate-questions] No target_roles for project ${project_id}, using default`,
      );
    }
    ...
    try {
      ...
      consola.log(
        "[BAML DEBUG] Calling generateQuestionSetCanonical with params:",
        canonicalParams,
      );
      ...
      consola.log("[BAML DEBUG] Canonical result:", {
        ...
        questions:
          questionSet?.questions?.slice(0, 3).map((q: any) => q.text) || [],
    ...
    try {
      ...
      const have = Array.isArray(questionSet?.questions)
      ...
      if (have < desired) {
        ...
        const seeds = (
          Array.isArray(questionSet?.questions)
            ? questionSet.questions.map((q: any) => String(q.text || ""))
            : []
        ) as string[];
    ...
    const sanitizeText = (text: string): string => {
      try {
        ...
        const end =
          lastPunct >= 40 ? lastPunct + 1 : lastSpace >= 40 ? lastSpace : MAX;
        return (
          cut
            .slice(0, end)
            .trim()
    ...
    const generatedQuestions = Array.isArray(questionSet?.questions)
    ...
    if (generatedQuestions.length === 0) {
      return Response.json(
        {
          error:
            "No questions were generated. Please adjust your inputs and try again.",
        },
        { status: 500 },
      );
    }
    ...
    const promptPayloads = generatedQuestions.map(
      (question: any, index: number) => {
        ...
        const estimatedMinutes =
          ...
          Number.isFinite(question.estimatedMinutes)
        ...
        return {
          ...
          status:
            typeof question.status === "string" && question.status.length > 0
              ? question.status
              : "selected",
          order_index: index + 1,
          scores,
          source:
            typeof question.source === "string" && question.source.length > 0
              ? question.source
              : "ai",
    ...
    if (promptError) {
      consola.error(
        "[api.generate-questions] Failed to persist interview prompts",
        promptError,
      );
      return Response.json(
        {
    ...
    const newQuestionIds = new Set(
      promptPayloads.map((p: PromptPayload) => p.id),
    );
    ...
    const sectionQuestionEntries = promptPayloads.map(
      (payload: PromptPayload, _index: number) => ({
        id: payload.id,
        text: payload.text,
    ...
    const sectionQuestions = [
      ...filteredExistingSectionQuestions,
    ...
    if (sectionError) {
      consola.error(
        "[api.generate-questions] Failed to update project section meta",
        sectionError,
      );
      return Response.json(
        {
