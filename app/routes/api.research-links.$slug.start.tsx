...
import { ResearchLinkCreatePersonSchema, ResearchLinkResponseStartSchema } from "~/features/research-links/schemas"
...
export async function action({ request, params }: ActionFunctionArgs) {
	...
	const { data: list, error: listError } = await supabase
		.from("research_links")
		.select("id, is_live, allow_chat, default_response_mode, account_id, project_id")
		.eq("slug", slug)
		.maybeSingle()
	...
	if (createPersonParsed.success) {
		return handleCreatePersonAndContinue(supabase, list, createPersonParsed.data)
	}
...
async function handleCreatePersonAndContinue(
	...
	// Check if person already exists (race condition check)
	const { data: existingPerson } = await supabase
		.from("people")
		.select("id")
		.eq("account_id", list.account_id)
