...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const { data: list, error: listError } = await supabase
    .from("research_links")
    .select(
      "id, is_live, allow_chat, default_response_mode, account_id, project_id, identity_mode, identity_field",
    )
    .eq("slug", slug)
    .maybeSingle();
  ...
  const identityMode: IdentityMode =
    (list.identity_mode as IdentityMode) || "identified";
  const identityField: IdentityField =
    (list.identity_field as IdentityField) || "email";
    ...
    return Response.json(
      {
        ...
        message:
          "This survey has reached its response limit. Please contact the survey owner.",
      },
      { status: 403 },
    );
  }
  ...
  if (identityMode === "identified") {
    const createPersonParsed =
      ResearchLinkCreatePersonSchema.safeParse(payload);
...
async function handlePhoneStart(
  ...
  if (!parsed.success) {
    return Response.json(
      {
        message:
          parsed.error.flatten().fieldErrors.phone?.[0] ?? "Invalid request",
      },
      { status: 400 },
    );
  }
  ...
  if (existing) {
    ...
    if (!personId) {
      ...
      if (backfillPerson) {
        ...
        await supabase
          .from("research_link_responses")
          .update({ person_id: backfillPerson.id })
          .eq("id", existing.id);
      }
    }
