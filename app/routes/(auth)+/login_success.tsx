...
import {
  clearUtmCookie,
  collectPersistedUtmParams,
  extractUtmParamsFromRequest,
} from "~/utils/utm.server";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  consola.log(
    "[LOGIN_SUCCESS] Incoming cookies:",
    incomingCookies ? "present" : "MISSING",
  );
  if (incomingCookies) {
  ...
  const utmParams = collectPersistedUtmParams(
    request,
    extractUtmParamsFromRequest(request),
  );
  ...
  if (!user) {
    consola.warn(
      "[LOGIN_SUCCESS] ⚠️  No authenticated user found - session lost between callback and login_success",
    );
  ...
  if (isNewUser && !inviteRedirect && shouldUseLastUsed) {
    const defaultProjectPath = await ensureDefaultAccountAndProject({
      supabase,
      userId: user.id,
    });
    ...
    const lastUsedPath = await resolveLastUsedProjectRedirect({
      supabase,
      userId: user.id,
    });
    if (lastUsedPath) {
...
async function ensureDefaultAccountAndProject({
  ...
  try {
    const { data: rawAccounts, error: accountsError } =
      await supabase.rpc("get_user_accounts");
    if (accountsError) {
      consola.warn(
        "[LOGIN_SUCCESS] Failed to load user accounts for default selection:",
        accountsError.message,
      );
      return null;
    }

    const accounts: AccountWithProjects[] = Array.isArray(rawAccounts)
      ? (rawAccounts as AccountWithProjects[])
      : [];
    if (accounts.length === 0) {
      consola.warn(
        "[LOGIN_SUCCESS] No accounts available for new user when ensuring default account/project",
      );
      return null;
    }
    ...
    const currentAccount =
      accounts.find((acc) => acc.personal_account === false) ?? accounts[0];
    const accountId = currentAccount.account_id;
    const projects = Array.isArray(currentAccount.projects)
    ...
    if (projects.length > 0) {
      ...
      if (createError || !createdProject) {
        consola.error(
          "[LOGIN_SUCCESS] Failed to auto-create default project for new user:",
          createError,
        );
        return null;
      }
    ...
    if (settingsError) {
      consola.warn(
        "[LOGIN_SUCCESS] Failed to persist default account/project in user_settings:",
        settingsError.message,
      );
    }

    // New users go to account setup first to set company context
    // This ensures the Define phase starts with company info before project goals
    return `/a/${accountId}/settings?onboarding=1`;
  } catch (error) {
    consola.warn(
      "[LOGIN_SUCCESS] Error ensuring default account/project:",
      error,
    );
    return null;
  }
...
async function captureSignupEvent({
  ...
  try {
    ...
    if (!posthog) {
      consola.warn(
        "[LOGIN_SUCCESS] PostHog server client unavailable; skipping signup event",
      );
      return;
    }
    ...
    await posthog.capture({
      ...
      properties: {
        ...
        signup_source:
          provider === "google" ? "oauth_google" : "email_password",
        $set_once: {
    ...
    await posthog.flush();
    consola.log(
      "[LOGIN_SUCCESS] Captured account_signed_up event for user:",
      userId,
    );
  } catch (error) {
...
async function resolveInviteRedirect({
  ...
  try {
    ...
    if (
      ...
      consola.log(
        "[LOGIN_SUCCESS] Preserving /accept-invite flow, not intercepting",
      );
      return next;
    }
  } catch {
  ...
  const { data: rawInvites, error } = await supabase.rpc(
    "list_invitations_for_current_user",
  );
  if (error) {
    consola.warn(
      "[LOGIN_SUCCESS] Unable to list invitations for current user:",
      error.message,
    );
    return null;
  }
  ...
  if (Array.isArray(rawInvites)) {
    ...
    try {
      ...
      if (Array.isArray(parsed))
        invites = parsed as Array<Record<string, unknown>>;
    } catch (parseError) {
      consola.warn(
        "[LOGIN_SUCCESS] Failed to parse invitations payload",
        parseError,
      );
    }
  }

  const firstInviteWithToken = invites.find(
    (inv) => typeof inv?.token === "string",
  );
...
async function computeManagePathFromToken({
  ...
  if (lookupError) {
    consola.warn(
      "[LOGIN_SUCCESS] lookup_invitation failed for token",
      lookupError,
    );
    return null;
  }
...
async function resolveLastUsedProjectRedirect({
  ...
  try {
    ...
    if (error) {
      consola.warn(
        "[LOGIN_SUCCESS] Failed to resolve user_settings for redirect:",
        error.message,
      );
      return null;
    }
    ...
    if (!accountId || !projectId) {
      consola.log(
        "[LOGIN_SUCCESS] No last_used preferences, attempting to set defaults",
      );
    ...
    if (projectError || !project) {
      consola.warn(
        "[LOGIN_SUCCESS] Last used project unavailable, setting new defaults",
        {
          accountId,
          projectId,
