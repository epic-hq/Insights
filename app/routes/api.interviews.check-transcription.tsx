  ...
  try {
    ...
    if (!assemblyaiId) {
      ...
      if (mediaUrl && isAudioVideo) {
        consola.info(
          `[check-transcription] No assemblyai_id but has media_url - submitting fresh to AssemblyAI`,
        );
        ...
        if (!presignedUrl) {
          return Response.json(
            { error: "Failed to generate presigned URL for media file" },
            { status: 500 },
          );
        }
        ...
        if (!transcriptResponse.ok) {
          ...
          consola.error(
            `[check-transcription] AssemblyAI submission failed:`,
            errorText,
          );
          return Response.json(
            {
        ...
        consola.info(
          `[check-transcription] AssemblyAI job created (recovery):`,
          assemblyData.id,
        );
