...
import { getGoogleCalendarAuthUrl, isPicaConfigured } from "~/lib/integrations/pica.server"
...
export async function action({ context, request }: ActionFunctionArgs) {
	...
	if (!hasFeature(planId, "calendar_sync")) {
		return Response.json({ error: "Calendar sync requires a paid plan" }, { status: 403 })
	}
	...
	try {
		...
		return Response.json({ error: "Failed to start calendar connection" }, { status: 500 })
	}
}
