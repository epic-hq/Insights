...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const run = await mastra
      .getWorkflow("signupOnboardingWorkflow")
      ?.createRunAsync();
    ...
    return {
      reply:
        "Let's continue — what business objective are you trying to achieve?",
      state: state ?? {},
      completed: false,
    };
  } catch (error) {
    ...
    return {
      ...
      reply:
        "Sorry — hit a hiccup. What business objective are you trying to achieve?",
      state: {},
      completed: false,
    };
  }
