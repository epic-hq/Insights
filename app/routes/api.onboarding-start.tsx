...
Please extract insights that specifically address these research questions and help understand ${primaryRole} at ${primaryOrg} better.`;
    ...
    if (isAudioVideo && mediaTypeInput === "voice_memo") {
      consola.info(
        "🎤 [ONBOARDING] Voice memo audio/video - transcribe only without analysis",
        {
          fileName: file.name,
          sourceType,
        },
    ...
    if (isAudioVideo) {
      ...
      if (r2Key) {
        ...
        if (!presignedResult) {
          ...
          return Response.json(
            { error: "Failed to generate presigned URL for uploaded file" },
            { status: 500 },
          );
        }
      ...
      try {
        ...
        if (!transcriptResponse.ok) {
          ...
          throw new Error(
            `AssemblyAI request failed: ${transcriptResponse.status} ${errorText}`,
          );
        }
        ...
        consola.success(
          "Transcription job queued, webhook will trigger processing when complete",
        );
      } catch (analysisError) {
        const message =
          analysisError instanceof Error
        ...
        traceEndPayload = { level: "ERROR", statusMessage: message };
        return Response.json(
          { error: "Failed to submit for transcription" },
          { status: 500 },
        );
      }
    }
