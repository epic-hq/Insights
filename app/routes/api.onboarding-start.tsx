...
export async function action({ request }: ActionFunctionArgs) {
	...
	try {
		...
		const originalVideoR2Key = formData.get("originalVideoR2Key") as string | null;
...
Please extract insights that specifically address these research questions and help understand ${primaryRole} at ${primaryOrg} better.`;
		...
		const interviewData: InterviewInsert = {
			...
			processing_metadata:
				originalVideoR2Key || thumbnailR2Key
					? {
							...(originalVideoR2Key ? { original_video_r2_key: originalVideoR2Key } : {}),
							...(thumbnailR2Key ? { client_thumbnail_r2_key: thumbnailR2Key } : {}),
						}
					: undefined,
		} as InterviewInsert;
		...
		if (isAudioVideo) {
			...
			try {
				...
				if (originalVideoR2Key) {
					try {
						const videoHandle = await tasks.trigger("interview.optimize-video", {
							interviewId: interview.id,
						...
						consola.warn("[ONBOARDING] Video optimization trigger failed:", videoError);
					}
				}
			} catch (triggerError) {
