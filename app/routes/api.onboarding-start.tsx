...
export async function action({ request }: ActionFunctionArgs) {
  ...
  let triggerRunInfo: { runId: string; publicToken: string | null } | null =
    null;
  ...
  try {
    ...
    const { data: userSettings } = await supabase
      .from("user_settings")
      .select(
        "last_used_account_id, first_name, last_name, email, image_url, title, role, company_name, industry",
      )
      .eq("user_id", user.sub)
      .single();
    ...
    if (!teamAccountId) {
      ...
      const teamAccount =
        accounts?.find((acc: any) => !acc.personal_account) || accounts?.[0];
    ...
    const effectiveContentType =
      file?.type || originalContentType || "application/octet-stream";

    const onboardingData: OnboardingData = JSON.parse(onboardingDataStr);
    trace?.update?.({
      metadata: {
        mediaType: onboardingData.mediaType,
        questionCount: Array.isArray(onboardingData.questions)
          ? onboardingData.questions.length
          : 0,
      },
    });

    consola.log(
      "Starting onboarding for account:",
      teamAccountId,
      "project:",
      projectId,
    );
    ...
    const researchGoalDetails =
      "research_goal_details" in onboardingData
        ? (onboardingData.research_goal_details ?? "")
        : "";
    const _customInstructionsInput =
      "custom_instructions" in onboardingData
        ? onboardingData.custom_instructions
        : undefined;
    ...
    if (!projectId) {
      ...
      let baseProjectName =
        researchGoal || `${primaryRole} at ${primaryOrg} Research`;
      ...
      const projectSections = [
        {
          ...
          meta: {
            target_orgs:
              (onboardingData as any).target_orgs ||
        ...
        },
        {
          ...
          meta: {
            target_roles:
              (onboardingData as any).target_roles ||
...
Please extract insights that specifically address these research questions and help understand ${primaryRole} at ${primaryOrg} better.`;
      ...
      if (personError || !person) {
        ...
        if (person.project_id !== projectId) {
          const targetProjectId =
            projectId ?? person.project_id ?? finalProjectId;
    ...
    const titleFromPerson = linkedPerson?.name
      ? `${linkedPerson.name} • ${dateLabel}`
      : undefined;
    ...
    if (sourceType === "document") {
      ...
      finalMediaType =
        mediaTypeInput === "voice_memo" ? "voice_memo" : "interview";
    }
    ...
    if (sourceType === "document" && !isTextFile) {
      consola.info(
        "📄 [ONBOARDING] Document file detected - saving without processing",
        {
          fileName: file.name,
          sourceType,
        },
    ...
    if (isTextFile && mediaTypeInput === "voice_memo") {
      consola.info(
        "📝 [ONBOARDING] Voice memo text file - saving to transcript without analysis",
        {
          fileName: file.name,
        },
      );
    ...
    if (isAudioVideo && mediaTypeInput === "voice_memo") {
      consola.info(
        "🎤 [ONBOARDING] Voice memo audio/video - transcribe via Trigger.dev",
        {
          fileName: file?.name || originalFilename,
          sourceType,
        },
      ...
      if (r2Key) {
        ...
        if (storageResult.error || !storageResult.mediaUrl) {
          ...
          return Response.json(
            { error: `Failed to store audio file: ${storageResult.error}` },
            { status: 500 },
          );
        }
        ...
        return Response.json(
          { error: "No file or r2Key provided" },
          { status: 500 },
        );
      }
    ...
    if (isAudioVideo) {
      ...
      if (r2Key) {
        consola.info(
          "⚡ [ONBOARDING] Using direct R2 upload - skipping server upload",
          {
            r2Key,
            elapsedMs: Date.now() - startTime,
          },
      ...
      try {
        ...
        // If original video was uploaded separately, trigger server-side optimization
        if (originalVideoR2Key) {
          try {
            const videoHandle = await tasks.trigger(
              "interview.optimize-video",
              {
                interviewId: interview.id,
                sourceR2Key: originalVideoR2Key,
                accountId: teamAccountId,
                projectId: finalProjectId,
              },
            );
            consola.info("🎬 [ONBOARDING] Video optimization triggered", {
              runId: videoHandle.id,
              sourceR2Key: originalVideoR2Key,
            });
          } catch (videoError) {
            // Non-fatal: transcription still works from audio
            consola.warn(
              "[ONBOARDING] Video optimization trigger failed:",
              videoError,
            );
          }
        }
