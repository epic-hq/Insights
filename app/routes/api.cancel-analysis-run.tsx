...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (!runId || !interviewId) {
    return Response.json(
      { error: "Missing runId or interviewId" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (!cancellableStatuses.includes(interview.status)) {
      return Response.json(
        { error: "Interview is not in a cancellable state" },
        { status: 400 },
      );
    }

    consola.info(
      `Cancelling analysis run ${runId} for interview ${interviewId}`,
    );
    ...
    return Response.json(
      { error: `Failed to cancel analysis: ${message}` },
      { status: 500 },
    );
  }
}
