...
import {
  getNextQuestionId,
  type ResponseRecord,
} from "~/features/research-links/branching";
import {
...
function computeReachablePath(
  ...
  const nextQuestion = surveyComplete
    ? null
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const questions = questionsResult.success
    ? questionsResult.data.filter(
        (q) => !q.hidden && q.prompt.trim().length > 0,
      )
    : [];
  ...
  const currentResponses =
    (responseRecord?.responses as Record<string, unknown>) ?? {};
  ...
  consola.info("research-link-chat: fetched responses from DB", {
    ...
    responseCount: Object.keys(currentResponses).filter(
      (k) =>
        currentResponses[k] !== undefined &&
        currentResponses[k] !== null &&
        currentResponses[k] !== "",
    ).length,
    questionCount: questions.length,
    firstQuestionId: questions[0]?.id,
    firstResponseKey: Object.keys(currentResponses)[0],
    questionIds: questions.map((q) => q.id),
  });

  // Compute reachable path using branching rules
  // This ensures chat mode respects the same skip logic as form mode
  const { answeredPath, nextQuestion, remainingQuestions, surveyComplete } =
    computeReachablePath(questions, currentResponses as ResponseRecord);

  consola.info("research-link-chat: computed path", {
    answeredCount: answeredPath.length,
    nextQuestionPrompt: nextQuestion?.prompt ?? "(none)",
    surveyComplete,
  });
  ...
  requestContext.set(
    ...
    nextQuestion
      ? JSON.stringify({
          ...
          likertScale: nextQuestion.likertScale ?? null,
          likertLabels: nextQuestion.likertLabels ?? null,
        })
      : "",
  );
