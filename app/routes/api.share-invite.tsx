...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  const inviterName =
    ...
    "A teammate";
  const inviterEmail =
    typeof ctx.claims?.email === "string" ? ctx.claims.email : undefined;
  ...
  try {
    ...
    const inviteUrl = `${host}/accept-invite?token=${encodeURIComponent(token)}&redirect=${encodeURIComponent(resourcePath)}`;
