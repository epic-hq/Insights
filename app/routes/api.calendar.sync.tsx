...
export async function action({ context, request }: ActionFunctionArgs) {
	...
	try {
		...
		const connection = await getCalendarConnection(ctx.supabase, userId, provider);

		if (!connection) {
			return Response.json({ error: "No calendar connection found" }, { status: 404 });
		}
		...
		return Response.json({ error: "Failed to start calendar sync" }, { status: 500 });
	}
}
