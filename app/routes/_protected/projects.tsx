 ...
 * Center-Left: ProjectStatusAgent chat sidebar (resizable)
...
} from "~/utils/project-status.server";
...
export const unstable_middleware: Route.MiddlewareFunction[] = [
  async ({ request: _request, context, params }) => {
    try {
      ...
      const _supabase =
        ctx.supabase as unknown as SupabaseClient<unknown> | null;
      ...
      if (
        project?.account_id &&
        urlAccountId &&
        project.account_id !== urlAccountId
      ) {
        consola.warn(
          "[projects middleware] URL accountId differs from project.account_id",
          {
            urlAccountId,
      ...
      consola.error(
        "_protected/projects Authentication middleware error:",
        error,
      );
...
export async function loader({ context, params }: Route.LoaderArgs) {
  try {
    ...
    const statusData = await getProjectStatusData(
      projectId,
      supabase as unknown as SupabaseClient<AppDatabase>,
    );
...
function ProjectLayout({
  ...
  const { isExpanded, registerForceExpandCallback } = useProjectStatusAgent();
  const [isChatCollapsed, setIsChatCollapsed] = useState(false);
  const chatPanelRef = useRef<ImperativePanelHandle | null>(null);
  const lastExpandedSize = useRef(22);

  // Register callback to clear local collapse state when force expand is requested
  useEffect(() => {
    registerForceExpandCallback(() => setIsChatCollapsed(false));
  ...
  const fullWidth = matches.some((match) => {
    const handle = match.handle as { fullWidth?: boolean };
    return handle?.fullWidth === true;
  });
...
`.trim();
  const projectSystemContext = [profileSection, projectSection]
    .filter(Boolean)
    .join("\n\n");
  ...
  const contentWrapper = fullWidth
    ? "h-full w-full"
    : "mx-auto h-full w-full max-w-7xl px-4 sm:px-6 lg:px-8";
  ...
  // DESKTOP: Main content only (chat is provided by AIAssistantPanel in SplitPaneLayout)
  return (
