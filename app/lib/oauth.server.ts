...
const TRUSTED_REDIRECT_DOMAINS = ["claude.ai", "claude.com", "chatgpt.com", "platform.openai.com", "localhost"];
...
export async function registerClient(
	...
	for (const uri of input.redirect_uris) {
		if (!isRedirectUriTrusted(uri)) {
			throw new Error(
				`Untrusted redirect_uri domain: ${uri}. ` + `Allowed domains: ${TRUSTED_REDIRECT_DOMAINS.join(", ")}`
			);
		}
	}
...
export async function validateClient(supabase: AnySupabaseClient, clientId: string): Promise<OAuthClient | null> {
	const { data, error } = await supabase
		.from("oauth_clients")
		.select(
...
export async function exchangeCodeForTokens(
	...
	if (markError) {
		consola.error("[oauth] Failed to mark authorization code as used", markError);
...
export async function refreshAccessToken(
	...
	if (existingKey.expires_at && new Date(existingKey.expires_at) < new Date()) {
		consola.warn("[oauth] Refresh token expired", { keyId: existingKey.id });
	...
	if (revokeError) {
		consola.error("[oauth] Failed to revoke old key during refresh", revokeError);
...
async function issueTokenPair(
	...
	const expiresAt = new Date(Date.now() + ACCESS_TOKEN_EXPIRES_IN * 1000).toISOString();
