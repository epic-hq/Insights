...
function resolveCostOptions(functionName: string, overridePrefix?: string) {
  const prefix =
    overridePrefix ?? `BAML_${camelToScreamingSnake(functionName)}`;
  ...
  return {
    ...
    completionCostPer1KTokens: Number.isFinite(completion)
      ? completion
      : undefined,
  };
}
...
export async function runBamlWithTracing<TResult>({
  ...
  try {
    ...
    await (langfuse as any)?.flushAsync?.().catch((error: unknown) => {
      consola.warn(
        `[langfuse] Failed to flush trace for baml.${functionName}:`,
        error,
      );
    });
  }
