...
export function summarizeCollectorUsage(
  ...
  if (
    typeof promptCostRate === "number" &&
    typeof promptTokenBase === "number"
  ) {
    summary.promptCostUsd = Number(
      ((promptTokenBase / 1000) * promptCostRate).toFixed(6),
    );
  }
  if (
    typeof completionCostRate === "number" &&
    typeof summary.outputTokens === "number"
  ) {
    summary.completionCostUsd = Number(
      ((summary.outputTokens / 1000) * completionCostRate).toFixed(6),
    );
  }
  if (
