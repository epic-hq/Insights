 ...
 *
 * Supports two modes:
 * - "single": per-interview analysis (Phase 1)
 * - "aggregated": cross-interview synthesis (future)
 *
 * analysisData is a passthrough — validated at ingestion by BAML, not here.
 */
export const conversationLensInsightsDataSchema = z.object({
	templateKey: z.string(),
	templateName: z.string().catch("Conversation Lens"),
	interviewCount: z.number().catch(1),
	mode: z.enum(["single", "aggregated"]).catch("single"),
	/** Raw analysis_data from conversation_lens_analyses — passthrough */
	analysisData: z.record(z.unknown()).nullable().catch(null),
	/** Raw synthesis_data for aggregated mode (future) */
	synthesisData: z.record(z.unknown()).nullable().catch(null),
	/** URL to the full lens detail page */
	lensDetailUrl: z.string().nullable().catch(null),
	/** Interview title for single mode */
	interviewTitle: z.string().nullable().catch(null),
	/** URL to the interview detail page */
	interviewUrl: z.string().nullable().catch(null),
	/** Person name associated with the interview */
	personName: z.string().nullable().catch(null),
});
...
export const progressRailPhaseSchema = z.object({
	id: z.enum(["frame", "collect", "validate", "commit", "measure"]).catch("frame"),
	label: z.string().catch(""),
	status: z.enum(["complete", "active", "upcoming", "blocked"]).catch("upcoming"),
...
export const decisionBriefDataSchema = z.object({
	projectId: z.string(),
	decisionQuestion: z.string().nullable().catch(null),
	targetCustomer: z.string().nullable().catch(null),
	deadline: z.string().nullable().catch(null),
	successMetric: z.string().nullable().catch(null),
	researchQuestions: z.array(z.string()).optional(),
	completeness: z
		.object({
			hasDecision: z.boolean().catch(false),
			hasTarget: z.boolean().catch(false),
			hasDeadline: z.boolean().catch(false),
			hasMetric: z.boolean().catch(false),
			hasQuestions: z.boolean().catch(false),
		})
		.catch({
			hasDecision: false,
			hasTarget: false,
	...
	readinessLabel: z.string().catch("Not started"),
	editUrl: z.string().optional(),
});
...
export const intakePathOptionSchema = z.object({
	id: z.enum(["upload", "record", "survey"]).catch("upload"),
	label: z.string().catch(""),
...
export const intakeItemSchema = z.object({
	...
	source: z.enum(["upload", "recording", "survey", "import"]).catch("upload"),
	status: z.enum(["queued", "processing", "ready", "failed"]).catch("queued"),
...
export const surveyOutreachRecipientSchema = z.object({
	...
	status: z.enum(["pending", "sent", "opened", "completed", "bounced"]).catch("pending"),
});
...
export const intakeHealthDataSchema = z.object({
	projectId: z.string(),
	confidenceTier: z.enum(["early_signal", "growing_confidence", "decision_ready"]).nullable().catch("early_signal"),
	confidenceLabel: z.string().nullable().catch("Early Signal"),
	summary: z.string().nullable().catch("Assessing your research intake..."),
	...
	sourceMix: z
		...
		.nullable()
		.catch({ interviews: 0, surveys: 0, documents: 0 }),
	totalEvidence: z.number().nullable().catch(0),
	daysSinceLastIntake: z.number().optional().nullable(),
	gaps: z.array(z.string()).optional(),
	gateStatus: z.enum(["insufficient", "marginal", "sufficient"]).nullable().catch("insufficient"),
	nextAction: z.string().nullable().catch("Collect more data"),
	nextActionUrl: z.string().optional().nullable(),
});
...
export const evidenceClusterSchema = z.object({
	label: z.string().catch(""),
	type: z.enum(["pain", "goal", "observation"]).catch("observation"),
	items: z.array(
		z.object({
			id: z.string(),
			verbatim: z.string().catch(""),
			speakerName: z.string().nullable().catch(null),
			speakerTitle: z.string().nullable().catch(null),
			interviewTitle: z.string().nullable().catch(null),
			detailUrl: z.string().optional(),
		})
	),
...
export const patternSchema = z.object({
	id: z.string(),
	name: z.string().catch(""),
	statement: z.string().nullable().catch(null),
	mentionCount: z.number().catch(0),
	confidenceTier: z.enum(["thin", "emerging", "strong", "validated"]).catch("emerging"),
	confidenceLabel: z.string().catch("Emerging"),
	topQuotes: z
		.array(
			z.object({
				verbatim: z.string().catch(""),
...
/**
 * Schema for DecisionSupport (Moment 7)
 */
...
export const decisionSupportDataSchema = z.object({
	projectId: z.string(),
	headline: z.string().optional(),
...
export const actionTrackingSchema = z.object({
	...
	status: z.enum(["not_started", "in_progress", "complete", "blocked"]).catch("not_started"),
	dueDate: z.string().nullable().catch(null),
});

export const researchPulseDataSchema = z.object({
	projectId: z.string(),
	periodLabel: z.string().catch("This week"),
	confidenceTier: z.enum(["early_signal", "growing_confidence", "decision_ready"]).catch("early_signal"),
	confidenceLabel: z.string().catch("Early Signal"),
	confidenceChange: z.enum(["improved", "stable", "declined"]).catch("stable"),
	deltas: z.array(weeklyDeltaSchema).catch([]),
	actions: z.array(actionTrackingSchema).optional(),
	newSignalSummary: z.string().optional(),
	nextStep: z.string().catch("Continue collecting data"),
...
class ComponentRegistry {
	...
	/**
	 * Register an alias so old/alternative names resolve to a registered component
	 */
	...
	get(type: string): ComponentDefinition | undefined {
		return this.components.get(this.resolve(type));
	}
	...
	has(type: string): boolean {
		return this.components.has(this.resolve(type));
	}
...
// Backwards-compat aliases for renamed components (agent may use old names)
componentRegistry.registerAlias("DecisionForcing", "DecisionSupport");
