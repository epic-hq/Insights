 ...
 */
export async function persistCanvasAction(action: A2UIAction, projectId: string): Promise<PersistResult> {
	try {
...
async function persistInterviewPromptsAction(action: A2UIAction, _projectId: string): Promise<PersistResult> {
	...
	switch (actionName) {
		case "reorder": {
			...
			const results = await Promise.all(dbIds.map((id, idx) => postIntent(id, "update-order", { order: String(idx) })));
