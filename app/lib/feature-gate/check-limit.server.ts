 ...
 */
export async function buildFeatureGateContext(accountId: string, userId: string): Promise<FeatureGateContext> {
...
async function getMonthlyAiAnalysisCount(accountId: string): Promise<number> {
	...
	// Count interviews that have been processed (exclude draft/upload states)
	const { count, error } = await supabase
		.from("interviews")
		.select("*", { count: "exact", head: true })
		.eq("account_id", accountId)
		.gte("created_at", startOfMonth.toISOString())
		.in("status", ["transcribed", "processing", "ready", "tagged"]);

	if (error) {
		consola.error("[checkLimit] Failed to get AI analysis count:", error);
		return 0;
	}
...
async function getMonthlyVoiceMinutes(accountId: string): Promise<number> {
	...
	if (error) {
		consola.error("[checkLimit] Failed to get voice minutes:", error);
		return 0;
	}
...
async function getActiveProjectCount(accountId: string): Promise<number> {
	...
	if (error) {
		consola.error("[checkLimit] Failed to get project count:", error);
		return 0;
	}
