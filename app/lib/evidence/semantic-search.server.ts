...
export async function searchEvidenceSemantic(
  ...
  const {
    query,
    interviewId,
    matchThreshold = SIMILARITY_THRESHOLDS.SEMANTIC_SEARCH,
    matchCount = 10,
  } = params;
...
export async function searchEvidenceMultipleQueries(
  ...
  for (const result of results) {
    if (result.status === "fulfilled") {
      ...
      console.error(
        "[searchEvidenceMultipleQueries] Query failed:",
        result.reason,
      );
    }
  }
  ...
  const queries = [
    {
      label: "budget",
      query:
        "budget constraints, pricing concerns, cost, investment, ROI, pricing model, payment terms, financial",
    },
    {
      label: "authority",
      query:
    ...
    },
    {
      label: "need",
      query:
        "pain points, problems, challenges, goals, needs, requirements, must-have features, critical issues",
    },
    {
      label: "timeline",
      query:
        "timeline, deadline, urgency, when, timeframe, schedule, launch date, implementation date",
    },
    {
      label: "nextSteps",
      query:
        "next steps, follow up, action items, who will do what, commitments, meeting scheduled",
    },
  ];
