...
export async function getResearchAnswerRollup(
  ...
  const interviewsById = new Map(
    (interviewsRes.data || []).map((row) => [row.id, row as { id: string; title: string | null; interview_date: string | null }])
  )
...
export async function getDecisionQuestionDetail(
  supabase: SupabaseClient<Database>,
...
export async function getResearchQuestionDetail(
  supabase: SupabaseClient<Database>,
...
export async function getProjectAnswerDetail(
  supabase: SupabaseClient<Database>,
