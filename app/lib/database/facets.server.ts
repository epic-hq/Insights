...
export class FacetResolver {
  ...
  private globalFacetCache = new Map<
    number,
    { label: string; kind_id: number; synonyms: string[] | null }
  >();
  ...
  private ensureSlug(kindId: number, label: string): string {
    const base =
      slugify(label, { separator: "_" }) || `facet_${kindId}_${Date.now()}`;
  ...
  async ensureFacetForRef(
    ...
    if (prefix === GLOBAL_PREFIX.slice(0, 1)) {
      ...
      const kindSlug =
        fallback.kindSlug ??
        (globalFacet ? this.kindIdToSlug?.get(globalFacet.kind_id) : undefined);
  ...
  async ensureFacet(options: {
    ...
    if (selectError) {
      consola.warn(
        "Failed to load existing facet_account",
        selectError.message,
      );
      return null;
    }
    ...
    if (existing?.id) {
      const mergedSynonyms = normalizeSynonyms([
        ...(existing.synonyms ?? []),
        ...normalizedSynonyms,
      ]);
  ...
  private async loadGlobalFacet(id: number) {
    if (this.globalFacetCache.has(id))
      return this.globalFacetCache.get(id) ?? null;
    const { data, error } = await this.db
      .from("facet_global")
...
export async function getFacetCatalog({
  db,
  accountId,
}: FacetCatalogOptions): Promise<FacetCatalog> {
  const [
    ...
    db
      .from("facet_kind_global")
      .select("id, slug, label, updated_at")
      .order("id"),
    db.from("facet_global").select("id, kind_id, label, synonyms, updated_at"),
    db
      .from("facet_account")
      .select("id, kind_id, label, synonyms, updated_at, is_active")
      .eq("account_id", accountId),
  ]);

  if (kindError)
    throw new Error(`Failed to load facet kinds: ${kindError.message}`);
  if (globalError)
    throw new Error(`Failed to load global facets: ${globalError.message}`);
  if (accountError)
    throw new Error(`Failed to load account facets: ${accountError.message}`);

  const kinds: FacetCatalogKind[] = (kindRows ?? []).map((row) => ({
    slug: row.slug,
    label: row.label,
  }));
  const kindIdToSlug = new Map<number, string>(
    (kindRows ?? []).map((row) => [row.id, row.slug]),
  );

  const entries = new Map<number, FacetCatalogEntry>();
  let newestTimestamp = getMaxTimestamp(kindRows ?? []);

  // Global facets are reference data only - they don't have facet_account_ids yet
  // When matched, they trigger FacetResolver.ensureFacet() to create account-specific entries
  for (const row of globalRows ?? []) {
    entries.set(row.id, {
      facet_account_id: 0, // Sentinel value - signals "create via FacetResolver"
  ...
  const facets = Array.from(entries.values()).filter(
    (entry) => entry.kind_slug && entry.label,
  );
...
export async function persistFacetObservations({
  ...
  for (const { personId, facets, scales } of observations) {
    if (Array.isArray(facets)) {
      for (const obs of facets) {
        ...
        const evidenceId =
          evidenceIndex !== null && evidenceIndex !== undefined
            ? (evidenceIds[evidenceIndex] ?? null)
            : null;
  ...
  if (facetRows.length) {
    ...
    if (facetError)
      consola.warn("Failed to upsert person facets", facetError.message);
  }
  ...
  if (scaleRows.length) {
    ...
    if (scaleError)
      consola.warn("Failed to upsert person scales", scaleError.message);
  }
}
