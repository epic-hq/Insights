...
export async function deleteGmailConnection(
	...
	const { error } = await supabase.from("gmail_connections").delete().eq("user_id", userId).eq("account_id", accountId);
...
export async function getBatchSurveySendStats(
	supabase: SupabaseClient<Database>,
	surveyIds: string[]
): Promise<Record<string, { total: number; sent: number; opened: number; completed: number }>> {
	if (surveyIds.length === 0) return {};

	const { data, error } = await supabase.from("survey_sends").select("survey_id, status").in("survey_id", surveyIds);
	...
	const result: Record<string, { total: number; sent: number; opened: number; completed: number }> = {};
 ...
 */
export async function getSurveySends(supabase: SupabaseClient<Database>, surveyId: string): Promise<SurveySend[]> {
	const { data, error } = await supabase
		.from("survey_sends")
		.select("*")
		.eq("survey_id", surveyId)
