...
function createMockSupabase() {
	...
	return {
		...
		_setTableResponse(table: string, response: { data: unknown; error: unknown }) {
...
describe("gmail.server", () => {
	...
	describe("getGmailConnection", () => {
		it("returns connection when found", async () => {
			const { getGmailConnection } = await import("~/lib/integrations/gmail.server");
			...
			const result = await getGmailConnection(mockSupabase as any, "user-1", "acc-1");
	...
	describe("getSurveySendStats", () => {
		it("returns null when no sends exist", async () => {
			const { getSurveySendStats } = await import("~/lib/integrations/gmail.server");
	...
	describe("getBatchSurveySendStats", () => {
		it("returns empty object for no survey IDs", async () => {
			const { getBatchSurveySendStats } = await import("~/lib/integrations/gmail.server");
		...
		it("groups stats by survey ID", async () => {
			...
			const result = await getBatchSurveySendStats(mockSupabase as any, ["s1", "s2"]);
	...
	describe("sendGmailEmail", () => {
		it("sends email via Pica wrapper and returns messageId/threadId", async () => {
			const { sendGmailEmail } = await import("~/lib/integrations/gmail.server");
	...
	describe("createSurveySends", () => {
		it("returns empty array for empty input", async () => {
			const { createSurveySends } = await import("~/lib/integrations/gmail.server");
		...
		it("inserts sends with nudge defaults", async () => {
			...
			mockSupabase.from = vi.fn(() => chain);
			chain.select = vi.fn(() => Promise.resolve({ data: insertedRecords, error: null }));
	...
	describe("markSurveySendCompleted", () => {
		it("updates matching sends to completed status", async () => {
			const { markSurveySendCompleted } = await import("~/lib/integrations/gmail.server");

			const chain = createChainMock({ data: null, error: null });
			mockSupabase.from = vi.fn(() => chain);
			chain.or = vi.fn(() => Promise.resolve({ error: null }));

			await markSurveySendCompleted(mockSupabase as any, "survey-1", "test@example.com");
