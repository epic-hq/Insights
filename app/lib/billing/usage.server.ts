 ...
 *
 * NOTE: The billing schema types are not yet in the generated types.
 * Once the migration is applied and types regenerated, remove the type
 * assertions and update to use proper typed queries.
 *
...
import { getAccountPlan as getAccountPlanFromGate } from "~/lib/feature-gate/check-limit.server";
...
/**
 * Get a client configured for the billing schema.
...
export async function recordUsageAndSpendCredits(
  ...
  try {
    ...
    const insertData: UsageEventInsert = {
    ...
    const { data: spendResult, error: spendError } = await serviceClient.rpc(
      "billing.spend_credits_atomic" as never,
      {
 ...
 * Uses public.record_usage_event function to insert into billing schema.
 */
...
/**
 * Get account's current plan and seat count.
 * Delegates to the shared getAccountPlan in check-limit.server.ts
 * which queries billing_subscriptions for real subscription data.
 */
async function getAccountPlan(
  accountId: string,
): Promise<{ planId: PlanId; seatCount: number }> {
  const planId = await getAccountPlanFromGate(accountId);
  // TODO: Query seat count from billing_subscriptions.quantity when per-seat billing is live
  return {
    planId,
    seatCount: 1,
  };
