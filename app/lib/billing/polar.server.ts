 ...
 */
export async function provisionLegacyTrial(params: { accountId: string; email?: string; planId?: PlanId }): Promise<{
	isOnTrial: boolean
	planName: string
	trialEnd: string
} | null> {
	...
	if (customerError) {
		consola.error("[legacy-trial] Failed to create billing customer", customerError)
		throw customerError
	}
	...
	if (subscriptionError) {
		consola.error("[legacy-trial] Failed to create subscription", subscriptionError)
