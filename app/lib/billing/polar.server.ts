...
/**
 * Remove trial records when a real Polar subscription activates.
 * Deletes the synthetic billing_subscription and billing_customer created
...
export async function removeTrial(params: { accountId: string }) {
  ...
  if ((subCount ?? 0) > 0 || (custCount ?? 0) > 0) {
    consola.info("[polar] Removed trial records", {
      accountId,
 ...
 */
export async function provisionTrial(params: {
  accountId: string;
  email?: string;
  planId?: PlanId;
}): Promise<{
 ...
 *
 * Only applies to subscriptions managed by Polar (not free trials).
 *
