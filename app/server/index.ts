...
export default await createHonoServer({
  ...
  configure(server, { upgradeWebSocket }) {
    ...
    async function getMcpToolsAndClass() {
      if (!mcpToolsPromise) {
        mcpToolsPromise = (async () => {
          ...
          const { generateProjectRoutesTool } =
            await import("../mastra/tools/generate-project-routes");
          const { fetchConversationLensesTool } =
            await import("../mastra/tools/fetch-conversation-lenses");
          const { fetchTopThemesWithPeopleTool } =
            await import("../mastra/tools/fetch-top-themes-with-people");
          const { fetchResearchPulseTool } =
            await import("../mastra/tools/fetch-research-pulse");
          const { generateResearchRecommendationsTool } =
            await import("../mastra/tools/generate-research-recommendations");
          const {
            fetchOpportunitiesTool,
            createOpportunityTool,
            updateOpportunityTool,
          } = await import("../mastra/tools/manage-opportunities");
          const { manageOrganizationsTool } =
            await import("../mastra/tools/manage-organizations");
          ...
          const allTools = {
            ...
            generate_research_recommendations:
              generateResearchRecommendationsTool,
    ...
    // Claude requests this with the resource path appended (e.g. /mcp)
