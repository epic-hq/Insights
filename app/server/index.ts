...
export default await createHonoServer({
	...
	configure(server, { upgradeWebSocket }) {
		...
		async function getMcpToolsAndClass() {
			if (!mcpToolsPromise) {
				mcpToolsPromise = (async () => {
					...
					const { semanticSearchEvidenceTool } = await import("../mastra/tools/semantic-search-evidence");
					const { fetchEvidenceTool } = await import("../mastra/tools/fetch-evidence");
					const { fetchThemesTool } = await import("../mastra/tools/fetch-themes");
					const { fetchPeopleDetailsTool } = await import("../mastra/tools/fetch-people-details");
					const { fetchSurveysTool } = await import("../mastra/tools/fetch-surveys");
					const { searchSurveyResponsesTool } = await import("../mastra/tools/search-survey-responses");
					const { fetchInterviewContextTool } = await import("../mastra/tools/fetch-interview-context");
					const { fetchPersonasTool } = await import("../mastra/tools/fetch-personas");
					const { fetchSegmentsTool } = await import("../mastra/tools/fetch-segments");
					const { semanticSearchPeopleTool } = await import("../mastra/tools/semantic-search-people");
					const { fetchProjectStatusContextTool } = await import("../mastra/tools/fetch-project-status-context");
					const { upsertPersonTool } = await import("../mastra/tools/upsert-person");
					const { managePeopleTool } = await import("../mastra/tools/manage-people");
					const { createTaskTool, deleteTaskTool, updateTaskTool } = await import("../mastra/tools/manage-tasks");
					const { markTaskCompleteTool } = await import("../mastra/tools/mark-task-complete");
					const { manageAnnotationsTool } = await import("../mastra/tools/manage-annotations");
					const { generateProjectRoutesTool } = await import("../mastra/tools/generate-project-routes");
					const { fetchConversationLensesTool } = await import("../mastra/tools/fetch-conversation-lenses");
					const { fetchTopThemesWithPeopleTool } = await import("../mastra/tools/fetch-top-themes-with-people");
					const { fetchResearchPulseTool } = await import("../mastra/tools/fetch-research-pulse");
					const { generateResearchRecommendationsTool } = await import("../mastra/tools/generate-research-recommendations");
					const { fetchOpportunitiesTool, createOpportunityTool, updateOpportunityTool } = await import("../mastra/tools/manage-opportunities");
					const { manageOrganizationsTool } = await import("../mastra/tools/manage-organizations");
					...
					function wrapForMcp(tool: any) {
						...
						return {
							...
							execute: async (input: any, ctx: any) => {
								...
								if (process.env.__MCP_PROJECT_ID) contextMap.set("project_id", process.env.__MCP_PROJECT_ID);
								if (process.env.__MCP_ACCOUNT_ID) contextMap.set("account_id", process.env.__MCP_ACCOUNT_ID);
								const safeCtx = {
					...
					const mcpTools = Object.fromEntries(Object.entries(allTools).map(([k, v]) => [k, wrapForMcp(v)]));
		...
		async function handleMcpRequest(rawRequest: Request) {
			...
			const mcpSdkPath = "@modelcontextprotocol/sdk/server/webStandardStreamableHttp.js";
			const { WebStandardStreamableHTTPServerTransport } = await import(/* @vite-ignore */ mcpSdkPath);
		...
		server.get("/.well-known/oauth-protected-resource", (c) => {
			...
			return c.json(getProtectedResourceMetadata(`${origin}/mcp`, origin), 200, { "Cache-Control": "no-store" });
		});
		...
		server.post("/oauth/token", async (c) => {
			...
			if (contentType.includes("application/x-www-form-urlencoded")) {
				...
				body = Object.fromEntries(Object.entries(parsed).map(([k, v]) => [k, String(v)]));
		...
		server.all("/mcp", async (c) => {
			...
			if (!rawKey) {
				return c.json({ error: "Missing Authorization: Bearer <api_key>" }, 401, { "WWW-Authenticate": "Bearer" });
			}
