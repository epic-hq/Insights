...
} from "~/lib/people/resolution.server";
...
describe("resolveOrCreatePerson", () => {
  ...
  describe("idempotency", () => {
    ...
    it("should handle retry without creating duplicates", async () => {
      ...
      // First call
      const result1 = await resolveOrCreatePerson(
        supabase,
        TEST_ACCOUNT_ID,
        TEST_PROJECT_ID,
        input,
  ...
  describe("match priority", () => {
    it("should use priority: email > platform_id > name_company > created", async () => {
      ...
      const emailResult = await resolveOrCreatePerson(
        supabase,
        TEST_ACCOUNT_ID,
        TEST_PROJECT_ID,
        {
      ...
      const platformResult = await resolveOrCreatePerson(
        supabase,
        TEST_ACCOUNT_ID,
        TEST_PROJECT_ID,
        {
      ...
      const nameResult = await resolveOrCreatePerson(
        supabase,
        TEST_ACCOUNT_ID,
        TEST_PROJECT_ID,
        {
