...
const SUPABASE_URL =
  getEnvVar("TEST_SUPABASE_URL") || getEnvVar("SUPABASE_URL");
const SUPABASE_ANON_KEY =
  getEnvVar("TEST_SUPABASE_ANON_KEY") || getEnvVar("SUPABASE_ANON_KEY");

// Ensure test environment variables are available (server-side only)
if (typeof process !== "undefined" && process.env) {
  if (!SUPABASE_URL) {
    throw new Error(
      "SUPABASE_URL or TEST_SUPABASE_URL environment variable is required for integration tests",
    );
  }
...
export async function getTestDbState() {
  const [interviews, people, links, insights, tags, junctionTags] =
    await Promise.all([
      testDb
        .from("interviews")
        .select("id,title")
