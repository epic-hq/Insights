...
if (!process.env.SUPABASE_URL) {
	throw new Error("SUPABASE_URL must be set in environment for integration tests. Run with: dotenvx run -- pnpm test:integration")
}
if (!process.env.SUPABASE_ANON_KEY) {
	throw new Error("SUPABASE_ANON_KEY must be set in environment for integration tests. Run with: dotenvx run -- pnpm test:integration")
}
