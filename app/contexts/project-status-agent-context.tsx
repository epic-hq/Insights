...
const ProjectStatusAgentContext =
  createContext<ProjectStatusAgentContextType | null>(null);

export function ProjectStatusAgentProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  const shouldDefaultCollapsed =
    isMobile && location.pathname.includes("/interviews");
...
export function useProjectStatusAgent() {
  ...
  if (!context) {
    throw new Error(
      "useProjectStatusAgent must be used within ProjectStatusAgentProvider",
    );
  }
  return context;
}
