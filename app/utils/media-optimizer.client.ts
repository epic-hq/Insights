  ...
  loadingPromise = (async () => {
    ...
    if (isCrossOriginIsolated()) {
      try {
        ...
        console.warn(
          "[MediaOptimizer] Multi-threaded load failed, falling back to single-threaded:",
          err,
        );
      }
    }
...
export function isVideoFile(file: File): boolean {
  ...
  const ext = file.name.split(".").pop()?.toLowerCase() || "";
  return ["mp4", "mov", "avi", "mkv", "webm", "m4v", "wmv", "flv"].includes(
    ext,
  );
}
...
export async function optimizeMediaFile(
  ...
  try {
    ...
    const optimizedFile = new File(
      [optimizedBlob],
      `${baseName}.${getOutputExtension(mediaKind)}`,
      {
        type: outputMimeType,
...
/**
 * Extract the audio track from a video file without re-encoding (stream copy).
 * This is near-instant since it copies the audio bitstream directly.
 *
 * If the file has no audio track (FFmpeg fails), falls back to returning
...
export async function extractAudioOnly(
  ...
  try {
    ...
    try {
      ...
      // Stream-copy audio, strip video, no re-encoding
      const args = ["-i", inputFile, "-vn", "-c:a", "copy", "-y", outputFile];
      await ffmpeg.exec(args);
    ...
    const audioBlob = new Blob([outputData], { type: "audio/mp4" });
    const audioSize = audioBlob.size;

    // If output is empty or suspiciously tiny, the file likely had no audio track
    if (audioSize < 100) {
      throw new Error("No audio track found in file");
    }

    const baseName = file.name.replace(/\.[^.]+$/, "");
    const audioFile = new File([audioBlob], `${baseName}.m4a`, {
      type: "audio/mp4",
      lastModified: Date.now(),
    });
    ...
    onProgress?.({
      phase: "done",
      percent: 100,
      message: `Audio extracted: ${formatBytes(originalSize)} → ${formatBytes(audioSize)}`,
      originalSize,
      optimizedSize: audioSize,
      multiThreaded: ffmpegMultiThreaded,
    });
    ...
    console.warn(
      "[MediaOptimizer] Audio extraction failed, returning original:",
      message,
    );
    ...
    onProgress?.({
      phase: "done",
      percent: 100,
      message: "Could not extract audio, using original file",
      originalSize,
      optimizedSize: originalSize,
      multiThreaded: ffmpegMultiThreaded,
    });
