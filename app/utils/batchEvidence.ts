...
const BATCH_SIZE = Math.min(
  MAX_BATCH_SIZE,
  readPositiveIntEnv("EVIDENCE_BATCH_SIZE", DEFAULT_BATCH_SIZE),
);
...
export async function batchExtractEvidence(
  ...
  if (!ENABLE_BATCHING) {
    ...
    consola.info(
      `⚡ Single-batch mode: ${speakerTranscripts.length} utterances (≤${BATCH_SIZE}, batching disabled)`,
    );
    const startTime = Date.now();
    const result = await withHeartbeatWhilePending(
      () => extractFn(speakerTranscripts),
  ...
  consola.info(
    `🚀 Batching enabled: processing ${speakerTranscripts.length} utterances in chunks of ${BATCH_SIZE}`,
  );
  ...
  consola.info(
    `📦 Created ${batches.length} batches (max ${MAX_CONCURRENT_BATCHES} concurrent)`,
  );
  ...
  const batchResults = await processWithConcurrency(
    ...
    async (batch, batchIndex) => {
      ...
      let result = await withHeartbeatWhilePending(
        () => extractFn(batch),
        BATCH_HEARTBEAT_INTERVAL_MS,
      );
      ...
      if (evidenceCount === 0 && batch.length >= 5) {
        consola.warn(
          `⚠️  ${batchLabel}: 0 evidence from ${batch.length} utterances — retrying`,
        );
        await heartbeats.yield();
        result = await withHeartbeatWhilePending(
          () => extractFn(batch),
          BATCH_HEARTBEAT_INTERVAL_MS,
        );
      ...
      consola.success(
        `✅ ${batchLabel}: completed in ${batchDuration}s (${result.evidence?.length ?? 0} evidence)`,
      );
...
async function withHeartbeatWhilePending<T>(
  operation: () => Promise<T>,
  intervalMs: number,
): Promise<T> {
