  ...
  try {
    ...
    if (updateError) {
      updateMetadataSpan?.end?.({
        level: "ERROR",
        statusMessage:
          updateError?.message ?? "Failed to update conversation_analysis",
      });
      throw new Error(
        `Failed to update conversation_analysis: ${updateError?.message}`,
      );
    }
    ...
    if (interviewFetchError || !interview) {
      fetchInterviewSpan?.end?.({
        level: "ERROR",
        statusMessage:
          interviewFetchError?.message ?? "Failed to fetch interview",
      });
      throw new Error(
        `Failed to fetch interview details: ${interviewFetchError?.message}`,
      );
    }
    fetchInterviewSpan?.end?.();
    ...
    consola.log(
      "Queued interview processing via Trigger.dev for interview:",
      interviewId,
    );
    ...
    const triggerEnv = process.env.TRIGGER_SECRET_KEY?.startsWith("tr_dev_")
      ? "dev"
      : "prod";
    consola.info(`Triggering interview pipeline in ${triggerEnv} environment`, {
      interviewId,
    });

    // Always use v2 modular workflow (v1 monolithic workflow is deprecated)
    consola.log("Triggering v2 orchestrator...", { forceReprocess });

    // Build trigger options - only use idempotency for non-forced runs
    // forceReprocess bypasses idempotency to allow intentional reprocessing
    const triggerOptions = forceReprocess
      ? {} // No idempotency key - allow reprocessing
      : {
          idempotencyKey: `interview-orchestrator-${interviewId}`,
    ...
    consola.log(
      `Trigger.dev v2 orchestrator triggered successfully with handle: ${handle.id}`,
    );
