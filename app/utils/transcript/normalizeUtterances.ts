...
const extractTime = (
  ...
  const start =
    toNumber(item.start) ?? toNumber(item.start_time) ?? toNumber(item.begin);
  const end =
    toNumber(item.end) ??
    toNumber(item.end_time) ??
    toNumber(item.finish) ??
    toNumber(item.stop) ??
    start;
  return { start, end };
};

const shouldConvertMilliseconds = (
  times: number[],
...
export function normalizeTranscriptUtterances(
  ...
  const useMilliseconds = shouldConvertMilliseconds(
    timings,
  ...
  return rawUtterances.map((item) => {
    ...
    return {
      speaker:
        typeof item.speaker === "string" && item.speaker.trim().length
          ? item.speaker
          : "A",
