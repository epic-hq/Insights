...
export function findTranscriptSegmentByTime(
	...
	contextWindowSeconds: number = 10
...
export function getTranscriptContext(
	...
	// Sort by start time
	const sortedSegments = [...speakerTranscripts].sort((a, b) => 
	...
	for (let i = 0; i < sortedSegments.length; i++) {
		...
		const distance = Math.min(
			Math.abs(targetTimeSeconds - segmentStart),
			Math.abs(targetTimeSeconds - segmentEnd)
		)
