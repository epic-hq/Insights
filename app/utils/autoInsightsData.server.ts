 ...
 */
export async function aggregateAutoInsightsData(
  request: Request,
  accountId: string
): Promise<AutoInsightsData> {
  ...
  const insights: InsightSummary[] = (insightsData || []).map(insight => {
    const tags = insightTagsData
      ?.filter(it => it.insight_id === insight.id)
      .map(it => (it.tags as any)?.tag)
      .filter(Boolean) || []

    const personas = insightPersonasData
  ...
  const personas: PersonaSummary[] = await Promise.all(
    (personasData || []).map(async persona => {
      ...
      const topPainPoints = personaInsights
        ?.map(pi => (pi.insights as any)?.pain)
        .filter(Boolean)
        .slice(0, 3) || []

      const topDesiredOutcomes = personaInsights
  ...
  const { data: tagCategories } = await db
    ...
    `)
    .in("tag_id", (tagFrequency || []).map(t => t.tag))

  const tags: TagSummary[] = (tagFrequency || []).map(tag => {
