...
export function createRouteDefinitions(projectPath = ""): RouteDefinitions {
	...
	return {
		...
		ask: {
			index: () => `${base}/ask`,
			new: () => `${base}/ask/new`,
			edit: (id: string) => `${base}/ask/${id}/edit`,
			responses: (id: string) => `${base}/ask/${id}/responses`,
			responseDetail: (surveyId: string, responseId: string) => `${base}/ask/${surveyId}/responses/${responseId}`,
			/** Public URL for respondents: /ask/<slug> */
			public: (slug: string) => `/ask/${slug}`,
		},
