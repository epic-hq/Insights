...
const MEDIA_EXTENSIONS = ["mp4", "mp3", "m4a", "wav", "webm", "ogg", "mov", "avi", "mkv", "flac", "aac"]
 ...
 */
async function extractVideoUrlWithLLM(html: string, pageUrl: string): Promise<string | null> {
	const openaiApiKey = process.env.OPENAI_API_KEY
	if (!openaiApiKey) {
		consola.warn("[extractMediaUrl] OPENAI_API_KEY not set, skipping LLM extraction")
		return null
	}
...
Return ONLY the URL, nothing else. If no media URL is found, return "NOT_FOUND".`,
		...
		if (extractedUrl && extractedUrl !== "NOT_FOUND" && extractedUrl.startsWith("http")) {
			consola.info(`[extractMediaUrl] LLM extracted media URL: ${extractedUrl}`)
			return extractedUrl
		}
 ...
 */
export async function extractAllMediaUrls(pageUrl: string): Promise<ExtractionResult> {
	...
	try {
		...
		if (!response.ok) {
			consola.warn(`[extractMediaUrl] Failed to fetch page: ${response.status}`)
		...
		if (jsonLdMatch) {
			try {
				...
				const contentUrl = jsonLd.contentUrl || jsonLd.embedUrl || jsonLd.video?.contentUrl
		...
		const streamingUrlPattern = /https?:\/\/[^\s"'<>\\]+\.(?:m3u8|mpd)(?:[^\s"'<>\\]*)?/gi
		...
		if (assets.length === 0) {
			consola.info("[extractMediaUrl] Regex patterns failed, trying LLM extraction...")
 ...
 */
export async function extractBestMediaUrl(pageUrl: string): Promise<string | null> {
...
export async function resolveToMediaUrl(
	...
	consola.info(`[extractMediaUrl] URL is not direct media, scanning page: ${url}`)
