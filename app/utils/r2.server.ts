...
export async function uploadToR2({
  ...
  contentType,
  useMultipart,
}: R2UploadParams): Promise<UploadResult> {
  const config = getR2Config();
  if (!config) {
  ...
  if (shouldUseMultipart && body.length > PART_SIZE) {
    consola.log(
      `Using multipart upload for ${(body.length / 1024 / 1024).toFixed(2)}MB file`,
    );
...
async function uploadToR2Multipart({
  ...
  try {
    ...
    consola.log(
      `Uploading ${numParts} parts (${PART_SIZE / 1024 / 1024}MB each)`,
    );
...
async function uploadPart({
  ...
  for (let attempt = 1; attempt <= MAX_PART_RETRIES; attempt++) {
    try {
      ...
      consola.error(
        `Part ${partNumber} attempt ${attempt}/${MAX_PART_RETRIES} failed:`,
        error,
      );
...
async function uploadPartAttempt({
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      consola.error(
        `Part ${partNumber} timed out after ${PART_UPLOAD_TIMEOUT}ms`,
      );
    }
    throw error;
  }
...
export async function createR2MultipartUpload(
  ...
  try {
    ...
    for (let partNumber = 1; partNumber <= totalParts; partNumber++) {
      ...
      const signedQuery = `${canonicalQuery}&X-Amz-Signature=${signature}`;
      partUrls[partNumber] =
        `${endpoint}/${bucket}/${encodedKey}?${signedQuery}`;
    }
 ...
 */
export async function abortMultipartUploadServer(params: {
  key: string;
  uploadId: string;
}): Promise<void> {
  const config = getR2Config();
...
export async function validateR2PresignedUrl(url: string): Promise<boolean> {
  try {
    ...
    if (!response.ok) {
      consola.warn(
        "Presigned URL validation failed with status:",
        response.status,
      );
      return false;
    }
...
function getR2Config(): R2Config | null {
  ...
  if (
    ...
    consola.warn(
      "Cloudflare R2 configuration is incomplete; uploads are disabled",
    );
    return null;
  }
