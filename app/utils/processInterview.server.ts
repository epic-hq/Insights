...
export async function extractEvidenceAndPeopleCore({
  ...
  try {
    ...
    personaSynthesis =
      await synthesisClient.DerivePersonaFacetsFromEvidence(evidenceResponse);
  ...
  if (insertedEvidenceIds.length) {
    ...
    // TODO(TrustCore): DEPRECATED - This UPDATE pattern is fragile and can silently drift.
    // The correct pattern is to set person_id at INSERT time (see extractEvidenceCore.ts line 1457).
    // To fix this legacy path, move person resolution BEFORE the evidence loop.
    // Tracked in bead: Insights-5ret
    //
    ...
    if (evidenceFacetRowsToInsert.length && evidenceIdsByPersonId.size) {
      consola.warn(
        "[TrustCore] Using legacy UPDATE pattern for evidence_facet.person_id - this path should be migrated to INSERT-time attribution",
      );
  ...
  try {
    ...
    if (!parityResult.passed) {
      consola.warn(
        "[TrustCore] Person attribution parity check failed in legacy processInterview",
        {
...
export async function analyzeThemesAndPersonaCore({
  ...
  if (!metadata.accountId && interviewRecord.account_id) {
    consola.warn(
      "[analyzeThemesAndPersonaCore] Backfilling accountId from interview record",
    );
    metadata.accountId = interviewRecord.account_id;
  }
  if (!metadata.projectId && interviewRecord.project_id) {
    consola.warn(
      "[analyzeThemesAndPersonaCore] Backfilling projectId from interview record",
    );
  ...
  try {
    ...
    consola.warn(
      "Auto theme generation failed; continuing without themes",
      themeErr,
    );
  }

  async function extractTranscriptInsightsWithRetry(
    text: string,
    instructions: string,
    attempts = 2,
  ) {
  ...
  if (interviewInsights) {
    ...
    try {
      ...
      try {
        response = await extractTranscriptInsightsWithRetry(
          fullTranscript,
          userCustomInstructions || "",
        );
        ...
        const errMsg =
          fallbackErr instanceof Error
            ...
            : String(fallbackErr);
        consola.error(
          "ExtractInsights ultimately failed after retries:",
          errMsg,
        );
  ...
  if (!metadata.accountId) {
    consola.warn(
      "[analyzeThemesAndPersonaCore] Skipping theme insert — accountId is missing",
    );
  } else {
    ...
    if (result.error)
      throw new Error(`Failed to insert themes: ${result.error.message}`);
    data = result.data;
  }
  ...
  const personName =
    ...
    evidenceResult.primaryPersonName ||
    generateFallbackName();
  const isNewNameGeneric =
    /^(Participant|Anonymous|Speaker|Person|Interviewee)\s*\d*$/i.test(
      personName,
    );
  ...
  const { firstname, lastname } = shouldUpdateName
  ...
  const personUpdatePayload: PeopleUpdate = {
    ...
    description:
      participant?.participantDescription?.trim() ||
      evidenceResult.primaryPersonDescription ||
      null,
    segment:
      participant?.segment?.trim() ||
  ...
  if (!shouldUpdateName) {
    consola.info(
      `Preserving user-entered name "${existingName}" (not overwriting with "${personName}")`,
    );
  }
  ...
  if (personUpdateErr) {
    consola.warn(
      "Failed to update primary person with participant details",
      personUpdateErr.message,
    );
  }

  try {
    let latestQuestions: Array<{
      id: string;
      text: string;
    ...
    if (metadata.projectId) {
      ...
      if (plannedError) {
        consola.warn(
          "Failed to load planned project_answers",
    ...
    if (
      Array.isArray(evidenceResult.evidenceUnits) &&
      evidenceResult.evidenceUnits.length &&
      metadata.projectId
    ) {
      for (let i = 0; i < evidenceResult.insertedEvidenceIds.length; i++) {
        ...
        const matchedSlug = (facetKinds || [])
          .map((slug) => String(slug))
          .find((slug) => categoryAlias.has(slug));
        const categoryKey = matchedSlug
          ? categoryAlias.get(matchedSlug)
          : undefined;
        const qRep = categoryKey
          ? categoryToQuestion.get(categoryKey)
          : undefined;
        if (!qRep) continue;
        ...
        if (existingAns?.id) {
          ...
          if (ansErr) {
            consola.warn(
              "Failed to upsert project_answers for evidence",
              ansErr.message,
            );
            continue;
          }
        ...
        if (
          !existingAnswer?.question_category &&
          (qRep?.categoryId || categoryKey)
        ) {
          answerUpdatePayload.question_category =
            qRep?.categoryId || categoryKey || null;
        }
        ...
        if (Object.keys(answerUpdatePayload).length) {
          ...
          if (answerUpdateErr) {
            consola.warn(
              "Failed to update project_answers metadata",
              answerUpdateErr.message,
            );
          }
        }
        ...
        if (evidenceUpdateErr) {
          consola.warn(
            "Failed to attach evidence to project_answer",
            evidenceUpdateErr.message,
          );
        }
      }
    }
  ...
  if (junctionError) {
    if (!junctionError.message?.toLowerCase().includes("duplicate key value")) {
      throw new Error(
        `Failed to link person to interview: ${junctionError.message}`,
      );
    }
  ...
  try {
    ...
    const { data: existingPersonas, error: personasError } =
      await personasQuery;

    if (personasError) {
      consola.warn(
        `Failed to fetch existing personas: ${personasError.message}`,
      );
    }
    ...
    if (
      personaDecision.action === "assign_existing" &&
      personaDecision.persona_id
    ) {
      personaId = personaDecision.persona_id;
      consola.log(
        `Assigning to existing persona: ${personaDecision.persona_name} (${personaId})`,
      );
    } else if (
      personaDecision.action === "create_new" &&
      personaDecision.new_persona_data
    ) {
    ...
    if (personaId && personData.id) {
      ...
      if (linkError) {
        ...
        consola.log(
          `Successfully linked person ${personData.id} to persona ${personaId}`,
        );
      }
    }
  } catch (personaErr) {
    consola.warn(
      "Persona assignment failed; continuing without persona assignment",
      personaErr,
    );
  }

  consola.log(
    `Successfully created/linked person "${personName}" to interview ${interviewRecord.id}`,
  );
  ...
  for (const tagName of uniqueTags) {
    ...
    for (let i = 0; i < insights.length; i++) {
      ...
      if (junctionError && !junctionError.message.includes("duplicate")) {
        consola.warn(
          `Failed to link insight ${storedInsight.id} to tag ${tagName}: ${junctionError.message}`,
        );
      }
    }
  }
  ...
  if (data?.length) {
    for (const insight of data) {
      const { error: personaLinkError } = await db.rpc(
      ...
      if (personaLinkError) {
        consola.warn(
          `Failed to auto-link persona insights for ${insight.id}: ${personaLinkError.message}`,
        );
      }
    }
  }
  ...
  const updatedInterview =
    (refreshedInterview as unknown as Interview) || interviewRecord;

  return {
    storedInsights: (data as unknown as InsightInsert[]) ?? [],
...
export async function attributeAnswersAndFinalizeCore({
  ...
  if (metadata.projectId) {
    try {
      ...
      consola.warn(
        "[processInterview] Evidence analysis failed",
        analysisError,
      );
    }
  }
  ...
  try {
    ...
    if (!posthog) {
      consola.debug(
        "[processInterview] PostHog not configured, skipping tracking",
      );
    } else {
...
export async function uploadMediaAndTranscribeCore({
  ...
  existingInterviewId,
  client,
}: UploadMediaAndTranscribePayload & {
  client: SupabaseClient<Database>;
}): Promise<UploadMediaAndTranscribeResult> {
  ...
  consola.info("[uploadMediaAndTranscribeCore] Sanitization check", {
    ...
    transcriptDataKeys:
      transcriptData && typeof transcriptData === "object"
  ...
  const normalizedTranscriptData = sanitizedTranscriptData as unknown as Record<
    string,
    unknown
  >;
  ...
  consola.info("[uploadMediaAndTranscribeCore] After sanitization", {
    ...
    speakerTranscriptsCount:
      sanitizedTranscriptData.speaker_transcripts?.length ?? 0,
  ...
  const fullTranscript = (sanitizedTranscriptData.full_transcript ??
    "") as string;
  const language =
  ...
  consola.log(
    ...
    sanitizedTranscriptData.audio_duration,
  );
  if (existingInterviewId) {
    ...
    if (fetchErr) {
      consola.error(
        "uploadMediaAndTranscribeCore: error fetching existing interview",
        {
          existingInterviewId,
          error: fetchErr,
          code: fetchErr.code,
      ...
      throw new Error(
        `Error fetching existing interview ${existingInterviewId}: ${fetchErr.message}`,
      );
    }

    if (!existing) {
      consola.error(
        "uploadMediaAndTranscribeCore: existing interview not found",
        { existingInterviewId },
      );
      throw new Error(
        `Existing interview ${existingInterviewId} not found in database`,
      );
    }
    ...
    if (updateErr) {
      consola.error(
        "uploadMediaAndTranscribeCore: error updating existing interview",
        {
          existingInterviewId,
          error: updateErr,
          code: updateErr.code,
      ...
      throw new Error(
        `Failed to update existing interview: ${updateErr.message}`,
      );
    }
    ...
    if (!updated) {
      ...
      throw new Error(
        `Failed to update existing interview ${existingInterviewId} - no data returned`,
      );
    }

    interviewRecord = updated as unknown as Interview;
    consola.info(
      "uploadMediaAndTranscribeCore: successfully updated existing interview",
      {
        interviewId: interviewRecord.id,
    ...
    if (!normalizedMetadata.accountId && interviewRecord.account_id) {
      consola.warn(
        "uploadMediaAndTranscribeCore: backfilling accountId from existing interview",
        {
    ...
    if (!normalizedMetadata.projectId && interviewRecord.project_id) {
      consola.warn(
        "uploadMediaAndTranscribeCore: backfilling projectId from existing interview",
        {
    ...
    const interviewData: InterviewInsert = {
      ...
      interview_date:
        normalizedMetadata.interviewDate ||
    ...
    if (interviewError || !created) {
      throw new Error(
        `Failed to create interview record: ${interviewError?.message}`,
      );
    }
  ...
  return {
    ...
    sanitizedTranscriptData: sanitizedTranscriptData as unknown as Record<
      string,
      unknown
    >,
...
export async function processInterviewTranscriptWithClient({
  ...
  try {
    conversationAnalysis = await generateConversationAnalysis({
      ...
      context: {
        meetingTitle:
          metadata.interviewTitle ||
          analysisResult.interview.title ||
          undefined,
    ...
    const { upsertConversationOverviewLens } =
      await import("~/lib/conversation-analyses/upsertConversationOverviewLens.server");
    ...
    const existingThemes = Array.isArray(
      analysisResult.interview.high_impact_themes,
    )
    ...
    if (!existingThemes.length && conversationAnalysis.key_takeaways.length) {
      takeawayStrings = conversationAnalysis.key_takeaways.map((takeaway) => {
        const priorityLabel = takeaway.priority
          ? `[${takeaway.priority.toUpperCase()}] `
          : "";
        const snippet = Array.isArray(takeaway.evidence_snippets)
          ? takeaway.evidence_snippets[0]
          : undefined;
    ...
    const openQuestionsText = conversationAnalysis.open_questions.length
      ? conversationAnalysis.open_questions
    ...
    if (analysisUpdateError) {
      consola.warn(
        "Failed to persist conversation analysis on interview",
        analysisUpdateError,
      );
    } else {
      analysisResult.interview.high_impact_themes =
        updatedInterview?.high_impact_themes ?? takeawayStrings;
      analysisResult.interview.open_questions_and_next_steps =
        updatedInterview?.open_questions_and_next_steps ??
        openQuestionsText ??
        null;
    }
  } catch (analysisError) {
    consola.warn(
      "Failed to generate conversation analysis for interview",
      analysisError,
    );
  }

  const projectId =
    uploadResult.metadata.projectId ?? analysisResult.interview.project_id;
  if (projectId) {
    ...
    if (!existingGoal) {
      try {
        ...
        consola.warn(
          "Failed to backfill research goal from conversation analysis",
          goalError,
        );
      }
    }
  ...
  return {
    stored: analysisResult.storedInsights,
