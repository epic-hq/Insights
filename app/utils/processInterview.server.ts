...
export async function extractEvidenceAndPeopleCore({
	...
	try {
		const parityResult = await validateAttributionParity(db, interviewRecord.id, "legacy-process");
		if (!parityResult.passed) {
			consola.warn("[TrustCore] Person attribution parity check failed in legacy processInterview", {
