...
export async function extractEvidenceAndPeopleCore({
  ...
  if (participants.length) {
    for (const [index, participant] of participants.entries()) {
      ...
      const personRecord = await upsertPerson(
        resolved.name,
...
export async function processInterviewTranscriptWithClient({
  ...
  try {
    ...
    const { data: updatedInterview, error: analysisUpdateError } = await db
      ...
      .select(
        "conversation_analysis, high_impact_themes, open_questions_and_next_steps",
      )
      .single();
    ...
    if (analysisUpdateError) {
      ...
      analysisResult.interview.conversation_analysis =
        updatedInterview?.conversation_analysis ??
        normalizedConversationAnalysis;
      analysisResult.interview.high_impact_themes =
