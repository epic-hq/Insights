...
export async function buildInitialSalesLensExtraction(
	...
	if (personIds.length > 0) {
		const { data: people, error: peopleError } = await db
			.from("people")
			.select("id, name, role, title, company, primary_email, default_organization_id, default_organization:organizations!default_organization_id(name)")
			.in("id", personIds);
		if (peopleError) {
			...
			for (const person of (people ?? []) as Array<PeopleRow & { default_organization?: { name: string | null } | null }>) {
				...
				peopleById.set(person.id, {
					...
					company: orgName || person.company ?? null,
