...
async function _getProjectAnalysisData(supabase: SupabaseClient, projectId: string, _accountId: string) {
	const [sectionsResult, insightsResult, peopleResult, personasResult] = await Promise.all([
		...
		// Phase 3: Select specific columns (company column dropped, use default_organization_id)
		supabase
			.from("people")
			.select(
				"id, account_id, project_id, name, firstname, lastname, primary_email, primary_phone, linkedin_url, title, segment, lifecycle_stage, role, job_function, location, description, image_url, persona_id, default_organization_id, person_type, contact_info, created_at, updated_at, created_by, updated_by"
			)
