...
type PeopleOrganization =
  Database["public"]["Tables"]["people_organizations"]["Row"];
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  const title = data?.organization?.name
    ? `${data.organization.name} | Organizations`
    : "Organization";
  return [
    { title },
    { name: "description", content: "Organization details and linked people" },
  ];
};
...
export async function loader({ params, context }: LoaderFunctionArgs) {
  ...
  const [
    ...
    supabase
      .from("opportunities")
      .select(
        "id, title, stage, amount, currency, next_step, next_step_due, close_date, description",
      )
    ...
    supabase
      ...
      .eq("project_id", projectId)
      .eq(
        "interview_people.people.people_organizations.organization_id",
        organizationId,
      )
  ...
  return {
    organization: data as Organization & {
      people_organizations: Array<
        PeopleOrganization & {
          person: Pick<
            Person,
            "id" | "name" | "image_url" | "segment" | "title"
          >;
        }
      >;
    },
    people:
      (people as Array<
        Pick<Person, "id" | "name" | "image_url" | "segment" | "title">
      >) ?? [],
    opportunities:
    ...
    conversations:
      (conversations as Array<
        Pick<Interview, "id" | "title" | "interview_date" | "status">
      >) ?? [],
  };
}
...
export default function OrganizationDetailPage() {
  const { organization, people, opportunities, conversations } =
    useLoaderData<typeof loader>();
  const actionData = useActionData<typeof action>();
  const { projectPath } = useCurrentProject();
  const routes = useProjectRoutes(
    projectPath ||
      `/a/${organization.account_id ?? ""}/${organization.project_id ?? ""}`,
  );
  ...
  const linkedPersonIds = new Set(
    (organization.people_organizations || []).map((link) => link.person.id),
  );
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  will also remove all links to people associated with this
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="space-y-1">
                ...
                <InlineEditableField
                  ...
                  entityIdKey="organizationId"
                  field="funding_stage"
                  value={organization.funding_stage}
                  type="select"
            ...
            <div className="mt-6 pt-4 border-t">
              <div className="relative overflow-hidden rounded-lg bg-gradient-to-r from-violet-500/10 via-purple-500/10 to-fuchsia-500/10 p-4">
                ...
                {!isResearching && (
                  <div className="pointer-events-none absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-gradient-to-r from-transparent via-white/20 to-transparent" />
                )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground text-xs">
                        {isResearching
                          ? "Gathering company information from the web"
                          : organization.website_url
                            ? "Research this company to fill in missing details"
                            : "Add a website URL above, then research to auto-fill details"}
                      </p>
                    </div>
                  </div>
                  <researchFetcher.Form
                    method="post"
                    action={`${routes.organizations.detail(organization.id)}/research`}
                  >
                    <Button
                      type="submit"
                      variant="default"
                      size="sm"
                      disabled={!organization.website_url || isResearching}
                    ...
                    >
                      {isResearching ? (
                        ...
                        <>
                          ...
                          Research Company
                          {/* Button shimmer */}
                          <span className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite_0.5s] bg-gradient-to-r from-transparent via-white/25 to-transparent" />
                        </>
                      )}
                    </Button>
                  </researchFetcher.Form>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              {opportunities.length === 1 ? "y" : "ies"} associated with this
              organization
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              {conversations.length !== 1 ? "s" : ""} with people from this
              organization
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 text-muted-foreground text-xs">
                        {conv.interview_date && (
                          <span>
                            {new Date(conv.interview_date).toLocaleDateString()}
                          </span>
                        )}
                        {conv.status && (
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr,1fr]">
          <Card className="border border-border/80">
            <CardHeader className="flex flex-col gap-2">
              ...
              <CardDescription>
                {(organization.people_organizations || []).length} people
