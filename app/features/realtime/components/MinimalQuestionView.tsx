...
type AnswerStatus =
  Database["public"]["Tables"]["project_answers"]["Row"]["status"];
...
function MinimalQuestionView({
  projectId,
  interviewId,
}: MinimalQuestionViewProps) {
  ...
  const updateStatus = useCallback(
    async (projectAnswerId: string, nextStatus: AnswerStatus) => {
      ...
      setAllQuestions((prev) =>
        prev.map((q) =>
          q.projectAnswerId === projectAnswerId
            ? { ...q, status: nextStatus }
            : q,
        ),
      );

      const payload: Database["public"]["Tables"]["project_answers"]["Update"] =
        { status: nextStatus };
