...
export async function getSegmentKindSummaries(
  supabase: DbClient,
  projectId: string,
): Promise<SegmentKindSummary[]> {
  ...
  if (segmentKindsError) {
    consola.error(
      "[getSegmentKindSummaries] Error fetching kinds",
      segmentKindsError,
    );
  ...
  if (segmentFacetsError) {
    consola.error(
      "[getSegmentKindSummaries] Error fetching facets",
      segmentFacetsError,
    );
  ...
  for (const facet of segmentFacets || []) {
    ...
    const slug =
      facetKind && typeof facetKind === "object" && "slug" in facetKind
        ? String(facetKind.slug)
        : undefined;
    if (slug) {
  ...
  if (facetIds.length > 0) {
    ...
    if (personFacetsError) {
      consola.error(
        "[getSegmentKindSummaries] Error fetching person facets",
        personFacetsError,
      );
...
export async function getSegmentsSummary(
  ...
  if (segmentKindsError) {
    consola.error(
      "[getSegmentsSummary] Error fetching kinds",
      segmentKindsError,
    );
    throw segmentKindsError;
  }
  ...
  if (personFacetsError) {
    consola.error(
      "[getSegmentsSummary] Error fetching person facets:",
      personFacetsError,
    );
    throw personFacetsError;
  }
  ...
  if (allPeopleIds.size === 0) {
    consola.info(
      "[getSegmentsSummary] No people found for facets in this project",
    );
    return [];
  }
  ...
  if (evidenceError) {
    consola.error(
      "[getSegmentsSummary] Error fetching evidence:",
      evidenceError,
    );
    throw evidenceError;
  }
  ...
  for (const facet of facets) {
    ...
    for (const personId of peopleInSegment) {
      ...
      for (const evidence of evidenceList) {
        ...
        const hasHighWtp =
          evidence.gains?.some((g: string) =>
            g.toLowerCase().includes("pay"),
          ) ?? false;
    ...
    if (
      options.minBullseyeScore === undefined ||
      bullseye_score >= options.minBullseyeScore
    ) {
      segments.push({
...
export async function getSegmentDetail(
  ...
  const { data: facet, error: facetError } = await supabase
    .from("facet_account")
    .select(
      "id, kind_id, slug, label, description, facet_kind_global!inner(slug)",
    )
  ...
  if (personError) {
    consola.error(
      "[getSegmentDetail] Error fetching person links:",
      personError,
    );
    return null;
  }
  ...
  for (const link of (personLinks as PersonFacetLink[]) || []) {
    ...
    const personEvidenceLinks: PersonEvidenceLink[] =
      link.people.evidence_people || [];
    for (const personLink of personEvidenceLinks) {
      ...
      // WTP signals
      const hasHighWtp =
        evidence.gains?.some((g: string) => g.toLowerCase().includes("pay")) ??
        false;
      if (hasHighWtp) highWtpCount++;
  ...
  consola.success(
    `[getSegmentDetail] Loaded segment ${segment.label} with ${segment.person_count} people`,
  );
