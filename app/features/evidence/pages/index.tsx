...
import { Frown, Grid3X3, List, Smile, Star, Target } from "lucide-react";
...
export async function loader({ context, params, request }: LoaderFunctionArgs) {
	...
	const minVotes = Number.parseInt(url.searchParams.get("min_votes") || "1", 10);
	...
	if (filterStarred) {
		...
		// Intersect with existing filters if present
		if (evidenceIdFilter) {
			evidenceIdFilter = evidenceIdFilter.filter((id) => starredIds.includes(id));
		} else {
	...
	if (filterUpvoted) {
		...
		if (votesErr) throw new Error(`Failed to load upvoted evidence: ${votesErr.message}`);
		...
		// Intersect with existing filters if present
		if (evidenceIdFilter) {
			evidenceIdFilter = evidenceIdFilter.filter((id) => upvotedIds.includes(id));
		} else {
	...
	if (filterFacetKind) {
		...
		// Intersect with existing filters if present
		if (evidenceIdFilter) {
			evidenceIdFilter = evidenceIdFilter.filter((id) => facetEvidenceIds.includes(id));
		} else {
		...
		<div className="space-y-4 p-4 sm:p-6">
			...
			<div className="relative max-w-3xl">
				<input
					...
					className="w-full rounded-lg border border-input bg-background px-4 py-2.5 pr-20 pl-10 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
				/>
				{isSearching && (
					<div className="-translate-y-1/2 absolute top-1/2 right-12 flex items-center gap-1 text-muted-foreground text-xs">
						<span>🧠</span>
						<span>Searching...</span>
					</div>
				)}
			...
			<div className="space-y-3">
				<div className="flex flex-wrap items-center justify-between gap-2">
					...
					<div className="flex flex-wrap items-center gap-2">
						{/* JTBD Quick Filters with active states */}
						<div className="flex flex-wrap gap-1.5">
							<Button
								...
								className={cn(
									...
									facetKind === "goal" &&
										"border-emerald-800 bg-emerald-950/50 text-emerald-200 hover:bg-emerald-950/70"
								)}
								onClick={() => updateParam("facet_kind", facetKind === "goal" ? "" : "goal")}
							>
							...
							<Button
								...
								onClick={() => updateParam("facet_kind", facetKind === "gain" ? "" : "gain")}
							>
							...
							<Button
								...
								className={cn(
									"h-7 gap-1 px-2 text-xs",
									facetKind === "pain" && "border-rose-800 bg-rose-950/50 text-rose-200 hover:bg-rose-950/70"
								)}
								onClick={() => updateParam("facet_kind", facetKind === "pain" ? "" : "pain")}
							>
					...
					{/* Right: Sort, Confidence, Method dropdowns */}
					<div className="flex flex-wrap items-center gap-2">
						{/* Compact Sort */}
						<select
			...
			{searchQuery && (
				<div className="rounded-lg border border-border bg-muted/30 px-4 py-3">
					<div className="flex items-center gap-4 text-sm">
						...
						<span className="text-muted-foreground">🔍 {keywordMatches.length} keyword matches</span>
						{semanticResults.length > 0 && (
							<>
								<span className="text-muted-foreground">•</span>
								<span className="text-muted-foreground">
									🧠 {semanticResults.filter((r) => !keywordMatches.some((k) => k.id === r.id)).length} semantic matches
								</span>
							</>
						)}
						<span className="text-muted-foreground">•</span>
						<span className="font-medium">Total: {filteredEvidence.length}</span>
					</div>
				</div>
			)}
			...
			{evidence.length === 0 ? (
				...
				<div className="rounded-lg border border-border border-dashed px-6 py-12 text-center text-muted-foreground text-sm">
					No evidence found matching "{searchQuery}". Try a different search term.
				</div>
			) : searchQuery && (keywordMatches.length > 0 || semanticResults.length > 0) ? (
				...
				<div className="space-y-6">
					...
					{keywordMatches.length > 0 && (
						<div>
							<div className="mb-3 flex items-center gap-3">
								<h3 className="font-semibold text-gray-300 text-sm uppercase tracking-wide">Exact Matches</h3>
							...
							<div className={listClassName}>
								{keywordMatches.map((item) => (
									<button
										...
										className="block w-full cursor-pointer text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
									>
										<EvidenceCard
											evidence={item}
											people={item.people}
					...
					{semanticResults.filter((r) => !keywordMatches.some((k) => k.id === r.id)).length > 0 && (
						<div>
							<div className="mb-3 flex items-center gap-3">
								<h3 className="font-semibold text-gray-300 text-sm uppercase tracking-wide">Related Insights</h3>
								<span className="text-gray-500 text-xs">
									({semanticResults.filter((r) => !keywordMatches.some((k) => k.id === r.id)).length} result
									{semanticResults.filter((r) => !keywordMatches.some((k) => k.id === r.id)).length !== 1 ? "s" : ""})
								</span>
							</div>
							<div className={listClassName}>
								{semanticResults
			...
			<EvidenceDetailModal evidenceId={selectedEvidenceId} onClose={() => setSelectedEvidenceId(null)} />
		</div>
	);
}
