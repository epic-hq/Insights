...
export async function loader({ context, params }: LoaderFunctionArgs) {
  ...
  let peopleByEvidence = new Map<string, { id: string; name: string | null; role: string | null; personas: Array<{ id: string; name: string }> }[]>()
  if (evidence.length) {
    ...
    for (const row of (evp ?? []) as Array<{ evidence_id: string; role: string | null; person: { id: string; name: string | null } | null }>) {
      if (row.person) personIds.add(row.person.id)
    }
    
    // Fetch personas for all people
    let personasByPerson = new Map<string, Array<{ id: string; name: string }>>()
    if (personIds.size > 0) {
  ...
  })) as Array<Pick<Evidence, "id" | "verbatim" | "support" | "confidence" | "created_at"> & { people: { id: string; name: string | null; role: string | null; personas: Array<{ id: string; name: string }> }[] }>
              ...
              <Card key={e.id} className="flex items-start justify-between gap-4 p-3">
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
                    ...
                    {/* Personas */}
                    {e.people && e.people.some(p => p.personas && p.personas.length > 0) && (
                      <>
                        <span>•</span>
                        <div className="flex flex-wrap gap-1">
                          {e.people.flatMap(p => p.personas || []).map((persona, i) => (
                            <Badge key={i} variant="outline" className="bg-green-50 text-green-800 border-green-200 text-xs">
                              {persona.name}
                            </Badge>
                          ))}
                        </div>
