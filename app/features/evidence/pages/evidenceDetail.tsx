...
import { getAnchorStartSeconds, type MediaAnchor } from "~/utils/media-anchors";
...
export async function loader({ context, params, request }: LoaderFunctionArgs) {
  ...
  if (peopleError) {
    consola.warn(
      `[evidence/detail] Failed to load evidence people: ${peopleError.message}`,
    );
  }
  ...
  if (facetError) {
    consola.warn(
      `[evidence/detail] Failed to load evidence facets: ${facetError.message}`,
    );
  }
  ...
  if (linkedPersonIds.size > 0) {
    ...
    if (personErr) {
      consola.warn(
        `[evidence/detail] Failed to load people names: ${personErr.message}`,
      );
    }
    ...
    if (Array.isArray(relatedEvidence) && relatedEvidence.length > 0) {
      ...
      if (relatedIds.length > 0) {
        ...
        if (relatedFacetError) {
          consola.warn(
            `[evidence/detail] Failed to load related facets: ${relatedFacetError.message}`,
          );
        }
        ...
        if (relatedPeopleError) {
          consola.warn(
            `[evidence/detail] Failed to load related people: ${relatedPeopleError.message}`,
          );
        }
        ...
        if (relatedPersonIds.size > 0) {
          let relatedPeopleQuery = supabase
            .from("people")
            .select("id, name")
          ...
          const { data: relatedPersonRows, error: relatedPersonErr } =
            await relatedPeopleQuery;
          if (relatedPersonErr) {
            consola.warn(
              `[evidence/detail] Failed to load related people names: ${relatedPersonErr.message}`,
            );
          }
        ...
        for (const row of relatedFacets ?? []) {
          ...
          list.push({
            ...
            person: personId
              ? { id: personId, name: relatedPersonNames.get(personId) ?? null }
              : null,
          });
...
function EvidenceChapterCard({
  evidence,
  isSelected,
  onClick,
}: EvidenceChapterCardProps) {
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2">
        ...
        {isDirty && (
          <span className="text-muted-foreground/60 text-xs">saving...</span>
        )}
      </div>
      {options ? (
        <select
...
export default function EvidenceDetail() {
  ...
  const allEvidence = useMemo(() => {
    ...
    return combined.sort((a, b) => {
      const aAnchors = Array.isArray(a.anchors)
        ? (a.anchors as MediaAnchor[])
        : [];
      const bAnchors = Array.isArray(b.anchors)
        ? (b.anchors as MediaAnchor[])
        : [];
  ...
  const evidenceWithAnchor = useMemo(() => {
    if (anchorFromUrl && selectedEvidence.id === evidence.id) {
      return {
        ...
        anchors: [
          anchorFromUrl,
          ...(Array.isArray(selectedEvidence.anchors)
  ...
  const [isArchived, setIsArchived] = useState(
    () => (selectedEvidence as any).is_archived === true,
  );
  ...
  const handleToggleArchive = useCallback(() => {
    ...
    archiveFetcher.submit(
      { _action: "toggle-archive", archived: String(newValue) },
      { method: "post" },
    );
        ...
        <TooltipProvider delayDuration={300}>
          <div className="mb-4 flex items-center gap-1.5">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                Archive — remove from views and analysis for your team
              </TooltipContent>
            </Tooltip>
            ...
            <AlertDialog>
              <Tooltip>
                ...
                <TooltipContent>
                  Delete — remove this evidence (recoverable for 30 days)
                </TooltipContent>
              </Tooltip>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will remove this evidence from all views and analysis.
                    You can recover it from Recently Deleted within 30 days.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
        ...
        {hasChapters && (
          <div className="mt-8">
            <h3 className="mb-3 font-medium text-muted-foreground text-sm">
              Related moments ({allEvidence.length})
            </h3>
