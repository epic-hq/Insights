...
import { BadgeInfo, Contrast, Dot, Users, GitMerge } from "lucide-react"
			...
			{ORDERED_FIELDS.map((k) => (
				<button
					...
					className={clsx(
						...
						value.includes(k) ? "border-foreground bg-foreground text-background" : "bg-transparent hover:bg-zinc-800 hover:text-zinc-200"
					)}
				>
					{FIELD_LABEL[k]}
				</button>
			...
			<div className="grid grid-cols-1 sm:grid-cols-[180px_repeat(2,minmax(0,1fr))]">
				...
				{fields.map((field) => (
					<React.Fragment key={field}>
						<div className="hidden bg-zinc-950/80 px-3 py-2 font-medium text-xs text-zinc-400 sm:block">{FIELD_LABEL[field]}</div>
...
function ChangeLogPanel({ changeLog = [{ version: "v0.1", note: "Initial personas created." }] }: { changeLog?: { version: string; note: string }[] }) {
	return (
			...
			<ul className="space-y-2">
				{changeLog.map((c) => (
					<li key={c.version} className="flex items-center gap-2 text-sm text-zinc-300">
						<span className="rounded-full bg-primary px-2 py-0.5 text-xs text-primary-foreground">{c.version}</span>
						<span>{c.note}</span>
					</li>
				))}
			</ul>
