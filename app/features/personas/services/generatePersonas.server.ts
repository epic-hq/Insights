...
async function clusterPeopleByFacets(
  ...
  consola.info(
    "[Persona Generation] Step 1: Clustering people by shared facets",
  );
  ...
  consola.info(
    `[Persona Generation] Found ${personFacets.length} person facet links`,
  );

  // Build person -> facets map
  const personFacetsMap = new Map<
    string,
    Array<{ id: number; label: string; kind: string }>
  >();
  ...
  const facetCombinationDetails = new Map<
    string,
    Array<{ id: number; label: string; kind: string }>
  >();
  ...
  consola.info(
    `[Persona Generation] Found ${facetCombinationMap.size} unique facet combinations`,
  );
...
async function aggregateEvidenceForClusters(
  ...
  consola.info(
    "[Persona Generation] Step 2: Aggregating evidence for each cluster (using person_id directly)",
  );
  ...
  for (const cluster of clusters) {
    ...
    const evidenceIds = [
      ...new Set(evidenceFacets.map((ef) => ef.evidence_id)),
    ];
    ...
    for (const ef of evidenceFacets || []) {
      ...
      if (ef.kind_slug === "behavior" || ef.kind_slug === "task")
        behaviors.add(ef.label);
    }
...
> {
  consola.info(
    `[Persona Generation] Step 3: Generating AI descriptions for ${clusters.length} clusters (in parallel)`,
  );
...
`;
    ...
    try {
      ...
      consola.error(
        "[Persona Generation] Error generating persona for cluster:",
        error,
      );
      return null;
    }
  ...
  consola.info(
    `[Persona Generation] Generated ${results.length} persona descriptions`,
  );

  // De-duplicate semantically similar personas
  const deduped = deduplicateSimilarPersonas(results);
  consola.info(
    `[Persona Generation] After semantic deduplication: ${deduped.length} personas (from ${results.length})`,
  );
...
function deduplicateSimilarPersonas(
  ...
  consola.info(
    `[Persona Generation] Starting semantic deduplication on ${personas.length} personas`,
  );
  ...
  for (const item of personas) {
    ...
    if (usedNames.has(nameLower)) {
      consola.info(
        `[Persona Generation] Skipping exact duplicate: "${item.persona.name}"`,
      );
      continue;
    }
    ...
    for (const existingName of usedNames) {
      ...
      if (overlap / newWords.length >= 0.5 && overlap >= 1) {
        isDuplicate = true;
        consola.info(
          `[Persona Generation] Skipping similar persona "${item.persona.name}" (matches "${existingName}")`,
        );
        break;
      }
    }
 ...
 */
async function generateContrastPersonas(
  corePersonas: Persona[],
): Promise<Persona[]> {
...
`;
  ...
  try {
    ...
    consola.info(
      `[Persona Generation] Generated contrast persona: ${contrast.name}`,
    );
  } catch (error) {
    consola.error(
      "[Persona Generation] Error generating contrast persona:",
      error,
    );
  }
...
export async function generatePersonasForProject(
  ...
  if (clusters.length === 0) {
    consola.warn(
      "[Persona Generation] No clusters found - need more people with facets",
    );
    return { personas: [], people_links: [] };
  }

  // Step 2: Aggregate evidence for each cluster
  const enrichedClusters = await aggregateEvidenceForClusters(
    supabase,
    projectId,
    clusters,
  );
