...
async function aggregateEvidenceForClusters(
	...
	for (const cluster of clusters) {
		// Get evidence facets for people in this cluster
		const { data: evidenceLinks } = await supabase
			.from("evidence_people")
			.select("evidence_id, person_id")
			.in("person_id", cluster.people_ids)

		if (!evidenceLinks || evidenceLinks.length === 0) continue

		const evidenceIds = [...new Set(evidenceLinks.map((el) => el.evidence_id))]

		// Get evidence facets (pains, goals, behaviors)
...
export async function generatePersonasForProject(
	...
	projectId: string,
	accountId: string
): Promise<{ personas: any[]; people_links: Array<{ persona_id: string; person_id: string }> }> {
