...
export default function LiveTranscriptionPage() {
  ...
  const startSession = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/a/${loaderData.accountId}/${loaderData.projectId}/api/live-transcription-clean`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          operation: "start",
      ...
      // Update session state BEFORE starting audio processing
      setSession(prev => ({
        ...prev,
        status: "recording",
        isRecording: true,
  ...
  const startTranscriptStream = useCallback((sessionId: string) => {
    const eventSource = new EventSource(`/a/${loaderData.accountId}/${loaderData.projectId}/api/live-transcription-clean/stream?sessionId=${sessionId}`)
  ...
  }, [loaderData.accountId, loaderData.projectId])
  ...
  const stopSession = useCallback(async (saveAsInterview = false) => {
    try {
      ...
      await fetch(`/a/${loaderData.accountId}/${loaderData.projectId}/api/live-transcription-clean`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          operation: "stop",
