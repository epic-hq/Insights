...
function SourceIcon({
  ...
  type: "interviews" | "surveys" | "documents";
}) {
  const icons = {
    interviews: Mic,
...
export function IntakeHealth({ data, isStreaming }: IntakeHealthProps) {
  ...
  const totalSources =
    data.sourceMix.interviews +
    data.sourceMix.surveys +
    data.sourceMix.documents;
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2.5">
              {data.coverage.map((segment) => {
                ...
                const levelColor = coverageLevelColor(
                  segment.count,
                  segment.target,
                );
                  ...
                  <div key={segment.label}>
                    <div className="mb-1 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn("font-medium text-xs", levelColor)}>
                          {level}
                        </span>
                      </div>
                    </div>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-muted-foreground text-sm">
          ...
          {data.daysSinceLastIntake != null && (
            <span className="flex items-center gap-1">
              ...
              {data.daysSinceLastIntake === 0
                ? "Updated today"
                : `${data.daysSinceLastIntake}d since last intake`}
            </span>
          )}
        </div>
