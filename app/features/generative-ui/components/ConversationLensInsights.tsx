...
function getSectionField(
  ...
  return sectionDataMap[sectionKey]?.fields?.find(
...
function distributeItemsByStep(
  steps: JobMapStep[],
  items: string[],
): string[][] {
...
function isJtbd(templateKey: string): boolean {
  return (
    templateKey.includes("jtbd") || templateKey.includes("jobs-to-be-done")
  );
}
      ...
      {opportunities.length > 0 && (
        <div className="space-y-0.5">
          ...
          {opportunities.slice(0, 2).map((item, i) => (
            <p
              key={i}
              className="line-clamp-2 text-xs leading-snug text-primary"
            >
              {item}
            </p>
          ))}
...
function JtbdLayout({
  ...
  // Job statement
  const jobStatementField = getSectionFieldFromPath(
    sectionMap,
    "core_job",
    undefined,
    "job_statement",
  ...
  // Forces of progress
  const pushField = getSectionField(
    sectionMap,
    "forces_of_progress",
    "push_forces",
  );
  const pullField = getSectionField(
    sectionMap,
    "forces_of_progress",
    "pull_forces",
  );
  const anxietyField = getSectionField(
    sectionMap,
    "forces_of_progress",
    "anxieties",
  );
  const habitField = getSectionField(
    sectionMap,
    "forces_of_progress",
    "habits_inertia",
  );
  ...
  // Journey matrix
  const jobStepsField = getSectionField(
    sectionMap,
    "job_map",
    "job_steps_summary",
  );
  ...
  const desiredOutcomes = getArrayValue(
    getSectionField(sectionMap, "outcomes", "desired_outcomes"),
  );
  const unmetOutcomes = getArrayValue(
    getSectionField(sectionMap, "outcomes", "unmet_outcomes"),
  );
  const allInsights = dedupeItems([...desiredOutcomes, ...unmetOutcomes]);
  const insightBuckets = distributeItemsByStep(steps, allInsights);

  const oppCandidates = getArrayValue(
    getSectionField(sectionMap, "opportunity_board", "opportunity_candidates"),
  );
  const topOpp = getArrayValue(
    getSectionField(
      sectionMap,
      "opportunity_board",
        ...
        <div className="border-b py-1">
          ...
          {recommendations.map((rec, i) => (
            <RecommendationRow
              key={i}
              priority={
                typeof rec.priority === "string" ? rec.priority : "medium"
              }
              description={
                ...
                const fieldLabel =
                  typeof field.field_label === "string"
                    ...
                    : typeof field.field_key === "string"
                      ...
                      : `Field ${fi + 1}`;
                const val =
                  field.value ?? field.text_value ?? field.text_array_value;
...
export function ConversationLensInsights({
  data,
  isStreaming,
}: ConversationLensInsightsProps) {
  const { analysisData, templateKey, templateName } = data;

  // Subtitle: person name + interview title
  const subtitle = [data.personName, data.interviewTitle]
    .filter(Boolean)
    .join(" \u00b7 ");
