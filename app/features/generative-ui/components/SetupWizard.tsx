...
type LensType = "bant" | "jtbd" | "empathy" | "problem-solution" | "research" | "support"
...
> = {
	bant: {
		...
		extractionFields: ["Budget range", "Decision maker", "Pain point", "Timeline"],
	},
	jtbd: {
		...
		extractionFields: ["Job to be done", "Desired outcome", "Current solution", "Constraints"],
	},
	empathy: {
		name: "Empathy Map",
	...
	"problem-solution": {
		...
		extractionFields: ["Pain intensity", "Current workaround", "Willingness to pay", "Segment"],
	},
	research: {
		...
		extractionFields: ["Research goal", "Research questions", "Hypotheses", "Methodology"],
	},
	support: {
				...
				{WIZARD_STEPS.map((step, index) => (
					<div key={step.key} className="flex flex-1 items-center">
						<div className="flex flex-col items-center">
							...
							>
								{index < currentStep ? <CheckCircle2 className="h-4 w-4" /> : <Circle className="h-4 w-4" />}
							</div>
							<span
								className={cn(
						...
						<Card className="border-primary/20 bg-gradient-to-br from-background to-muted/30">
								...
								<div className="flex items-start justify-between">
									<div className="flex-1">
										<CardTitle className="flex items-center gap-2 text-xl">
											<Sparkles className="h-5 w-5 text-primary" />I recommend {lensConfig.name}
										</CardTitle>
										<CardDescription className="mt-1.5">
											Based on your goal: <span className="font-medium italic">"{userGoal}"</span>
										</CardDescription>
									</div>
							...
							<CardContent className="space-y-4">
								...
								<div className="rounded-lg border bg-background/50 p-4">
									<p className="mb-3 text-muted-foreground text-sm">{lensConfig.description}</p>
									<div className="space-y-2">
										<p className="font-medium text-sm">What I'll extract from your conversations:</p>
								...
								<div className="flex items-center justify-between gap-3">
									<Button variant="outline" size="sm" onClick={() => setShowAllLenses(!showAllLenses)}>
														...
														>
															<div className="flex items-start justify-between">
																<div className="flex-1">
																	...
																	<p className="text-muted-foreground text-xs">{lens.useCase}</p>
																</div>
					...
					>
						<Card>
							<CardHeader>
								...
								<CardDescription>Tell us about your company and research goals</CardDescription>
							</CardHeader>
							<CardContent>
								<p className="text-muted-foreground text-sm">[Context gathering component would go here]</p>
							</CardContent>
						</Card>
					</motion.div>
				)}
