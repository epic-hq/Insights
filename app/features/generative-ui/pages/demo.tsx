...
export default function GenerativeUIDemo() {
	...
	const [scorecardState, setScorecardState] = useState<"empty" | "partial" | "streaming" | "complete">("empty")
		...
		<div className="mx-auto min-h-screen max-w-6xl space-y-6 p-6">
			...
			<div className="space-y-2">
				...
				<p className="text-muted-foreground">
					Demonstrates how agents dynamically render components based on user context and data state
				</p>
			</div>
			...
			<Tabs defaultValue="wizard" className="w-full">
				...
				<TabsContent value="wizard" className="space-y-4">
					<div className="rounded-lg border bg-muted/30 p-4">
						<h2 className="mb-2 font-semibold text-lg">Agent-Driven Lens Selection</h2>
						<p className="mb-4 text-muted-foreground text-sm">
							When user says: <span className="font-medium italic">"I need to qualify enterprise deals"</span>
							<br />
							Agent recommends BANT lens and shows this wizard.
						</p>
						<div className="flex gap-2">
							<Button onClick={() => setWizardStep(0)} variant={wizardStep === 0 ? "default" : "outline"} size="sm">
								Step 1: Lens Selection
							</Button>
							<Button onClick={() => setWizardStep(1)} variant={wizardStep === 1 ? "default" : "outline"} size="sm">
								Step 2: Context
							</Button>
						</div>
					</div>
				...
				<TabsContent value="scorecard" className="space-y-4">
					<div className="rounded-lg border bg-muted/30 p-4">
						<h2 className="mb-2 font-semibold text-lg">Real-Time Evidence Extraction</h2>
						<p className="mb-4 text-muted-foreground text-sm">
							During voice recording or conversation, the scorecard updates in real-time as evidence is extracted.
						</p>
						<div className="flex gap-2">
							<Button onClick={() => setScorecardState("empty")} variant="outline" size="sm">
								Reset (Empty)
							</Button>
							<Button
							...
							>
								{scorecardState === "streaming" ? "Streaming..." : "Simulate Recording"}
							</Button>
							<Button onClick={() => setScorecardState("complete")} variant="outline" size="sm">
								Show Complete
							</Button>
						</div>
					</div>
					...
					<BANTScorecard
						...
						onPlayClip={(timestamp) => {
							...
							alert(
								`Would jump to ${Math.floor(timestamp / 60)}:${Math.floor(timestamp % 60)
									.toString()
									.padStart(2, "0")}`
							)
						}}
				...
				<div className="space-y-4 text-sm">
					<div>
						...
						<p className="text-muted-foreground">
							...
							<br />• Returns <code className="rounded bg-muted px-1 py-0.5">SetupWizard</code> component with props
						</p>
					</div>
					<div>
						...
						<p className="text-muted-foreground">
							...
							<code className="rounded bg-muted px-1 py-0.5">createStreamableUI()</code>
						</p>
					</div>
					<div>
						...
						<p className="text-muted-foreground">
							...
							<br />• 0 deals → <code className="rounded bg-muted px-1 py-0.5">SetupWizard</code>
							<br />• 1-3 deals → <code className="rounded bg-muted px-1 py-0.5">DealCards</code>
							<br />• 10+ deals → <code className="rounded bg-muted px-1 py-0.5">BANTMatrix</code> (aggregated view)
						</p>
					</div>
				</div>
			</div>
