		...
		async (
			...
			try {
				...
				if (isAudioVideo && uploadProjectId) {
					...
					// Step 1: Get presigned upload URL from server (with retry for transient 503s)
		...
		async (files: File[]) => {
			...
			try {
				for (let i = 0; i < files.length; i++) {
					...
					setUploadProgress({
						bytesSent: 0,
						totalBytes: 0, // 0 = show % instead of bytes (counts, not bytes)
					...
					if (!presignedResponse.ok) {
						const errorData = await presignedResponse.json().catch(() => ({}));
						throw new Error(
							errorData.error || `Failed to get upload URL for ${file.name} (${presignedResponse.status})`
						);
					}
					...
					formData.append("sourceType", file.type.startsWith("audio/") ? "audio_upload" : "video_upload");
					...
					if (!startResponse.ok) {
						...
						throw new Error(errorData.error || `Failed to start processing for ${file.name}`);
					}
