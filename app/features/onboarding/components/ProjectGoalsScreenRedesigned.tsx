...
const projectGoalsSchema = z.object({
  ...
  decision_questions: z
    .array(z.string())
    .min(1, "At least one decision question is required"),
...
export default function ProjectGoalsScreen({
  ...
  const [
    _showDecisionQuestionSuggestions,
    _setShowDecisionQuestionSuggestions,
  ] = useState(false);
  const [showDecisionQuestionInput, setShowDecisionQuestionInput] =
    useState(false);
  const [showOrgSuggestions, setShowOrgSuggestions] = useState(false);
  const [showRoleSuggestions, setShowRoleSuggestions] = useState(false);
  const [_showOfferingSuggestions, _setShowOfferingSuggestions] =
    useState(false);
  const [showAssumptionSuggestions, setShowAssumptionSuggestions] =
    useState(false);
  ...
  const [activeSuggestionType, setActiveSuggestionType] = useState<
    string | null
  >(null);
  const [shownSuggestionsByType, setShownSuggestionsByType] = useState<
  ...
  const { isEnabled: isSetupChatEnabled, isLoading: isFeatureFlagLoading } =
    usePostHogFeatureFlag("ffSetupChat");
  ...
  const ensureResearchStructure = useCallback(
    async (projectIdToUse: string | undefined) => {
      ...
      if (!research_goal.trim() || target_roles.length === 0)
        return projectIdToUse;

      setEnsuringStructure(true);
      try {
        const checkResponse = await fetch(
          `/api/check-research-structure?project_id=${projectIdToUse}`,
        );
        ...
        if (!checkResponse.ok) {
          throw new Error(
            checkBody.error ||
              checkBody.details ||
              "Failed to check research structure",
          );
        }

        const summary = (checkBody.summary as Record<string, unknown>) || {};
        const hasExistingStructure = Boolean(
          summary.has_decision_questions && summary.has_research_questions,
        );
        ...
        if (customer_problem.trim())
          formData.append("customer_problem", customer_problem);
        ...
        if (offerings.length > 0)
          formData.append("offerings", offerings.join(", "));
        ...
        if (!generateResponse.ok || !generateBody?.success) {
          throw new Error(
            generateBody?.details ||
              generateBody?.error ||
              "Failed to generate research structure",
          );
        }

        consola.log(
          "[ProjectGoals] Automatically generated research structure",
          {
            projectId: projectIdToUse,
            decisionQuestions:
              generateBody?.structure?.decision_questions?.length ?? 0,
            researchQuestions:
              generateBody?.structure?.research_questions?.length ?? 0,
          },
        );
        return projectIdToUse;
      } catch (error) {
  ...
  const loadProjectData = useCallback(async () => {
    ...
    try {
      ...
      if (ctx?.merged) {
        ...
        const hasAny =
          (typeof m.customer_problem === "string" &&
            (m.customer_problem as string).length > 0) ||
          (Array.isArray(m.target_orgs) &&
            (m.target_orgs as unknown[]).length > 0) ||
          (Array.isArray(m.target_roles) &&
            (m.target_roles as unknown[]).length > 0) ||
          (Array.isArray(m.offerings) &&
            (m.offerings as unknown[]).length > 0) ||
          (typeof m.offerings === "string" &&
            (m.offerings as string).length > 0) ||
          (Array.isArray(m.competitors) &&
            (m.competitors as unknown[]).length > 0) ||
          (typeof m.research_goal === "string" &&
            (m.research_goal as string).length > 0) ||
          (Array.isArray(m.decision_questions) &&
            (m.decision_questions as unknown[]).length > 0) ||
          (Array.isArray(m.assumptions) &&
            (m.assumptions as unknown[]).length > 0) ||
          (Array.isArray(m.unknowns) && (m.unknowns as unknown[]).length > 0) ||
          (typeof m.custom_instructions === "string" &&
            (m.custom_instructions as string).length > 0);
        ...
        if (hasAny) {
          ...
          if (Array.isArray(loadedOfferings)) {
            ...
            typeof loadedOfferings === "string" &&
            loadedOfferings.trim()
          ) {
            setOfferings([loadedOfferings]);
          } else {
            setOfferings([]);
          ...
          const mergedMode =
            (m.research_mode as ResearchMode | undefined) ??
            (m.conversation_type as ResearchMode | undefined);
      ...
      if (!populatedFromContext) {
        const response = await fetch(
          `/api/load-project-goals?projectId=${currentProjectId}`,
        );
        const result = await response.json();
        if (result.success && result.data) {
          ...
          const fallbackMode =
            (data.research_mode as ResearchMode | undefined) ??
            (data.conversation_type as ResearchMode | undefined);
  ...
  useEffect(() => {
    ...
    if (noData) {
      ...
      if (Array.isArray(prefill.offerings)) {
        ...
        typeof prefill.offerings === "string" &&
        prefill.offerings.trim()
      ) {
        setOfferings([prefill.offerings]);
      } else {
        setOfferings([]);
      ...
      if (currentProjectId) {
        if (prefill.customer_problem)
          saveSection("customer_problem", prefill.customer_problem);
        if ((prefill.target_orgs || []).length > 0)
          saveSection("target_orgs", prefill.target_orgs);
        if ((prefill.target_roles || []).length > 0)
          saveSection("target_roles", prefill.target_roles);
        if ((prefill.offerings || []).length > 0)
          saveSection("offerings", prefill.offerings);
        if ((prefill.competitors || []).length > 0)
          saveSection("competitors", prefill.competitors);
        if (prefill.research_goal)
          saveSection("research_goal", prefill.research_goal);
        if ((prefill.assumptions || []).length > 0)
          saveSection("assumptions", prefill.assumptions);
        if ((prefill.unknowns || []).length > 0)
          saveSection("unknowns", prefill.unknowns);
        if (prefill.custom_instructions)
          saveSection("custom_instructions", prefill.custom_instructions);
      }
  ...
  const addDecisionQuestion = async () => {
    if (
      newDecisionQuestion.trim() &&
      !decision_questions.includes(newDecisionQuestion.trim())
    ) {
      await createProjectIfNeeded();
  ...
  const handleNext = useCallback(async () => {
    ...
    if (!validationResult.success) {
      ...
      consola.log(
        "[ProjectGoals] Raw validation errors:",
      ...
      consola.log(
        "[ProjectGoals] About to show toast with message:",
        errorMessage,
      );
    ...
    try {
      const resolvedProjectId =
        (await createProjectIfNeeded()) || currentProjectId;
    ...
    <TooltipProvider>
      <div className="mx-auto min-h-screen max-w-7xl bg-background px-2 py-4 text-foreground sm:p-4 md:p-6 lg:p-8">
        ...
        <div className="mx-auto max-w-4xl space-y-4">
          {/* Website CTA - encourage AI-powered auto-fill */}
          <WebsiteResearchCard
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>
                        Tell us about your business and the customer problem
                        you're addressing.
                      </p>
                    </TooltipContent>
                  </Tooltip>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>
                        Define your research goal - what you're trying to learn
                        or validate.
                      </p>
                    </TooltipContent>
                  </Tooltip>
          ...
          <Collapsible
            ...
            onOpenChange={() =>
              setOpenAccordion(
                openAccordion === "target-market" ? null : "target-market",
              )
            }
          >
            <Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                          <p>
                            Identify the organizations and roles of people you
                            want to interview.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    ...
                    {!showOrgSuggestions ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <ContextualSuggestions
                          ...
                          apiPath={apiPath}
                          shownSuggestions={
                            shownSuggestionsByType.organizations || []
                          }
                          isActive={
                            activeSuggestionType === null ||
                            activeSuggestionType === "organizations"
                          }
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-2">
                          <Textarea
                            ...
                            onKeyDown={(e) =>
                              e.key === "Enter" && !e.shiftKey && addRole()
                            }
                            className="flex-1 resize-none"
                            rows={2}
                            autoFocus
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>
                        Describe your solutions - the products and services that
            ...
            <CardHeader className="cursor-pointer p-4 transition-colors hover:bg-muted/50">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold text-lg">
                    What other products are customers using?
                  </h2>
                  <span className="rounded-md px-2 py-1 font-medium text-foreground/75 text-xs">
                    {competitors.length}
                  </span>
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>
                        List the competitive alternatives or solutions customers
                        might be using or considering.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </div>
          ...
          <Collapsible
            ...
            onOpenChange={() =>
              setOpenAccordion(
                openAccordion === "key-questions" ? null : "key-questions",
              )
            }
          >
            <Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                          <p>
                            These questions will guide your research and help
                            shape the interviews.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex gap-2">
                        <Textarea
                          ...
                          onChange={(e) =>
                            setNewDecisionQuestion(e.target.value)
                          }
                          onKeyDown={(e) =>
                            e.key === "Enter" &&
                            !e.shiftKey &&
                      ...
                      <ContextualSuggestions
                        ...
                        apiPath={apiPath}
                        shownSuggestions={
                          shownSuggestionsByType.decision_questions || []
                        }
                        isActive={
                          activeSuggestionType === null ||
                          activeSuggestionType === "decision_questions"
                        }
                        onSuggestionClick={async (suggestion) => {
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                          <p>
                            Address your riskiest assumptions to gain confidence
                            in your decisions.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                      ...
                      {!showAssumptionSuggestions ? (
                        ...
                        <div className="space-y-3">
                          ...
                          <ContextualSuggestions
                            ...
                            apiPath={apiPath}
                            shownSuggestions={
                              shownSuggestionsByType.assumptions || []
                            }
                            isActive={
                              activeSuggestionType === null ||
                              activeSuggestionType === "assumptions"
                            }
                        ...
                        <div className="space-y-3">
                          <div className="flex gap-2">
                            <Textarea
                              ...
                              onKeyDown={(e) =>
                                e.key === "Enter" && !e.shiftKey && addUnknown()
                              }
                              className="flex-1 resize-none"
                              rows={2}
                              autoFocus
                          ...
                          <ContextualSuggestions
                            ...
                            apiPath={apiPath}
                            shownSuggestions={
                              shownSuggestionsByType.unknowns || []
                            }
                            isActive={
                              activeSuggestionType === null ||
                              activeSuggestionType === "unknowns"
                            }
                            onSuggestionClick={async (suggestion) => {
          ...
          <Collapsible
            ...
            onOpenChange={() =>
              setOpenAccordion(
                openAccordion === "type-scope" ? null : "type-scope",
              )
            }
          >
            <Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                          <p>
                            Set the interview type, duration, and target number
                            to guide question generation.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="rounded-lg border border-muted-foreground/30 border-dashed p-4">
                        ...
                        <ul className="list-disc space-y-2 pl-5 text-muted-foreground text-sm">
                          <li>
                            Exploratory: Use when you're still framing the
                            problem.
                          </li>
                          <li>
                            Validation: Use when testing specific hypotheses.
                          </li>
                          <li>
                            User Testing: Use when refining usability or
                            adoption.
                          </li>
                        </ul>
                      </div>
        ...
        <div className="mb-8">
          ...
          >
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs">
                <p>
                  Add specific instructions for AI analysis or interview
                  generation (optional).
                </p>
              </TooltipContent>
            </Tooltip>
