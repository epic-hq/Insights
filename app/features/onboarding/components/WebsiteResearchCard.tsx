...
export function WebsiteResearchCard({
  onResearchComplete,
}: WebsiteResearchCardProps) {
  const [websiteUrl, setWebsiteUrl] = useState("");
  const [researchResults, setResearchResults] = useState<ResearchData | null>(
    null,
  );
  ...
  if (showResults && researchResults) {
    const targetList =
      researchResults.target_customers || researchResults.target_orgs;
    const hasAnyData =
      ...
      <div className="rounded-lg border bg-muted/30 p-4">
        ...
        {!hasAnyData ? (
          <p className="text-sm text-muted-foreground">
            No useful information could be extracted. Try a different URL or
          ...
          <div className="space-y-2 text-sm">
            ...
            {researchResults.offerings &&
              researchResults.offerings.length > 0 && (
                <div>
                  <span className="font-medium text-muted-foreground">
                    Products/Services:
                  </span>{" "}
                  <span className="text-foreground">
                    {researchResults.offerings.join(", ")}
                  </span>
                </div>
              )}
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Review above. Click "Use Results" to populate fields, or "Discard"
              to ignore.
            </p>
          </div>
        )}
