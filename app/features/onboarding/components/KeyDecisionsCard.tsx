							...
							<div className="flex items-start justify-between gap-2">
								<div>
									...
									<p className="text-muted-foreground text-xs">
										{decision.metrics.answered_answer_count ?? 0} answered · {decision.metrics.open_answer_count ?? 0}{" "}
										open
									</p>
								</div>
								<div className="rounded-full border border-border px-2 py-0.5 text-[11px] text-muted-foreground">
									{decision.research_questions.length} research question
									{decision.research_questions.length === 1 ? "" : "s"}
								</div>
							</div>
							{decision.research_questions.length > 0 && (
								<div className="mt-2 space-y-1">
									{decision.research_questions.slice(0, 2).map((rq) => (
										<div key={rq.id} className="flex items-start justify-between gap-3 text-muted-foreground text-xs">
											<span className="line-clamp-2">{rq.text}</span>
											<span className="whitespace-nowrap">{rq.metrics.answered_answer_count ?? 0} answered</span>
										</div>
									))}
