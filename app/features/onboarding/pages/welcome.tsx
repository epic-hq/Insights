...
import {
  FeatureTour,
  SplashScreen,
} from "~/features/onboarding/components/FeatureTour";
...
export async function loader({ context }: LoaderFunctionArgs) {
  ...
  const onboardingSteps = (userSettings?.onboarding_steps || {}) as Record<
    string,
    boolean
  >;
  ...
  if (tourCompleted) {
    if (
      ...
      return redirect(
        `/a/${userSettings.last_used_account_id}/${userSettings.last_used_project_id}`,
      );
    }
    return redirect("/");
  }
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  const currentSteps = (userSettings?.onboarding_steps || {}) as Record<
    string,
    boolean
  >;

  // Update with tour completed and mark onboarding as done
  const updatedSteps = {
    ...currentSteps,
    [TOUR_STEP_KEY]: true,
    completed: true,
...
export default function WelcomePage() {
  const { showTour, isAuthenticated, redirectTo } =
