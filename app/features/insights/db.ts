...
> => {
	...
	for (const link of links) {
		...
		if (link.created_at >= fourteenDaysAgo) {
			recentCounts.set(link.theme_id, (recentCounts.get(link.theme_id) ?? 0) + 1);
		} else {
...
export const getTopVoicesForThemes = async ({
	...
	projectId: string;
	themeIds: string[];
}): Promise<Map<string, Array<{ id: string; name: string; title: string | null }>>> => {
	const result = new Map<string, Array<{ id: string; name: string; title: string | null }>>();
	...
	const { data: evidencePeople } = await supabase
		.from("evidence_people")
		.select("evidence_id, person_id, people:person_id!inner(id, name, organizations:organization_id(name))")
		.eq("project_id", projectId)
		.in("evidence_id", Array.from(allEvidenceIds));
	...
	const evidenceToPersons = new Map<string, Map<string, { id: string; name: string; title: string | null }>>();
...
export const getStakeholderSummaries = async ({
	...
	const [facetResult, evidencePeopleResult, interviewPeopleResult] = await Promise.all([
		supabase
			.from("evidence_facet")
			.select("evidence_id, person_id")
			.eq("project_id", projectId)
			.not("person_id", "is", null),
		supabase.from("evidence_people").select("evidence_id, person_id").eq("project_id", projectId),
		supabase.from("interview_people").select("interview_id, person_id").in("person_id", personIds),
	]);
	...
	for (let i = 0; i < evidenceIdArray.length; i += BATCH_SIZE) {
		...
		const { data } = await supabase.from("theme_evidence").select("theme_id, evidence_id").in("evidence_id", batch);
	...
	const themeMap = new Map<string, { id: string; name: string; statement: string | null }>();
	...
	for (const person of people) {
		...
		const personName = person.name || [person.firstname, person.lastname].filter(Boolean).join(" ") || "Unknown";
		...
		const repQuote = topThemeId ? (themeMap.get(topThemeId)?.statement ?? null) : null;
