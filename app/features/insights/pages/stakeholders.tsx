...
export default function StakeholdersPage() {
	const { stakeholders, commonGround, sharedConcern, themes } = useLoaderData<typeof loader>();

	const [groupBy, setGroupBy] = useState<"job_function" | "seniority" | "top_theme">("job_function");
	...
	const weakSignals = useMemo<WeakSignal[]>(() => {
		...
		const sorted = [...withSignal].sort((a, b) => b.signal_strength - a.signal_strength);
	...
	const suggestedActions = useMemo<SuggestedAction[]>(() => {
		...
		actions.push({
			...
			description: "AI-detected conflicting positions will surface here in Phase C.",
			confidence: "low",
			cta: "Coming Soon",
		});
		...
		<div className="space-y-6">
			...
			{stakeholders.length > 0 ? (
						...
						<div className="mb-4 flex flex-wrap items-center justify-between gap-3">
							...
							<div className="flex items-center gap-2">
								...
								<ToggleGroup
									type="single"
									value={groupBy}
									onValueChange={(value) => value && setGroupBy(value as "job_function" | "seniority" | "top_theme")}
									size="sm"
								>
									<ToggleGroupItem value="job_function">Function</ToggleGroupItem>
								...
								<div key={role}>
									...
									<div className="grid gap-4 md:grid-cols-2">
										{roleGroups[role].map((s) => (
											<StakeholderCard key={s.person.id} stakeholder={s} highlightId={highlightPersonId} />
										))}
									</div>
								</div>
							))}
				...
				<div className="rounded-lg border p-8 text-center">
					<p className="text-lg text-muted-foreground">No stakeholders yet</p>
					<p className="mt-1 text-muted-foreground text-sm">
						Upload conversations with identified participants to see stakeholder perspectives here.
					</p>
				</div>
			)}
		</div>
...
function getSeniorityBucket(title: string | null | undefined, jobFunction: string | null | undefined): string {
	const source = `${title || ""} ${jobFunction || ""}`.toLowerCase();
	if (!source.trim()) return "Unknown";

	if (
		/\b(ceo|cto|cfo|coo|cio|cmo|chief|founder|co-founder|owner|president|partner|principal|executive)\b/.test(source)
	) {
		return "Executive";
	}
