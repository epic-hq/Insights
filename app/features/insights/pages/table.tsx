...
import {
  type InsightSegmentData,
  InsightsDataTable,
} from "~/features/insights/components/InsightsDataTableTS";
...
function inferJobFunction(title: string): string | null {
  ...
  if (t.includes("finance") || t.includes("accounting") || t.includes("cfo"))
    return "Finance";
  if (t.includes("hr") || t.includes("human resources") || t.includes("people"))
    return "HR";
  if (t.includes("legal") || t.includes("counsel") || t.includes("compliance"))
    return "Legal";
  if (t.includes("data") || t.includes("analytics") || t.includes("bi "))
...
function inferSeniority(title: string): string | null {
  ...
  if (t.includes("founder") || t.includes("co-founder") || t.includes("owner"))
    return "C-Level";
  if (
    t.includes("vp ") ||
  ...
  if (t.includes("manager") || t.includes("lead") || t.includes("principal"))
    return "Manager";
...
export async function loader({ context, params }: LoaderFunctionArgs) {
  ...
  if (insightIds.length > 0) {
    ...
    if (themeEvidence && themeEvidence.length > 0) {
      const evidenceIds = [
        ...new Set(themeEvidence.map((te) => te.evidence_id)),
      ];
      ...
      const personIds = [
        ...new Set(
          (evidencePeopleLinks ?? [])
            .filter((ep) => ep.person_id)
            .map((ep) => ep.person_id as string),
        ),
      ];
      ...
      const evidenceToInsight = new Map(
        themeEvidence.map((te) => [te.evidence_id, te.theme_id]),
      );
      ...
      for (const ep of evidencePeopleLinks ?? []) {
        ...
        const jobFunc =
          person.job_function ||
          (person.title ? inferJobFunction(person.title) : null);
        if (jobFunc) {
          insightSegments[insightId].jobFunction[jobFunc] =
            (insightSegments[insightId].jobFunction[jobFunc] || 0) + 1;
        }
        // Count seniority_level (fall back to title if seniority not set)
        const seniority =
          person.seniority_level ||
          (person.title ? inferSeniority(person.title) : null);
        if (seniority) {
          insightSegments[insightId].seniority[seniority] =
            (insightSegments[insightId].seniority[seniority] || 0) + 1;
        }
  ...
  consola.log(
    `[table] Loaded ${insights.length} insights with segment data for ${Object.keys(insightSegments).length}`,
  );
