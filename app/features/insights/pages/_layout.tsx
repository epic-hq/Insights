...
export default function InsightsLayout() {
  ...
  const totalPeopleInChart = useMemo(
    () => topThemes.reduce((sum, t) => sum + t.person_count, 0),
    [topThemes],
  );
  const pieData = useMemo(
    () =>
      topThemes.map((t) => ({
        ...
        percent:
          totalPeopleInChart > 0
            ? Math.round((t.person_count / totalPeopleInChart) * 100)
            : 0,
        id: t.id,
      })),
  ...
  const isConsolidatingTask =
    consolidateRunId !== null &&
    !consolidateProgress.isComplete &&
    !consolidateProgress.hasError;
  const isConsolidating =
    consolidateFetcher.state !== "idle" || isConsolidatingTask;
                      ...
                      >
                        {pieData.map((entry, index) => (
                          <Cell
                            ...
                            onClick={() =>
                              navigate(routes.insights.detail(entry.id))
                            }
                          />
                        ))}
                        <Label
                      ...
                      return (
                        <g>
                          <rect
                            ...
                            height={height}
                            fill={
                              PIE_COLORS[(index as number) % PIE_COLORS.length]
                            }
                            className="cursor-pointer transition-opacity hover:opacity-80"
                            onClick={() =>
                              navigate(routes.insights.detail(entry.id))
