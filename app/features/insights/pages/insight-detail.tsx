	...
	try {
		...
		if (evidenceIds.length > 0) {
			...
			const interviewIdsForPeople = [
				...new Set(evidence.map((e) => e.interview_id).filter((id): id is string => id !== null)),
			];
...
export default function InsightDetail() {
	const { insight, evidence, projectPath, accountId, peopleAffected } = useLoaderData<typeof loader>();
