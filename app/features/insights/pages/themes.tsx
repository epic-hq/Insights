...
import { getInsights, getTopVoicesForThemes, getTrendingData } from "~/features/insights/db";
import type { SuggestedAction, ThemeWithSignal, WeakSignal } from "~/features/insights/types";
...
export async function loader({ context, params, request }: LoaderFunctionArgs) {
	...
	const filterInterviewId = url.searchParams.get("interview_id") || null;
	...
	const themesWithSignal = insights.map((insight) => {
		const evidenceCount = (insight as { evidence_count?: number }).evidence_count ?? 0;
		const personCount = (insight as { person_count?: number }).person_count ?? 0;
	...
	const sorted = [...themesWithSignal].sort((a, b) => b.signal_strength - a.signal_strength);
	...
	// Optional: filter to themes linked to a specific interview
	...
	if (filterInterviewId) {
		...
		const { data: interviewRow } = await supabase
			.from("interviews")
			.select("title")
			.eq("id", filterInterviewId)
			.maybeSingle();
		filterInterviewTitle = interviewRow?.title || null;
	}
	...
	return {
		...
		filterInterviewId,
		filterInterviewTitle,
	};
}

export default function ThemesPage() {
	const { themes, totalPeople, paging, filterInterviewId, filterInterviewTitle } = useLoaderData<typeof loader>();
	...
	useEffect(() => {
		...
		setNextOffset(typeof data.paging?.nextOffset === "number" ? data.paging.nextOffset : nextOffset);
	...
	const suggestedActions = useMemo<SuggestedAction[]>(() => {
		...
		const sorted = [...allThemes].sort((a, b) => b.signal_strength - a.signal_strength);
		...
		const growing = sorted.find((t) => t.trend === "growing" && t !== sorted[0]);
		if (growing) {
			actions.push({
	...
	return (
		<div className="space-y-6">
			...
			{/* Interview filter badge */}
			{filterInterviewId && (
				<div className="flex items-center gap-2">
					<Badge variant="secondary" className="gap-1.5 py-1 pr-1.5 pl-3">
						<span className="text-xs">Filtered by: {filterInterviewTitle || "Interview"}</span>
						<button
							type="button"
			...
			{allThemes.length > 0 ? (
				...
				<div className="rounded-lg border p-8 text-center">
					<p className="text-lg text-muted-foreground">No themes yet</p>
