...
import { CheckSquareIcon, ChevronDown, ChevronRight, Columns3, Link2 } from "lucide-react"
				...
				<TableHeader>
					<TableRow>
						{table
							...
							.map((column) => (
								<TableHead key={column.id}>{flexRender(column.columnDef.header, { column } as any)}</TableHead>
							))}
					</TableRow>
				</TableHeader>
				<TableBody>
		...
		<div className="space-y-4">
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				...
				<div className="flex items-center gap-2">
					...
					{/* View mode toggle */}
					<div className="flex items-center gap-2">
						<Label htmlFor="view-mode" className="text-muted-foreground text-sm">
							Flat
						</Label>
						<Switch
							id="view-mode"
			...
			{viewMode === "grouped" ? (
				<div className="rounded-md border">
					{grouped.map(({ category, insights: categoryInsights }, idx) => (
						<div key={category}>
							{/* Group header - styled like priorities route */}
							<button
								type="button"
								onClick={() => toggleCategory(category)}
								className="flex w-full items-center gap-2 border-b bg-slate-200 px-4 py-2.5 text-left font-semibold hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700"
							>
								...
								<span className="text-slate-700 text-xs uppercase tracking-wide dark:text-slate-200">
									{category} ({categoryInsights.length})
								</span>
							</button>

							{/* Expanded content - data rows */}
							{expandedCategories.has(category) && (
								<div className="border-b last:border-b-0">{renderCategoryTable(categoryInsights)}</div>
							)}
						</div>
					))}
				</div>
