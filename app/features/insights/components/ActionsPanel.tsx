		...
		<div className="space-y-3">
			{actions.slice(0, 3).map((action, i) => (
				...
				>
					...
					<div className="flex shrink-0 items-center gap-2.5">
						<span
							className={cn(
								"hidden items-center rounded border px-2 py-0.5 font-medium text-[11px] sm:inline-flex",
								CONFIDENCE_BADGE_STYLES[action.confidence] ?? CONFIDENCE_BADGE_STYLES.low
							)}
						>
			...
			<div className="flex items-center justify-between gap-4 rounded-lg border border-dashed bg-card p-3.5 opacity-60">
				<div className="flex items-center gap-3">
					...
					<div>
						<p className="font-semibold text-foreground text-sm">Share Discovery Brief</p>
						<p className="text-muted-foreground text-xs">Compile themes, evidence, and gaps into a shareable view</p>
					</div>
				</div>
				<Button variant="default" size="sm" className="text-xs" disabled>
