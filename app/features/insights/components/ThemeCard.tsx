...
const SIGNAL_BADGE_CONFIG: Record<string, { label: string; trendIcon: string; className: string }> = {
	high: {
		label: "High Signal",
		trendIcon: "",
...
export function ThemeCard({ theme, totalPeople, highlightId }: ThemeCardProps) {
	...
	const truncatedStatement = displayStatement.length > 140 ? `${displayStatement.slice(0, 137)}...` : displayStatement;

	const badge = SIGNAL_BADGE_CONFIG[theme.signal_level] ?? SIGNAL_BADGE_CONFIG.low;
	...
	return (
		...
		>
			<Link to={routes.insights.detail(theme.id)} className="block p-4 no-underline">
				{/* Signal badge */}
				<span
					className={cn(
						"inline-flex items-center gap-1 rounded px-2 py-0.5 font-semibold text-[11px]",
						badge.className
					)}
				>
				...
				{/* Title */}
				<h3 className="mt-2.5 font-semibold text-foreground text-sm leading-snug">{displayName}</h3>
				...
				<div className="mt-3 flex items-center gap-3 text-muted-foreground text-xs">
					<span>
						{theme.person_count} {theme.person_count === 1 ? "person" : "people"}
					</span>
