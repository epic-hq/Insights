...
function toHowMightWe(insight: Insight): string {
  ...
  const outcome =
    insight.desired_outcome || insight.name || insight.statement || "";
      ...
      <DialogContent className="sm:max-w-[550px]">
        <fetcher.Form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Turn this insight into an actionable task. The title uses HMW (How
              Might We) framing to inspire solutions.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            {insight?.name && (
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/30">
                <span className="text-amber-700 text-xs uppercase dark:text-amber-400">
                  From insight
                </span>
                <p className="mt-1 font-medium text-amber-900 text-sm dark:text-amber-100">
                  {insight.name}
                </p>
              </div>
            )}
