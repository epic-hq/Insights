			...
			<CardContent className="p-8">
				...
				<div className="mb-8 space-y-4">
					<div className="space-y-3">
						...
						{insight.statement && (
							<p className="text-lg text-muted-foreground leading-relaxed">{insight.statement}</p>
						)}
					</div>
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						{(insight as any).evidence_count > 0 && (
							<Link to={routes.evidence.index() + `?theme_id=${insight.id}`}>
								<Badge variant="outline" className="gap-1.5 px-3 py-1 hover:bg-accent cursor-pointer">
					...
					{((insight as any).people?.length > 0 || (insight as any).organizations?.length > 0) && (
						<div className="space-y-6 rounded-lg border bg-muted/30 p-6">
							<div className="flex items-center gap-2">
								...
								<h4 className="font-semibold text-foreground text-base">Coverage</h4>
							</div>
										...
										<div className="flex flex-wrap gap-2">
											{(insight as any).organizations.map((org: any, idx: number) => (
												<Link key={idx} to={routes.evidence.index() + `?theme_id=${insight.id}`}>
													<Badge variant="secondary" className="gap-1.5 px-3 py-1 hover:bg-secondary/80 cursor-pointer">
														{org.name}
								...
								{(insight as any).people?.length > 0 && (
									<div className="space-y-3">
										<div className="flex items-center gap-2">
											...
											<div className="font-medium text-sm uppercase tracking-wide text-muted-foreground">People</div>
										</div>
										<div className="flex flex-wrap gap-2">
