          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {insight.insight_tags?.map((tag) => {
                ...
                const frequency =
                  typeof tag.frequency === "number" ? tag.frequency : undefined;
                return (
                  <StyledTag
                    key={tagName}
                    name={tagName}
          ...
          <div className="space-y-6 rounded-lg border bg-muted/30 p-6">
            {(insight as any).organizations?.length > 0 && (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {(insight as any).organizations.map(
                    (org: any, idx: number) => (
                      <Link
                        key={idx}
                      ...
                      >
                        <Badge
                          variant="secondary"
                          className="cursor-pointer gap-1.5 px-3 py-1 hover:bg-secondary/80"
                        >
                          {org.name}
            ...
            {(insight as any).people?.length > 0 && (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {(insight as any).people
                    .slice(0, 10)
                    .map((person: any, idx: number) => (
                      <Link key={idx} to={routes.people.detail(person.id)}>
                        <Badge
                          variant="outline"
                          className="cursor-pointer gap-1.5 px-3 py-1 hover:bg-accent"
                        >
                          {person.name}
                          {person.role && (
                            <span className="text-[10px] opacity-70">
