...
export function BreadthGrid({ covered, total, level, size = "default" }: BreadthGridProps) {
		...
		<div className="flex items-center gap-2">
			<div className={cn("flex flex-wrap", gap)}>
				{Array.from({ length: total }, (_, i) => (
					<div key={i} className={cn("rounded-sm", squareSize, i < covered ? LEVEL_COLORS[level] : "bg-muted/30")} />
				))}
			</div>
