...
export async function loader({ context, params, request }: LoaderFunctionArgs) {
  ...
  let gmailConnection: LoaderData["gmailConnection"] = null;
  if (userId && accountId) {
    ...
    if (researchFetcher.state === "idle" && researchFetcher.data) {
      ...
      if (result.success && result.data) {
        ...
        const updatedContext: CompanyContext = {
          ...
          company_description:
            result.data?.description || localCompanyContext.company_description,
          customer_problem:
            result.data?.customer_problem ||
            localCompanyContext.customer_problem,
          offerings: result.data?.offerings || localCompanyContext.offerings,
          target_orgs:
            result.data?.target_orgs || localCompanyContext.target_orgs,
          target_roles:
            result.data?.target_roles || localCompanyContext.target_roles,
          competitors:
            result.data?.competitors || localCompanyContext.competitors,
      ...
      <div className="space-y-6">
        ...
        <Card className="border-border/60">
          <CardHeader>
            ...
            <CardDescription>
              The name displayed in the sidebar and account switcher.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex gap-3">
              ...
              <Button
                ...
                disabled={
                  !accountNameHasChanges ||
                  accountNameFetcher.state === "submitting"
                }
              >
                {accountNameFetcher.state === "submitting"
                  ? "Saving..."
