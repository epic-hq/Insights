...
export async function getEnhancedProjectContext(
  ...
  try {
    ...
    const [accountResult, themesResult, personasResult, lensesResult] =
      await Promise.all([
        // Account context
        project?.account_id
          ? supabase
    ...
    if (personasResult.data) {
      for (const p of personasResult.data) {
        personas.push({
          ...
          frustrations: Array.isArray(p.frustrations)
            ? p.frustrations.filter(Boolean)
            : [],
    ...
    return {
      ...
      target_orgs:
        joinArray(meta.target_orgs) || joinArray(account?.target_orgs),
      target_roles:
        joinArray(meta.target_roles) || joinArray(account?.target_roles),
      research_goal:
        (meta.research_goal as string) ||
        (meta.customer_problem as string) ||
        "",
      ...
      customer_problem:
        account?.customer_problem ?? (meta.customer_problem as string) ?? null,
 ...
 */
export function buildEnhancedCustomInstructions(
  ...
  if (context.themes.length > 0) {
    const themesList = context.themes
      .map((t) => `- ${t.name}${t.statement ? `: ${t.statement}` : ""}`)
      .join("\n");
    parts.push(
  ...
  if (context.personas.length > 0) {
    const personasList = context.personas
      .map((p) => {
        ...
        if (p.goals.length)
          details.push(`Goals: ${p.goals.slice(0, 3).join("; ")}`);
        if (p.pains.length)
          details.push(`Pains: ${p.pains.slice(0, 3).join("; ")}`);
        return `- ${p.name}${p.percentage ? ` (${p.percentage}%)` : ""}: ${details.join(". ")}`;
      })
      .join("\n");
    parts.push(
      `KNOWN PERSONAS (tailor questions to their specific perspectives):\n${personasList}`,
    );
  }
  ...
  if (context.lenses.length > 0) {
    const lensesList = context.lenses
      .map(
        (l) =>
          `- ${l.templateName}: ${l.primaryObjective || "No objective specified"}`,
      )
      .join("\n");
    parts.push(
      `ACTIVE ANALYSIS FRAMEWORKS (ensure questions help fill these frameworks):\n${lensesList}`,
    );
  }
