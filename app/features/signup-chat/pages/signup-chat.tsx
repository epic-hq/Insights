...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (action === "save_chat_data") {
    const chatData = JSON.parse(
      formData.get("chatData") as string,
    ) as AgentState;
    ...
    <div className="flex min-h-screen flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <main className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="h-full min-h-0 w-full flex-1 p-0">
          ...
          <ChatWithChecklist
            existingChatData={onboardingData}
            messages={loaderMessages}
          />
...
function _ModernChatInterface({
  existingChatData,
}: {
  existingChatData?: any;
}) {
      ...
      {messages.map((message) => {
        switch (message.role) {
          case "user":
            return (
              <div key={message.id}>
                {message.parts.map((part) => part).join("")}
              </div>
            );
          case "assistant":
            return (
...
function ChatWithChecklist({
  ...
  const { messages: copilotMessages, setMessages } =
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-1 flex-col">
      <div
        className={cn(
          "h-full min-h-0 w-full flex-1 rounded-lg bg-white shadow-xl dark:bg-gray-800",
        )}
      >
        <CopilotChat
