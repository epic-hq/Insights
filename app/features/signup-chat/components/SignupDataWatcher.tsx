...
import { CheckCircle2, XCircle, User, Target, AlertTriangle, ListChecks, MessageSquareText, ChevronDown } from "lucide-react"
...
export function SignupDataWatcher({
	...
	const normalized = useMemo(() => {
		...
		return {
			...
			problem:
				typeof d.problem === "string" ? d.problem.trim() || undefined : d.problem?.toString?.() ?? undefined,
			...
			<div>
				...
				<div className="fixed left-2 right-2 top-2 z-50 md:hidden">
					...
					>
						<div className="flex min-w-0 items-center gap-3">
							<h3 className="truncate text-sm font-medium text-neutral-700 dark:text-neutral-200">{title}</h3>
							<span
								className={
									...
									(normalized.completed
										...
										: "bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-200 dark:bg-amber-900/30 dark:text-amber-300 dark:ring-amber-800")
								}
							>
					...
					<div
						className={
							...
							(isOpen ? "mt-2 max-h-[80dvh] opacity-100" : "max-h-0 opacity-0")
						}
					>
						<div className="overflow-hidden rounded-xl border border-neutral-200/60 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
							<div className="grid gap-5 p-4 grid-cols-1 @xl:grid-cols-2">
								{/* Name */}
									...
									<div className="flex flex-wrap gap-2">
										{normalized.challenges.length ? (
											normalized.challenges.map((c, i) => (
												<span
													key={`${c}-${i}`}
													className="inline-flex items-center rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-700 ring-1 ring-inset ring-neutral-200 dark:bg-neutral-800 dark:text-neutral-200 dark:ring-neutral-700"
												>
													{c}
												</span>
											))
				...
				<div className="hidden md:block">
					<div className="rounded-xl border border-neutral-200/60 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
						{/* Header */}
						<div className="flex items-center justify-between gap-3 border-b border-neutral-200/60 px-4 py-3 dark:border-neutral-800">
