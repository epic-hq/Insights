...
export const meta: MetaFunction = () => [
  { title: "Dashboard | UpSight Customer Intelligence" },
];
...
export async function loader({ request, context, params }: LoaderFunctionArgs) {
  ...
  // Always redirect to the journey map — it's the primary project home
  const { redirect } = await import("react-router");
  throw redirect(`/a/${accountId}/${projectId}/journey`);
  ...
  const [
    ...
    supabase
      .from("interviews")
      .select(
        "id, status, participant_pseudonym, source_type, interview_type, media_type",
      )
  ...
  const templateMap = new Map(
    lensTemplates.map((t) => [
      t.template_key,
      { name: t.template_name, category: t.category || "general" },
    ]),
  );
  ...
  const lensAnalysesByTemplate = new Map<
    string,
  ...
  const researchGoalSection = projectSections.find(
    (s) => s.kind === "research_goal",
  );
  ...
  const activityFeedItems: LensActivityItem[] = recentLensActivity.map(
    (analysis) => {
      const template = templateMap.get(analysis.template_key);
      const analysisData = analysis.analysis_data as Record<
        string,
        unknown
      > | null;
      ...
      if (analysisData) {
        ...
        if (typeof analysisData.executive_summary === "string") {
          ...
          typeof analysisData.sections === "object"
        ) {
      ...
      const interviewTitle =
        interviewData?.participant_pseudonym || "Untitled Conversation";
