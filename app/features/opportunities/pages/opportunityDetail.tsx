...
export async function loader({ params, context }: LoaderFunctionArgs) {
  ...
  if (!accountId || !projectId || !opportunityId) {
    throw new Response(
      "Account ID, Project ID, and Opportunity ID are required",
      { status: 400 },
    );
  }
...
export default function OpportunityDetail() {
  ...
  const handleOpportunityUpdate = (field: string, value: string) => {
    if (!currentProjectContext?.accountId || !currentProjectContext?.projectId)
      return;
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div className="flex-1">
          ...
          <div className="mt-3 flex items-center gap-2">
            <Badge
              variant="outline"
              className={getKanbanStatusColor(
          ...
          <CardContent>
            ...
            >
              <InlineEdit
                ...
                placeholder="$0"
                onSubmit={(value) =>
                  handleOpportunityUpdate("amount", value.replace(/[$,]/g, ""))
                }
                submitOnBlur
          ...
          <CardContent>
            ...
            >
              <InlineEdit
                ...
                onSubmit={(value) =>
                  handleOpportunityUpdate("close_date", value)
                }
                submitOnBlur
                type="date"
                textClassName="font-bold text-foreground text-2xl"
          ...
          <Card>
            ...
            <CardContent>
              <Select
                ...
                onValueChange={(value) =>
                  handleOpportunityUpdate("stage", value)
                }
              >
        ...
        <CardContent>
          ...
          >
            <InlineEdit
              ...
              onSubmit={(value) =>
                handleOpportunityUpdate("description", value)
              }
              submitOnBlur
              multiline
        ...
        <CardContent>
          ...
          >
            <InlineEdit
              ...
              onSubmit={(value) =>
                handleOpportunityUpdate("product_description", value)
              }
              submitOnBlur
              multiline
      ...
      <Card className="mt-8">
        ...
        <CardContent>
          {salesLensData?.linkedInterviews &&
          salesLensData.linkedInterviews.length > 0 ? (
...
function StakeholderMatrix({
  ...
  useEffect(() => {
    ...
    stakeholderFetcher.state,
    stakeholderFetcher.data,
    revalidator.revalidate,
  ]);
  ...
  useEffect(() => {
    ...
    addStakeholderFetcher.state,
    addStakeholderFetcher.data,
    revalidator.revalidate,
  ]);
  ...
  const getStakeholderType = (stakeholder: (typeof stakeholders)[0]) => {
    ...
    if (
      stakeholder.labels.some(
        (l) =>
          l.toLowerCase().includes("influencer") ||
          l.toLowerCase().includes("champion"),
      )
    ) {
      return "I";
    }
    ...
    if (
      stakeholder.labels.some(
        (l) =>
          ...
          l.toLowerCase().includes("objection"),
      )
    ) {
      return "B";
    }
    ...
    <>
      <div className="mb-4 flex justify-end">
        <Button
          ...
          disabled={addStakeholderFetcher.state === "submitting"}
          size="sm"
        >
          {addStakeholderFetcher.state === "submitting"
            ? "Adding..."
            : "Add Stakeholder"}
        </Button>
      </div>
          ...
          <TableBody>
            {stakeholders.map((stakeholder) => {
              const stakeholderSteps =
                nextStepsByOwner.get(stakeholder.displayName) || [];
                  ...
                  <TableCell>
                    ...
                    >
                      <InlineEdit
                        ...
                        onSubmit={(value) =>
                          handleStakeholderUpdate(stakeholder.id, "role", value)
                        }
                        submitOnBlur
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Select a person from your CRM or company contacts to add as a
              stakeholder.
            </DialogDescription>
          </DialogHeader>
          <div className="max-h-60 space-y-2 overflow-y-auto">
            {peopleFetcher.data?.people?.length > 0 ? (
              peopleFetcher.data.people.map(
                (person: {
                  id: string;
                  display_name?: string;
                  name?: string;
...
function AIAdvisorSection({
  ...
  const isGenerating =
    advisorFetcher.state === "submitting" || advisorFetcher.state === "loading";
      ...
      <CardHeader className={latestRecommendation ? "" : "pb-4"}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className="bg-card text-indigo-600 text-xs"
            >
              AI
            </Badge>
          </div>
        ...
        {hasError && (
          <p className="mt-2 text-red-600 text-sm">
            Error:{" "}
            {(advisorFetcher.data as { error?: string })?.error ||
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-lg border-2 border-border bg-card p-4">
              <div className="mb-2 flex items-center gap-2">
                ...
                <ConfidenceBarChart
                  level={latestRecommendation.confidence}
                  variant="bars"
                  size="sm"
                />
              </div>
              <p className="text-foreground">
                {latestRecommendation.status_assessment}
              </p>
            </div>

            {/* Recommendations */}
            {latestRecommendation.recommendations &&
              latestRecommendation.recommendations.length > 0 && (
                <div className="rounded-lg border-2 border-border bg-card p-4">
            ...
            {/* Risks */}
            {latestRecommendation.risks &&
              latestRecommendation.risks.length > 0 && (
                <div className="rounded-lg border-2 border-border bg-card p-4">
                  <div className="mb-3 flex items-center gap-2">
                  ...
                  <div className="space-y-2">
                    {latestRecommendation.risks.map(
