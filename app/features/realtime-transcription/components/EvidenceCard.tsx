...
export function EvidenceCard({
  ...
  const speakerDisplay =
    speakerNames[speakerKey] || evidence.speaker_label || speakerKey;
  ...
  if (compact && !expanded) {
    return (
      <div
        className={cn(
          "group flex cursor-pointer items-start gap-2 rounded-md border-l-2 py-2 pl-3 pr-2 transition-colors hover:bg-muted/50",
  ...
  return (
    <div
      className={cn(
        "rounded-md border-l-2 py-2 pl-3 pr-2 transition-all",
        evidence.isQuestion ? "border-l-blue-400" : "border-l-emerald-400",
        isNew && "slide-in-from-right-4 fade-in animate-in duration-500",
      )}
    >
      ...
      {evidence.verbatim && (
        <p className="mt-2 border-l border-muted-foreground/30 pl-2 text-muted-foreground text-xs italic">
          "{evidence.verbatim}"
        </p>
      )}
    </div>
