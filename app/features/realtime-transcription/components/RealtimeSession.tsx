...
export function RealtimeSession({
    ...
    try {
      ...
      if (data.people?.length) {
        setPeople((prev) => {
          ...
          const fresh = data.people.filter(
            (p: Person) => !existingKeys.has(p.person_key),
          );
          return [...prev, ...fresh];
        });
      }
  ...
  const addTurn = useCallback(
    (turn: TranscriptTurn) => {
      ...
      const newTurnsSinceExtract =
        allTurnsRef.current.length - lastExtractedCountRef.current;
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      const ws = new WebSocket(
        `${protocol}://${location.host}/ws/realtime-transcribe`,
      );
      wsRef.current = ws;
          ...
          if (msg.type === "Turn") {
            if (msg.end_of_turn) {
              const turn: TranscriptTurn = {
                ...
                speaker: "SPEAKER A",
                text: msg.transcript,
                startMs: msg.words?.[0]?.start
                  ? msg.words[0].start * 1000
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <h1 className="font-semibold text-lg">
                Realtime Transcription & Evidence
              </h1>
            </div>
              ...
              <div className="space-y-2">
                ...
                {recommendations.length > 5 && (
                  <p className="text-muted-foreground text-xs">
                    +{recommendations.length - 5} more insights
                  </p>
                )}
              </div>
            </div>
                ...
                <div className="space-y-2 py-8 text-center">
                  {extractionStatus === "processing" ? (
                    ...
                    <>
                      ...
                      <p className="text-muted-foreground text-sm">
                        Evidence will appear here as key moments are identified
