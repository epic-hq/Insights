...
export async function loader({ context, params }: LoaderFunctionArgs) {
	...
	// Get ICP score distribution with people names
	const { data: icpScores } = await supabase
		.from("person_scale")
		.select("band, score, confidence, person_id, people(name, title, company)")
	...
	// Build scored people list for UI
	const icpScoredPeople = (icpScores || []).map((s) => ({
		name: (s.people as any)?.name || "Unknown",
		title: (s.people as any)?.title || null,
		company: (s.people as any)?.company || null,
