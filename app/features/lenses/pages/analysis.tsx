 ...
 *
 * Replaces the old Library page (template management) with an insight-first
 * experience organized into three tabs:
 ...
 *
 * Template management is moved to a settings dialog (gear icon).
 */
...
import { ManageLensesDialog } from "../components/ManageLensesDialog"
...
export const meta: MetaFunction = () => {
	return [
		...
		{ name: "description", content: "Unified analysis across all conversation lenses" },
	]
}
...
export default function AnalysisPage() {
	...
	const totalAnalyses = overview.lensStats.reduce((sum, ls) => sum + ls.completedCount, 0)
				...
				<div className="space-y-1">
					...
					<p className="text-muted-foreground">
						{overview.interviewCount > 0 ? (
							<>
								<span className="font-medium text-foreground">{overview.interviewCount}</span>{" "}
								conversation{overview.interviewCount !== 1 ? "s" : ""}
								{overview.surveyResponseCount > 0 && (
									<>
										{" "}
										&middot;{" "}
										<span className="font-medium text-foreground">{overview.surveyResponseCount}</span>{" "}
										survey response{overview.surveyResponseCount !== 1 ? "s" : ""}
									</>
								)}
								{" "}&middot;{" "}
								<span className="font-medium text-foreground">{overview.peopleCount}</span>{" "}
								people
								{" "}&middot;{" "}
								<span className="font-medium text-foreground">{activeLensCount}</span>{" "}
								lens{activeLensCount !== 1 ? "es" : ""} active
							</>
						) : (
