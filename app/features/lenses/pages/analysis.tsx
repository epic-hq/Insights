...
export async function loader({ context, params }: LoaderFunctionArgs) {
  ...
  for (const ep of evidencePeople || []) {
    evidenceCountByPerson.set(
      ep.person_id,
      (evidenceCountByPerson.get(ep.person_id) || 0) + 1,
    );
  }
  ...
  const peopleWithCompany = pageData.people.filter(
...
export async function action({ context, params, request }: ActionFunctionArgs) {
  ...
  if (intent === "synthesize-cross-lens") {
    ...
    try {
      const result = await tasks.trigger<typeof synthesizeCrossLensTask>(
        "lens.synthesize-cross-lens",
        {
          projectId,
          accountId,
      ...
      consola.info(
        `[analysis] Triggered cross-lens synthesis, run ID: ${result.id}`,
      );
      return { success: true, runId: result.id };
    } catch (error) {
      consola.error(
        "[analysis] Failed to trigger cross-lens synthesis:",
        error,
      );
      return {
        success: false,
  ...
  if (intent === "update_lens_settings") {
    const enabledLensesJson =
      (formData.get("enabled_lenses") as string) || "[]";
    ...
    if (!previousLenses || previousLenses.length === 0) {
      ...
      const metadata = (acctSettings?.metadata ||
      ...
      if (newlyEnabled.length > 0) {
        ...
        if (interviews && interviews.length > 0) {
          try {
            ...
            consola.info(
              `[analysis] Triggered backfill for ${interviews.length} interviews`,
            );
          } catch (err) {
...
export default function AnalysisPage() {
  ...
  const handleSynthesizeCrossLens = (force: boolean) => {
    fetcher.submit(
      { intent: "synthesize-cross-lens", force: force.toString() },
      { method: "post" },
    );
  ...
  const activeLensCount = overview.lensStats.filter(
    (ls) => ls.completedCount > 0,
  ).length;
      ...
      >
        ...
        <TabsContent value="manage" className="mt-6">
          <ManageLensesTab
            ...
            userId={userId}
            onCreateLens={() =>
              setSearchParams({ tab: "manage", create: "true" })
            }
          />
        </TabsContent>
      </Tabs>
