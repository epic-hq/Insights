...
import {
  type LensTemplate,
  loadLensTemplates,
} from "../lib/loadLensAnalyses.server";
...
export async function loader({ context, params }: LoaderFunctionArgs) {
  ...
  if (accountId) {
    ...
    if (accountSettings?.metadata) {
      ...
      if (
        Array.isArray(metadata.default_lens_keys) &&
        metadata.default_lens_keys.length > 0
      ) {
  ...
  if (projectId) {
    ...
    if (project?.project_settings) {
      ...
      if (
        Array.isArray(settings.enabled_lenses) &&
        settings.enabled_lenses.length > 0
      ) {
...
export async function action({ request, context, params }: ActionFunctionArgs) {
  ...
  if (!previousLenses || previousLenses.length === 0) {
    ...
    if (accountId) {
      ...
      const metadata = (accountSettings?.metadata ||
        {}) as AccountSettingsMetadata;
  ...
  if (applyToExisting && project?.account_id) {
    const newlyEnabled = enabledLenses.filter(
      (lens) => !previousLenses.includes(lens),
    );
      ...
      if (interviews && interviews.length > 0) {
        try {
          const { applyAllLensesTask } =
            await import("~/../src/trigger/lens/applyAllLenses");
          ...
          consola.info(
            `[LensSettings] Triggered backfill for ${interviews.length} interviews`,
          );
        } catch (err) {
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        ...
        <CardHeader className="pb-2">
          <div className="flex items-start justify-between gap-3">
            ...
            <DialogTrigger asChild>
              ...
              >
                <div
                  className={`rounded-lg p-2.5 ${colors.iconBg} ${colors.text}`}
                >
      ...
      <DialogContent className="max-h-[80vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isCustom && isOwner && onUpdateField ? (
              <InlineEdit
                ...
                onSubmit={(value) =>
                  onUpdateField(template.template_key, "template_name", value)
                }
              />
            ) : (
              template.template_name
          ...
          {isCustom && isOwner && onUpdateField ? (
            <InlineEdit
              ...
              onSubmit={(value) =>
                onUpdateField(template.template_key, "summary", value)
              }
            />
          ) : (
        ...
        <div className="mt-4 space-y-6">
          ...
          {/* Entities */}
          {template.template_definition.entities &&
            template.template_definition.entities.length > 0 && (
              <div>
                <h4 className="mb-3 font-semibold text-muted-foreground text-sm uppercase tracking-wide">
                  Entities Extracted
                </h4>
...
export default function LensLibrary() {
  const { templates, enabledLenses, projectId, accountId, userId } =
  ...
  const handleDelete = (templateKey: string) => {
    if (
      !accountId ||
      !confirm("Delete this custom lens? This cannot be undone.")
    )
      return;
  ...
  const enabledCount = currentEnabledLenses.filter((key) =>
    ...
    <div className="container max-w-6xl py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-lg text-muted-foreground">
          Lenses automatically extract structured insights from your
          conversations. Enable lenses to populate feeds, power dashboards, and
      ...
      <div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {systemTemplates.map(renderLensCard)}
        </div>
      </div>
