	...
	const filteredData = useMemo(() => {
		...
		const filterByInterviews = <T extends { interview_id?: string; interviews?: Array<{ id: string }> }>(items: T[]): T[] => {
			return items.filter((item) => {
				...
				if (item.interviews) {
					return item.interviews.some((int) => filteredInterviewIds.has(int.id))
				}
				return true
			})
		}
	...
	return (
		<div className="space-y-6 p-6">
					...
					<CardContent>
						<div className="grid gap-4 md:grid-cols-2">
							{filteredData.opportunity_fields.map((field) => (
								<FieldCard key={field.field_key} field={field} onSelect={setSelectedField} projectPath={projectPath} />
							))}
						</div>
					</CardContent>
				</Card>
			...
			{filteredData.next_steps.length > 0 && (
				<div id="next-steps">
					<NextStepsSection nextSteps={filteredData.next_steps} projectPath={projectPath} />
				</div>
			)}

			{/* Hygiene Gaps - only shows orgs with flagged gaps */}
			<HygieneGapsSection gaps={filteredData.hygiene_gaps} projectPath={projectPath} />
		...
		<div className="flex items-start justify-between gap-4">
			<div>
				...
				<p className="text-muted-foreground">
					Aggregated qualification data across {totalConversations || 0} conversation
			...
			<CardContent>
				<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
					{sortedFields.map((field) => (
						<FieldCard key={field.field_key} field={field} onSelect={onSelect} projectPath={projectPath} />
					))}
				</div>
			</CardContent>
		</Card>
 ...
 */
function summarizeDealSizes(values: Array<{ value: string; organization_name: string | null }>): string {
	const parsed: number[] = []
	const orgCount = new Set(values.map((v) => v.organization_name).filter(Boolean)).size
	...
	if (values.length > 0) {
		...
		const uniqueValues = [...new Set(values.map((v) => v.value.trim()).filter(Boolean))]
...
function FieldCard({
	...
	for (const v of field.values) {
		if (v.organization_name && !orgsMap.has(v.organization_name)) {
			orgsMap.set(v.organization_name, { id: v.organization_id, name: v.organization_name })
		}
	}
	...
	const getCoverageText = () => {
		...
		if (totalOrgs === 0) return `${valueCount} response${valueCount !== 1 ? "s" : ""}`
	...
	return (
		...
		>
			...
			<div className={cn("mb-1 pr-12 text-xs", isDealSize ? "font-semibold text-foreground" : "text-muted-foreground")}>
				{getCoverageText()}
			</div>
			{orgList.length > 0 && (
			...
			<CardContent>
				<div className="space-y-2">
					{stakeholders.slice(0, 8).map((stakeholder, i) => {
						...
						const orgName = stakeholder.interviews.find((int) => int.organization_name)?.organization_name
			...
			<CardContent>
				<div className="space-y-2">
					{objections.slice(0, 8).map((objection, i) => {
						const orgName = objection.interviews.find((int) => int.organization_name)?.organization_name
						return (
							<button
								key={i}
...
const GAP_CODE_LABELS: Record<string, { label: string; description: string }> = {
	missing_budget: { label: "Budget", description: "No budget or pricing discussion captured" },
	missing_authority: { label: "Authority", description: "Decision maker not identified" },
	missing_need: { label: "Need", description: "Core pain/need not clearly articulated" },
	missing_timeline: { label: "Timeline", description: "No timeline or urgency mentioned" },
	missing_deal_size: { label: "Deal Size", description: "No deal value or contract size mentioned" },
	no_champion: { label: "Champion", description: "No internal advocate identified" },
...
function HygieneGapsSection({
	...
	const orgGapMap = new Map<string, { id: string | null; name: string; gapCodes: Set<string> }>()
	...
	const activeGapCodes = [...new Set(gaps.map((g) => g.code))].filter((code) => GAP_CODE_LABELS[code])
					...
					<table className="w-full text-sm">
						<thead>
							<tr className="border-b">
								<th className="py-2 pr-4 text-left font-medium">Organization</th>
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>Suggested actions based on analysis - create tasks to track follow-ups</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="space-y-2">
	...
	return (
		<div className="flex items-start gap-3 rounded-lg border bg-card p-3">
			...
			<fetcher.Form method="post">
				...
				<input type="hidden" name="title" value={rec.description.slice(0, 100)} />
				<input type="hidden" name="description" value={rec.description} />
				<input type="hidden" name="sourceInterview" value={rec.interview_title} />
				<input type="hidden" name="sourceOrg" value={rec.organization_name || ""} />
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					{interviews.length} conversation{interviews.length !== 1 ? "s" : ""} with Sales BANT analysis
				</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="space-y-2">
				...
				{interviews.length > 5 && (
					<Button variant="ghost" onClick={onToggle} className="mt-3 w-full">
						{expanded ? (
							...
							<>
								Show All {interviews.length} Conversations <ChevronDown className="ml-1 h-4 w-4" />
							</>
						)}
					</Button>
				)}
								...
								>
									<div className="min-w-0 flex-1">
										...
										{int.organization_name && (
											<div className="text-muted-foreground text-xs">{int.organization_name}</div>
										)}
									</div>
