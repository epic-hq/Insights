...
export async function loadAnalysisPageData(
	...
	// We'd need another query for this, but let's keep it simple for now
				...
				for (const section of data.sections || []) {
					for (const field of section.fields || []) {
						if (field.value && typeof field.value === "string" && field.value.length > 0) {
							...
							fields.push({ key: fieldKey, name: fieldName, value: field.value })
						}
					}
				}
	...
	// Sort people by interview count (most active first), then by name
	people.sort((a, b) => {
		if (b.interviewCount !== a.interviewCount) return b.interviewCount - a.interviewCount
