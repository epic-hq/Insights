...
export async function loadAnalysisPageData(
  ...
  const [
    ...
    db
      .from("people")
      .select(
        "id, firstname, lastname, name, title, company, image_url, created_at",
      )
      .eq("project_id", projectId)
    ...
    db
      .from("evidence")
      .select(
        "id, research_link_response_id, gist, verbatim, pains, gains, feels",
      )
      .eq("project_id", projectId)
  ...
  const templates: LensTemplate[] = (templatesResult.data || []).map((t) => ({
    ...
    template_definition:
      t.template_definition as LensTemplate["template_definition"],
  ...
  const analysesByInterview = new Map<
    string,
    Array<{ templateKey: string; data: any }>
  >();
  for (const a of analysesResult.data || []) {
    analysesByTemplate.set(
      a.template_key,
      (analysesByTemplate.get(a.template_key) || 0) + 1,
    );
  ...
  for (const sr of surveyResponsesByPersonResult.data || []) {
    if (sr.person_id) {
      surveyCountByPerson.set(
        sr.person_id,
        (surveyCountByPerson.get(sr.person_id) || 0) + 1,
      );
  ...
  const people: PersonAnalysisSummary[] = (peopleResult.data || []).map((p) => {
    ...
    const personSurveyResponses =
      surveyResponseIdsByPerson.get(p.id) || new Set();
...
function deduplicateHighlights(
  highlights: PersonLensHighlight[],
): PersonLensHighlight[] {
