...
export async function generatePainMatrix(opts: {
  ...
  consola.info(
    `[generatePainMatrix] Starting for project ${projectId}${forceRefresh ? " (forced refresh)" : ""}`,
  );
  ...
  if (!forceRefresh) {
    ...
    if (cached) {
      ...
      const deltaPercent =
        cached.evidence_count > 0
          ? Math.abs(evidenceDelta) / cached.evidence_count
          : 0;
  ...
  // 2. Get all pain facets with person_id directly from evidence_facet table
  // OPTIMIZED: Uses evidence_facet.person_id for direct person attribution (no junction table needed)
  // NOTE: We ONLY use evidence_facet table (kind_slug='pain') because the old pains[] column
  // contained corrupted data in "when X blocked by..." format. The evidence_facet table has clean data.

  const { data: painFacets, error: facetError } = await supabase
    .from("evidence_facet")
    .select(
      `
      id,
  ...
  // Group facets by evidence_id and collect person_ids directly
  ...
  for (const facet of painFacets || []) {
    const ev = facet.evidence as any;
    const evidenceId = facet.evidence_id;
    ...
    if (facet.label) entry.pain_labels.push(facet.label);
    if (facet.person_id && !entry.person_ids.includes(facet.person_id)) {
      entry.person_ids.push(facet.person_id);
    }
  }

  // Normalize to common structure (one row per pain label for clustering)
  const evidenceWithPains: Array<{
    id: string;
    verbatim: string;
    confidence: string | null;
    pain_label: string;
  ...
  for (const [_, entry] of evidenceMap) {
    // Create one row per unique pain label (for clustering)
    for (const painLabel of [...new Set(entry.pain_labels)]) {
      evidenceWithPains.push({
        id: entry.id,
        ...
        priority: entry.confidence || "medium",
  ...
  consola.info(
    `[generatePainMatrix] Found ${evidenceWithPains.length} pain facets from ${evidenceMap.size} evidence items (using person_id directly)`,
  );

  // No need for separate evidence_people query - person_ids come directly from evidence_facet.person_id
  const evidenceWithPeopleIds = evidenceWithPains;
  const evidenceWithPersonLinks = evidenceWithPeopleIds.filter(
    (e) => e.person_ids.length > 0,
  ).length;

  consola.info(
    `[generatePainMatrix] ${evidenceWithPersonLinks}/${evidenceWithPeopleIds.length} evidence items have person_id links`,
  );
  ...
  consola.info(
    `[generatePainMatrix] Clustered into ${painThemes.length} pain themes using semantic embeddings`,
  );
  ...
  const filteredPainThemes = painThemes.filter((p) =>
    painThemesInCells.has(p.id),
  );
  ...
  const highImpactCells = cells.filter(
    (c) => c.metrics.impact_score >= 1.0,
  ).length;

  const orphanedPains = painThemes.length - filteredPainThemes.length;
  if (orphanedPains > 0) {
    consola.warn(
      `[generatePainMatrix] Filtered out ${orphanedPains} pain themes with no user group mapping`,
    );
  }
  ...
  if (accountId) {
    try {
      consola.info(
        `[generatePainMatrix] Saving cache for account ${accountId}, project ${projectId}`,
      );
      ...
      if (error) {
        ...
        consola.success(
          `[generatePainMatrix] Saved matrix to cache (id: ${data[0]?.id})`,
        );
      } else {
        consola.warn(
          "[generatePainMatrix] Cache save returned no data or error",
        );
      }
    } catch (err) {
    ...
    consola.warn(
      "[generatePainMatrix] No accountId provided, skipping cache save",
    );
  }
...
async function clusterPainsBySemantic(opts: {
  ...
  if (validPainLabels.length === 0) {
    consola.warn(
      "[clusterPainsBySemantic] No pain labels passed minEvidence threshold",
    );
    return [];
  }
  ...
  if (error) {
    consola.error(
      "[clusterPainsBySemantic] Error fetching pain facets:",
      error,
    );
  ...
  if (!painFacets || painFacets.length === 0) {
    consola.warn(
      "[clusterPainsBySemantic] No embeddings available, falling back to label clustering",
    );
    return clusterPainsByLabel(evidenceWithPains, minEvidencePerPain);
  }
  ...
  if (clusterError) {
    consola.error(
      "[clusterPainsBySemantic] Error finding clusters:",
      clusterError,
    );
    return clusterPainsByLabel(evidenceWithPains, minEvidencePerPain);
  }
  ...
  for (const [root] of clusterGroups) {
    ...
    const representative = labelsInCluster.reduce((a, b) =>
      (painLabelMap.get(a)?.length || 0) > (painLabelMap.get(b)?.length || 0)
        ? a
        : b,
    );
...
async function buildMatrixCell(opts: {
  ...
  const relevantEvidence = evidenceWithPains.filter((ev) => {
    ...
    const hasUserInGroup = ev.person_ids?.some((personId: string) =>
      userGroup.member_ids.includes(personId),
    );
  ...
  const uniquePersonIds = [...new Set(allPersonIds)].filter((id) =>
    userGroup.member_ids.includes(id),
  );
  const personCount = uniquePersonIds.length;

  // Calculate frequency: what % of this user group mentions this pain?
  const frequency =
    userGroup.member_count > 0 ? personCount / userGroup.member_count : 0;
  ...
  const wtpSignals = relevantEvidence
    .map((e) => e.willingness_to_pay)
    .filter(Boolean);
