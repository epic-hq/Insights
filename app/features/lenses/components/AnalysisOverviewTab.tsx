...
function FindingCard({ finding, totalPeople }: { finding: CrossLensFinding; totalPeople: number }) {
	...
	return (
		<div className="flex gap-3 rounded-lg border p-4 transition-colors hover:bg-muted/30">
			<div className={`mt-1 h-2.5 w-2.5 flex-shrink-0 rounded-full ${getSeverityDot(finding.severity)}`} />
			<div className="min-w-0 flex-1 space-y-1">
				...
				<p className="text-foreground/70 text-sm leading-relaxed">{finding.description}</p>
				<div className="flex items-center gap-3 pt-1">
					...
					{finding.supportingLenses.length > 0 && (
						<span className="text-foreground/50 text-xs">via {finding.supportingLenses.join(", ")}</span>
					)}
				</div>
			</div>
		</div>
			...
			<div className="space-y-8">
				<Card className="border-primary/20 bg-primary/5">
					<CardContent className="flex items-center gap-4 py-8">
						...
						<div>
							<h3 className="font-semibold text-lg">Generating Analysis Overview</h3>
							<p className="text-muted-foreground">
								Synthesizing {crossLensSynthesis.analysisCount} analyses across all lenses...
							</p>
						</div>
					</CardContent>
				</Card>
			...
			<div className="space-y-8">
				<Alert variant="destructive">
					...
					<AlertDescription className="flex items-center justify-between">
						...
						<Button variant="outline" size="sm" onClick={() => onSynthesize(true)} disabled={isSubmitting}>
		...
		<div className="space-y-8">
			...
			<div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
				<div className="rounded-lg border bg-card p-3 text-center">
					<div className="font-semibold text-2xl text-foreground">{peopleCount}</div>
				...
				<div className="rounded-lg border bg-card p-3 text-center">
					<div className="font-semibold text-2xl text-foreground">{interviewCount}</div>
					<div className="flex items-center justify-center gap-1 text-muted-foreground text-xs">
				...
				<div className="rounded-lg border bg-card p-3 text-center">
					<div className="font-semibold text-2xl text-foreground">{surveyResponseCount}</div>
					<div className="flex items-center justify-center gap-1 text-muted-foreground text-xs">
				...
				<CardHeader className="pb-3">
					...
					{isStale && (
						<p className="text-amber-600 text-sm">
							{totalAnalyses - crossLensSynthesis.analysisCount} new analysis(es) since last synthesis.
						</p>
					)}
				</CardHeader>
						...
						<div className="space-y-3">
							...
							<div className="space-y-2">
								{crossLensSynthesis.keyFindings.map((finding, i) => (
									<FindingCard key={i} finding={finding} totalPeople={peopleCount} />
								))}
							</div>
						</div>
					)}
			...
			{crossLensSynthesis.processedAt && (
				<p className="text-muted-foreground text-xs">
					Last synthesized {new Date(crossLensSynthesis.processedAt).toLocaleDateString()} at{" "}
					{new Date(crossLensSynthesis.processedAt).toLocaleTimeString()}
					{crossLensSynthesis.overallConfidence && (
						<> &middot; {Math.round(crossLensSynthesis.overallConfidence * 100)}% confidence</>
					)}
				</p>
			)}
		</div>
