...
import { COMPANY_SIZE_RANGES, INDUSTRIES, JOB_FUNCTIONS, SENIORITY_LEVELS } from "~/lib/constants/options";
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="flex flex-col sm:max-w-md">
				<SheetHeader>
					...
					<SheetDescription>Edit ICP-relevant fields for better scoring accuracy</SheetDescription>
				</SheetHeader>
				...
				<ScrollArea className="flex-1 py-4">
					<div className="space-y-5 px-1">
						...
						<div className="space-y-2">
							...
							/>
							<p className="text-muted-foreground text-xs">Used for ICP role matching (weight: 40%)</p>
						</div>

						<div className="space-y-2">
							<Label>Job Function</Label>
							<Select value={jobFunction} onValueChange={(v) => setJobFunction(v === "__clear__" ? "" : v)}>
								<SelectTrigger>
						...
						<div className="space-y-2">
							...
							<Select value={seniority} onValueChange={(v) => setSeniority(v === "__clear__" ? "" : v)}>
								<SelectTrigger>
						...
						<div className="space-y-2">
							...
							<Popover open={orgPopoverOpen} onOpenChange={setOrgPopoverOpen}>
								<PopoverTrigger asChild>
									<Button
										variant="outline"
										className={cn("w-full justify-start font-normal", !selectedOrgName && "text-muted-foreground")}
									>
										...
										<CommandList>
											...
											<CommandGroup>
												{filteredOrgs.map((org) => (
													...
													>
														<Check className={cn("h-4 w-4", selectedOrgId === org.id ? "opacity-100" : "opacity-0")} />
														{org.name || "Unnamed"}
													</CommandItem>
												))}
											</CommandGroup>
											{orgSearch && !filteredOrgs.some((o) => o.name?.toLowerCase() === orgSearch.toLowerCase()) && (
												<CommandGroup>
													<CommandItem onSelect={handleCreateOrg} className="flex items-center gap-2">
														<Plus className="h-4 w-4" />
														Create "{orgSearch}"
													</CommandItem>
												</CommandGroup>
							...
							<p className="text-muted-foreground text-xs">Used for ICP organization matching (weight: 30%)</p>
						</div>

						<div className="space-y-2">
							<Label>Industry</Label>
							<Select value={industry} onValueChange={(v) => setIndustry(v === "__clear__" ? "" : v)}>
								<SelectTrigger>
							...
							<p className="text-muted-foreground text-xs">Organization industry for ICP org matching</p>
						</div>

						<div className="space-y-2">
							<Label>Company Size</Label>
							<Select value={sizeRange} onValueChange={(v) => setSizeRange(v === "__clear__" ? "" : v)}>
								<SelectTrigger>
							...
							<p className="text-muted-foreground text-xs">Used for ICP size matching (weight: 30%)</p>
						</div>
					</div>
				</ScrollArea>
