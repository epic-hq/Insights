...
export function LensSelector({
  ...
  useEffect(() => {
    if (
      ...
      fetcher.data?.ok &&
      fetcher.data?.taskId &&
      fetcher.data?.publicAccessToken
    ) {
    ...
    <div className={cn("flex flex-wrap items-center gap-3", className)}>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button
          ...
          disabled={!selectedLens || isProcessing}
          size="sm"
        >
      ...
      {isProcessing && (
        <Badge variant="outline" className="bg-blue-50 text-blue-700">
          ...
          {progressInfo.label ||
            (pendingCount > 0 ? `${pendingCount} processing` : "Processing...")}
        </Badge>
      )}
...
export function LensSelectorCompact({
  ...
  const unappliedLenses = templates.filter(
    (t) =>
      !analyses[t.template_key] || analyses[t.template_key].status === "failed",
  );
