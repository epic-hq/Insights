...
export async function getTopConcernsForSegment(opts: {
  ...
  consola.info(
    `[getTopConcernsForSegment] Finding top concerns for ${segmentPersonIds.length} people in segment`,
  );
  ...
  if (facetError) {
    consola.error(
      "[getTopConcernsForSegment] Error fetching facets:",
      facetError,
    );
    throw facetError;
  }
  ...
  if (linkError) {
    consola.error(
      "[getTopConcernsForSegment] Error fetching theme links:",
      linkError,
    );
    throw linkError;
  }
...
export async function getUsersWithThemes(opts: {
  ...
  consola.info(
    `[getUsersWithThemes] Finding users with ${themeIds.length} themes (requireAll: ${requireAll})`,
  );
  ...
  if (linkError) {
    consola.error(
      "[getUsersWithThemes] Error fetching theme links:",
      linkError,
    );
    throw linkError;
  }
  ...
  consola.success(
    `[getUsersWithThemes] Found ${results.length} people matching theme criteria`,
  );
...
export async function getOrphanedThemes(opts: {
  ...
  consola.info(
    `[getOrphanedThemes] Found ${orphaned.length} themes with 0 evidence`,
  );
...
export async function deleteOrphanedThemes(opts: {
  ...
  consola.success(
    `[deleteOrphanedThemes] Deleted ${orphanedIds.length} themes with 0 evidence`,
  );
