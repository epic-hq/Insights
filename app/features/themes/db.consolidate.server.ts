...
export async function consolidateExistingThemes({
	...
	const { data: duplicatePairs, error: dupError } = await supabase.rpc("find_duplicate_themes", {
	...
	onProgress?.("clustering", 20, `Found ${pairs.length} duplicate pairs, building clusters...`);
	...
	onProgress?.("merging", 40, `Processing ${clusters.length} theme clusters...`);

	// Step 4: Process each cluster
	let processedClusters = 0;
	for (const cluster of clusters) {
		processedClusters++;
		const progress = 40 + Math.round((processedClusters / clusters.length) * 50);
		onProgress?.("merging", progress, `Merging cluster ${processedClusters}/${clusters.length}...`);
		...
		const totalEvidence = themesInCluster.reduce((sum, t) => sum + t.evidenceCount, 0);
		if (totalEvidence === 0) {
			consola.debug(`[consolidate] Cluster has 0 total evidence, deleting ${duplicateThemes.length} duplicates`);
		}
		...
		for (const duplicate of duplicateThemes) {
			...
			if (moveError) {
				consola.error(`[consolidate] Error moving evidence from ${duplicate.id}:`, moveError);
			...
			const { error: deleteError } = await supabase.from("themes").delete().eq("id", duplicate.id);

			if (deleteError) {
				consola.error(`[consolidate] Error deleting theme ${duplicate.id}:`, deleteError);
	...
	onProgress?.("complete", 100, `Consolidated ${result.themesDeleted} duplicate themes`);
	consola.success(
...
function buildClusters(pairs: DuplicateThemePair[]): string[][] {
	...
	for (const pair of pairs) {
		if (!parent.has(pair.theme_id_1)) parent.set(pair.theme_id_1, pair.theme_id_1);
		if (!parent.has(pair.theme_id_2)) parent.set(pair.theme_id_2, pair.theme_id_2);
	}
...
async function moveEvidenceLinks(
	...
	const { data: existingLinks } = await supabase.from("theme_evidence").select("evidence_id").eq("theme_id", toThemeId);

	const existingEvidenceIds = new Set((existingLinks || []).map((l) => l.evidence_id));

	// Filter to only new links (not already linked to target)
	const newLinks = sourceLinks.filter((link) => !existingEvidenceIds.has(link.evidence_id));
	...
	const { error: deleteError } = await supabase.from("theme_evidence").delete().eq("theme_id", fromThemeId);
