...
export async function loader({ context, params }: LoaderFunctionArgs) {
  ...
  console.log(
    `[ThemesIndex] Loaded ${themes?.length || 0} themes, error:`,
    tErr,
  );
  if (themes?.length > 0) {
  ...
  console.log(
    `[ThemesIndex] Loaded ${insights?.length || 0} insights from getInsights, error:`,
    iErr,
  );
  ...
  if (epErr)
    throw new Error(`Failed to load evidence_people: ${epErr.message}`);
  ...
  if (personIdSet.size) {
    ...
    if (ppErr)
      throw new Error(`Failed to load people_personas: ${ppErr.message}`);
  ...
  const evidenceById = new Map<
    string,
    { personas: string[]; interview_id?: string }
  >();
  ...
  for (const persona of personas) {
    ...
    for (const theme of enriched) {
      ...
      const coverage =
        totalInterviews > 0 ? personaInterviews.size / totalInterviews : 0;
  ...
  const hasConsolidated = Boolean(
    (project?.project_settings as { insights_consolidated_at?: string } | null)
      ?.insights_consolidated_at,
  );
...
export default function ThemesIndex() {
  ...
  const visibleInsights = useMemo(() => {
    ...
    console.log(
      `[ThemesIndex Client] Filtered ${insights.length} insights -> ${filtered.length} visible`,
    );
  ...
  const filteredCardInsights = useMemo(() => {
    ...
    return visibleInsights.filter(
      (insight: Insight & { [key: string]: any }) => {
        const haystack = [
          insight.name,
          insight.statement,
          insight.inclusion_criteria,
    ...
    return (
      <PageContainer className="flex min-h-[60vh] flex-col items-center justify-center space-y-4 text-center">
        ...
        <p className="max-w-md text-foreground/70 text-sm">
          Add themes or connect evidence to personas to unlock persona coverage
          insights.
        </p>
      </PageContainer>
    );
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1.5">
          ...
          <p className="max-w-2xl text-foreground/70 text-sm">
            Review insights, vote or comment to drive alignment and prioritize
            actions.
          </p>
        </div>
