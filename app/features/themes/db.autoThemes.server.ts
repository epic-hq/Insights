...
async function findSimilarEvidenceForTheme(
  ...
  const embedding = await generateEmbeddingWithBillingOrThrow(
    billingCtx,
    themeText,
    {
...
export async function autoGroupThemesAndApply(
  ...
  if (project_id && created_theme_ids.length > 0) {
    try {
      ...
      if (deleted_count > 0) {
        consola.warn(
          `[autoGroupThemesAndApply] Deleted ${deleted_count} orphaned themes with 0 evidence`,
        );
        ...
        const finalThemeIds = created_theme_ids.filter(
      ...
      consola.error(
        "[autoGroupThemesAndApply] Failed to clean up orphaned themes:",
        cleanupErr,
      );
