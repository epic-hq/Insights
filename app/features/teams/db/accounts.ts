...
export const getAccounts = async ({
  ...
  const teamAccounts = data?.filter(
    (account) => account?.personal_account === false,
  );
  const personalAccount = data?.filter(
    (account) => account?.personal_account === true,
  )[0];
...
export const getAccount = async ({
  supabase,
  account_id,
}: { supabase: SupabaseClient } & GetAccountRequest) => {
...
export const leaveAccount = async ({
  supabase,
  account_id,
}: { supabase: SupabaseClient } & LeaveAccountRequest) => {
