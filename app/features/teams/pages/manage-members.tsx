...
export async function loader({ request, params }: LoaderFunctionArgs) {
	...
	// All team members can see pending invitations
	// Graceful fallback if the migration hasn't been applied yet (old SQL only allows owners)
	const { data: invitationsData, error: invitationsError } = await dbGetAccountInvitations({
		supabase: client,
		account_id: accountId,
	});
	if (invitationsError) {
		consola.warn("[MANAGE TEAM] Could not fetch invitations", {
			error: invitationsError.message,
		});
	}
	invitations = invitationsError ? [] : ((invitationsData as GetAccountInvitesResponse | null) ?? []);
...
export default function ManageTeamMembers() {
	...
	const handleResendInvitation = useCallback(
		async (email: string, role: string) => {
			...
			submit({ intent: "resendInvite", email, role }, { method: "post", replace: true });
	...
	return (
		<PageContainer size="sm" padded={false} className="container max-w-3xl py-6">
			...
			<TeamInvite
				...
				pendingInvitations={localInvitations.map((inv) => ({
					invitation_id: inv.invitation_id,
					email: inv.email ?? null,
					account_role: inv.account_role,
				...
				onResendInvitation={canManage ? handleResendInvitation : undefined}
				onRevokeInvitation={
					canManage
						? (invitationId) => {
								const inv = localInvitations.find((i) => i.invitation_id === invitationId);
