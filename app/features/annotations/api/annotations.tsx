...
export const loader: LoaderFunction = async ({ context, request, params }) => {
  ...
  const accountId =
    ctx_project.accountId ?? params?.accountId ?? ctx.account_id;
  const projectId = ctx_project.projectId ?? params?.projectId ?? null;

  if (!accountId || !projectId) {
    return Response.json(
      { error: { message: "Missing account or project context" } },
      { status: 400 },
    );
  }
  ...
  const annotationType = url.searchParams.get("annotationType") as
    | AnnotationType
    | undefined;
  ...
  if (!entityType || !entityId) {
    return Response.json(
      { error: { message: "Missing entityType or entityId" } },
      { status: 400 },
    );
  }
...
export const action: ActionFunction = async ({ context, request, params }) => {
  ...
  if (!accountId || !projectId || !userId) {
    return Response.json(
      { error: { message: "Missing authentication context" } },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    switch (action) {
      case "add-comment": {
        ...
        const contentJsonbStr = formData.get("contentJsonb") as
          | string
          | undefined;
        ...
        try {
          ...
          consola.warn(
            "[ANNOTATION_CREATED] PostHog tracking failed:",
            trackingError,
          );
        }
      ...
      case "update-annotation": {
        ...
        try {
          ...
          return Response.json(
            { error: { message: "Invalid updates JSON" } },
            { status: 400 },
          );
        }
      ...
      case "delete-annotation": {
        ...
        if (!annotationId) {
          return Response.json(
            { error: { message: "Missing annotation ID" } },
            { status: 400 },
          );
        }
