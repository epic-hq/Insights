		...
		<div className="space-y-6">
			{columns.map((col) => (
				<div key={col.title} className="space-y-3">
					{/* Column title outside the card */}
					<div className="flex items-center justify-between">
						<h3 className="font-semibold text-gray-900 text-sm dark:text-gray-100">{col.title}</h3>
						<span className="rounded-full bg-gray-100 px-2 py-0.5 text-gray-600 text-xs dark:bg-gray-800 dark:text-gray-400">
							{col.items.length}
						</span>
					</div>

					{/* Column card without internal header */}
					<div
						className={`rounded-lg border bg-white dark:bg-gray-900 ${dragOverColumn === col.title ? "ring-2 ring-blue-400" : ""}`}
						onDragOver={(e) => handleDragOverColumn(col.title, e)}
						onDrop={handleDragEnd}
					>
						<div
							className="min-h-[120px] flex-1 space-y-2 overflow-auto p-4"
							onDragOver={(e) => handleDragOverColumn(col.title, e)}
						>
							{col.items.map((item) => (
								<div
							...
							{col.items.length === 0 && (
								<div className="flex h-full min-h-[60px] items-center justify-center rounded-lg border-2 border-dashed p-4 text-gray-400 text-sm italic">
									Drop items here
								</div>
							)}
						</div>
