		...
		<div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-6 py-10">
			...
			<div className="flex items-center justify-between">
				<div className="space-y-1">
					...
					<p className="text-muted-foreground">Platform-wide usage metrics and trends</p>
				</div>
			...
			<div className="grid gap-4 md:grid-cols-4">
				<Card>
					...
					<CardContent>
						<div className="font-bold text-2xl">{totals.events.toLocaleString()}</div>
						<p className="text-muted-foreground text-xs">LLM API calls</p>
					</CardContent>
				</Card>
				<Card>
					<CardHeader className="flex flex-row items-center justify-between pb-2">
						<CardTitle className="font-medium text-sm">Active Accounts</CardTitle>
						<Users className="h-4 w-4 text-muted-foreground" />
					</CardHeader>
					<CardContent>
						<div className="font-bold text-2xl">{totals.accounts}</div>
						<p className="text-muted-foreground text-xs">With usage in period</p>
					</CardContent>
				</Card>
				<Card>
					<CardHeader className="flex flex-row items-center justify-between pb-2">
				...
				<Card>
					...
					<CardContent>
						<div className="font-bold text-2xl">{totals.credits.toLocaleString()}</div>
						<p className="text-muted-foreground text-xs">Internal credits consumed</p>
					</CardContent>
				</Card>
			</div>
			...
			<div className="grid gap-4 md:grid-cols-4">
				<Card>
					...
					<CardContent>
						<div className="font-bold text-2xl">${costMetrics.perDay.toFixed(4)}</div>
						<p className="text-muted-foreground text-xs">Average daily LLM spend</p>
					</CardContent>
				</Card>
				<Card>
					<CardHeader className="flex flex-row items-center justify-between pb-2">
					...
					<CardContent>
						<div className="font-bold text-2xl">${costMetrics.perWeek.toFixed(2)}</div>
						<p className="text-muted-foreground text-xs">Average weekly LLM spend</p>
					</CardContent>
				</Card>
				<Card>
					<CardHeader className="flex flex-row items-center justify-between pb-2">
						<CardTitle className="font-medium text-sm">Avg Cost/Month</CardTitle>
						<DollarSign className="h-4 w-4 text-muted-foreground" />
					</CardHeader>
					<CardContent>
						<div className="font-bold text-2xl">${costMetrics.perMonth.toFixed(2)}</div>
						<p className="text-muted-foreground text-xs">Average monthly LLM spend</p>
					</CardContent>
				</Card>
				<Card>
					<CardHeader className="flex flex-row items-center justify-between pb-2">
					...
					<CardContent>
						<div className="font-bold text-2xl">${costMetrics.perUser.toFixed(4)}</div>
						<p className="text-muted-foreground text-xs">Average cost per active user</p>
					</CardContent>
				</Card>
			</div>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Daily credits by feature (stacked bar chart)</CardDescription>
				</CardHeader>
				<CardContent>
					{chartData.length > 0 ? (
						<ResponsiveContainer width="100%" height={280}>
							<BarChart data={chartData}>
								...
								<Tooltip
									...
									formatter={(value: number, name: string) => [value, formatFeatureSource(name)]}
								/>
								<Legend formatter={(value) => formatFeatureSource(value)} wrapperStyle={{ fontSize: "12px" }} />
								{featureSources.length > 0 ? (
									...
									<Bar dataKey="credits" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} name="Credits" />
								)}
							</BarChart>
						</ResponsiveContainer>
					) : (
			...
			<div className="grid gap-6 lg:grid-cols-2">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>Top accounts by credit consumption</CardDescription>
					</CardHeader>
					<CardContent>
								...
								<TableBody>
									{accountUsage.slice(0, 10).map((account) => (
										<TableRow key={account.account_id}>
											...
											<TableCell className="text-right">{(account.event_count ?? 0).toLocaleString()}</TableCell>
											<TableCell className="text-right">
												<Badge variant="secondary">{(account.total_credits ?? 0).toLocaleString()}</Badge>
											</TableCell>
											<TableCell className="text-right">${(account.total_cost_usd ?? 0).toFixed(4)}</TableCell>
										</TableRow>
									))}
								</TableBody>
							</Table>
						) : (
							<div className="py-8 text-center text-muted-foreground">No account usage data</div>
						)}
					</CardContent>
				</Card>
				...
				<Card>
					...
					<CardContent>
						{featureUsage.length > 0 ? (
								...
								<TableBody>
									{featureUsage.map((feature) => (
										<TableRow key={feature.feature_source}>
											<TableCell className="font-medium">{formatFeatureSource(feature.feature_source)}</TableCell>
											<TableCell className="text-right">{(feature.event_count ?? 0).toLocaleString()}</TableCell>
											<TableCell className="text-right">{(feature.total_tokens ?? 0).toLocaleString()}</TableCell>
											<TableCell className="text-right">
												<Badge variant="secondary">{(feature.total_credits ?? 0).toLocaleString()}</Badge>
											</TableCell>
										</TableRow>
									))}
								</TableBody>
							...
							<div className="py-8 text-center text-muted-foreground">No feature usage data</div>
						)}
					</CardContent>
				</Card>
			</div>
			...
			<Card>
				...
				<CardContent>
					{userUsage.length > 0 ? (
							...
							<TableBody>
								{userUsage.slice(0, 20).map((user) => (
									<TableRow key={user.user_id}>
										...
										<TableCell className="text-right">{(user.event_count ?? 0).toLocaleString()}</TableCell>
										<TableCell className="text-right">{(user.total_tokens ?? 0).toLocaleString()}</TableCell>
										<TableCell className="text-right">${(user.total_cost_usd ?? 0).toFixed(4)}</TableCell>
										<TableCell className="text-right">
											<Badge variant="secondary">{(user.total_credits ?? 0).toLocaleString()}</Badge>
										</TableCell>
									</TableRow>
								))}
							</TableBody>
						...
						<div className="py-8 text-center text-muted-foreground">No user usage data</div>
					)}
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Last 5 logins with their recent feature usage</CardDescription>
				</CardHeader>
				<CardContent>
					{recentLoginActivity.length > 0 ? (
						<div className="space-y-6">
							{recentLoginActivity.map((activity) => (
								<div key={activity.user_id} className="rounded-lg border bg-card p-4">
									<div className="mb-3 flex items-center justify-between">
										...
										<div className="text-right">
											...
											<p className="font-medium text-sm">{new Date(activity.last_sign_in_at).toLocaleString()}</p>
										</div>
									</div>
									{activity.recent_features.length > 0 ? (
										<div className="mt-3 border-t pt-3">
											<p className="mb-2 text-muted-foreground text-xs uppercase tracking-wide">Recent Features Used</p>
											<div className="flex flex-wrap gap-2">
												{activity.recent_features.map((feature, idx) => (
													<Badge key={`${activity.user_id}-${idx}`} variant="outline" className="text-xs">
														{formatFeatureSource(feature.feature)}
														<span className="ml-1 text-muted-foreground">({feature.credits} cr)</span>
													</Badge>
												))}
											</div>
										</div>
									) : (
										<p className="mt-3 text-muted-foreground text-sm">No recent feature usage</p>
									)}
								</div>
							))}
						</div>
					) : (
						<div className="py-8 text-center text-muted-foreground">No recent login activity</div>
					)}
				</CardContent>
			</Card>
		</div>
