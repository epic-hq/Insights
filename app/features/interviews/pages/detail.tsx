        ...
        <div className="w-full space-y-6">
          ...
          <div className="mb-6 space-y-3">
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem
                        onClick={() => {
                          try {
                            ...
                            consola.error(
                              "Reprocess evidence submit failed",
                              e,
                            );
                          }
                        }}
                      ...
                      <DropdownMenuItem
                        onClick={() => {
                          try {
                            ...
                            consola.error(
                              "Generate sales lens submit failed",
                              e,
                            );
                          }
                        }}
                      ...
                      {linkedOpportunity ? (
                        <DropdownMenuItem asChild>
                          <Link
                            to={routes.opportunities.detail(
                              linkedOpportunity.id,
                            )}
            ...
            {(() => {
              ...
              const hasUnlinkedParticipants = participants.some(
                (p) => !p.people?.id,
              );
              ...
              const needsAttention =
                ...
                (hasUnlinkedParticipants || hasPlaceholderNames);
              ...
              const linkedParticipants = participants.filter((p) => {
                ...
                return (
                  person?.id &&
                  person?.name &&
                  !/^(Participant|Interviewer)\s*\d*$/i.test(person.name)
                );
              });
            ...
            <div className="flex flex-wrap items-center gap-4 text-muted-foreground text-sm">
              ...
              {interview.duration_sec && (
                <>
                  <span>•</span>
                  <span>
                    {Math.floor(interview.duration_sec / 60)}m{" "}
                    {interview.duration_sec % 60}s
                  </span>
                </>
              )}
              {evidence.length > 0 && (
                ...
                <div className="mb-4 space-y-3 rounded-lg border border-muted/60 bg-muted/40 p-4">
                  ...
                  <ul className="space-y-3">
                    {aiKeyTakeaways.map((takeaway, index) => {
                      const badgeStyles = badgeStylesForPriority(
                        takeaway.priority,
                      );
                      return (
                        <li
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <p className="mb-3 text-muted-foreground text-xs">
                            Optionally provide custom instructions to guide the
                            AI's analysis
                          </p>
                        </div>
                        <Textarea
              ...
              <InlineEdit
                ...
                onSubmit={(value) => {
                  try {
                    ...
                    consola.error(
                      "❌ Failed to update observations_and_notes:",
                      error,
                    );
                  }
                }}
          ...
          {evidence.length > 0 && (
            <PlayByPlayTimeline evidence={evidence} className="mb-8" />
          )}
