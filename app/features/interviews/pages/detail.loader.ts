...
import {
  type EvidenceRow,
  processEmpathyMap,
} from "../lib/processEmpathyMap.server";
  ...
  try {
    ...
    try {
      const { data: participantData, error: participantError } =
        await getInterviewParticipants({
          supabase,
          projectId,
    ...
    if (linkedTaskError) {
      consola.warn(
        "Could not fetch linked tasks for interview:",
        linkedTaskError.message,
      );
    }
    ...
    const linkedTasks = (
      ...
      .filter(
        (task, idx, arr) =>
          arr.findIndex((candidate) => candidate.id === task.id) === idx,
      );
    ...
    if (conversationAnalysis?.keyTakeaways && evidence?.length) {
      matchTakeawaysToEvidence(
        ...
        evidenceWithFacets as EvidenceRecord[],
      );
    }
    ...
    if (
      ...
      interviewData.status === "ready"
    ) {
      const ext = (interviewData.file_extension ?? "").toLowerCase();
      const videoExts = ["mp4", "mov", "avi", "mkv", "webm", "m4v"];
      const isVideo =
        videoExts.includes(ext) ||
        videoExts.some((e) =>
          interviewData.media_url?.toLowerCase().includes(`.${e}`),
        );
      if (isVideo) {
        ...
        import("@trigger.dev/sdk")
          .then(({ tasks }) =>
            tasks.trigger("generate-thumbnail", {
              mediaKey: interviewData.media_url!,
              interviewId: interviewData.id,
              timestampSec: 1,
              accountId,
            }),
          ...
          .then(() =>
            consola.info(
              "[detail.loader] Auto-triggered thumbnail for",
              interviewId,
            ),
          )
          .catch((err) =>
            consola.warn(
              "[detail.loader] Failed to auto-trigger thumbnail:",
              err,
            ),
          );
      }
