...
export const meta: MetaFunction = () => {
  return [
    { title: "New Interview | Insights" },
    { name: "description", content: "Create a new interview" },
  ];
};
...
export async function action({ request, params, context }: ActionFunctionArgs) {
  ...
  // Check AI analysis limit before creating
  if (ctx.claims?.sub) {
    const gateCtx = await buildFeatureGateContext(accountId, ctx.claims.sub);
    const limitCheck = await checkLimitAccess(gateCtx, "ai_analyses");
    if (!limitCheck.allowed) {
      return {
        error: `You've reached your plan's limit of ${limitCheck.limit} AI analyses this month. Upgrade to create more.`,
      };
    }
  }
  ...
  try {
    ...
    return redirect(
      `/a/${accountId}/projects/${projectId}/interviews/${data.id}`,
    );
  } catch (_error) {
    return { error: "Failed to create interview" };
  }
...
export default function NewInterview() {
  ...
  const [newPeople, setNewPeople] = useState<
    Array<{ name: string; segment: string; tempId: string }>
  >([]);
    ...
    <PageContainer size="lg" padded={false} className="max-w-4xl">
      <div className="mb-8">
        ...
        <p className="mt-2 text-muted-foreground">
          Create a comprehensive interview record with participant details
        </p>
      </div>
      ...
      <Form method="post" className="space-y-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <Label htmlFor="participant_pseudonym">
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-muted-foreground text-sm">
              Link existing people or add new participants for better tracking
          ...
          <CardContent>
            ...
            <input
              ...
              value={JSON.stringify(
                newPeople.map((p) => ({ name: p.name, segment: p.segment })),
              )}
            />
                ...
                <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-4">
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <Select
                        value={newPersonSegment}
                        onValueChange={setNewPersonSegment}
                      >
                        <SelectTrigger className="mt-1">
                  ...
                  {newPeople.map((person) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        onClick={() => removeNewPerson(person.tempId)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
                ...
                {(selectedPeople.length > 0 || newPeople.length > 0) && (
                  <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-950/20">
                    <p className="text-blue-700 text-sm dark:text-blue-300">
                      ...
                      {selectedPeople.length + newPeople.length > 1
                        ? "s"
                        : ""}{" "}
                      will be linked
                    </p>
            ...
            {people.length === 0 && (
              <div className="py-8 text-center text-muted-foreground">
                ...
                <p className="mt-1 text-xs">
                  Use "Add Person" above to create participants for this
                  interview.
                </p>
              </div>
            )}
