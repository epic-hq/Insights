			...
			{aiKeyTakeaways.length > 0 && (
				<div className="space-y-3 rounded-lg border border-muted/60 bg-muted/40 p-4">
					<div className="flex items-center justify-between gap-4">
						<label className="font-semibold text-foreground text-lg">Key Takeaways</label>
						{conversationUpdatedLabel && (
						...
						{aiKeyTakeaways.map((takeaway, index) => {
							...
							return (
								<li key={`${takeaway.summary}-${index}`} className="flex gap-3">
									<Badge variant={styles.variant} color={styles.color} className="mt-0.5 uppercase">
										{takeaway.priority}
									</Badge>
									<div className="space-y-1">
										...
										{takeaway.evidenceSnippets.length > 0 && (
											<p className="text-muted-foreground text-sm">&ldquo;{takeaway.evidenceSnippets[0]}&rdquo;</p>
										)}
									</div>
								</li>
							);
							...
							<div className="space-y-4">
								<div>
									<h4 className="mb-2 font-medium text-sm">
										{normalizedKeyTakeaways ? "Regenerate" : "Generate"} takeaways
									</h4>
