/**
 * InterviewDetailHeader — lightweight header replacing the heavy InterviewScorecard.
 * Shows title (inline-editable), participants, metadata links, status, and actions.
 */
...
export function InterviewDetailHeader({
  ...
  const insightsFilterLink = `${routes.insights.themes()}?interview_id=${encodeURIComponent(interview.id)}`;
  ...
  const needsAttention =
    participants.length > 0 && (hasUnlinkedParticipants || hasPlaceholderNames);
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Metadata line with cross-page links */}
