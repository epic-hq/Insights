		...
		<Tabs value={activeTab} onValueChange={onTabChange}>
			<TabsList className="flex-wrap">
				...
				{completedLenses.map((template) => (
					<TabsTrigger key={template.template_key} value={`lens:${template.template_key}`}>
			...
			{completedLenses.map((template) => {
				...
				return (
					<TabsContent key={template.template_key} value={`lens:${template.template_key}`}>
						<GenericLensView analysis={analysis} template={template} editable evidenceMap={evidenceMap} />
					</TabsContent>
				);
			})}
