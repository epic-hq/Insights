 ...
 *
 * Includes inline action bar (vote, star, archive, delete) and related moments.
 */
import {
  Archive,
  ArrowUpRight,
...
interface EvidenceVerificationDrawerProps {
  ...
  /** Called when user clicks a related moment card to switch evidence */
...
function getSpeakerName(
  ...
  const match = participants.find((p) => {
    ...
    return (
      tk === normalizedKey ||
      tk === `SPEAKER ${normalizedKey}` ||
      `SPEAKER ${tk}` === normalizedKey
    );
  });

  if (match) {
    return (
      match.people?.name || match.display_name || `Speaker ${normalizedKey}`
    );
  }
  ...
  if (
    ...
    !/^SPEAKER\s+[A-Z0-9]+$/.test(normalizedKey)
  ) {
    return speakerKey;
  }
...
export function EvidenceVerificationDrawer({
  ...
  // Signed thumbnail URL for the selected evidence
  const [signedThumbUrl, setSignedThumbUrl] = useState<string | null>(null);
  useEffect(() => {
    setSignedThumbUrl(null);
    const thumbKey = selectedEvidence?.thumbnail_url;
    if (!thumbKey) return;
    let cancelled = false;
    const run = async () => {
      try {
        const res = await fetch("/api/media/signed-url", {
          ...
          body: JSON.stringify({ mediaUrl: thumbKey, intent: "playback" }),
        });
        if (!res.ok) return;
        const data = (await res.json()) as { signedUrl?: string };
        if (!cancelled && data.signedUrl) setSignedThumbUrl(data.signedUrl);
      } catch {
  ...
  // Entity flags (star, archive) for the selected evidence
  ...
  }, [deleteFetcher.state, deleteFetcher.data, onOpenChange]);

  // Related moments: all evidence except the selected one
  const relatedMoments = useMemo(() => {
    if (!selectedEvidence) return [];
    return allEvidence.filter((e) => e.id !== selectedEvidence.id);
      ...
      try {
        ...
        const audioDurationParsed =
          typeof audioDurationRaw === "string"
            ? Number.parseFloat(audioDurationRaw)
            : audioDurationRaw;
        const audioDurationSec =
          typeof audioDurationParsed === "number" &&
          Number.isFinite(audioDurationParsed)
            ? audioDurationParsed
            : (interview.duration_sec ?? null);

        const normalized = normalizeTranscriptUtterances(
          data.transcript_formatted?.speaker_transcripts || [],
          {
            audioDurationSec,
          },
        );
  ...
  const selectedRange = useMemo(() => {
    ...
    return extractEvidenceTimeRange(
      selectedEvidence.id,
      selectedEvidence.anchors,
    );
  }, [selectedEvidence]);
      ...
      return (
        <div
          ...
          className={cn(
            ...
            isHighlighted &&
              !isSelected &&
              "border-amber-400 border-l-4 bg-amber-50 dark:bg-amber-900/20",
            !isHighlighted && !isUncoded && "hover:bg-muted/30",
            isUncoded &&
              "border-muted-foreground/20 border-l-4 border-dashed bg-muted/10 opacity-60",
          )}
        >
            ...
            <div className="space-y-0">
              ...
              <div className="border-border border-b bg-muted/20 p-4">
                <div className="space-y-2">
                  ...
                  {Array.isArray(selectedEvidence.speakerNames) &&
                    selectedEvidence.speakerNames.length > 0 && (
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {selectedEvidence.support && (
                      <Badge
                        ...
                        className={cn(
                          "text-[10px]",
                          selectedEvidence.support === "supports" &&
                            "border-green-300 text-green-700",
                          selectedEvidence.support === "opposes" &&
                  ...
                  {/* Action bar: vote, star, archive, delete, open detail */}
                          ...
                          <TooltipTrigger asChild>
                            ...
                            >
                              <ThumbsUp
                                className={cn(
                                  "h-3 w-3",
                                  voteCounts.user_vote === 1 && "fill-current",
                                )}
                              />
                              {voteCounts.upvotes > 0 && voteCounts.upvotes}
                            </Button>
                          </TooltipTrigger>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            ...
                            >
                              ...
                              />
                              {voteCounts.downvotes > 0 && voteCounts.downvotes}
                            </Button>
                          </TooltipTrigger>
                          ...
                          <TooltipTrigger asChild>
                                ...
                                archiveFetcher.submit(
                                  {
                                    ...
                                    archived: String(!flags.archived),
                                  },
                                  {
                                    method: "post",
                                    action: evidenceDetailRoute(
                                      selectedEvidence.id,
                                    ),
                                  },
                                );
                            ...
                            >
                              <Archive
                                className={cn(
                                  "h-3 w-3",
                                  flags.archived && "text-orange-500",
                                )}
                              />
                            </Button>
                          </TooltipTrigger>
              ...
              {!loading &&
                ...
                !interview.hasFormattedTranscript && (
                  <div className="flex flex-col items-center gap-2 p-8 text-center">
                    ...
                    <p className="text-muted-foreground text-sm">
                      No transcript available to verify against.
                    </p>
                  </div>
                )}
              ...
              {transcriptLoaded && utterances.length > 0 && (
                <div className="p-3">
                    ...
                    <div className="space-y-1">
                      {overlappingIndices.size > 0 ? (
                        Array.from(overlappingIndices)
                          .sort((a, b) => a - b)
                          .map((idx) =>
                            renderUtterance(utterances[idx], idx, true, true),
                          )
                      ) : (
                  ...
                  {view === "context" && (
                    <div className="space-y-1">
                      {utterances
                        .slice(contextRange.start, contextRange.end + 1)
                        .map((u, relIdx) => {
              ...
              {relatedMoments.length > 0 && (
                <div className="border-border border-t p-4">
                  <h4 className="mb-2 font-medium text-foreground text-xs">
                    Related Moments ({relatedMoments.length})
                  </h4>
                  <div className="flex gap-2 overflow-x-auto pb-1">
                    {relatedMoments.map((moment) => (
                      <button
                        key={moment.id}
                        type="button"
                        className="flex w-44 shrink-0 flex-col gap-1 rounded-md border border-border bg-muted/20 p-2 text-left transition-colors hover:bg-muted/40"
                        onClick={() => onEvidenceSelect?.(moment.id)}
                      >
                        <p className="line-clamp-2 text-foreground text-xs leading-snug">
                          {moment.gist
                            ? moment.gist.length > 50
                              ? `${moment.gist.slice(0, 50)}...`
                              : moment.gist
                            : moment.verbatim
                              ? `"${moment.verbatim.length > 50 ? `${moment.verbatim.slice(0, 50)}...` : moment.verbatim}"`
                              : "No summary"}
                        </p>
                        {moment.topic && (
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this evidence item. This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                ...
                deleteFetcher.submit(
                  ...
                  {
                    method: "post",
                    action: evidenceDetailRoute(selectedEvidence.id),
                  },
                );
              }}
            >
