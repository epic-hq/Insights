...
export function TranscriptResults({
  ...
  const getSpeakerName = (speakerKey: string): string => {
    ...
    if (participantByKey) {
      return (
        participantByKey.people?.name ||
        participantByKey.display_name ||
    ...
    if (normalizedKey === "B" || normalizedKey === "SPEAKER B") {
      const anyWithName = participants.find(
        (p) => p.people?.name && p.role !== "interviewer",
      );
  ...
  const getSpeakerColor = (speaker: string) => {
    switch (speaker.toUpperCase()) {
      ...
      case "D":
        return {
          badge:
            "bg-red-100 text-red-900 border-red-200 dark:bg-red-900/20 dark:text-red-300 dark:border-red-800",
  ...
  const formatTopicDetection = () => {
    const topCategories = Object.entries(
      data?.iab_categories_result?.summary || {},
    )
      .sort(([, a], [, b]) => b - a)
      .slice(0, 10);
    ...
    result += topCategories
      .map(
        ([category, relevance]) =>
          `${category.replace(/>/g, " > ")}: ${(relevance * 100).toFixed(0)}%`,
      )
      .join("\n");
  ...
  const topCategories = Object.entries(
    ...
    .sort(([, a], [, b]) => b - a)
    .slice(0, 10);
  const topicResults = (data?.iab_categories_result?.results || []).slice(
    0,
    MAX_TOPIC_RESULTS,
  );
  ...
  const hasTopicData =
    data?.iab_categories_result &&
    Object.keys(data.iab_categories_result.summary || {}).length > 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="speakers" className="w-full">
        <TabsList
          className={`grid w-full ${hasTopicData ? "grid-cols-2" : "grid-cols-1"}`}
        >
              ...
              {hasFormattedData ? (
                  ...
                  <div className="space-y-4">
                    {(data?.utterances || []).map((utterance, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className={`${colors.border} border-l-4 py-3 pl-4`}
                        >
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <button
                                      type="button"
                                      onClick={() =>
                                        onSpeakerClick(utterance.speaker)
                                      }
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center text-muted-foreground text-sm">
                    <p>
                      Raw transcript content - no speaker breakdown available
                    </p>
                  </div>
                </div>
              )}
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      copyToClipboard(formatTopicDetection(), "topics")
                    }
                    className="flex items-center gap-2"
                  >
                    {copiedStates.topics ? (
                ...
                <div className="space-y-6">
                  ...
                  {truncatedTopics && (
                    <p className="text-muted-foreground text-xs">
                      Showing first {topicResults.length.toLocaleString()} of{" "}
                      {totalTopicResults.toLocaleString()} topic matches.
                    </p>
                  )}
                </div>
              </CardContent>
