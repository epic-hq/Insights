...
export function ManagePeopleAssociations({
  ...
  const [addDialogMode, setAddDialogMode] = useState<"search" | "create">(
    "search",
  );
  ...
  const addNewParticipant = (name: string) => {
    ...
    const submitName =
      first && last ? `${first} ${last}` : first || fallbackName;
    setSubmitError(null);
    pendingAddDialogCloseRef.current = true;
    fetcher.submit(
      {
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {participants.map((participant, idx) => {
            let speakerLabel = formatSpeakerLabel(
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isUnlinked
                    ? "border-muted-foreground/40 border-dashed bg-muted/10"
                    : "bg-muted/30",
                )}
              >
                <div
                  className={cn(
                    "min-w-[100px] text-sm",
                    isUnlinked ? "text-muted-foreground italic" : "font-medium",
                  )}
                >
                  {speakerLabel}
                </div>
                        ...
                        <CommandGroup>
                          {availablePeople.map((person) => (
                            <CommandItem
                              ...
                              onSelect={() =>
                                linkPerson(participant.id, person.id)
                              }
                            >
                              <Check
                                className={cn(
          ...
          {unassignedSpeakers.length > 0 && (
            <div className="space-y-2 border-b pb-4">
              <Label className="font-medium text-sm">
                Link to transcript speaker (optional)
              </Label>
              <div className="flex flex-wrap gap-2">
                ...
                {unassignedSpeakers.map((speaker) => (
                  <Button
                    ...
                    type="button"
                    variant={
                      selectedSpeakerKey === speaker.key ? "default" : "outline"
                    }
                    size="sm"
              ...
              {selectedSpeakerKey && (
                <p className="text-muted-foreground text-xs">
                  ...
                  {unassignedSpeakers.find((s) => s.key === selectedSpeakerKey)
                    ?.label || selectedSpeakerKey}{" "}
                  in the transcript.
                </p>
              )}
            </div>
          ...
          {addDialogMode === "search" ? (
            <div className="space-y-4 py-4">
              ...
              <Command className="rounded-lg border">
                <CommandInput
                  ...
                  onValueChange={setAddSearchInput}
                />
                  ...
                  <CommandGroup>
                    {availablePeople
                      .filter((p) => {
                        ...
                        return p.name
                          ?.toLowerCase()
                          .includes(addSearchInput.toLowerCase());
                      })
                      .map((person) => (
                        <CommandItem
                          key={person.id}
            ...
            <div className="space-y-4 py-4">
              <Button
                ...
                size="sm"
                className="-ml-2 mb-2"
                onClick={() => setAddDialogMode("search")}
              >
                ← Back to search
              </Button>
              ...
              <div className="flex items-center gap-2">
                ...
                {voiceStatus === "unsupported" && (
                  <span className="text-muted-foreground text-xs">
                    Voice fill not supported in this browser
                  </span>
                )}
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            {addDialogMode === "create" && (
              <Button
                onClick={() =>
                  addNewParticipant(
                    newPersonName || `${newPersonFirst} ${newPersonLast}`,
                  )
                }
                disabled={
                  (!newPersonFirst.trim() && !newPersonName.trim()) ||
                  isSubmitting
                }
              >
                Create & Add
