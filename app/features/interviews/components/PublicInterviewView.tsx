...
function parseKeyTakeaways(
	...
	return keyTakeaways
		.filter(
			(t): t is { priority?: string; summary?: string; evidence?: string[] } => typeof t === "object" && t !== null
		)
		.map((t) => ({
		...
		<div className="min-h-screen bg-background">
			...
			<main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
				...
				<div className="mb-8 space-y-4">
					<h1 className="font-bold text-2xl tracking-tight sm:text-3xl">{interview.title || "Untitled Interview"}</h1>
						...
						<div className="flex flex-wrap items-center gap-2">
							...
							{linkedParticipants.map((p) => (
								<Badge key={p.id} variant="secondary">
									...
									{p.people?.segment && p.people.segment !== "Unknown" && ` (${p.people.segment})`}
								</Badge>
							))}
						</div>
					)}
							...
							<ul className="space-y-3">
								{keyTakeaways.map((takeaway, index) => (
									<li key={`takeaway-${index}`} className="flex gap-3">
										<Badge variant={badgeVariantForPriority(takeaway.priority)} className="mt-0.5 shrink-0 uppercase">
											{takeaway.priority}
										</Badge>
				...
				{interview.key_takeaways && (
					<section className="mb-8">
						...
						<div className="rounded-lg border bg-muted/40 p-4">
							<Streamdown className="prose prose-sm max-w-none text-foreground">{interview.key_takeaways}</Streamdown>
						</div>
					</section>
				)}
					...
					<Card className="overflow-hidden border-dashed">
						...
						<CardContent className="py-6">
							<div className="flex flex-col items-center justify-center text-center">
								...
								<p className="mb-2 font-medium text-foreground">Unlock AI-powered analysis frameworks</p>
								<p className="mb-4 max-w-md text-muted-foreground text-sm">
									Apply Sales BANT, Empathy Maps, Customer Journey, and more to extract structured insights from this
									conversation.
								</p>
								<Button variant="outline" size="sm" asChild>
									<a href="/sign-up?ref=share-lenses">Create Free Account to View</a>
								</Button>
							</div>
						</CardContent>
					</Card>
						...
						<Card className="overflow-hidden">
							...
							<CardContent className="p-0">
								<div className="px-4 py-2">
									<Timeline
										items={displayedTopics.map(({ topic, firstEvidence, firstSeconds, count }) => ({
											id: firstEvidence.id,
											title: (
												<div className="flex items-center justify-between gap-2">
													<span className="line-clamp-1 font-medium text-foreground">{topic}</span>
							...
							<div className="space-y-4">
								...
								<TranscriptResults
									data={{
										...
										iab_categories_result: transcriptData?.topicDetection as any,
										sentiment_analysis_results: [],
									}}
									rawTranscript={transcriptData?.fullTranscript || undefined}
									participants={participants.map((p) => ({
										id: typeof p.id === "string" ? Number.parseInt(p.id, 10) || 0 : p.id,
										role: p.role,
			...
			<footer className="border-t bg-muted/30 py-12">
				<div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
					<h3 className="mb-2 font-semibold text-foreground text-lg">Want more insights and AI chat?</h3>
					<p className="mb-6 text-muted-foreground">Create a free account and join the team to unlock full access.</p>
