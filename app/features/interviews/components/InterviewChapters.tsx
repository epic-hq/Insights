...
function extractAnchorSeconds(item: Evidence): number | null {
  const anchors = Array.isArray(item.anchors)
    ? (item.anchors as Array<Record<string, unknown>>)
    : [];
...
export function InterviewChapters({
  evidence,
  onChapterClick,
  className,
}: InterviewChaptersProps) {
  ...
  const visibleChapters = useMemo(
    () => chapters.slice(pageStart, pageEnd),
    [chapters, pageStart, pageEnd],
  );
  ...
  const handleNext = useCallback(() => {
    setPageStart((prev) =>
      Math.min(totalChapters - PAGE_SIZE, prev + PAGE_SIZE),
    );
  }, [totalChapters]);
        ...
        {visibleChapters.map((chapter, localIndex) => {
          ...
          return (
            ...
            >
              ...
              {chapter.description && (
                <div className="mt-1 line-clamp-1 text-muted-foreground text-xs opacity-0 transition-opacity group-hover:opacity-100">
                  {chapter.description}
                </div>
              )}
            </button>
