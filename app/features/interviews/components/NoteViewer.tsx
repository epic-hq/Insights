...
export function NoteViewer({
  ...
  const [linked_organizations, set_linked_organizations] = useState<
    LinkedItem[]
  >([]);
  const [available_organizations, set_available_organizations] = useState<
    AvailableItem[]
  >([]);
  const [linked_opportunities, set_linked_opportunities] = useState<
    LinkedItem[]
  >([]);
  const [available_opportunities, set_available_opportunities] = useState<
    AvailableItem[]
  >([]);
  ...
  const fetch_links = useCallback(async () => {
    ...
    try {
      const [
        { data: linkedPeopleData, error: linkedPeopleError },
        { data: peopleData, error: peopleError },
      ] = await Promise.all([
        supabase
          .from("interview_people")
          .select("id, people(id, name)")
      ...
      if (linkedOrganizationsError) {
        consola.warn(
          "Failed to fetch linked organizations:",
          linkedOrganizationsError,
        );
      } else {
      ...
      if (organizationsError) {
        consola.warn(
          "Failed to fetch available organizations:",
          organizationsError,
        );
      } else {
        set_available_organizations(
          (organizationsData || []).map((o) => ({ id: o.id, label: o.name })),
        );
      }
      ...
      if (linkedOpportunitiesError) {
        consola.warn(
          "Failed to fetch linked opportunities:",
          linkedOpportunitiesError,
        );
      } else {
      ...
      if (opportunitiesError) {
        consola.warn(
          "Failed to fetch available opportunities:",
          opportunitiesError,
        );
      } else {
        set_available_opportunities(
          (opportunitiesData || []).map((o) => ({ id: o.id, label: o.title })),
        );
      }
    } finally {
    ...
    const fetchTranscript = async () => {
      ...
      try {
        ...
        const transcriptText =
          data?.transcript || formatted?.full_transcript || null;
          ...
          <div className="flex items-center gap-2">
            {needsTranscription ? (
              <Button
                ...
                title="Transcribe audio and extract insights"
              >
                {isReprocessing ? (
                  <>
              ...
              <Button
                ...
                title={
                  hasContent
                    ? isIndexed
                      ? "Re-index this note for semantic search"
                      : "Index this note for semantic search"
              ...
              >
                {isIndexing || isReprocessing ? (
                  <>
                    <Loader2 className="mr-1 h-4 w-4 animate-spin" />
                    {isReprocessing ? "Processing..." : "Indexing..."}
                  </>
                ) : (
                  <>
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={handleApplyLenses}
              disabled={isApplyingLenses || !hasContent}
              title="Apply conversation lenses to extract insights"
            >
              {isApplyingLenses ? (
                <>
                  <Loader2 className="mr-1 h-4 w-4 animate-spin" />
          ...
          <AddLink
            ...
            kinds={[
              ...
              },
              {
                ...
                on_unlink: (link_id) =>
                  handleUnlinkOpportunity({ interviewOpportunityId: link_id }),
          ...
          <div className="mb-6">
            <audio controls className="w-full">
              <source
                ...
                type={`audio/${interview.file_extension || "mp3"}`}
              />
