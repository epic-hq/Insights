...
describe("parseFullName", () => {
  it("splits 'Jane Doe' into first + last", () => {
    expect(parseFullName("Jane Doe")).toEqual({
      firstname: "Jane",
      lastname: "Doe",
    });
  });
  ...
  it("handles single name", () => {
    expect(parseFullName("Cher")).toEqual({
      firstname: "Cher",
      lastname: null,
    });
  });
  ...
  it("trims whitespace", () => {
    expect(parseFullName("  Bob   Smith  ")).toEqual({
      firstname: "Bob",
      lastname: "Smith",
    });
  });
...
describe("normalizeMultilineText", () => {
  ...
  it("preserves existing bullet markers", () => {
    const result = normalizeMultilineText([
      "- Already bulleted",
      "* Star marker",
      "1. Numbered",
    ]);
...
describe("matchTakeawaysToEvidence", () => {
  const evidence: EvidenceRecord[] = [
    {
      id: "ev-1",
      verbatim: "It takes forever to get started with the onboarding",
      gist: "Slow onboarding",
    },
    {
      id: "ev-2",
      verbatim:
        "I don't want to wait for a sales call to begin using the product",
      gist: "Self-serve preference",
    },
    {
      id: "ev-3",
      verbatim: "The dashboard is great once you're in",
      gist: "Dashboard satisfaction",
    },
  ];
    ...
    const takeaways: KeyTakeaway[] = [
      {
        ...
        summary: "Direct link",
        evidenceSnippets: [
          "This snippet should be ignored when direct ids are present",
        ],
    ...
    const shortEvidence: EvidenceRecord[] = [
      {
        id: "ev-short",
        verbatim:
          "The onboarding process is painful and slow for most new customers",
        gist: null,
      },
    ];
  ...
  it("matches via token overlap when no substring match exists", () => {
    const takeaways: KeyTakeaway[] = [
      {
        priority: "high",
        summary: "Onboarding friction",
        evidenceSnippets: [
          "getting started with the product onboarding takes too long",
        ],
      },
    ];
  ...
  it("matches takeaway summary against evidence gist when no snippets match", () => {
    const gistEvidence: EvidenceRecord[] = [
      {
        id: "ev-g1",
        verbatim: "completely unrelated verbatim text here",
        gist: "Users prefer self-service onboarding over sales calls",
      },
      {
        id: "ev-g2",
        verbatim: "another unrelated thing",
        gist: "Dashboard performance metrics",
      },
    ];
    const takeaways: KeyTakeaway[] = [
      {
        priority: "medium",
        summary: "Self-service onboarding preference over sales",
        evidenceSnippets: [],
      },
    ];

    matchTakeawaysToEvidence(takeaways, gistEvidence);
    expect(takeaways[0].evidenceId).toBe("ev-g1");
  });

  it("matches reverse gist containment (gist inside snippet)", () => {
    const takeaways: KeyTakeaway[] = [
      {
        priority: "medium",
        summary: "Dashboard",
        evidenceSnippets: ["Dashboard satisfaction is high among users"],
      },
    ];
  ...
  it("matches short verbatim (>10 chars) via reverse containment", () => {
    const shortEvidence: EvidenceRecord[] = [
      { id: "ev-short", verbatim: "painful onboarding", gist: null },
    ];
    const takeaways: KeyTakeaway[] = [
      {
        priority: "high",
        summary: "Onboarding issues",
        evidenceSnippets: [
          "The painful onboarding process frustrates new users",
        ],
      },
    ];
