...
describe("matchTakeawaysToEvidence", () => {
	...
	it("matches reverse containment (verbatim inside snippet) for long verbatims", () => {
		const shortEvidence: EvidenceRecord[] = [
			{
				id: "ev-short",
				verbatim: "The onboarding process is painful and slow for most new customers",
				gist: null,
			},
		];
		...
		const takeaways: KeyTakeaway[] = [
			{
				...
				evidenceSnippets: ["getting started with the product onboarding takes too long"],
			},
		];
	...
	it("matches short verbatim (>10 chars) via reverse containment", () => {
		const shortEvidence: EvidenceRecord[] = [{ id: "ev-short", verbatim: "painful onboarding", gist: null }];
		const takeaways: KeyTakeaway[] = [
			{
				priority: "high",
				summary: "Onboarding issues",
				evidenceSnippets: ["The painful onboarding process frustrates new users"],
			},
		];
