...
type Project_SectionInsert =
  Database["public"]["Tables"]["project_sections"]["Insert"];
type Project_SectionUpdate =
  Database["public"]["Tables"]["project_sections"]["Update"];
...
export const getProjectSectionKinds = async ({
  supabase,
}: {
  supabase: SupabaseClient<Database>;
}) => {
...
export const upsertProjectSection = async ({
  ...
  const position =
    data.position ??
    DEFAULT_SECTION_POSITION[(data as { kind?: string }).kind ?? ""];
...
const _deleteProjectSection = async ({
  supabase,
  id,
}: {
  supabase: SupabaseClient<Database>;
