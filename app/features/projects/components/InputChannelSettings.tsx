...
export function MethodSettingsButton({
  projectId,
  accountId,
  projectName,
}: MethodSettingsProps) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure how external participants can provide feedback via a
...
function MethodSettingsContent({
    ...
    const load = async () => {
      ...
      try {
        ...
        if (data) {
          setResearchLink({
            ...data,
            default_response_mode: data.default_response_mode as
              | "form"
              | "chat",
          });
          ...
          setSlug(
            generateSlug(projectName || `project-${projectId.slice(0, 8)}`),
          );
        }
      } finally {
        setIsLoading(false);
  ...
  const hasChanges = researchLink
    ? researchLink.allow_chat !== chatEnabled ||
      researchLink.is_live !== isLive ||
      researchLink.slug !== slug
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-xs">
                Structured questions, one at a time
              </p>
            </div>
          </div>
          <Switch
        ...
        <div className="flex items-center justify-between rounded-lg border p-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-xs">
                Conversational AI guides discussion
              </p>
            </div>
          </div>
          <Switch
        ...
        <div className="space-y-4 rounded-lg border bg-muted/30 p-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground text-xs">
                Only live links are accessible to participants
              </p>
            </div>
            <Switch
              checked={isLive}
          ...
          {!isLive && researchLink && (
            <p className="text-amber-600 text-xs dark:text-amber-400">
              This link is not live. Enable "Make link live" to allow
              participants to access it.
            </p>
          )}
        </div>
      )}
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button
          ...
          disabled={
            isSaving ||
            !hasChanges ||
            (!surveyEnabled && !chatEnabled && !researchLink)
          }
        >
          {isSaving ? (
            <>
