				...
				<div className="flex items-start gap-2.5">
					...
					<div className="min-w-0 flex-1">
						...
						<div className="mt-1.5">
							{isArray && Array.isArray(field.value) && field.value.length > 0 ? (
								<TagList values={field.value} />
							) : (
								<p className="text-foreground/90 text-sm leading-relaxed">{formatValue(field.value)}</p>
							)}
						</div>
						{field.sourceText && (
							<p className="mt-2 text-muted-foreground/70 text-xs italic">From: "{field.sourceText}"</p>
						)}
					</div>
				</div>
				{onEdit && (
 ...
 */
function UncapturedFieldCard({ field, onAsk }: { field: CapturedField; onAsk?: () => void }) {
	return (
		<div className="rounded-lg border border-border/40 border-dashed bg-muted/20 p-3 opacity-70 transition-opacity hover:opacity-100">
			<div className="flex items-start gap-2.5">
				...
				<div className="min-w-0 flex-1">
					<p className="font-medium text-muted-foreground text-sm">{field.label}</p>
					{field.description && <p className="mt-0.5 text-muted-foreground/70 text-xs">{field.description}</p>}
					{onAsk && (
						<Button
							variant="ghost"
							size="sm"
		...
		<>
				...
				<div className="flex border-b">
					...
					>
						...
						<span className="text-xs opacity-60">
							({companyFields.filter((f) => hasValue(f.value)).length}/{companyFields.length})
						</span>
					</button>
					<button
						type="button"
					...
					>
						...
						Project
						<span className="text-xs opacity-60">
							({projectFields.filter((f) => hasValue(f.value)).length}/{projectFields.length})
						</span>
					</button>
				</div>
			)}
			...
			<div className={cn("overflow-y-auto p-4", !isSidebar && "max-h-[60vh]")}>
				...
				{capturedFields.length > 0 && (
					<div className="mb-4">
						<h3 className="mb-2 font-medium text-muted-foreground text-xs uppercase tracking-wide">Captured</h3>
						<div className="space-y-2">
							{capturedFields.map((field) => (
								<CapturedFieldCard
									key={field.key}
									field={field}
									onEdit={onEditField ? () => onEditField(field.key) : undefined}
								/>
							))}
						</div>
					</div>
				...
				{uncapturedFields.length > 0 && (
					<div>
						<h3 className="mb-2 font-medium text-muted-foreground text-xs uppercase tracking-wide">Not Yet Captured</h3>
						<div className="space-y-2">
							{uncapturedFields.map((field) => (
								<UncapturedFieldCard
									key={field.key}
									field={field}
									onAsk={onAskAboutField ? () => onAskAboutField(field.key) : undefined}
								/>
							))}
						</div>
					</div>
				...
				{/* All captured state */}
				{capturedFields.length === displayFields.length && displayFields.length > 0 && (
					<div className="rounded-lg bg-emerald-50 p-4 text-center dark:bg-emerald-950/30">
						...
						<p className="font-medium text-emerald-700 text-sm dark:text-emerald-300">
							{shouldShowTabs ? `All ${activeTab} fields captured` : "All fields captured"}
						</p>
					</div>
				)}
				...
				{displayFields.length === 0 && (
					<div className="py-8 text-center text-muted-foreground text-sm">No fields configured</div>
				)}
			</div>
		</>
	)
