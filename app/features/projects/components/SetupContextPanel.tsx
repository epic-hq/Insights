...
function SectionCard({
	...
	return (
		<div className="space-y-2" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
			...
			<div className={cn("rounded-lg border bg-muted/30 p-3 text-sm", status === "empty" && "border-dashed")}>
				{children}
			</div>
		</div>
	)
				...
				{!collapsed && (
					...
					>
						<div className="space-y-2 p-4">
							...
							>
								{companyStatus === "empty" ? (
									<p className="text-muted-foreground">Share your website to auto-fill company info</p>
								) : (
									<div className="space-y-1">
										{company?.company_description && <p className="line-clamp-2">{company.company_description}</p>}
										{company?.customer_problem && (
											<p className="text-muted-foreground text-xs">Solves: {company.customer_problem}</p>
										)}
										{company?.target_orgs && company.target_orgs.length > 0 && (
											<p className="text-muted-foreground text-xs">
												For: {company.target_orgs.slice(0, 3).join(", ")}
												{company.target_orgs.length > 3 && ` +${company.target_orgs.length - 3}`}
											</p>
										)}
									</div>
								)}
							...
							>
								{researchStatus === "empty" ? (
									<p className="text-muted-foreground">What do you want to learn from customers?</p>
								) : (
									<div className="space-y-1.5">
										{research?.research_goal && (
											<div className="flex gap-1.5">
												<span className="shrink-0 font-medium text-muted-foreground">Goal:</span>
												<span className="line-clamp-2">{research.research_goal}</span>
											</div>
										)}
										{research?.assumptions && research.assumptions.length > 0 && (
											<div className="flex gap-1.5">
												<span className="shrink-0 font-medium text-muted-foreground">Assumption:</span>
												<span className="line-clamp-1">{research.assumptions[0]}</span>
											</div>
										)}
										{research?.unknowns && research.unknowns.length > 0 && (
											<div className="flex gap-1.5">
												<span className="shrink-0 font-medium text-muted-foreground">Unknown:</span>
												<span className="line-clamp-1">{research.unknowns[0]}</span>
											</div>
										)}
									</div>
								)}
								...
								{questionsStatus === "empty" ? (
									...
									<div className="flex items-center justify-between">
										...
										<div className="flex gap-2">
											{onViewQuestions && (
												<Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={onViewQuestions}>
													View
												</Button>
											)}
											{onEditQuestions && (
												<Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={onEditQuestions}>
													Edit
												</Button>
											)}
										</div>
