...
> = {
	roles: {
		pattern: /\b(roles?|job titles?|who.*talk|target.*roles?|buyers?|users?)\b/i,
		suggestionType: "roles",
	},
	organizations: {
		pattern: /\b(organizations?|companies|industries|target.*org|types? of.*compan)/i,
		...
		<div className="border-border/60 border-t bg-muted/30">
			...
			>
				<div className="flex items-center gap-2">
					<span className="font-medium text-foreground/80 text-xs">Context</span>
						...
						{fields.slice(0, 6).map((field) => (
							<div
								...
								className={cn(
									...
									hasFieldValue(field.value) ? "bg-emerald-500" : "bg-muted-foreground/30"
								)}
								title={`${field.label}: ${hasFieldValue(field.value) ? "Captured" : "Not captured"}`}
							/>
						))}
						{fields.length > 6 && (
							<span className="ml-0.5 text-[10px] text-muted-foreground">+{fields.length - 6}</span>
						)}
					</div>
					{expanded ? (
			...
			<AnimatePresence>
				{expanded && (
					...
					>
						<div className="max-h-48 space-y-3 overflow-y-auto border-border/40 border-t px-3 py-2">
							{/* Company fields */}
							{companyFields.length > 0 && (
								<div>
									<h4 className="mb-1.5 font-medium text-[10px] text-muted-foreground uppercase tracking-wide">
										Company
									</h4>
...
function CapturedFieldChip({ field, onAsk }: { field: CapturedField; onAsk?: () => void }) {
		...
		return (
			<div className="flex items-start gap-1.5 rounded-md bg-card px-2 py-1.5 ring-1 ring-border/50">
				...
				<div className="min-w-0 flex-1">
					<p className="truncate font-medium text-[11px] text-foreground">{field.label}</p>
	...
	return (
		...
		>
			...
			<div className="min-w-0 flex-1">
				<p className="truncate font-medium text-[11px] text-foreground/70">{field.label}</p>
				{onAsk && (
