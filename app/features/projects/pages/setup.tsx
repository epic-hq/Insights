...
import { ArrowRight, Building2, Check, FolderKanban, Pencil } from "lucide-react"
...
export async function loader({ context, params, request }: LoaderFunctionArgs) {
	...
	const uiPreferences = (ctx.user_settings?.ui_preferences as Record<string, unknown>) || {}
...
function SetupCapturedPane({
	...
	const capturedFields: CapturedField[] = CAPTURED_FIELD_DEFINITIONS.map((fieldDef) => {
		...
		const accountValue = accountData?.[fieldDef.key as keyof typeof accountData]
		...
		if (hasLocalField) {
			...
			value = localValue && (Array.isArray(localValue) ? localValue.length > 0 : localValue) ? localValue : null
		} else if (sectionValue && (Array.isArray(sectionValue) ? sectionValue.length > 0 : sectionValue)) {
			value = sectionValue as string | string[] | null
		} else if (accountValue && (Array.isArray(accountValue) ? accountValue.length > 0 : accountValue)) {
...
const PROJECT_QUESTIONS = [
	...
	},
	{
		...
		description: "What don't you know yet that you need to find out from customers?",
		fieldType: "tags" as const,
		required: false,
						...
						return (
							...
							>
								<div className="min-w-0 flex-1">
									<p className="font-medium text-muted-foreground text-xs">{field.label}</p>
									<p className="mt-0.5 text-foreground text-sm leading-relaxed">{displayValue}</p>
								</div>
								<Button
									variant="ghost"
									size="sm"
		...
		>
			<div>
				...
				<p className="mt-2 text-muted-foreground text-sm">
					Here's what we've captured. Edit anything that needs changing, then continue.
				</p>
			</div>

			<div className="space-y-6">
				{renderFieldGroup("Company", <Building2 className="h-4 w-4" />, companyFields, companyValues)}
				{renderFieldGroup("Project", <FolderKanban className="h-4 w-4" />, projectFields, projectValues)}
			</div>
...
export default function ProjectSetupPage() {
	...
	// Mode: default to "form" during welcome flow, otherwise saved preference or "chat"
	const savedMode = (uiPreferences?.setupMode as SetupMode) || "chat"
	const [mode, setMode] = useState<SetupMode | null>(isWelcomeFlow ? "form" : savedMode)
	...
	useEffect(() => {
		if (accountData) {
			setCompanyValues((prev) => ({
				...
				company_description: accountData.company_description || prev.company_description,
	...
	const lastSavedProjectRef = useRef<Record<string, string | string[]>>(
		Object.fromEntries(
			Object.entries(initialSections || {}).map(([k, v]) => [k, Array.isArray(v) ? v : String(v || "")])
		)
	)
	...
	const hasValuesChanged = (
		...
		for (const key of Object.keys(current)) {
			...
			if (Array.isArray(currentVal) && Array.isArray(savedVal)) {
				if (currentVal.length !== savedVal.length || currentVal.some((v, i) => v !== savedVal[i])) {
					return true
				}
	...
	const currentQuestions =
		formPhase === "company" ? COMPANY_QUESTIONS : formPhase === "project" ? PROJECT_QUESTIONS : []
	...
	useEffect(() => {
		if (researchFetcher.state === "idle" && researchFetcher.data) {
			...
			if (result.success && result.data) {
				...
				// Show success toast
				if (insightCount > 0) {
					toast.success(`Found ${insightCount} key insights from your website`, {
	...
	const handleFormBack = () => {
		...
		if (formPhase === "review") {
			// Go back to last project question from review
			setFormPhase("project")
	...
	const renderModeContent = () => {
		...
		if (mode === "form") {
			// Form question content (shared between welcome and normal layout)
			const formQuestionContent =
				formPhase === "review" ? (
					...
					<TypeformQuestion
						...
						onBack={formStep > 0 || formPhase === "project" ? handleFormBack : undefined}
						...
						researchGoal={formValues.research_goal?.toString() || companyValues.company_description?.toString() || ""}
					/>
				) : null
			...
			return (
				...
				>
					...
					<div className="flex items-center justify-between border-b px-6 py-3">
						...
						{!isWelcomeFlow && (
							<SetupModeToggle mode={mode} onModeChange={handleModeChange} showVoice={isVoiceEnabled} />
						)}
					</div>
					...
					<div className="flex-1 overflow-y-auto">
						{isWelcomeFlow ? (
							// Welcome flow: 60/40 split with sidebar CapturedPane
							<div className="flex h-full flex-col gap-6 p-6 lg:flex-row">
								<div className="flex-[3] lg:max-w-2xl">
									<AnimatePresence mode="wait">{formQuestionContent}</AnimatePresence>
								</div>
							...
							// Normal layout: centered, full width
	...
	const FIELD_PROMPTS: Record<string, string> = {
		// Company fields
		website_url: "I'd like to add my company website so you can learn about us.",
		company_description: "Help me describe what my company does.",
		industry: "What industry should I categorize my company in?",
		customer_problem: "Help me articulate the main problem we solve for customers.",
		offerings: "Help me list our main products or services.",
		target_orgs: "What types of organizations should I be targeting for research?",
		...
		// Project fields
		research_goal: "Help me define my research goal - what do I want to learn from customers?",
		research_goal_details: "Can you help me add more detail to my research objectives?",
		decision_questions: "What key business questions should this research help me answer?",
		unknowns:
	...
	const EDIT_FIELD_PROMPTS: Record<string, string> = {
		...
		research_goal_details: "I want to update the details of my research objectives.",
		decision_questions: "I'd like to change the business questions I'm trying to answer.",
	...
	const handleAskAboutField = (fieldKey: string) => {
		console.log("[handleAskAboutField] called with:", fieldKey, "current mode:", mode)
		...
		const prompt =
			FIELD_PROMPTS[fieldKey] || `Help me fill in ${fieldDef.label.toLowerCase()}. ${fieldDef.description || ""}`
	...
	const handleEditField = (fieldKey: string) => {
		console.log("[handleEditField] called with:", fieldKey, "current mode:", mode)
		...
		if (mode === "form") {
			...
			if (companyIndex !== -1) {
				console.log("[handleEditField] navigating to company question:", companyIndex)
				setFormPhase("company")
			...
			if (projectIndex !== -1) {
				console.log("[handleEditField] navigating to project question:", projectIndex)
				setFormPhase("project")
		...
		const prompt = EDIT_FIELD_PROMPTS[fieldKey] || `I'd like to update ${fieldDef.label.toLowerCase()}.`
	...
	return (
		<ProjectSetupProvider projectId={projectId} initialData={initialSections as Record<string, unknown>}>
			...
			{/* Floating CapturedPane - hidden when sidebar variant is showing in welcome flow form mode */}
			{mode !== null && !(isWelcomeFlow && mode === "form") && (
				<SetupCapturedPane
					localFields={localCapturedFields}
					accountData={accountData}
					onAskAboutField={handleAskAboutField}
