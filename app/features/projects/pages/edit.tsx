...
export async function action({
  ...
  if (intent === "update_analysis_settings") {
    const themeDedup = Number.parseFloat(
      (formData.get("theme_dedup_threshold") as string) ||
        `${DEFAULT_THEME_DEDUP_THRESHOLD}`,
    );
    const evidenceLink = Number.parseFloat(
      (formData.get("evidence_link_threshold") as string) ||
        `${DEFAULT_EVIDENCE_LINK_THRESHOLD}`,
    );
...
export default function EditProject() {
  ...
  const initialThemeDedup =
    analysisSettings?.theme_dedup_threshold ?? DEFAULT_THEME_DEDUP_THRESHOLD;
