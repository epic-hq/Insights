		...
		if (initialData && Object.keys(initialData).length > 0) {
			...
			for (const [key, value] of Object.entries(initialData)) {
				if (value !== undefined && value !== null) {
					...
					if (hasValue) {
						dataToMerge[key as keyof ProjectSectionData] = value as ProjectSectionData[keyof ProjectSectionData]
					}
				}
			}
			if (Object.keys(dataToMerge).length > 0) {
				...
				consola.debug("Synced project setup with server data", Object.keys(dataToMerge))
			}
		}
	}, [initialData, store])
