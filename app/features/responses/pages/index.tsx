...
export const meta: MetaFunction = () => {
  return [
    { title: "Responses | Insights" },
    { name: "description", content: "Survey and chat responses" },
  ];
};
...
export async function loader({ context, params }: LoaderFunctionArgs) {
  ...
  if (error) {
    ...
    throw new Response(
      `Error fetching responses: ${(error as PostgrestError).message}`,
      {
        status: 500,
      },
    );
  ...
  const surveyMap = new Map<
    string,
    { id: string; title: string; slug: string; response_mode: string | null }
  >();
  ...
  for (const response of responses || []) {
    const linkId = (response as { research_link_id?: string | null })
      .research_link_id;
  ...
  const surveyGroups = Array.from(grouped.entries()).map(
    ([surveyId, items]) => ({
...
function ResponseCard({
  ...
  const getResponseTypeBadge = () => {
    ...
    if (
      response.media_type === "voice_memo" ||
...
export default function ResponsesIndex() {
  const { surveyGroups, ungroupedResponses, totalCount } =
            ...
            <div className="space-y-1">
              ...
              {totalCount > 0 && (
                <p className="text-muted-foreground text-sm">
                  {totalCount} response{totalCount !== 1 ? "s" : ""} from
                  surveys and chats
                </p>
              )}
            </div>
              ...
              <div key={survey.id}>
                ...
                <div className="space-y-2">
                  {responses.map((response) => (
                    <ResponseCard
                      key={response.id}
                      response={response as ResponseItem}
                      routes={routes}
                    />
                  ))}
                </div>
