...
import { getBatchSurveySendStats } from "~/lib/integrations/gmail.server";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
	...
	// Load email distribution stats for all surveys in one query
	const surveyIds = (data ?? []).map((l) => l.id);
	const emailStatsMap = await getBatchSurveySendStats(supabase, surveyIds);

	return { accountId, projectId, origin, lists: data ?? [], emailStatsMap };
}
...
interface LoaderData {
	...
	projectId: string;
	origin: string;
	emailStatsMap: Record<string, EmailStats>;
	lists: Array<{
		id: string;
		name: string;
		slug: string;
...
function AskLinkCard({ list, questions, responsesCount, publicUrl, routes, emailStats }: AskLinkCardProps) {
					...
					{emailStats && (
						<div className="mt-3 space-y-1.5">
							<div className="flex items-center gap-2 text-xs">
								...
								{emailStats.sent > 0 && <span className="text-muted-foreground">· {emailStats.sent} pending</span>}
							</div>
			...
			<AlertDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This will permanently delete "{list.name}" and all its responses. This action cannot be undone.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
