						...
						<Card className="border-violet-200 bg-gradient-to-r from-violet-50/50 to-background shadow-sm dark:border-violet-800 dark:from-violet-950/30">
							<CardContent className="p-6">
								...
								<p className="mb-4 text-muted-foreground text-sm">
									Based on your research progress, here are some surveys that could help:
								</p>
										...
										>
											<div className="flex items-start justify-between gap-3">
												<div className="min-w-0 flex-1">
													...
													<p className="mt-1 text-muted-foreground text-sm">{suggestion.description}</p>
													<p className="mt-2 text-muted-foreground/80 text-xs">{suggestion.reasoning}</p>
												</div>
								...
								<div className="mt-4 text-center">
									<span className="text-muted-foreground text-sm">Or start from scratch below</span>
								</div>
							</CardContent>
						</Card>
					</motion.div>
							...
							<Card className="overflow-hidden border-2 border-primary/20 bg-gradient-to-br from-primary/5 via-background to-violet-500/5 shadow-xl">
								<CardContent className="p-8">
									<div className="text-center">
										<h1 className="mb-2 font-bold text-2xl">Create with your voice</h1>
										<p className="mx-auto mb-8 max-w-sm text-muted-foreground">
											Just tell me what you're looking for and I'll create the survey, even with branches and questions
											for special groups
										</p>
										...
										<div className="flex flex-col items-center gap-4">
											{isGeneratingFromVoice ? (
												<div className="flex flex-col items-center gap-3">
													...
													<p className="text-muted-foreground text-sm">Creating your survey...</p>
													{voiceTranscript && (
												...
												<div className="flex flex-col items-center gap-3">
													...
													<p className="font-medium text-red-600 text-sm">Listening... tap to stop</p>
													<p className="max-w-xs text-muted-foreground/70 text-xs">
														Describe who you want to learn from and what you want to know
													</p>
												</div>
											) : (
												<div className="flex flex-col items-center gap-4">
										...
										{/* Example prompts */}
										{!isRecording && !isTranscribing && !isGeneratingFromVoice && (
											<div className="mt-8 space-y-2">
												<p className="text-muted-foreground/60 text-xs">Try saying something like:</p>
												<div className="flex flex-wrap justify-center gap-2">
													{[
														"I want to understand pricing sensitivity",
														"Get feedback from enterprise customers about their workflow",
														"Learn what makes sponsors renew their sponsorship",
													].map((example) => (
														<span
															key={example}
															className="rounded-full bg-muted/50 px-3 py-1 text-muted-foreground/70 text-xs"
														>
															"{example}"
														</span>
													))}
							...
							<CardContent className="p-8">
								...
								{(!showVoiceInput || !isVoiceSupported || name) && (
									<>
										...
										<p className="mb-6 text-muted-foreground">What are you trying to learn?</p>
									</>
								)}
								...
								<div className="space-y-6">
									...
									<Button type="button" onClick={() => setStep(2)} disabled={!step1Valid} size="lg" className="w-full">
										Next: Add Questions
						...
						<Card className="shadow-lg">
							<CardContent className="p-8">
								<div className="mb-6">
									...
									<p className="text-muted-foreground">What do you want to ask?</p>
								</div>
										...
										<div className="flex items-start gap-3">
											...
											<div className="min-w-0 flex-1">
												...
												<p className="mt-1 text-amber-700 text-sm dark:text-amber-300">
													I set up some branching logic but want to make sure I got it right:
												</p>
												<ul className="mt-3 space-y-2">
													{clarifications.map((c) => (
														<li key={c.guidelineId} className="flex items-start gap-2 text-sm">
															<AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-amber-500" />
															<div>
																<span className="font-medium text-amber-900 dark:text-amber-100">{c.summary}</span>
																<span className="ml-1 text-amber-700 dark:text-amber-300">— {c.question}</span>
															</div>
														</li>
													))}
												</ul>
												<div className="mt-4 flex gap-2">
													...
													<Button
														...
														onClick={() => {
															// TODO: Open edit mode for guidelines
															setShowClarifications(false)
														}}
													>
														I'll adjust in the editor
													</Button>
												</div>
											</div>
										</div>
								...
								<div className="space-y-6">
									...
									{questions.length === 0 ? (
										<div className="space-y-4">
											...
											/>
											{bulkText.trim() && (
												<Button type="button" onClick={handleBulkSubmit} variant="secondary" className="w-full">
													<Plus className="mr-2 h-4 w-4" />
													Add {bulkText.split("\n").filter((l) => l.trim()).length} question
												...
												{questions.map((question, index) => (
													<div key={question.id} className="group flex gap-2">
														...
														<Textarea
															...
															onChange={(e) => {
																setQuestions(
																	questions.map((q) => (q.id === question.id ? { ...q, prompt: e.target.value } : q))
																)
															}}
											...
											<div className="flex gap-2">
												...
												<Popover open={aiPopoverOpen} onOpenChange={setAiPopoverOpen}>
													<PopoverTrigger asChild>
														<Button type="button" variant="ghost" disabled={isGenerating} className="gap-2">
															{isGenerating ? (
													...
													<PopoverContent className="w-80" align="end" side="top">
														<div className="space-y-3">
															<div className="space-y-1">
																<h4 className="font-medium text-sm">Edit with AI</h4>
																<p className="text-muted-foreground text-xs">
																	Describe what questions you want to add or change.
																</p>
															</div>
															<Textarea
																value={aiPrompt}
