/**
 * Stepper-based research link creation wizard
 * 3 simple steps: Basics → Questions → Review & Share
 * Each step is a separate "page" - only one visible at a time
 */
import slugify from "@sindresorhus/slugify"
import { AnimatePresence, motion } from "framer-motion"
import { ArrowLeft, ArrowRight, Check, Copy, Loader2, Plus, Sparkles, X } from "lucide-react"
...
export const meta: MetaFunction = () => {
	return [{ title: "Create Survey | Upsight" }, { name: "description", content: "Create a survey in under a minute" }]
}
...
export async function action({ request, params }: ActionFunctionArgs) {
	...
	if (!data) {
		return Response.json({ errors: { _form: "Unable to create survey" } }, { status: 500 })
	}
...
export default function CreateResearchLinkPage() {
	...
	useEffect(() => {
		if (name) {
			setSlug(slugify(name, { lowercase: true }))
		}
	}, [name])
	...
	const publicUrl = `${typeof window !== "undefined" ? window.location.origin : ""}${routes.researchLinks.public(slug || "your-survey")}`
	...
	const handleGenerate = () => {
		generateFetcher.submit(
			...
			{
				method: "POST",
				action: `/a/${accountId}/research-links/api/generate-questions`,
			}
		)
	}
					...
					<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
						<Card className="shadow-lg">
							<CardContent className="p-8">
								<h1 className="mb-2 font-bold text-2xl">Name your survey</h1>
								...
								<div className="space-y-6">
									...
									<div className="space-y-2">
										...
										<Textarea
											...
											placeholder="What should respondents know about this survey?"
											rows={3}
										/>
									</div>
...
What would make you switch to a new solution?`}
												...
												autoFocus
											/>
											<Button type="button" onClick={handleBulkSubmit} disabled={!bulkText.trim()} variant="secondary">
								...
								<CardContent className="space-y-6 p-8">
									...
									<div className="flex items-center justify-between rounded-lg border p-4">
										<div>
											...
											<p className="text-muted-foreground text-sm">Make the survey available at this URL</p>
										</div>
