			...
			<div className="mx-auto max-w-xl px-4">
						...
						<Card className="border-violet-200 bg-gradient-to-r from-violet-50/50 to-background shadow-sm dark:border-violet-800 dark:from-violet-950/30">
							<CardContent className="p-6">
								...
								<div className="mt-3 text-center">
									<span className="text-muted-foreground text-xs">Or start from scratch below</span>
								</div>
							</CardContent>
						</Card>
					</motion.div>
				...
				{step === 1 && (
					...
					>
						...
						{showVoiceInput && isVoiceSupported && !name && activeInput !== "form" && (
						...
						{showVoiceInput && isVoiceSupported && !name && activeInput !== "voice" && activeInput !== "form" && (
													...
													<div key={question.id} className="group space-y-1">
														<div className="flex gap-2">
															...
															<div className="min-w-0 flex-1 space-y-1">
																...
																/>
																{/* AI guidance hints */}
																{question.branching?.rules && question.branching.rules.length > 0 && (
																	<div className="space-y-0.5 pl-1">
																		{question.branching.rules.map((rule) => (
																			<div key={rule.id} className="flex items-start gap-1.5 text-xs">
																				<Sparkles className="mt-0.5 h-3 w-3 shrink-0 text-violet-500" />
																				<span className="text-muted-foreground">
																					{rule.summary || rule.guidance}
																					{rule.summary && rule.guidance && (
																						<span className="text-violet-500/70"> — {rule.guidance}</span>
																					)}
																				</span>
																			</div>
																		))}
