			...
			{aiInsight && (
				<div className="mt-4 space-y-3 rounded-lg border border-primary/20 bg-primary/5 p-4">
					...
					{aiInsight.common_answers.length > 0 && !compact && (
						<div>
							<h5 className="mb-1 font-medium text-muted-foreground text-xs">Common Answers</h5>
							<ul className="space-y-1">
					...
					{aiInsight.notable_outliers.length > 0 && !compact && (
						<div>
							<h5 className="mb-1 font-medium text-muted-foreground text-xs">Notable Outliers</h5>
							<ul className="space-y-1">
...
export default function ResearchLinkResponsesPage() {
	...
	const getQuestionInsight = (questionText: string): QuestionInsight | undefined => {
							...
							{detailedResult && (
								<div className="space-y-4">
											...
											<div className="space-y-3">
												{detailedResult.top_themes.map((theme, idx) => (
													<div key={idx} className="rounded-lg border bg-muted/30 p-3">
														...
														<p className="mt-1 text-muted-foreground text-sm">{theme.description}</p>
														{theme.example_quotes.length > 0 && (
															<div className="mt-2 space-y-1">
																{theme.example_quotes.slice(0, 2).map((quote, qIdx) => (
																	<p
																		key={qIdx}
									...
									{detailedResult.actionable_insights.length > 0 && (
										<div>
											<h4 className="mb-2 font-medium text-sm">Actionable Insights</h4>
											<ul className="space-y-1 text-muted-foreground text-sm">
									...
									<div className="flex flex-col gap-3 border-t pt-3">
										<div className="flex items-center justify-between">
											<span className="text-muted-foreground text-xs">
												{detailedResult.total_responses} responses · {detailedResult.completion_rate}% completion rate
											</span>
											<div className="flex items-center gap-2">
												<Button
													variant="ghost"
								...
								<div className="flex flex-col gap-3 sm:gap-4">
									...
									<div className="space-y-2">
										...
										<div className="flex flex-wrap items-center gap-2 text-muted-foreground text-xs">
											<span>
												Question {activeQuestionIndex + 1} of {questionStats.length}
											</span>
										...
										<div className="flex gap-1">
											{questionStats.map((_, idx) => (
												<button
													...
													className={`h-2 w-2 rounded-full transition-colors ${idx === activeQuestionIndex
															? "bg-primary"
									...
									<div className="h-[55vh] overflow-y-auto pr-1 sm:h-[60vh] sm:pr-2">
										<QuestionBreakdown
											...
											showDivider={false}
											hideHeader
											aiInsight={getQuestionInsight(activeBreakdown.questionText)}
										/>
									</div>
								</div>
							)}
