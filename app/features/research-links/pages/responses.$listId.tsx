...
export const meta: MetaFunction = () => {
	return [
		{ title: "Research link responses" },
		{ name: "description", content: "Review and export responses from your research link." },
	]
}
...
export async function loader({ params, request }: LoaderFunctionArgs) {
	...
	return {
		...
		publicUrl: `${origin}/research/${list.slug}`,
	}
}

export default function ResearchLinkResponsesPage() {
	const { accountId, list, responses, questions, publicUrl } = useLoaderData<typeof loader>()
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div className="space-y-2">
					<div className="flex items-center gap-2">
						<h1 className="font-semibold text-3xl">Research link responses</h1>
				...
				<div className="flex flex-wrap gap-2">
					<Button asChild variant="outline">
						<Link to={routes.researchLinks.edit(list.id)}>Edit link</Link>
					</Button>
			...
			{responses.length === 0 ? (
				<Card className="border-dashed bg-muted/30">
					...
					<CardContent className="space-y-4 text-muted-foreground text-sm">
						<p>Share your public research link to start collecting responses.</p>
				...
				<ResearchLinkResponsesDataTable questions={questions} responses={responses} />
			)}
		</PageContainer>
	)
}
