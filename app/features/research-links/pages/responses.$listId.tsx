...
import { getSurveySendStats, getSurveySends } from "~/lib/integrations/gmail.server";
...
function sanitizeQuestionInsight(
	insight: QuestionInsight | undefined,
	responseCount: number
...
function QuestionBreakdown({
	...
	return (
		<div className={`space-y-3 ${showDivider ? "border-b pb-6 last:border-b-0 last:pb-0" : ""}`}>
						...
						{stat.choices.options.map((option) => (
							<div key={option.value} className="flex items-center gap-2">
								...
								{option.percentage < 15 && (
									<span className="min-w-[100px] truncate text-muted-foreground text-sm">{option.value}</span>
								)}
			...
			{aiInsight && (
				<div className="space-y-3 rounded-lg border border-primary/20 bg-primary/5 p-4">
					...
					{aiInsight.answer_distribution && aiInsight.answer_distribution.length > 0 && (
						<div>
							<h5 className="mb-1.5 font-medium text-muted-foreground text-xs">Distribution</h5>
							<div className="space-y-1">
								{aiInsight.answer_distribution.map((item, i) => (
									<div key={i} className="flex items-center gap-2 text-sm">
										...
										<span className="shrink-0 text-muted-foreground text-xs tabular-nums">{item.percentage}%</span>
										<span className="shrink-0 text-xs">{item.answer}</span>
										<span className="shrink-0 text-muted-foreground text-xs">({item.count})</span>
									</div>
								))}
							</div>
						</div>
					...
					{aiInsight.key_findings.length > 0 && (
						<div>
							<h5 className="mb-1 font-medium text-muted-foreground text-xs">Key Findings</h5>
			...
			{showTextSamples && stat.effectiveType === "text" && stat.text && (
				<div className="space-y-2">
					<h5 className="font-medium text-muted-foreground text-xs uppercase tracking-wide">Responses</h5>
					{stat.text.sampleResponses.length > 0 ? (
						<div className="space-y-2">
							{stat.text.sampleResponses.map((response, i) => (
								<div key={i} className="rounded-lg border-muted-foreground/30 border-l-2 bg-muted/30 py-2 pr-2 pl-3">
									<p className={`${compact ? "line-clamp-3" : "whitespace-pre-wrap"} text-sm`}>"{response}"</p>
								</div>
							))}
...
export default function ResearchLinkResponsesPage() {
	...
	// Get AI insight for a specific question (by id/text/index with defensive fallback)
	const getQuestionInsight = (
		...
		const normalizedPrompt = normalizeInsightText(questionText);
		const byId = detailedResult.question_insights.find((insight) => insight.questionId === questionId);
		const byExactText = normalizedPrompt
			? detailedResult.question_insights.find((insight) => normalizeInsightText(insight.question) === normalizedPrompt)
			: undefined;
		const byContainsText = normalizedPrompt
			? detailedResult.question_insights.find((insight) => {
					const normalizedInsight = normalizeInsightText(insight.question);
					return normalizedInsight.includes(normalizedPrompt) || normalizedPrompt.includes(normalizedInsight);
				})
			: undefined;
		const byIndex = detailedResult.question_insights[questionIndex];
		return sanitizeQuestionInsight(
			byId ?? byExactText ?? byContainsText ?? byIndex,
			questionStats[questionIndex]?.responseCount ?? 0
		);
	};
		...
		<PageContainer className="space-y-6 pb-12">
			...
			{responses.length === 0 ? (
				...
				<>
							...
							<CardContent className="space-y-4">
								...
								<div className="grid grid-cols-4 gap-3">
									<div className="rounded-md border p-3 text-center">
										<div className="font-semibold text-lg">{emailStats.total}</div>
										<div className="text-muted-foreground text-xs">Sent</div>
									</div>
									<div className="rounded-md border p-3 text-center">
										<div className="font-semibold text-lg">{emailStats.opened}</div>
										<div className="text-muted-foreground text-xs">Opened</div>
									</div>
									<div className="rounded-md border p-3 text-center">
										<div className="font-semibold text-lg">{emailStats.completed}</div>
										<div className="text-muted-foreground text-xs">Responded</div>
									</div>
									<div className="rounded-md border p-3 text-center">
										<div className="font-semibold text-lg">{emailStats.pendingNudge}</div>
										<div className="text-muted-foreground text-xs">Pending Nudge</div>
									</div>
								</div>
								...
								{emailStats.total > 0 && (
									<div className="space-y-1">
										<div className="flex justify-between text-muted-foreground text-xs">
											...
											<span>{Math.round((emailStats.completed / emailStats.total) * 100)}%</span>
										</div>
												...
												{surveySends.map((send) => (
													<tr key={send.id} className="border-t text-xs">
														<td className="px-3 py-1.5">
															{send.to_name ? (
																<span>
																	{send.to_name} <span className="text-muted-foreground">({send.to_email})</span>
																</span>
															) : (
																send.to_email
															)}
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-2">
							{!analysisResult && !detailedResult && (
								<Button size="sm" onClick={() => handleAnalyze("detailed")} disabled={isAnalyzing}>
									{isAnalyzing ? (
							...
							{!analysisResult &&
								...
								!(analyzeFetcher.data && "error" in analyzeFetcher.data) && (
									<p className="text-muted-foreground text-sm">
										Click Analyze to get AI insights including key themes, per-question analysis, and actionable
										recommendations.
									</p>
								)}
							{analysisResult && (
										...
										{showCustomInstructions && (
											<div className="space-y-2">
												...
												<div className="flex justify-end gap-2">
													...
													<Button
														size="sm"
														onClick={() => handleAnalyze("detailed", customInstructions)}
														disabled={isAnalyzing}
													>
														{isAnalyzing ? (
							...
							{detailedResult && (
								<div className="space-y-3">
									...
									{detailedResult.top_themes.length > 0 && (
										<ul className="space-y-1">
											{detailedResult.top_themes.slice(0, 5).map((theme, idx) => (
												<li key={idx} className="flex gap-2 text-sm">
													<span className="text-primary">•</span>
													<span className="font-medium">{theme.theme}</span>
													{theme.percentage != null && (
														<span className="text-muted-foreground tabular-nums">{theme.percentage}%</span>
													)}
													<span className="text-muted-foreground">— {theme.description.split(".")[0]}</span>
												</li>
											))}
										</ul>
									)}
									...
									<div className="flex items-center justify-between border-t pt-3">
										<span className="text-muted-foreground text-xs">
											{detailedResult.total_responses} responses · {detailedResult.completion_rate}% completion
										</span>
										<div className="flex items-center gap-2">
											<Button
												variant="ghost"
						...
						<>
							<div className="flex items-center justify-between">
								...
								<div className="flex gap-2">
									...
									<Button variant="outline" size="sm" onClick={() => openBreakdownModal(0)}>
										<Maximize2 className="mr-2 h-4 w-4" />
										View full breakdown
									</Button>
								</div>
					...
					>
						...
						>
							<DialogHeader className="sr-only">
								...
								<DialogDescription>View detailed statistics for each question</DialogDescription>
							</DialogHeader>
								...
								{isOnVideoCard ? (
													...
													>
														...
														<div className="p-3">
															<p className="font-medium text-sm">
																{response.person?.name || response.email || "Anonymous"}
															</p>
															<p className="text-muted-foreground text-xs">
																{response.completed ? "Completed" : "Partial response"}
															</p>
														</div>
													</div>
												))}
									...
									<>
										...
										<div className="flex items-start justify-between gap-2">
											<div className="space-y-1">
												<h3 className="font-semibold text-lg leading-snug sm:text-xl">
													{activeQuestionIndex + 1}. {activeBreakdown.questionText}
												</h3>
												<p className="text-muted-foreground text-sm">
													{activeBreakdown.responseCount}/{activeBreakdown.totalResponses} answered
												</p>
											</div>
											{!detailedResult && (
												<Button
								...
								<div className="flex items-center justify-between gap-2 rounded-md border bg-muted/50 px-2 py-1.5 sm:px-3 sm:py-2">
									...
									<div className="flex flex-wrap items-center justify-center gap-1.5">
										...
										{hasVideoCard && (
											<button
												type="button"
												onClick={() => setActiveQuestionIndex(questionStats.length)}
												className={`flex h-6 w-6 items-center justify-center rounded-full font-medium text-xs transition-all ${
													isOnVideoCard
														? "bg-primary text-primary-foreground ring-2 ring-primary/30"
														: "bg-muted text-muted-foreground hover:bg-muted-foreground/20"
					...
					>
						...
						>
							<DialogHeader className="sr-only">
								...
								<DialogDescription>Full analysis with themes and recommendations</DialogDescription>
							</DialogHeader>

							{detailedResult && (
								<div className="flex flex-col gap-4">
									{/* Card content based on activeAnalysisCard */}
									<div className="h-[60vh] space-y-4 overflow-y-auto pr-1 sm:pr-2">
										{/* Card 0: Summary */}
										{activeAnalysisCard === 0 && (
											<div className="space-y-5">
												<div className="space-y-3">
													<h3 className="font-semibold text-xl sm:text-2xl">Summary</h3>
													<p className="text-base leading-relaxed">{detailedResult.executive_summary}</p>
													<p className="text-muted-foreground text-sm">
														{detailedResult.total_responses} responses · {detailedResult.completion_rate}% completion
													</p>
												</div>
												{detailedResult.top_themes.length > 0 && (
													<div className="space-y-3">
														<h4 className="font-medium text-base">Key Themes</h4>
																	...
																	<span>
																		...
																		{theme.percentage != null && (
																			<span className="ml-1.5 text-muted-foreground text-sm">
																				({theme.frequency} responses, {theme.percentage}%)
																			</span>
																		)}
																	</span>
																</li>
										...
										{activeThemeForCard && (
											<div className="space-y-4">
													...
													<h3 className="font-semibold text-xl sm:text-2xl">
														...
														{activeThemeForCard.percentage != null && (
															<span className="ml-2 font-normal text-base text-muted-foreground">
																{activeThemeForCard.percentage}% ({activeThemeForCard.frequency})
															</span>
														)}
													</h3>
												...
												{activeThemeForCard.example_quotes.length > 0 && (
													<div className="space-y-3">
														{activeThemeForCard.example_quotes.map((quote, qIdx) => (
															<div
																key={qIdx}
																className="rounded-lg border-muted-foreground/30 border-l-2 bg-muted/30 py-3 pr-3 pl-4"
															>
										...
										{activeAnalysisCard === 1 + detailedResult.top_themes.length &&
											detailedResult.actionable_insights.length > 0 && (
												<div className="space-y-4">
													<h3 className="font-semibold text-xl sm:text-2xl">Recommendations</h3>
													<ul className="space-y-3">
														{detailedResult.actionable_insights.map((insight, idx) => (
															<li key={idx} className="flex gap-2 text-base leading-relaxed">
																<span className="text-primary">•</span>
																{insight}
															</li>
														))}
									...
									{analysisCardCount > 1 && (
										<div className="flex items-center justify-between gap-2 rounded-md border bg-muted/50 px-2 py-1.5 sm:px-3 sm:py-2">
											<Button
												variant="ghost"
												size="sm"
												onClick={() => setActiveAnalysisCard((prev) => Math.max(prev - 1, 0))}
											...
											<div className="flex items-center gap-1.5">
												{Array.from({ length: analysisCardCount }).map((_, idx) => (
													<button
														type="button"
														key={idx}
														...
														aria-label={
															idx === 0
																? "Summary"
																: idx === analysisCardCount - 1 && detailedResult.actionable_insights.length > 0
																	? "Recommendations"
																	: `Theme ${idx}`
														}
													>
											...
											<Button
												variant="ghost"
												size="sm"
												onClick={() => setActiveAnalysisCard((prev) => Math.min(prev + 1, analysisCardCount - 1))}
					...
					<Dialog open={showCustomInstructionsDialog} onOpenChange={setShowCustomInstructionsDialog}>
						<DialogContent className="sm:max-w-md">
							<DialogHeader>
								<DialogTitle>Custom Analysis Prompt</DialogTitle>
								<DialogDescription>
									Add specific instructions to focus the AI analysis on what matters most to you.
								</DialogDescription>
							</DialogHeader>
							<div className="space-y-4">
								<textarea
					...
					>
						<DialogContent className="max-h-[92vh] max-w-[calc(100%-1rem)] overflow-hidden p-4 sm:max-w-4xl sm:p-6">
							<DialogHeader className="sr-only">
								...
								<DialogDescription>Watch video responses from survey participants</DialogDescription>
							</DialogHeader>
							...
							{videoResponses.length > 0 && activeVideoResponse && (
								<div className="flex flex-col gap-4">
									...
									<div className="flex items-center justify-between">
										<div className="space-y-1">
											<p className="font-medium text-sm">
												{activeVideoResponse.person?.name || activeVideoResponse.email || "Anonymous"}
											</p>
											<p className="text-muted-foreground text-xs">
												{activeVideoResponse.completed ? "Completed" : "Partial response"}
											</p>
										</div>
										<Link
									...
									{videoResponses.length > 1 && (
										<div className="flex items-center justify-between gap-2 rounded-md border bg-muted/50 px-2 py-1.5 sm:px-3 sm:py-2">
											<Button
												variant="ghost"
												size="sm"
												onClick={() => setActiveVideoIndex((prev) => Math.max(prev - 1, 0))}
											...
											<Button
												variant="ghost"
												size="sm"
												onClick={() => setActiveVideoIndex((prev) => Math.min(prev + 1, videoResponses.length - 1))}
