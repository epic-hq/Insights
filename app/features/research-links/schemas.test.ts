...
describe("ResearchLinkResponseSaveSchema", () => {
  it("should accept a valid completed response", () => {
    ...
    if (result.success) {
      ...
      expect(result.data.responseId).toBe(
        "22c66a0c-cfbb-4caf-a1c6-704ac5596bda",
      );
...
describe("ResearchLinkQuestionSchema", () => {
  ...
  it("should accept question without prompt (defaults to empty string)", () => {
    const result = ResearchLinkQuestionSchema.safeParse({
      id: "q1",
    });
    expect(result.success).toBe(true);
...
describe("ResearchLinkPayloadSchema — respondentFields", () => {
  ...
  it("should parse a JSON string of field names", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.respondentFields).toEqual([
        "first_name",
        "company",
        "phone",
      ]);
