...
export async function getProjectResearchContext({
	...
	const [projectResult, sectionsResult, themesResult, promptsResult, surveysResult, interviewCountResult] =
		await Promise.all([
			...
			supabase
				.from("projects")
				.select("account_id, accounts(company_description, customer_problem, offerings, target_roles)")
				.eq("id", projectId)
				.single(),
	...
	if (themeIds.length > 0) {
		const { data: evidenceCounts } = await supabase.from("evidence").select("theme_id").in("theme_id", themeIds)

		// Count evidence per theme
		for (const e of evidenceCounts ?? []) {
			if (e.theme_id) {
				evidenceCountMap.set(e.theme_id, (evidenceCountMap.get(e.theme_id) ?? 0) + 1)
			}
		}
	}
	...
	if (accountId) {
		...
		if (peopleStats) {
			dataQuality = {
				totalPeople: peopleStats.length,
...
export async function getResearchLinkWithResponses({ supabase, accountId, listId }: GetLinksArgs & { listId: string }) {
	const [listResult, responsesResult] = await Promise.all([
		supabase.from("research_links").select("*").eq("account_id", accountId).eq("id", listId).maybeSingle(),
		supabase
