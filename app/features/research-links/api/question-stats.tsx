...
export async function loader({ request, params }: LoaderFunctionArgs) {
	...
	if (!listId || !questionId) {
		return Response.json({ error: "listId and questionId are required" }, { status: 400 });
	}

	// Fetch survey (for question metadata) and responses in parallel
	const [listResult, responsesResult] = await Promise.all([
		supabase.from("research_links").select("questions").eq("id", listId).eq("account_id", accountId).maybeSingle(),
		supabase.from("research_link_responses").select("responses, completed").eq("research_link_id", listId),
	]);
	...
	const questions = Array.isArray(listResult.data.questions) ? listResult.data.questions : [];
	const rawQuestion = questions.find((q: unknown) => (q as Record<string, unknown>).id === questionId);
	if (!rawQuestion) {
	...
	if (question.type === "single_select" || question.type === "multi_select" || question.type === "image_select") {
		...
		stats.likertAvg = likertCount > 0 ? Math.round((sum / likertCount) * 10) / 10 : null;
	} else {
