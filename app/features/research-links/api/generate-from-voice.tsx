...
const GuidelineSchema = z.object({
  summary: z
    .string()
    .describe(
      "Human-readable summary like 'For sponsors, focus on budget questions'",
    ),
  triggerQuestionIndex: z
    .number()
    .describe("Which question triggers this rule (0-indexed)"),
  triggerValue: z.string().describe("What value/response triggers this rule"),
  action: z.enum(["skip_to", "end_survey"]),
  targetQuestionIndex: z
    .number()
    .optional()
    .describe("For skip_to: which question to jump to"),
  guidance: z
    .string()
    .optional()
...
const SurveyGenerationSchema = z.object({
  ...
  insights: z
    .string()
    .optional()
    .describe("Any insights about the user's research goals"),
});
...
Generate a survey that will help the user gather the insights they described.`,
    ...
    const guidelines = (result.object.guidelines ?? []).map((g, index) => {
      ...
      const targetQuestion =
        g.targetQuestionIndex !== undefined
          ? questions[g.targetQuestionIndex]
          : null;
