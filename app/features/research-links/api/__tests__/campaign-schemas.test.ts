...
describe("CreateCampaignSchema", () => {
	...
	it("accepts all strategy types", () => {
		for (const strategy of ["pricing_validation", "sparse_data_discovery", "theme_validation", "general_research"]) {
...
describe("generateCampaignName", () => {
	...
	it("generates correct name for each strategy", () => {
		...
		expect(generateCampaignName("pricing_validation", date)).toBe("Pricing Validation - Feb 11");
		expect(generateCampaignName("sparse_data_discovery", date)).toBe("Discovery Campaign - Feb 11");
		expect(generateCampaignName("theme_validation", date)).toBe("Theme Validation - Feb 11");
		expect(generateCampaignName("general_research", date)).toBe("Research Campaign - Feb 11");
	});

	it("falls back for unknown strategy", () => {
		const date = new Date(2026, 2, 15); // month is 0-indexed
		expect(generateCampaignName("unknown", date)).toBe("Survey Campaign - Mar 15");
	});
});
