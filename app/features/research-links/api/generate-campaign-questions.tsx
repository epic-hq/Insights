...
import {
  fetchPersonContext,
  fetchProjectContext,
} from "~/features/research-links/lib/person-context.server";
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (!accountId || !projectId) {
    return Response.json(
      { error: "Missing account or project ID" },
      { status: 400 },
    );
  }
  ...
  for (const survey of surveys) {
    try {
      const personContext = await fetchPersonContext(
        supabase,
        survey.person_id,
        projectId,
      );

      const surveyGoal = survey.survey_goal as
        | "validate"
        | "discover"
        | "deep_dive"
        | "pricing";
      ...
      consola.error(
        `Failed to generate questions for person ${survey.person_id}:`,
        error,
      );
      results.push({
        personId: survey.person_id,
