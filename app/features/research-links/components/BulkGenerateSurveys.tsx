...
import {
  SurveyQuickView,
  type PersonalizedSurveyItem,
} from "./SurveyQuickView";
...
export function BulkGenerateSurveys({
  open,
  onOpenChange,
  selectedPeople,
  onComplete,
}: BulkGenerateSurveysProps) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "[BulkGenerate] Generation response:",
  ...
  const handleEditQuestion = useCallback(
    async (surveyId: string, questionId: string, newText: string) => {
      ...
      <Dialog open={open && !quickViewOpen} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Create AI-personalized survey questions for{" "}
          ...
          {step === "done" && (
            <div className="flex flex-col items-center gap-3 py-6">
              ...
              <p className="text-sm font-medium">
                Generated surveys for {progress.current} of {progress.total}{" "}
                people
              </p>
              {progress.current < progress.total && (
                <p className="text-xs text-muted-foreground">
                  {progress.total - progress.current} failed — you can retry
                  later
                </p>
              )}
            </div>
