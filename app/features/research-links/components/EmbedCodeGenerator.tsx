...
export function EmbedCodeGenerator({
  ...
  const emailPreviewButtonLabel =
    heroCtaLabel || config.buttonText || "Open survey";
...
<script src="https://getupsight.com/embed.js" async></script>`;
  ...
  const emailPreviewCode = useMemo(() => {
    ...
    const emailPreviewImageSrc = hasThumbnail
    ...
    <div className="min-w-0 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {LAYOUT_OPTIONS.map((option) => (
            <button
              ...
              type="button"
              onClick={() =>
                setConfig((prev) => ({ ...prev, layout: option.value }))
              }
              className={cn(
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Select
            value={config.theme}
            onValueChange={(v) =>
              setConfig((prev) => ({ ...prev, theme: v as EmbedTheme }))
            }
          >
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(v) =>
              setConfig((prev) => ({ ...prev, borderRadius: Number(v) }))
            }
          >
            <SelectTrigger className="h-9">
              <SelectValue />
        ...
        <div className="space-y-2 sm:col-span-2">
          ...
          <Input
            ...
            onChange={(e) =>
              setConfig((prev) => ({ ...prev, successMessage: e.target.value }))
            }
        ...
        <div className="space-y-2 sm:col-span-2">
          ...
          />
          <p className="text-muted-foreground text-xs">
            Email clients can’t play video. Use a thumbnail or GIF that links to
            your survey. If you leave this empty, the HTML uses a placeholder
            you should replace.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between rounded-md border px-3 py-2 sm:col-span-2">
          <div>
            ...
            <p className="text-muted-foreground text-xs">
              Display "Powered by UpSight"
            </p>
          </div>
          <Switch
            checked={config.showBranding}
            onCheckedChange={(v) =>
              setConfig((prev) => ({ ...prev, showBranding: v }))
            }
          />
        </div>
      </div>
        ...
        <CardContent className="min-w-0 overflow-hidden">
          <Tabs defaultValue="html" className="w-full min-w-0 overflow-hidden">
            ...
            <TabsContent value="html" className="min-w-0 space-y-2">
              <p className="text-muted-foreground text-xs">
                Best for most websites. Add this code where you want the form to
                appear.
              </p>
            ...
            <TabsContent value="advanced" className="min-w-0 space-y-2">
              <p className="text-muted-foreground text-xs">
                Programmatic API for SPAs and custom integrations.
              </p>
              <div className="relative min-w-0 overflow-hidden">
                <pre className="max-h-[200px] overflow-auto rounded-lg bg-muted p-3 font-mono text-xs">
                  {scriptCode}
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-muted-foreground text-xs">
          <p>
            <strong className="text-foreground">For waitlists:</strong> Use the
            Compact or Email Capture layout. Set a compelling button text like
            "Join Waitlist" or "Get Early Access".
          </p>
          <p>
            <strong className="text-foreground">For feedback:</strong> Use the
            Full Form layout to show your video introduction and collect
            detailed responses.
          </p>
          <p>
            <strong className="text-foreground">Custom events:</strong> Listen
            for <code className="rounded bg-muted px-1">upsight:signup</code>{" "}
            events to trigger custom actions when someone submits their email.
          </p>
        </CardContent>
      </Card>
    </div>
