...
function QuestionAdvancedSection({
  ...
  const hasContent =
    Boolean(question.helperText) ||
    Boolean(question.videoUrl) ||
  ...
  const badgeCount = [
    question.helperText,
    question.videoUrl,
    question.branching?.rules?.length,
      ...
      const remaining = questions
        ...
        .map((question) => {
          ...
          const cleanedRules = question.branching.rules.filter((rule) => {
            // Remove rule if it targets the deleted question
            if (rule.targetQuestionId === id) return false;
            // Remove rule if any condition references the deleted question
                ...
                {(question.type === "single_select" ||
                  ...
                  <div className="space-y-2">
                    ...
                    {question.imageOptions?.length ? (
                      ...
                      // Text options mode - use OptionsInput for proper comma handling
                      <OptionsInput
