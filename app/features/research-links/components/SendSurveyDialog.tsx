...
export function SendSurveyDialog({
	...
	const [subject, setSubject] = useState(`Quick feedback request: ${surveyName}`);
	...
	const filteredPeople = useMemo(() => {
		...
		return people
			.filter((p) => p.primary_email && !addedEmails.has(p.primary_email.toLowerCase()))
			.filter(
				(p) =>
					!peopleSearch ||
	...
	const addEmailsFromInput = useCallback(() => {
		...
		for (const email of emails) {
			...
			const matched = people.find((p) => p.primary_email?.toLowerCase() === email.toLowerCase());
			newRecipients.push({
				email,
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
				...
				{step === "recipients" ? (
					<div className="space-y-4">
						...
						<div className="space-y-1.5">
							<Label className="text-muted-foreground text-xs">Add emails</Label>
						...
						{people.length > 0 && (
							<div className="space-y-1.5">
								<Label className="text-muted-foreground text-xs">From your contacts</Label>
								<Input
									...
									className="h-9"
								/>
								{(peopleSearch || filteredPeople.length <= 8) && filteredPeople.length > 0 && (
									<div className="max-h-40 overflow-y-auto rounded-md border">
										{filteredPeople.map((person) => (
											...
											>
												...
												<span className="ml-auto truncate text-muted-foreground text-xs">{person.primary_email}</span>
											</button>
										))}
									</div>
								)}
						...
						{recipients.length > 0 && (
							<div className="space-y-1.5">
								<Label className="text-muted-foreground text-xs">Recipients ({recipients.length})</Label>
								<div className="flex max-h-32 flex-wrap gap-1.5 overflow-y-auto rounded-md border bg-muted/20 p-2">
									{recipients.map((r) => (
										<Badge key={r.email} variant="secondary" className="gap-1 pr-1 text-xs">
					...
					<div className="space-y-4">
						...
						<div className="space-y-1.5">
							...
							<div className="rounded-md border bg-white p-4 text-sm dark:bg-zinc-950">
								<p className="mb-1 text-muted-foreground text-xs">From: {fromEmail}</p>
								<p className="mb-1 text-muted-foreground text-xs">
									To: {recipients.length} recipient
									{recipients.length === 1 ? "" : "s"}
								</p>
								<p className="mb-3 text-muted-foreground text-xs">Subject: {subject}</p>
								<div className="border-t pt-3">
									<p className="mb-2 text-foreground">Hi {recipients[0]?.name || "there"},</p>
									<p className="mb-3 text-foreground">
										{customMessage || "I'd love to get your feedback. It only takes a few minutes."}
									</p>
									<div className="mb-3">
										<span className="inline-block rounded-md bg-sky-600 px-4 py-2 font-medium text-white text-xs">
											Take the Survey
										</span>
									</div>
				...
				<DialogFooter className="gap-2 sm:gap-0">
					{step === "preview" && (
						<Button type="button" variant="ghost" size="sm" onClick={() => setStep("recipients")} disabled={isSending}>
							Back
						</Button>
					)}
					{step === "recipients" ? (
