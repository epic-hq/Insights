...
function createMockSupabase(overrides: Record<string, unknown> = {}) {
  ...
  const defaultThemes = [
    { theme_name: "Onboarding friction" },
    { theme_name: "Customer retention" },
  ];
  ...
  return {
    from: vi.fn((table: string) => {
      if (table === "people")
        return chainable(
          "person" in overrides ? overrides.person : defaultPerson,
        );
      if (table === "person_organization")
        return chainable("org" in overrides ? overrides.org : defaultOrg);
      if (table === "person_scale")
        return chainable("icp" in overrides ? overrides.icp : defaultIcp);
      ...
      if (table === "evidence_people")
        return chainable(
          "lastEvidence" in overrides ? overrides.lastEvidence : null,
        );
...
describe("fetchPersonContext", () => {
  ...
  it("returns complete context for a rich profile", async () => {
    ...
    const { fetchPersonContext } =
      await import("~/features/research-links/lib/person-context.server");

    const ctx = await fetchPersonContext(
      mockSb as never,
      "person-1",
      "project-1",
    );
    ...
    expect(ctx.conversation_themes).toEqual([
      "Onboarding friction",
      "Customer retention",
    ]);
  ...
  it("computes correct ICP bands", async () => {
    ...
    // Strong: >= 0.7
    const strong = await fetchPersonContext(
      createMockSupabase({ icp: { score: 0.75 } }) as never,
      "p",
      "proj",
    );
    ...
    // Moderate: >= 0.5
    const moderate = await fetchPersonContext(
      createMockSupabase({ icp: { score: 0.55 } }) as never,
      "p",
      "proj",
    );
    ...
    // Weak: < 0.5
    const weak = await fetchPersonContext(
      createMockSupabase({ icp: { score: 0.3 } }) as never,
      "p",
      "proj",
    );
  ...
  it("throws when person not found", async () => {
    ...
    await expect(
      fetchPersonContext(mockSb as never, "nonexistent", "project-1"),
