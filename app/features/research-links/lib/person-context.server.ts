...
export async function fetchPersonContext(
  ...
  const [
    ...
    supabase
      .from("people")
      .select("id, firstname, lastname, title, primary_email")
      .eq("id", personId)
      .single(),
  ...
  return {
    ...
    conversation_themes: (themesResult.data || []).map(
      (t: { theme_name: string }) => t.theme_name,
    ),
...
export async function fetchProjectContext(
  ...
  return {
    research_goals: project?.description
      ? [project.description]
