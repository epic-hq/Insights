...
export interface Recommendation {
	...
	reasoning: string;
	/** The type of action */
	actionType: "setup" | "interview" | "survey" | "validate" | "deep_dive" | "analyze" | "decide" | "data_quality";
 ...
 */
export function determineProjectStage(context: ProjectResearchContext): ProjectStage {
...
export function generateRecommendations(context: ProjectResearchContext): Recommendation[] {
	...
	if (!context.hasGoals) {
		recommendations.push({
			...
			description: "Define your research goals to get personalized guidance and recommendations.",
	...
	if (context.interviewCount === 0 && context.surveyCount === 0) {
		recommendations.push({
			...
			description: "Run discovery interviews or create a survey to learn about your customers.",
	...
	if (context.interviewCount > 0 && context.themes.length === 0 && recommendations.length < 3) {
		recommendations.push({
	...
	const highEvidenceThemes = context.themes.filter((t) => t.evidence_count >= 5);
	...
	if (pricingThemes.length > 0 && recommendations.length < 3) {
		...
		recommendations.push({
			...
			description: "Understand willingness to pay and pricing sensitivity with a focused survey.",
	...
	if (!hasRecentSurvey && context.surveyResponseCount > 0 && recommendations.length < 3) {
		recommendations.push({
			id: "nps-check",
			priority: 6,
			title: "NPS & satisfaction check",
			description: "Measure customer satisfaction and loyalty with a quick NPS survey.",
	...
	if (context.themes.length >= 5 && context.interviewCount >= 5 && recommendations.length < 3) {
		recommendations.push({
			id: "synthesize-findings",
			priority: 7,
			title: "Synthesize your findings",
			description: "You have enough data to draw conclusions. Review themes and make decisions.",
 ...
 */
export function getSurveyRecommendations(context: ProjectResearchContext): Recommendation[] {
	...
	const surveyActionable = allRecommendations.filter(
		(r) => r.actionType === "survey" || r.actionType === "validate" || r.actionType === "deep_dive"
	);
		...
		return [
			{
				...
				description: "Learn about customer pains, needs, and motivations with a broad discovery survey.",
				reasoning: "Start with open-ended questions to identify themes and patterns.",
				actionType: "survey",
				navigateTo: "/ask/new",
				metadata: {
