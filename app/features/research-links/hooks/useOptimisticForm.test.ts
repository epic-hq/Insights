...
import { extractFormFields, type SurveyFormFields, serializeToFormData } from "./useOptimisticForm";
...
function makeFields(overrides: Partial<SurveyFormFields> = {}): SurveyFormFields {
	return {
		name: "My Survey",
		slug: "my-survey",
		heroTitle: "Welcome",
...
describe("round-trip: loader → fields → formData → schema", () => {
	it("produces valid action payload from loader data", () => {
		const list = makeLoaderList();
		const questions = [makeQuestion({ id: "q1", prompt: "What do you think?" })];
