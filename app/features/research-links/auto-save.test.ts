...
// ============================================================================
function cleanupBranchingOnDelete(
  ...
  return questions
    ...
    .map((question) => {
      ...
      const cleanedRules = question.branching.rules.filter((rule) => {
        ...
        if (rule.conditions.conditions.some((c) => c.questionId === deletedId))
          return false;
        return true;
      });
      const cleanedDefaultNext =
        question.branching.defaultNext === deletedId
          ? undefined
...
describe("buildFormData", () => {
  ...
  it("should serialize questions as JSON", () => {
    const questions = [
      makeQuestion({ id: "q1" }),
      makeQuestion({ id: "q2", prompt: "Second" }),
    ];
    const fd = buildFormData({
      name: "Test",
      slug: "test",
        ...
        rules: [
          ...
          makeRule({
            ...
            conditions: {
              logic: "and",
              conditions: [
                { questionId: "q3", operator: "equals", value: "B" },
              ],
            },
            targetQuestionId: "q4",
          }),
