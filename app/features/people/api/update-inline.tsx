...
export const action: ActionFunction = async ({ context, request, params }) => {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      newOrganizationName =
        (formData.get("newOrganizationName") as string) || undefined;
    ...
    if (field === "organization") {
      ...
      if (newOrganizationName && !organizationId) {
        ...
        if (createError) {
          consola.error(
            "[people/update-inline] Create org error:",
            createError,
          );
          return Response.json(
      ...
      if (targetOrgId) {
        ...
        if (linkError) {
          ...
          return Response.json(
            { error: `Failed to link organization: ${linkError.message}` },
            { status: 500 },
          );
        }
        ...
        if (defaultOrgError) {
          consola.error(
            "[people/update-inline] Default org update error:",
            defaultOrgError,
          );
        }
    ...
    if (orgField && organizationId) {
      ...
      if (allowedOrgFields.includes(orgField)) {
        ...
        if (updateError) {
          consola.error(
            "[people/update-inline] Org update error:",
            updateError,
          );
          return Response.json(
      ...
      if (allowedLinkFields.includes(orgField)) {
        ...
        if (updateError) {
          consola.error(
            "[people/update-inline] Link update error:",
            updateError,
          );
          return Response.json(
      ...
      return Response.json(
        { error: `Organization field '${orgField}' is not editable` },
        { status: 400 },
      );
    }
