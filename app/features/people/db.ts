...
export const updatePerson = async ({
	...
	if (error) {
		consola.error("[updatePerson] Supabase update failed:", {
			id,
			projectId,
			error,
		});
		throw new Response(`Failed to update person: ${error.message}`, {
			status: 500,
		});
	}
...
/**
 * Verify a person exists and belongs to the given project.
 * Returns the person row on success, or a Response error to return directly.
 */
export const verifyPersonBelongsToProject = async ({
	supabase,
	personId,
	projectId,
}: {
	...
	personId: string;
	projectId: string;
}): Promise<{ ok: true; person: { id: string; project_id: string | null } } | { ok: false; response: Response }> => {
	...
	if (personErr || !personRow) {
		return {
			ok: false,
			response: Response.json({ ok: false, error: "Selected person not found" }, { status: 400 }),
		};
	}
	if (personRow.project_id !== projectId) {
		return {
			ok: false,
			response: Response.json({ ok: false, error: "Selected person belongs to a different project" }, { status: 400 }),
		};
	}
	return { ok: true, person: personRow };
};
