...
import {
  PeopleDataTable,
  type PersonTableRow,
} from "~/features/people/components/PeopleDataTable";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const scope =
    (url.searchParams.get("scope") as "project" | "account") || "project";

  const [{ data: people, error }, catalog, { data: allOrganizations }] =
    await Promise.all([
  ...
  if (person_ids.length) {
    ...
    if (evidenceError) {
      consola.warn(
        "Failed to load evidence counts for people index",
...
export default function PeopleIndexPage() {
  const { people, catalog, evidence_counts, scope, organizationsList } =
  ...
  const handleMergeDuplicates = async (
    ...
    try {
      ...
      if (data.error) {
        ...
        alert(
          `Merged ${data.mergedIds.length} duplicate(s) into primary record.`,
        );
  ...
  const handleAutoMergeDuplicates = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        alert(
          `Auto-merged ${data.peopleMerged} duplicate(s) across ${data.groupsProcessed} groups.`,
        );
        setShowDuplicatesDialog(false);
        setDuplicateGroups([]);
        revalidator.revalidate();
    ...
    return people.map((person) => {
      const personFacets = (person.person_facet ?? []).map((row) => {
        ...
        return {
          ...
          kind_slug:
            facetMeta?.kind_slug || joinedFacet?.facet_kind_global?.slug || "",
  ...
  const organizations = useMemo(() => {
    ...
    for (const { person } of peopleWithFacets) {
      const primaryOrgLink =
        person.people_organizations?.find((link) => link.is_primary) ??
        person.people_organizations?.[0];
      const label =
        primaryOrgLink?.organization?.name ||
        primaryOrgLink?.organization?.website_url ||
        null;
      if (!label) continue;
      seen.add(label);
    }
  ...
  const stakeholder_statuses = useMemo(() => {
    ...
    for (const { person } of peopleWithFacets) {
      const primaryOrgLink =
        ...
        person.people_organizations?.[0];
      const status =
        primaryOrgLink?.relationship_status || person.lifecycle_stage || null;
      if (!status) continue;
      seen.add(status);
    }
    return Array.from(seen).sort((a, b) => a.localeCompare(b));
  ...
  const filteredPeopleWithFacets = useMemo(() => {
    ...
    return peopleWithFacets.filter(({ person }) => {
      ...
      const organization_label =
        ...
        primaryOrgLink?.organization?.website_url ||
        null;
      const stakeholder_status =
        primaryOrgLink?.relationship_status || person.lifecycle_stage || null;
      const job_title = person.title || null;

      if (
        organizationFilter !== "all" &&
        organization_label !== organizationFilter
      )
        return false;
      if (
        stakeholderFilter !== "all" &&
        stakeholder_status !== stakeholderFilter
      )
        return false;
      ...
      if (icpFilter !== "all") {
        const personScale = (person as Record<string, unknown>).person_scale as
          ...
          | undefined;
        const band =
          personScale?.find((s) => s.kind_slug === "icp_match")?.band ?? null;
  ...
  const tableRows = useMemo<PersonTableRow[]>(() => {
    return filteredPeopleWithFacets.map(({ person }) => {
      ...
      return {
        ...
        organization: primaryOrganization
          ? {
              ...
              name:
                ...
                primaryOrganization.website_url ||
                null,
        ...
        // Segment data
        jobFunction:
          (person as { job_function?: string | null }).job_function ?? null,
        seniority:
          (person as { seniority_level?: string | null }).seniority_level ??
          null,
      ...
      <PageContainer className="space-y-6">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="text-muted-foreground text-sm">
            {filteredPeopleWithFacets.length} results
          </div>
        </div>
        ...
        {people.length === 0 ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {filteredPeopleWithFacets.map(({ person, facets }) => {
              const personScale = (person as Record<string, unknown>)
                .person_scale as
                | Array<{
                    kind_slug: string;
                    band: string | null;
                ...
                | null
                | undefined;
              const icpMatch = personScale?.find(
                (s) => s.kind_slug === "icp_match",
              );
              return (
                <EnhancedPersonCard
                  key={person.id}
                  person={{
                    ...person,
                    people_personas: (person.people_personas || []).map(
                      (pp) => ({
      ...
      >
        <AlertDialogContent className="max-h-[80vh] max-w-2xl overflow-y-auto">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Found {duplicateGroups.length} groups of potential duplicate
              people. Review each group and choose how to handle them.
            </AlertDialogDescription>
          </AlertDialogHeader>
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="truncate text-muted-foreground text-sm">
                          {[person.title, (person as any).people_organizations?.[0]?.organization?.name || person.company, person.primary_email]
                            .filter(Boolean)
                            .join(" · ")}
                        </div>
                      </div>
