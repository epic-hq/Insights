...
export default function PersonDetail() {
  const { person, catalog, organizations, relatedAssets } =
  ...
  const quickFacts: Array<{ label: string; value: string }> = [
    ...
    person.updated_at
      ...
      : null,
    peoplePersonas.length
      ? { label: "Persona Links", value: String(peoplePersonas.length) }
      : null,
                        ...
                        >
                          <h3 className="font-medium text-foreground transition-colors hover:text-primary">
                            {link.interviews?.title ||
                              `Interview ${link.interviews?.id?.slice(0, 8) || "Unknown"}`}
                          </h3>
                    ...
                    >
                      <CardContent className="p-4">
                        ...
                        >
                          <h3 className="font-medium text-foreground transition-colors hover:text-primary">
                            {link.interviews?.title ||
                              `Note ${link.interviews?.id?.slice(0, 8) || "Unknown"}`}
                          </h3>
                          <p className="text-muted-foreground text-sm">
                            {link.interviews?.created_at &&
                              new Date(
