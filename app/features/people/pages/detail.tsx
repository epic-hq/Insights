	...
	if (intent === "enrich-person") {
		try {
			...
			if (result.data.industry) {
				const primaryOrgId = (primaryOrgLink?.organization as { id?: string } | null)?.id;
				if (primaryOrgId) {
					...
					if (orgErr) {
						consola.warn("[enrich-person] Failed to update org industry:", orgErr.message);
					}
				}
			}
...
export default function PersonDetail() {
	...
	const icpScaleEntry = useMemo(() => {
		...
		return scales.find((s) => s.kind_slug === "icp_match") ?? scales.find((s) => s.kind_slug === "icp") ?? null;
	}, [person]);
	...
	const handleDelete = () => {
		if (
			window.confirm(`Are you sure you want to delete ${person.name || "this person"}? This action cannot be undone.`)
		) {
	...
	useEffect(() => {
		if (hasIcpScore || isScoringICP || !hasIcpScorableSignals || autoIcpRequestedRef.current) return;
