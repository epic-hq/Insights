...
export async function loader({ params, context }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const surveyLinksForLoader = allInterviewLinksForLoader.filter(
      (ip: { interviews?: { source_type?: string } }) =>
        ip.interviews?.source_type === "survey_response",
    );
    const lastContactDates = allInterviewLinksForLoader
      .map(
        (link: { interviews?: { created_at?: string } }) =>
    ...
    const lastContactDateForLoader =
      lastContactDates.length > 0
        ? lastContactDates.reduce((latest: Date, d: Date) =>
            d > latest ? d : latest,
          )
        : null;

    const primaryOrgForLoader =
      person.people_organizations?.find(
        (po: { is_primary?: boolean | null }) => po.is_primary,
      )?.organization ??
      person.people_organizations?.[0]?.organization ??
      null;

    const icpScales = (person as Record<string, unknown>)
      .person_scale as Array<{
      kind_slug: string;
      score: number | null;
      band: string | null;
    }> | null;
    const icpEntry =
      icpScales?.find((s) => s.kind_slug === "icp_match") ?? null;
    ...
    const [facetSummaries, aiNextSteps] = await Promise.all([
      ...
      generatePersonNextSteps({
        ...
        companyName:
          (primaryOrgForLoader as { name?: string | null } | null)?.name ??
          null,
        icpMatch: icpEntry
          ? { band: icpEntry.band, score: icpEntry.score }
          : null,
...
export async function action({ request, params, context }: ActionFunctionArgs) {
  ...
  if (intent === "score-icp") {
    try {
      ...
      const { error: upsertError } = await supabase.from("person_scale").upsert(
        {
          person_id: personId,
      ...
      if (upsertError) {
        return { scoreIcp: { error: "Failed to save ICP score." } };
      }
      ...
      return {
        scoreIcp: {
          ...
          band: scoreBreakdown.band,
          confidence: scoreBreakdown.confidence,
          indeterminate: scoreBreakdown.overall_score == null,
        },
      };
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Failed to score ICP match.";
  ...
  if (intent === "enrich-person") {
    try {
      ...
      const orgName =
        (primaryOrgLink?.organization as { name?: string | null } | null)
          ?.name ?? null;
      ...
      // Persist only null fields on the person record
      const updates: Record<string, string | null> = {};
      if (result.data.title && !person.title) updates.title = result.data.title;
      if (result.data.role && !(person as Record<string, unknown>).role)
      ...
      // Industry belongs on the organization, not the person
      if (result.data.industry) {
        const primaryOrgId = (
          primaryOrgLink?.organization as { id?: string } | null
        )?.id;
        if (primaryOrgId) {
          const { error: orgErr } = await supabase
            ...
            .eq("id", primaryOrgId)
            .is("industry", null); // only fill if not already set
          if (orgErr) {
            consola.warn(
              "[enrich-person] Failed to update org industry:",
              orgErr.message,
            );
          }
        }
      ...
      // Track all updated fields for UI feedback (person + org)
      const allFieldsUpdated = [...Object.keys(updates)];
      if (result.data.industry) allFieldsUpdated.push("industry");
      ...
      // Re-throw Response objects so React Router handles them (e.g. from updatePerson)
      if (error instanceof Response) throw error;
      const message =
        error instanceof Error ? error.message : "Failed to enrich person.";
...
export default function PersonDetail() {
  ...
  const [pendingAction, setPendingAction] = useState<
    "refresh-description" | "score-icp" | "enrich-person" | null
  >(null);
  ...
  const fetcherData = refreshFetcher.data as
    | {
  ...
  const icpScaleEntry = useMemo(() => {
    ...
    return (
      scales.find((s) => s.kind_slug === "icp_match") ??
      scales.find((s) => s.kind_slug === "icp") ??
      null
    );
  }, [person]);

  const icpMatch = useMemo(() => {
    if (!icpScaleEntry) return null;
    return {
      band: icpScaleEntry.band,
      score: icpScaleEntry.score,
      confidence: icpScaleEntry.confidence,
    };
  }, [icpScaleEntry]);
  ...
  const isFetcherBusy = refreshFetcher.state !== "idle";
  const activeFetcherAction =
    (refreshFetcher.formData?.get("_action") as string | null) ?? pendingAction;
  const isRefreshingDescription =
    isFetcherBusy && activeFetcherAction === "refresh-description";
  ...
  const handleRefreshDescription = () => {
    setPendingAction("refresh-description");
  ...
  useEffect(() => {
    if (refreshFetcher.state !== "idle" || !pendingAction) return;
    const success =
  ...
  }, [fetcherData, pendingAction, refreshFetcher.state, revalidator]);

  useEffect(() => {
    if (
      hasIcpScore ||
      isScoringICP ||
      !hasIcpScorableSignals ||
      autoIcpRequestedRef.current
    )
      return;
    autoIcpRequestedRef.current = true;
    setPendingAction("score-icp");
    refreshFetcher.submit({ _action: "score-icp" }, { method: "post" });
  }, [hasIcpScorableSignals, hasIcpScore, isScoringICP, refreshFetcher]);
      ...
      <PageContainer className="space-y-8 pb-16">
        ...
        <section id="scorecard">
          <PersonScorecard
            ...
            icpScored={hasIcpScore}
            isScoringICP={isScoringICP}
            showAutoScoringHint={
              !hasIcpScore && (isScoringICP || autoIcpRequestedRef.current)
            }
