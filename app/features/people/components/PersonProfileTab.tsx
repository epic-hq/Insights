...
export function PersonProfileTab({
  ...
  const hasSegmentData =
    person.job_function || person.seniority_level || industryValue;

  // Empty state
  const hasFacetLenses = facetLensGroups.length > 0;
  const hasAnyContent =
    hasFacetLenses || hasSegmentData || hasContactInfo || icpMatch;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="max-w-md text-muted-foreground text-sm">
          Profile data will appear here as you add conversations and evidence
          ...
          {hasSegmentData && (
            <div className="flex flex-wrap gap-2">
              {jobFunctionLabel && (
                <Badge
                  variant="outline"
                  className="gap-1.5 border-border/60 bg-muted/30"
                >
            ...
            <div className="flex items-center gap-3">
              ...
              {icpMatch.score != null && (
                <span className="text-muted-foreground text-xs">
                  Score: {Math.round(icpMatch.score * 100)}%
                </span>
              )}
            </div>
          </CardContent>
              ...
              <CollapsibleTrigger asChild>
                <Button variant="ghost" size="sm" className="h-8 w-8 p-0">
                  ...
                  <span className="sr-only">
                    {contactExpanded ? "Collapse" : "Expand"}
                  </span>
                </Button>
              </CollapsibleTrigger>
            </div>
              ...
              {!hasPrimaryEmail &&
                ...
                contactItems.length === 0 && (
                  <p className="py-4 text-center text-muted-foreground text-sm">
                    No contact information available. Use the fields above to
                    add email, phone, or LinkedIn.
                  </p>
                )}
            </CardContent>
          </CollapsibleContent>
