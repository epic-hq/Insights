...
function EditableSelectCell({
	...
	return (
		<Select value={displayValue ?? ""} onValueChange={handleChange} disabled={isPending}>
			<SelectTrigger className={cn("h-7 w-[130px] text-xs", isPending && "opacity-70")}>
				{isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <SelectValue placeholder={placeholder || "—"} />}
			</SelectTrigger>
			<SelectContent>
...
function CompanySizeCell({
	...
	return (
		<Select value={displayValue ?? ""} onValueChange={handleChange} disabled={isPending}>
			<SelectTrigger className={cn("h-7 w-[110px] text-xs", isPending && "opacity-70")}>
				{isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <SelectValue placeholder="—" />}
			</SelectTrigger>
			<SelectContent>
				<SelectItem value="__clear__">
					<span className="text-muted-foreground">Clear</span>
