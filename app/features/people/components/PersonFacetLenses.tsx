    ...
    <section className="space-y-3">
      ...
      >
        {groups.map((group) => {
          ...
          const linkedFacetIds = new Set(
            group.facets.map((f) => f.facet_account_id),
          );
          const unlinkedFacets = availableFacets.filter(
            (f) => !linkedFacetIds.has(f.id),
          );
          // Sort facets by confidence (highest first)
          const sortedFacets = [...group.facets].sort(
          ...
          return (
            ...
            >
              <AccordionTrigger className="group flex w-full items-center gap-3 px-4 py-3 text-left hover:no-underline">
                ...
                >
                  <iconConfig.icon
                    className={cn("h-4 w-4", iconConfig.color)}
                  />
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sortedFacets.length === 0 ? (
                      <p className="text-muted-foreground text-xs italic">
                        No signals yet. Add one to get started.
                      </p>
                    ) : (
                      sortedFacets.map((facet) => (
                        ...
                        >
                          ...
                          <Button
                            type="button"
                            onClick={() =>
                              handleRemoveFacet(facet.facet_account_id)
                            }
                            variant="ghost"
                            size="icon"
