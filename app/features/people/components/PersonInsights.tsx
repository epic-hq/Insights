...
import { ChevronDown, ChevronUp, RefreshCw, Sparkles, Target } from "lucide-react";
...
function generateNextSteps({
	...
	conversationCount,
	icpMatch,
}: Pick<PersonInsightsProps, "lastContactDate" | "surveyCount" | "conversationCount" | "icpMatch">): NextStep[] {
	...
	if (!hasDescription && !hasThemes) {
		return (
			<div className="flex flex-col items-center justify-center rounded-xl border border-border/60 bg-card py-16 text-center">
				...
				<h3 className="mb-1 font-semibold text-base text-foreground">No insights yet</h3>
				<p className="max-w-sm text-muted-foreground text-sm">
					Add conversations or surveys linked to this person to generate AI-powered insights, themes, and next steps.
				</p>
			</div>
		);
	}
		...
		<div className="rounded-xl border border-border/60 bg-card">
			...
			{hasThemes && (
				<div className="px-6 pb-5">
					<h3 className="mb-3 font-semibold text-muted-foreground text-sm">Themes</h3>
