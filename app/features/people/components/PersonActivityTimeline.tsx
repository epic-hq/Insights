 ...
 * for long lists. Survey/research data appears in a dedicated section at the bottom.
 */
...
export function PersonActivityTimeline({
	...
	const hasImportedData = surveyResponses.length > 0 || researchLinkResponses.length > 0;

	// ---- Categories that actually have items (for filter pills) ----

	const availableCategories = useMemo(() => {
		const categories: Exclude<SourceFilter, "all">[] = ["interviews", "notes", "surveys", "chats", "assets"];
	...
	if (allItems.length === 0 && !hasImportedData) {
		return (
			<div className="flex flex-col items-center justify-center py-16 text-center">
				...
				<h3 className="mb-1 font-medium text-base text-foreground">No activity yet</h3>
		...
		<div className="space-y-6">
					...
					{visibleItems.map((item) => {
						...
						return (
							<Link
								key={item.id}
								to={item.linkTo}
								className="group relative grid grid-cols-[4rem_1rem_1fr] items-center gap-x-2 rounded-md px-2 py-2.5 transition-colors hover:bg-muted/50"
							>
								{/* Date on left */}
			...
			{filteredItems.length === 0 && allItems.length > 0 && (
				<div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12 text-center">
					<p className="text-muted-foreground text-sm">No {filter === "all" ? "activity" : `${filter}`} found.</p>
				</div>
			)}
			...
			{hasImportedData && (
				<div>
					<div className="flex items-center gap-2 px-4 py-3">
						<ClipboardList className="h-4 w-4 text-muted-foreground" />
						<span className="flex-1 font-medium text-sm">Surveys</span>
								...
								<Card key={response.id}>
									<CardHeader className="pb-2">
										<div className="flex items-center justify-between">
											<CardTitle className="text-sm">{response.research_links?.name || "Survey"}</CardTitle>
											<div className="flex items-center gap-2">
												<Badge variant={response.completed ? "default" : "secondary"}>
													{response.completed ? "Responded" : "In Progress"}
												</Badge>
											...
											answeredQuestions.map((question) => {
												...
												return (
													<div key={question.id} className="space-y-1">
														...
														<p className="text-foreground text-sm">{formatResearchAnswer(answer, question)}</p>
													</div>
												);
											})
										) : (
