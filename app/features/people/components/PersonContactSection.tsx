...
import { ExternalLink, Globe, Instagram, Linkedin, Mail, MapPin, Pencil, Phone, Plus, Twitter } from "lucide-react";
 ...
 */
function parseContactItems(person: PersonContactSectionProps["person"]): ContactItem[] {
	...
	if (info) {
		...
		if (info.website) {
			const displayValue = info.website.replace(/^https?:\/\/(www\.)?/, "").replace(/\/$/, "");
			items.push({
				type: "Website",
				value: info.website,
				displayValue,
...
export function PersonContactSection({ person, embedded = false }: PersonContactSectionProps) {
	...
	const content = (
		<>
			...
			<div className="mb-3 flex items-center justify-between">
				<p className="font-semibold text-muted-foreground text-xs uppercase tracking-wider">Contact</p>
				{hasAnyContact && (
					<Button
						type="button"
			...
			{isEditing && (
				<div className="grid gap-4 sm:grid-cols-3">
					...
					<div className="space-y-1">
						<label className="text-[11px] text-muted-foreground uppercase tracking-wide">LinkedIn</label>
						<InlineEditableField
							value={person.linkedin_url}
							entityId={person.id}
							entityIdKey="personId"
			...
			{!isEditing && hasAnyContact && (
				<div className="flex flex-wrap items-center gap-y-1">
					{contactItems.map((item, index) => {
						...
						const isExternal = item.href && !item.href.startsWith("mailto:") && !item.href.startsWith("tel:");

						return (
							<span key={`${item.type}-${item.value}`} className="flex items-center">
								{index > 0 && <span className="mx-2 text-muted-foreground/50 text-xs">&middot;</span>}
								{item.href ? (
									<a
										href={item.href}
									...
									>
										...
										{isExternal && <ExternalLink className="h-3 w-3 shrink-0 text-muted-foreground/50" />}
									</a>
								) : (
			...
			{!isEditing && !hasAnyContact && (
				<div className="space-y-2">
					<p className="text-muted-foreground/60 text-sm italic">No contact information yet.</p>
					<Button
						type="button"
	...
	return <div className="rounded-xl border border-border/60 bg-card px-5 py-4">{content}</div>;
}
