...
export function PersonOverviewTab({
  ...
  const recentEvidence = allInterviewLinks
    ...
    .sort((a, b) => {
      const dateA = a.interviews?.created_at
        ? new Date(a.interviews.created_at).getTime()
        : 0;
      const dateB = b.interviews?.created_at
        ? new Date(b.interviews.created_at).getTime()
        : 0;
      return dateB - dateA;
    })
    .slice(0, 3);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="max-w-md text-muted-foreground text-sm">
          Add conversations linked to this person to see key takeaways, themes,
          and insights.
        </p>
      </div>
    );
          ...
          <CardContent>
            <div className="space-y-3">
              {recentEvidence.map((link) => {
                ...
                const { icon, label } = getSourceInfo(
