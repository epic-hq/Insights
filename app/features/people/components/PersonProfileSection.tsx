...
function DemographicStat({ label, value }: { label: string; value: string | null }) {
	return (
		<div className="space-y-1">
			<p className="text-[11px] text-muted-foreground uppercase tracking-wide">{label}</p>
			<p className={cn("font-medium text-sm", value ? "text-foreground" : "text-muted-foreground/60 italic")}>
				{value || "---"}
			</p>
		</div>
	);
		...
		return (
			<div className="rounded-xl border border-border/60 bg-card p-5">
				<div className="flex flex-col items-center justify-center py-8 text-center">
					...
					<p className="font-medium text-foreground text-sm">No profile data yet</p>
					<p className="mt-1 max-w-xs text-muted-foreground text-xs">
						Profile data will appear here as conversations and evidence are linked to this person.
					</p>
				</div>
			</div>
		);
	...
	return (
		<div className="rounded-xl border border-border/60 bg-card p-5">
			...
			{hasOrganizations ? (
				<div>
					<div className="mb-3 flex items-center justify-between">
						<p className="font-semibold text-muted-foreground text-xs uppercase tracking-wider">Organizations</p>
						<LinkOrganizationDialog
							personId={person.id}
							availableOrganizations={availableOrganizations}
							triggerButton={
								<button type="button" className="cursor-pointer text-primary text-xs hover:underline">
									Manage
								</button>
							}
						/>
								...
								<div key={link.id} className="flex items-center gap-2 py-2">
									...
									{link.job_title && (
										<>
											...
											<span className="text-muted-foreground text-sm">{link.job_title}</span>
										</>
									)}
									{link.is_primary && (
										<Badge variant="outline" className="ml-1 px-1.5 py-0 text-[10px] uppercase">
											Primary
										</Badge>
									)}
								</div>
				...
				<div>
					...
					<p className="text-muted-foreground/60 text-sm italic">No organizations linked yet.</p>
				</div>
			)}
			...
			{hasDemographics || isEditing ? (
				<div>
					<div className="mb-3 flex items-center justify-between">
						<p className="font-semibold text-muted-foreground text-xs uppercase tracking-wider">Demographics</p>
						<button
							type="button"
					...
					{isEditing ? (
						<div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
							<div className="space-y-1">
								<label className="text-[11px] text-muted-foreground uppercase tracking-wide">Job Function</label>
								<InlineEditableField
							...
							<div className="space-y-1">
								<label className="text-[11px] text-muted-foreground uppercase tracking-wide">Seniority</label>
								<InlineEditableField
							...
							<div className="space-y-1">
								<label className="text-[11px] text-muted-foreground uppercase tracking-wide">Industry</label>
								<InlineEditableField
							...
							<div className="space-y-1">
								<label className="text-[11px] text-muted-foreground uppercase tracking-wide">Company Size</label>
								{primaryOrg ? (
									<InlineEditableField
				...
				<div>
					...
					<p className="pb-2 text-muted-foreground/60 text-sm italic">
						No demographic data. Click &ldquo;Add&rdquo; to set job function, seniority, or industry.
					</p>
				</div>
			)}
