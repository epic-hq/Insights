...
export async function calculateICPScore(opts: {
  ...
  const roleResult = scoreRoleMatch(
    person as PersonWithOrg,
    criteria.target_roles,
  );
  const orgResult = scoreOrgMatch(
    person as PersonWithOrg,
    criteria.target_orgs,
  );
  const sizeResult = scoreSizeMatch(
    person as PersonWithOrg,
  ...
  if (criteria.target_roles.length > 0)
    criteriaDimensions.push({ result: roleResult, weight: 0.4 });
  if (criteria.target_orgs.length > 0)
    criteriaDimensions.push({ result: orgResult, weight: 0.3 });
  if (criteria.target_size_ranges.length > 0)
    criteriaDimensions.push({ result: sizeResult, weight: 0.3 });
