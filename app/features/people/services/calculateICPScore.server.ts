...
async function scoreFacetMatch(
  ...
  const matchedFacets = targetFacets.filter((tf) =>
    personFacetIds.has(tf.facet_account_id),
  );
...
export async function calculateICPScore(opts: {
  ...
  const [roleResult, orgResult, sizeResult, facetResult] = await Promise.all([
    Promise.resolve(
      scoreRoleMatch(person as PersonWithOrg, criteria.target_roles),
    ),
    Promise.resolve(
      scoreOrgMatch(person as PersonWithOrg, criteria.target_orgs),
    ),
    Promise.resolve(
      scoreSizeMatch(person as PersonWithOrg, criteria.target_size_ranges),
    ),
  ...
  const criteriaDimensions: Array<{ result: DimensionResult; weight: number }> =
    [];
  if (criteria.target_roles.length > 0)
    criteriaDimensions.push({ result: roleResult, weight: 0.33 });
  if (criteria.target_orgs.length > 0)
    criteriaDimensions.push({ result: orgResult, weight: 0.25 });
  if (criteria.target_size_ranges.length > 0)
    criteriaDimensions.push({ result: sizeResult, weight: 0.22 });
  if (criteria.target_facets.length > 0)
    criteriaDimensions.push({ result: facetResult, weight: 0.2 });
  ...
  const overall_score = scorableDimensions.reduce(
    (sum, d) => sum + (d.result.score * d.weight) / totalWeight,
    0,
  );

  // Confidence = how many dimensions we could actually score
  const confidence =
    dimensions_total > 0 ? dimensions_scored / dimensions_total : 0;
