 ...
 */
function scoreRoleMatch(person: PersonWithOrg, targetRoles: string[]): { score: number; matched?: string } {
 ...
 * Returns { score: 0-1, matched: string | undefined }
 */
function scoreOrgMatch(person: PersonWithOrg, targetOrgs: string[]): { score: number; matched?: string } {
 ...
 * Returns { score: 0-1, matched: string | undefined }
 */
function scoreSizeMatch(person: PersonWithOrg, targetSizes: string[]): { score: number; matched?: string } {
...
export async function calculateICPScore(opts: {
	...
	const roleMatch = scoreRoleMatch(person as PersonWithOrg, criteria.target_roles)
	const orgMatch = scoreOrgMatch(person as PersonWithOrg, criteria.target_orgs)
	const sizeMatch = scoreSizeMatch(person as PersonWithOrg, criteria.target_size_ranges)

	// Calculate weighted average (0.4, 0.3, 0.3)
	const rawScore = roleMatch.score * 0.4 + orgMatch.score * 0.3 + sizeMatch.score * 0.3
