...
export async function resolveInternalPerson({
  ...
  if (!resolvedSettings) {
    const { data, error } = await supabase
      .from("user_settings")
      .select(
        "first_name, last_name, title, job_function, company_name, industry, email, image_url",
      )
      .eq("user_id", userId)
      .maybeSingle();
    if (error) {
  ...
  const { data: existingByUser, error: existingByUserError } = await supabase
    .from("people")
    .select(
      "id, name, firstname, lastname, primary_email, image_url, title, job_function, person_type, user_id",
    )
