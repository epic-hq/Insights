...
export function standardizeSizeRange(raw: string | null | undefined): string | null {
	if (!raw) return null;
...
export async function syncOrgDataToPersonFacets({
	...
	if (orgData.industry) {
		try {
			...
			consola.error("[syncOrgDataToPersonFacets] Error syncing industry:", error);
		}
	}
	...
	if (standardSize) {
		...
		try {
			...
			consola.error("[syncOrgDataToPersonFacets] Error syncing company_size:", error);
		}
	}
}
...
async function syncOrgFacet({
	...
	if (existing) {
		...
		if (createError) {
			consola.error(`[syncOrgFacet] Failed to create facet_account for ${kindSlug}:`, createError);
			return;
		}
	...
	if (allKindFacets && allKindFacets.length > 0) {
		const staleFacetIds = allKindFacets.map((f) => f.id).filter((id) => id !== facetAccountId);
	...
	if (linkError) {
		consola.error(`[syncOrgFacet] Failed to upsert person_facet for ${kindSlug}:`, linkError);
	}
}
