...
export async function syncPeopleFieldsToFacets({
	...
	for (const { kindSlug, value } of fieldMap) {
		try {
			...
			consola.error(`[syncPeopleFieldsToFacets] Error syncing ${kindSlug}:`, error);
		}
	}
}
...
async function syncSingleFieldToFacet({
	...
	if (kindError || !kind) {
		consola.warn(`[syncSingleFieldToFacet] Kind '${kindSlug}' not found:`, kindError);
		return;
	}
	...
	if (existing) {
		...
		if (createError) {
			consola.error(`[syncSingleFieldToFacet] Failed to create facet_account for ${kindSlug}:`, createError);
			return;
		}
	...
	if (linkError) {
		consola.error(`[syncSingleFieldToFacet] Failed to upsert person_facet for ${kindSlug}:`, linkError);
	}
}
