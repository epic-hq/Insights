...
describe("isCardComplete", () => {
  ...
  it("should use completionCheck when provided", () => {
    ...
    expect(
      isCardComplete(
        card,
        {},
        { ...emptyJourneyProgress, contextComplete: true },
      ),
    ).toBe(false);
    expect(
      isCardComplete(
        card,
        {},
        { ...emptyJourneyProgress, promptsComplete: true },
      ),
    ).toBe(false);
    expect(
      isCardComplete(
  ...
  it("should use completionKey when provided", () => {
    ...
    expect(isCardComplete(card, {}, emptyJourneyProgress)).toBe(false);
    expect(
      isCardComplete(card, { surveyResponses: 0 }, emptyJourneyProgress),
    ).toBe(false);
    expect(
      isCardComplete(card, { surveyResponses: 1 }, emptyJourneyProgress),
    ).toBe(true);
    expect(
      isCardComplete(card, { surveyResponses: 10 }, emptyJourneyProgress),
    ).toBe(true);
  });
  ...
  it("should return false when card has no completion criteria", () => {
    ...
    expect(isCardComplete(dummyCard, { test: 5 }, fullJourneyProgress)).toBe(
      false,
    );
  });
});
...
describe("Phase isComplete functions", () => {
  ...
  it("Phase 2 (gather): requires 3+ encounters", () => {
    ...
    expect(phase.isComplete({}, emptyJourneyProgress)).toBe(false);
    expect(phase.isComplete({ encounters: 2 }, emptyJourneyProgress)).toBe(
      false,
    );
    expect(phase.isComplete({ encounters: 3 }, emptyJourneyProgress)).toBe(
      true,
    );
    expect(phase.isComplete({ encounters: 10 }, emptyJourneyProgress)).toBe(
      true,
    );
  });
  ...
  it("Phase 3 (patterns): requires themes and insights", () => {
    ...
    expect(
      phase.isComplete({ themes: 1, insights: 1 }, emptyJourneyProgress),
    ).toBe(true);
  });
  ...
  it("Phase 4 (action): requires high priority tasks", () => {
    ...
    expect(phase.isComplete({}, emptyJourneyProgress)).toBe(false);
    expect(
      phase.isComplete({ highPriorityTasks: 0 }, emptyJourneyProgress),
    ).toBe(false);
    expect(
      phase.isComplete({ highPriorityTasks: 1 }, emptyJourneyProgress),
    ).toBe(true);
  });
});
