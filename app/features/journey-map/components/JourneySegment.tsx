	...
	return (
		<div className={cn("relative z-10 w-full cursor-pointer", state === "locked" && "opacity-60")}>
			...
			>
				<div
					className={cn(
						...
						state === "active" &&
							"animate-[pulse-glow_3s_ease-in-out_infinite] bg-gradient-to-br from-indigo-500 to-violet-500 shadow-[0_0_30px_rgba(99,102,241,0.4),0_0_60px_rgba(99,102,241,0.15)]",
						state === "upcoming" && "border-2 border-indigo-500/30 bg-indigo-500/15",
						state === "locked" && "border-2 border-slate-500/40 border-dashed bg-slate-700/60"
					)}
				>
					{/* Phase number badge */}
					<span
						className={cn(
							"-top-2 -right-2 absolute z-20 flex h-7 w-7 items-center justify-center rounded-full font-extrabold text-white text-xs",
							state === "active" &&
								"bg-gradient-to-br from-amber-400 to-orange-500 shadow-[0_2px_8px_rgba(245,158,11,0.4)]",
							state === "completed" && "bg-emerald-500",
							(state === "upcoming" || state === "locked") && "bg-slate-700/80 text-slate-400"
						)}
					>
						{phase.number}
					</span>
					...
					{state === "completed" ? (
						<Check className="h-6 w-6 text-white lg:h-8 lg:w-8" strokeWidth={2} />
					) : state === "locked" ? (
			...
			<div className="mb-4 text-center">
				<h3 className="font-bold text-[13px] lg:text-[15px]">{phase.title}</h3>
			...
			<div className={cn("relative overflow-visible transition-all duration-400", expanded ? "h-auto" : "h-0")}>
				<div
					className={cn(
						"flex flex-col gap-2 transition-all duration-350",
						expanded
							? "pointer-events-auto translate-y-0 opacity-100"
							: "-translate-y-2.5 pointer-events-none opacity-0"
					)}
				>
