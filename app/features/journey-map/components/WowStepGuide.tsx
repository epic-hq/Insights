...
export function WowStepGuide({
	...
	useEffect(() => {
		...
		stepStatuses.forEach((isComplete, index) => {
			if (isComplete && !prevCompleted.has(index) && !completedSteps.includes(index)) {
		...
		if (newlyCompleted.length > 0) {
			...
			const allDone = pathConfig.steps.every((_, i) => stepStatuses[i] || allCompleted.includes(i));
	...
	// Polling: revalidate loader every 10s when there's an incomplete step
	// so counts refresh after async AI processing completes
			...
			<div className="w-full max-w-lg space-y-8">
				...
				<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
					<div className="mb-2 flex items-center justify-center gap-2">
						<span className={cn("inline-block rounded-full px-3 py-1 font-medium text-xs", accentClasses.badge)}>
							{pathConfig.label} path
						</span>
						<button
							type="button"
							onClick={() => resetFetcher.submit({ _action: "reset_wow_path" }, { method: "POST" })}
				...
				<div className="space-y-3">
					<AnimatePresence mode="sync">
						{pathConfig.steps.map((step, index) => {
							const isComplete = stepStatuses[index] || completedSteps.includes(index);
	...
	return (
		<div
			className={cn(
				...
				isComplete && "border-green-300 bg-green-50/50 dark:border-green-800 dark:bg-green-950/20",
				isActive && cn(accentClasses.activeBorder, accentClasses.activeBg, "ring-2", accentClasses.ring),
		...
		>
			<div className="flex items-start gap-3">
				...
				<div className="min-w-0 flex-1">
					<h3 className={cn("font-semibold text-sm", isComplete && "text-green-700 dark:text-green-400")}>
						{step.title}
					</h3>
					<p className="mt-0.5 text-muted-foreground text-xs leading-relaxed">{step.description}</p>
						...
						>
							<Button asChild size="sm" className="gap-1.5">
								<Link to={step.getRoute(routes)}>
									{step.cta}
									{step.timeHint && <span className="text-primary-foreground/60">({step.timeHint})</span>}
