...
export async function action({ context, request }: ActionFunctionArgs) {
  ...
  if (intent === "create") {
    ...
    const priority =
      Number.parseInt(formData.get("priority") as string, 10) || 3;
...
function TaskTitleCell({
  ...
  return (
    <div className="flex items-center gap-2">
      <Link
        to={detailHref}
        className="flex-1 font-medium text-foreground text-sm hover:text-primary hover:underline"
      >
        {value}
      </Link>
    </div>
...
function EditableStatusCell({
  ...
  const displayValue = React.useMemo(() => {
    if (
      fetcher.formData &&
      fetcher.formData.get("taskId") === taskId &&
      fetcher.formData.get("field") === "status"
    ) {
...
function EditablePriorityCell({
  ...
  const displayValue = React.useMemo(() => {
    if (
      fetcher.formData &&
      fetcher.formData.get("taskId") === taskId &&
      fetcher.formData.get("field") === "priority"
    ) {
      const optimisticValue = fetcher.formData.get("value");
      return optimisticValue
        ? (Number.parseInt(optimisticValue as string, 10) as Priority)
        : value;
    }
    return value;
  }, [fetcher.formData, taskId, value]);
...
function EditableImpactCell({
  ...
  const displayValue = React.useMemo(() => {
    if (
      fetcher.formData &&
      fetcher.formData.get("taskId") === taskId &&
      fetcher.formData.get("field") === "impact"
    ) {
      const optimisticValue = fetcher.formData.get("value");
      return optimisticValue
        ? (Number.parseInt(optimisticValue as string, 10) as Impact)
        : value;
    }
    return value;
  }, [fetcher.formData, taskId, value]);
          ...
          <div className="space-y-1">
            {[3, 2, 1].map((i) => (
              ...
              >
                ...
                <span className="ml-2">
                  {priorityConfig[i as 1 | 2 | 3].label}
                </span>
              </Button>
            ))}
          </div>
    ...
    <TooltipProvider>
      <Tooltip delayDuration={300}>
        <Popover open={open} onOpenChange={setOpen}>
          ...
          <TooltipContent className="max-w-xs">
            <p className="text-xs">
              The current status of the task in the workflow
            </p>
          </TooltipContent>
    ...
    <TooltipProvider>
      <Tooltip delayDuration={300}>
        <Popover open={open} onOpenChange={setOpen}>
          ...
          <TooltipContent className="max-w-xs">
            <p className="text-xs">
              The priority level of the task (High/Medium/Low)
            </p>
          </TooltipContent>
          <PopoverContent className="w-48" align="start">
            <div className="space-y-1">
              ...
              {filterOptions.map((option) => (
                ...
                >
                  <div className="flex items-center">
                    {option.color && (
                      <span
                        className={`mr-2 h-2 w-2 rounded-full bg-${option.color}-600`}
                      />
                    )}
                    <span className={option.className}>{option.label}</span>
                  </div>
...
const createColumns = (
  ...
  },
  {
    ...
    header: () => {
      return (
        <PriorityFilterHeader currentFilter={priorityFilter} tasks={tasks} />
      );
    },
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => {
      return (
        <ColumnHeader
          title="Due"
          tooltip="The target date for completing this task"
        />
      );
    },
    cell: ({ row }) => (
      ...
      <PopoverContent className="w-80" align="end">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-xs">
            Get AI insights about "{row.feature}" including implementation
            considerations, user impact, and prioritization rationale.
          </p>
...
export default function FeaturePrioritizationPage() {
  const { tasks, statusFilter, priorityFilter, projectPath } =
  ...
  const data = React.useMemo(
    () => filteredTasks.map(taskToFeatureRow),
    [filteredTasks],
  );
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <input
            ...
            onChange={(e) =>
              table.getColumn("feature")?.setFilterValue(e.target.value)
            }
              ...
              {table.getRowModel().rows.map((row) => {
                if (row.getIsGrouped()) {
                  ...
                  return (
                    <tr
                      key={row.id}
                      className="border-b bg-slate-200 font-semibold dark:bg-slate-800"
                    >
