...
export function useInterviewProgress({
  ...
  runId,
  accessToken,
}: UseInterviewProgressOptions) {
  const [interview, setInterview] = useState<Interview | null>(null);
  const [_conversationAnalysis, setConversationAnalysis] =
  ...
  const { run, error: realtimeError } = useRealtimeRun<
    typeof uploadMediaAndTranscribeTask
  >(realtimeRunId, realtimeOptions);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (interviewData && !interviewError) {
        ...
        const analysis =
          interviewData.conversation_analysis as ConversationAnalysis | null;
        if (analysis) {
          setConversationAnalysis(analysis);
        }
      } else {
        consola.error(
          "[useInterviewProgress] Failed to fetch interview",
          interviewError,
        );
      }
    };
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const analysis =
            newInterview.conversation_analysis as ConversationAnalysis | null;
          if (analysis) {
            setConversationAnalysis(analysis);
          }
        },
  ...
  useEffect(() => {
    ...
    const isComplete =
      currentStep === "complete" || interview.status === "ready";
    const hasError =
      interview.status === "error" || Boolean(metadata.failed_at);
    ...
    const label =
      statusDetail || (currentStep ? stepLabels[currentStep] : "Processing...");
  ...
  useEffect(() => {
    ...
    const displayProgress =
      isComplete || hasError ? baseProgress : syntheticProgress;
