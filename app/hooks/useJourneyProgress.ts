  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const [contextResult, promptsResult, interviewsResult, insightsResult] =
          await Promise.all([
        ...
        if (!isCancelled) {
          ...
          const merged = contextResult.data?.merged as Record<
            string,
            unknown
          > | null;
          const researchGoal = merged?.research_goal;
          const contextComplete =
            typeof researchGoal === "string" && researchGoal.trim().length > 0;
