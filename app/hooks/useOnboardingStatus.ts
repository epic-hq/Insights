...
export function useOnboardingStatus(): OnboardingStatus {
  ...
  const completed =
    protectedData?.user_settings?.onboarding_completed ||
    walkthrough?.completed ||
    false;
  const jobFunction = walkthrough?.job_function || metadata?.job_function || "";
  const primaryUseCase =
    walkthrough?.primary_use_case || metadata?.primary_use_case || "";
...
export function buildOnboardingContext(status: OnboardingStatus): string {
  ...
  if (status.jobFunction) {
    ...
    parts.push(
      `User Role: ${roleMap[status.jobFunction] || status.jobFunction}`,
    );
  }
  ...
  if (status.primaryUseCase) {
    ...
    parts.push(
      `Primary Goal: ${useCaseMap[status.primaryUseCase] || status.primaryUseCase}`,
    );
  }
  ...
  if (status.companySize) {
    ...
    parts.push(
      `Company Size: ${sizeMap[status.companySize] || status.companySize}`,
    );
  }
