...
async function main() {
  ...
  while (offset < totalMissing) {
    ...
    consola.info(
      `Processing batch ${Math.floor(offset / BATCH_SIZE) + 1} (${evidenceBatch.length} records)...`,
    );
    ...
    for (const evidence of evidenceBatch) {
      ...
      if (updateError) {
        consola.error(
          `Failed to update evidence ${evidence.id}:`,
          updateError.message,
        );
        errors++;
      } else {
    ...
    // Progress indicator
    consola.info(
      `Progress: ${processed + errors}/${totalMissing} (${errors} errors)`,
    );
