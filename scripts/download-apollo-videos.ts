...
async function main() {
  ...
  console.log(
    "⏳ Waiting for login... (script will auto-continue once logged in)",
  );
  ...
  for (let i = 0; i < URLS.length; i++) {
    ...
    try {
      ...
      if (!videoUrl) {
        ...
        await page
          .click('video, [role="button"][aria-label*="play"]')
          .catch(() => {});
        await page.waitForTimeout(2000);
      ...
      // Download video
      const extension = videoUrl.includes(".mp4")
        ? "mp4"
