...
async function testRecommendationEngine() {
  ...
  try {
    ...
    console.log(
      `📈 Generated ${result.metadata.total_potential_recommendations} potential recommendations`,
    );
    ...
    const hasCategoryDiversity =
      new Set(result.recommendations.map((r) => r.category)).size > 1;
    const hasReasoning = result.recommendations.every(
      (r) => r.reasoning.length > 50,
    );

    console.log(
      `   ${hasHighPriority ? "✅" : "❌"} Has priority 1 (critical) recommendation`,
    );
    console.log(
      `   ${hasCategoryDiversity ? "✅" : "❌"} Category diversity (not all same type)`,
    );
    console.log(
      `   ${hasReasoning ? "✅" : "❌"} All recommendations have detailed reasoning`,
    );
  } catch (error) {
...
async function generateRecommendations(
  ...
  const lowConfidenceThemes = themesWithConfidence.filter(
    (t: any) => t.confidence < 0.8 && t.evidence_count < 10,
  );

  // DEBUG: Log what data we found
  console.log("📊 Data Summary:");
  console.log(
    `   Research questions with gaps: ${unansweredQuestions?.length || 0}`,
  );
