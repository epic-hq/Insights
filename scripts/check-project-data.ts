...
async function checkProjectData() {
  ...
  const { data: personScales } = await supabase
    .from("person_scale")
    .select("*")
    .limit(10);
  ...
  if (themes && themes.length > 0) {
    ...
    themes.forEach((t) => {
      console.log(
        `   ${t.name}: ${t.evidence_count} mentions (validation: ${t.validation_status || "none"})`,
      );
    });
    console.log("");
  }
  ...
  if (lensAnalyses && lensAnalyses.length > 0) {
    ...
    templates.forEach((template) => {
      const count = lensAnalyses.filter(
        (l) => l.template_key === template,
      ).length;
