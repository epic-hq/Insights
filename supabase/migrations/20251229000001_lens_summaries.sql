...
-- Auto-update updated_at timestamp (using a simple function instead of moddatetime)
CREATE OR REPLACE FUNCTION public.update_lens_summaries_timestamp()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
CREATE TRIGGER update_lens_summaries_updated_at
  BEFORE UPDATE ON public.conversation_lens_summaries
  FOR EACH ROW
  EXECUTE FUNCTION public.update_lens_summaries_timestamp();
