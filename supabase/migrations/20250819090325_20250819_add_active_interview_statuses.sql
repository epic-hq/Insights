...
alter type "public"."interview_status" rename to "interview_status__old_version_to_be_dropped";

create type "public"."interview_status" as enum ('draft', 'scheduled', 'uploading', 'uploaded', 'transcribing', 'transcribed', 'processing', 'ready', 'tagged', 'archived', 'error');

alter table "public"."interviews" alter column status type "public"."interview_status" using status::text::"public"."interview_status";

alter table "public"."interviews" alter column "status" set default 'draft'::interview_status;

drop type "public"."interview_status__old_version_to_be_dropped";
...
alter table "public"."entity_flags" add constraint "entity_flags_entity_type_check" CHECK ((entity_type = ANY (ARRAY['insight'::text, 'persona'::text, 'opportunity'::text, 'interview'::text, 'person'::text, 'project'::text]))) not valid;

alter table "public"."entity_flags" validate constraint "entity_flags_entity_type_check";

alter table "public"."votes" add constraint "votes_entity_type_check" CHECK ((entity_type = ANY (ARRAY['insight'::text, 'persona'::text, 'opportunity'::text, 'interview'::text, 'person'::text, 'project'::text]))) not valid;
