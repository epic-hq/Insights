drop policy "Users can delete their own interviews" on "public"."interviews";

drop policy "Users can insert their own interviews" on "public"."interviews";

drop policy "Users can select their own interviews" on "public"."interviews";

drop policy "Users can update their own interviews" on "public"."interviews";
...
create or replace view "public"."persona_distribution" as  WITH persona_interview_counts AS (
 ...
 SELECT pic.persona_id,
  ...
  create policy "Account members can insert interviews"
...
with check ((account_id IN ( SELECT accounts.get_accounts_with_role() AS get_accounts_with_role)));
  ...
  create policy "Account members can select interviews"
  on "public"."interviews"
  as permissive
  for select
  to authenticated
using ((account_id IN ( SELECT accounts.get_accounts_with_role() AS get_accounts_with_role)));
  ...
  create policy "Account members can update interviews"
  on "public"."interviews"
  as permissive
  for update
  to authenticated
using ((account_id IN ( SELECT accounts.get_accounts_with_role() AS get_accounts_with_role)));
  ...
  create policy "Account owners can delete interviews"
  on "public"."interviews"
  as permissive
  for delete
  to authenticated
