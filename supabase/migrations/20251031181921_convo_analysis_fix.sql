drop policy "account members can manage sales lens hygiene events" on "public"."sales_lens_hygiene_events";

drop policy "account members can manage sales lens slots" on "public"."sales_lens_slots";

drop policy "account members can manage sales lens stakeholders" on "public"."sales_lens_stakeholders";
...
drop index if exists "public"."sales_lens_stakeholders_account_idx";

alter table "public"."interviews" add column "conversation_analysis" jsonb;
...
create policy "Account members can manage sales lens hygiene events"
on "public"."sales_lens_hygiene_events"
as permissive
for all
to public
using ((summary_id IN ( SELECT sales_lens_summaries.id
   FROM sales_lens_summaries
  WHERE (sales_lens_summaries.account_id IN ( SELECT accounts.get_accounts_with_role() AS get_accounts_with_role)))))
with check ((summary_id IN ( SELECT sales_lens_summaries.id
   FROM sales_lens_summaries
  WHERE (sales_lens_summaries.account_id IN ( SELECT accounts.get_accounts_with_role() AS get_accounts_with_role)))));


create policy "Account members can manage sales lens slots"
on "public"."sales_lens_slots"
as permissive
for all
to public
using ((summary_id IN ( SELECT sales_lens_summaries.id
...
create policy "Account members can manage sales lens stakeholders"
on "public"."sales_lens_stakeholders"
as permissive
for all
to public
...
create policy "Account members can manage sales lens summaries"
on "public"."sales_lens_summaries"
as permissive
for all
to public
using ((account_id IN ( SELECT accounts.get_accounts_with_role() AS get_accounts_with_role)))
