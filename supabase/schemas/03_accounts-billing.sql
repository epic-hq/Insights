...
(
    ...
    id                   text primary key,
    account_id           uuid references accounts.accounts (id) on delete cascade          not null,
    billing_customer_id  text references accounts.billing_customers (id) on delete cascade not null,
...
create policy "Can only view own billing customer data." on accounts.billing_customers for
    select
    using (
...
create policy "Can only view own billing subscription data." on accounts.billing_subscriptions for
    select
    using (
    accounts.has_role_on_account(account_id) = true
    );
