...
-- Partial index for upcoming customer meetings (frequently queried)
CREATE INDEX IF NOT EXISTS idx_calendar_events_upcoming_customer
    ON public.calendar_events(account_id, start_time)
    WHERE start_time > now() AND is_customer_meeting = true;
