...
-- New: survey_response, public_chat
COMMENT ON COLUMN public.interviews.source_type IS
  'Source of the content. Values: realtime_recording | audio_upload | video_upload | document_upload | transcript_paste | transcript_import | survey_response | public_chat';
...
CREATE INDEX IF NOT EXISTS idx_interviews_draft_surveys
  ...
  WHERE status = 'draft' AND source_type IN ('survey_response', 'public_chat');
